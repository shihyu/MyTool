...
#define BAS_MODE_NAME   'Visual Basic'
#define BAS_LANGUAGE_ID 'bas'
#define BAS_WORD_CHARS  'a-zA-Z0-9_$!%#&'
...
defload()
{
   // set up qbasic (Visual Basic) extension (bas)
   _str setup_info='MN='BAS_MODE_NAME',TABS=+3,MA=1 74 1,':+
                   'KEYTAB=qbasic-keys,WW=0,IWT=0,ST='DEFAULT_SPECIAL_CHARS',';
   _str compile_info='0 bc *;';
   _str syntax_info='3 1 1 0 0 3 0';
   _str be_info='(FOR)|(NEXT) (WHILE)|(WEND) (DO)|(LOOP);i';

   _CreateLanguage(BAS_LANGUAGE_ID, BAS_MODE_NAME,
                   setup_info, compile_info,
                   syntax_info, be_info, '',
                   BAS_WORD_CHARS, BAS_MODE_NAME);
   _CreateExtension('vb',BAS_LANGUAGE_ID);
   _CreateExtension('bas',BAS_LANGUAGE_ID);
   _CreateExtension('frm',BAS_LANGUAGE_ID);
   LanguageSettings.setAutoBracket(BAS_LANGUAGE_ID, AUTO_BRACKET_ENABLE|AUTO_BRACKET_PAREN|AUTO_BRACKET_BRACKET|AUTO_BRACKET_DOUBLE_QUOTE);
   LanguageSettings.setReferencedInLanguageIDs(BAS_LANGUAGE_ID, "cs");
}
 ...
 */
_str _bas_keyword_case(_str kw, boolean confirm = true)
{
   return _word_case(kw, confirm);
}
...
#define BAS_ENTER_WORDS ' case do for if select type while '
#define BAS_FIRST_WORDS  ' function sub '
/* Space words must be in sorted order */
#define BAS_SPACE_WORDS (' beep bload bsave call chain chdir circle clear close':+\
...
#define BAS_SPACE_WORDS2 (' get gosub goto if input input$ ioctl key kill':+\
               ' left line locate lock loop lprint lset mid$ on open out paint palette':+\
                ' pcopy pen play poke preset print pset put ')
#define BAS_SPACE_WORDS3 (' randomize read redim rem reset restore resume return':+\
               ...
               ' timer troff tron type unlock view wait while width window':+\
                ' write ')
#define BAS_EXPAND_WORDS (' beep bload bsave call case chain chdir circle clear close':+\
               ' cls color com const data date$ declare':+\
               ' def defdbl defint deflng defsng defstr dim do draw else':+\
               ' elseif end environ erase error exit field files for function ')

#define BAS_EXPAND_WORDS2 (' get gosub goto if input input$ ioctl key kill':+\
               ' left line locate lock loop lprint lset mid$ module on open out paint palette':+\
                ' pcopy pen play poke preset print pset put ')
#define BAS_EXPAND_WORDS3 (' randomize read redim rem reset restore resume return':+\
               ' rmdir rset run screen seek select shared':+\
               ' shell sleep sound static stop strig sub swap system time$':+\
               ' timer troff tron type unlock view wait while width window':+\
                ' write ')
#define BAS_DECL_WORDS (' integer real string ')
/*
...
boolean _bas_expand_enter()
{
   ...
   if ( first_word=='for' && name_on_key(ENTER)=='nosplit-insert-line' ) {
      ...
      get_line(new_line);
      if ( rest=='' && new_line=='' ) {
         replace_line(indent_string(p_col-1)_word_case('Case '));
         _end_line();
      ...
      replace_line(substr(line,1,pos(word,line)-1)_word_case(first_word) " "rest);
...
static _str qbasic_expand_space()
{
   ...
   _str aliasfilename='';
   _str word=min_abbrev2(orig_word,bas_space_words,name_info(p_index),aliasfilename);
   ...
   if ( word=='if' ) {
      ...
      replace_line(_word_case(line):+' =  ':+_word_case('to'):+_word_case('  step '));
      ...
      insert_line('');
      insert_line(indent_string(width)_word_case('Loop'));
      set_surround_mode_end_line();
      up();up(); _end_line();right();

   } else if ( word=='addhandler' ) {

      replace_line(_word_case(line:+'  addressof '));
      p_col=width+12;

   } else if (word=='dim' ) {

      replace_line(_word_case(line):+_word_case('  as '));
      p_col=width+5;
      if ( ! _insert_state() ) {
         _insert_toggle();
      };
      ...
      insert_line(indent_string(width)_word_case('catch'));
      insert_line('');
      ...
      if (strip(third)=='') {
         insert_line('');
         insert_line(indent_string(width)_word_case('end')' '_word_case(second));
         up();up();_end_line();
         right();
      } else {
         return(1);
      ...
      insert_line('');
      insert_line(indent_string(width)_word_case('end')' '_word_case('class'));
      up();up();_end_line();
      right();
      ...
      insert_line('');
      insert_line(indent_string(width)_word_case('end')' '_word_case('property'));
      up();up();_end_line();
      right();
      ...
      replace_line(_word_case(line));
      insert_line(indent_string(width)_word_case('wend'));
      set_surround_mode_end_line();
      up();p_col=width+7;
      if ( ! _insert_state() ) {
         _insert_toggle();
      ...
      replace_line(_word_case(line)' '_word_case('case'));
      insert_line(indent_string(width)_word_case('end')' '_word_case('select'));
      set_surround_mode_end_line();
      up();_end_line();right();
      ...
      replace_line(_word_case(line));
      insert_line(indent_string(width)_word_case('end')' '_word_case('type'));
      up();p_col=width+6;
      if (! _insert_state()) {
         _insert_toggle();
      }
      ...
      if (strip(third)=='') {
         if (second == 'sub' ) {
            insert_line('');
            insert_line(_word_case('end')' '_word_case('sub'));
         } else {
            insert_line('');
 ...
 */
boolean _bas_is_continued_statement()
{
   get_line(auto line);
 ...
 *
 */
int bas_smartpaste(boolean char_cbtype, int first_col, int Noflines,boolean allow_col_1=false)
{
   _str comment_col='';
   int line_no1 = 0, line_no2 = 0, status2 = 0;
...
*/
#define BAS_COMMON_END_OF_STATEMENT_RE 'if|while|throw|select|for|case|default|public|private|protected|static|class|break|resume|do|else|goto|return|synclock|catch|finally|module|structure|set|removehandler|redim|randomize|raiseevent|property|option|on|interface|erase|error|event|exit|enum'
#define BAS_MORE_END_OF_STATEMENT_RE 'extern|structure|overridable|mustoverride|import|namespace'
#define BAS_NOT_FUNCTION_WORDS  ' int long double single float boolean short byte date char catch do for if return while for call class declare dim end erase error exit goto on raiseevent stop set select case try finally with while return resume removehandler redim '
 ...
 * @since 11.0
 */
int _bas_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                                  ...
                                  VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
      ...
      case '.':
         if (package_name != '') {
            ...
            if (pos('/',package_name)) {
               ...
               _str new_package_name = stranslate(package_name,ch,VS_TAGSEPARATOR_package);
               new_package_name = stranslate(new_package_name,ch,VS_TAGSEPARATOR_class);
               if (tag_check_for_package(new_package_name, tag_files, true, false)) {
                  verified_package=new_package_name;
                  package_name = new_package_name :+ VS_TAGSEPARATOR_package;
               } else {
                  package_name = package_name :+ VS_TAGSEPARATOR_class;
               }
            } else if (tag_check_for_package(package_name, tag_files, true, false)) {
               // this is a known package
               verified_package=stranslate(package_name,ch,VS_TAGSEPARATOR_class);
               package_name = package_name :+ VS_TAGSEPARATOR_package;
            } else if (tag_check_for_package(package_name:+ch, tag_files, false, false)) {
   ...
   if (allow_local_class) {
      ...
      if (qualified_name=='') {
         ...
         _str outer_qualified='';
         if (outer_name!='') {
            status = tag_qualify_symbol_name(outer_qualified, outer_name, search_class_name, file_name, tag_files, false);
         ...
         if (outer_name=='' && search_class_name!='') {
            status = tag_qualify_symbol_name(qualified_name, package_inner, search_class_name, file_name, tag_files, false);
         } else {
            status = tag_qualify_symbol_name(qualified_name, package_inner, outer_name, file_name, tag_files, false);
         }
      }
      if (qualified_name=='') {
   ...
   if (depth < VSCODEHELP_MAXRECURSIVETYPESEARCH) {
      ...
      if (tag_check_for_typedef(/*found_type*/qualified_inner, tag_files, false, qualified_outer)) {
                                   ...
                                   VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   if ((strieq(symbol,'me') || strieq(symbol,'myclass')) && !(cur_tag_flags & VS_TAGFLAG_static)) {
      if (search_class_name :== '' && context_id > 0) {
         ...
         tag_qualify_symbol_name(rt.return_type,inner_name,outer_name,p_buf_name,tag_files,false);
 ...
 * based on the pushtag_mask and bas_return_flags.  The number of
...
static int _bas_match_return_type_of(_str (&errorArgs)[], typeless tag_files,
                                     ...
                                     int pushtag_mask, int bas_return_flags,
                                     struct VS_TAG_RETURN_TYPE &rt,
                                     VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   //say("_bas_match_return_type_of("symbol","search_class_name")");
   // filter out mutual recursion
   _str input_args='match;'symbol';'search_class_name';'cur_class_name';'min_args';'maybe_class_name';'pushtag_mask';'bas_return_flags;
   if (visited._indexin(input_args)) {
      if (visited:[input_args].return_type==null) {
      ...
      for (i=1; i<=num_matches; i++) {
         ...
         if (check_args && num_matches>1 && tag_tree_type_is_func(type_name) &&
            ...
            for (;;) {
               ...
               if (pos('[',parm) && !pos(',',substr(signature,arg_pos))) {
                  num_args = min_args;
                  break;
               }
         ...
         if (_chdebug) {
            isay(depth, "_bas_get_type_of_matches: WHERE proc_name="proc_name" class="class_name" return_type="return_type);
         }
         if (rt.taginfo == '') {
                                 ...
                                 int &reference_count, int depth=0)
{
   ...
   case '.':     // member access operator
      ...
      if (previous_id != '') {
         ...
         status = _bas_get_return_type_of(errorArgs, tag_files,
                                          ...
                                          VS_TAGFILTER_ANYSTRUCT|VS_TAGFILTER_ANYDATA|VS_TAGFILTER_ENUM|VS_TAGFILTER_PACKAGE,
                                          true,rt,visited);
         if (status) {
            return status;
         }
         ...
         if (previous_builtin) {
            ...
            if (new_match_class == '') {
               ...
               tag_list_symbols_in_context(previous_id, rt.return_type, 0, 0, tag_files, '',
                                           ...
                                           VS_TAGFILTER_ANYSTRUCT|VS_TAGFILTER_TYPEDEF|VS_TAGFILTER_PACKAGE,
   ...
   case 'mybase':
      status = _bas_get_return_type_of(errorArgs, tag_files, 'Me', '', 0,
                                       VS_TAGFILTER_ANYDATA, false, rt, visited);
      if (status) {
         return status;
      }
      _str tag_dbs;
      _str parents = cb_get_normalized_inheritance(rt.return_type, tag_dbs, tag_files, true);
   ...
   default:
      ...
      if (rt.return_type=='') {
         ...
         while (orig_prefix != '') {
            ...
            if (tag_check_for_package(package_name, tag_files, true, false)) {
                           ...
                           VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   _str constructor_class='';
   VS_TAG_RETURN_TYPE rt;tag_return_type_init(rt);
   if (prefixexp!='') {
      int status = _bas_get_type_of_prefix(errorArgs, prefixexp, rt, visited, depth);
      if (status) {
         tag_pop_matches();
         return status;
      } else {
   ...
   tag_clear_matches();
   num_matches := 0;
   int context_flags2=(rt.return_flags & VSCODEHELP_RETURN_TYPE_GLOBALS_ONLY)? 0:VS_TAGCONTEXT_ALLOW_locals;
   ...
   tag_list_symbols_in_context(lastid, rt.return_type, 0, 0, tag_files, '',
                               ...
                               exact_match, false, visited, depth, rt.template_args);
                     ...
                     VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   _str common='[,#{};()]|'BAS_COMMON_END_OF_STATEMENT_RE'|'BAS_MORE_END_OF_STATEMENT_RE;
   ...
   for (;;--stack_top) {
      ...
      status=_bas_get_expression_info(true,idexp_info,visited,depth);
      idexp_info.errorArgs[1] = idexp_info.lastid;

      if (_chdebug) {
         tag_idexp_info_dump(idexp_info,"_bas_fcthelp_get");
         say("_bas_fcthelp_get: status="status);
      }
      if (!status) {
         ...
         if (idexp_info.info_flags & VSAUTOCODEINFO_VAR_OR_PROTOTYPE_DECL) {
            ...
            _str cmatch_class = tag_join_class_name(match_symbol, match_class, tag_files, false);
            tag_clear_matches();
            _UpdateLocals(true);
         ...
         if (num_matches == 0) {
            if (pos('new ',idexp_info.prefixexp) == 1) {
               // handle 'new' expressions as a special case
               _str outer_class = substr(idexp_info.prefixexp, 5);
               if (last_char(outer_class)==':') {
               ...
               if (outer_class=='') {
                  ...
                  tag_qualify_symbol_name(rt.return_type,match_symbol,'',p_buf_name,tag_files,false);
               } else {
                  rt.return_type = tag_join_class_name(match_symbol, outer_class, tag_files, false);
               }
               rt.pointer_count = 1;
               status = 0;
               ...
               if (pos('new ',idexp_info.prefixexp)==1) {
                  if (match_symbol=='' && rt.return_type!='') {
                     int colon_pos=lastpos('[:/]',rt.return_type,1,'r');
                     if (colon_pos) {
         ...
         int i,num_unique = unique_indexes._length();
         for (i=0; i<num_unique; i++) {
            int j = unique_indexes[i];
            _str tag_file,proc_name,type_name,file_name;
            int line_no;
            _str class_name;
            ...
            if (match_list._length()>0 || i+1<num_unique) {
               ...
               if (tag_tree_type_is_package(type_name) || tag_tree_type_is_class(type_name)) {
                  continue;
               }
         ...
         if (match_list._length()>0) {
            ...
            for (i=0; i<match_list._length(); i++) {
               ...
               _str as_return_type=(return_type=='')? '':' As ':+return_type;
               FunctionHelp_list[k].prototype = match_tag_name'('signature')':+as_return_type;
               ...
               while (argument != '') {
                  ...
                  if (j == ParamNum) {
                     if (pos("^["word_chars"]*([=]?*|)$",argument,1,'r')) {
                        ...
                        boolean utf8=p_UTF8;
                        int psindex = find_index(pslang'_proc_search',PROC_TYPE);
                        int temp_view_id;
 ...
 */
boolean _bas_is_builtin_type(_str return_type, boolean no_class_types=false)
{
 ...
 */
boolean _bas_builtin_assignment_compatible(_str expected_type,
                                           _str candidate_type,
 ...
 * @param bas_access_flags     bitset of VS_TAGFLAG_* (VS_TAGFLAG_access)
                                  ...
                                  _str class_signature='')
{
   ...
   // show access flags
   if ((cm.flags & VS_TAGFLAG_access) != bas_access_flags) {
      bas_access_flags = (cm.flags & VS_TAGFLAG_access);
   ...
   int show_inline=0;
   if (!in_class_scope &&
       file_eq(cm.file_name,p_buf_name) &&
                   ...
                   _str access_indent_string="")
{
   ...
   case 'subproc':      // Nested procedure or cobol paragraph
      ...
      if (show_access) {
         int bas_access_flags = (tag_flags & VS_TAGFLAG_access);
         switch (bas_access_flags) {
         case VS_TAGFLAG_public:
         case VS_TAGFLAG_package:
            before_tag='';
...
int _bas_insert_constants_of_type(struct VS_TAG_RETURN_TYPE &rt_expected,
                                  ...
                                  _str lastid_prefix="",
                                  boolean exact_match=false, boolean case_sensitive=false)
{
                           ...
                           int tree_wid, int tree_index)
{
   ...
   boolean expected_is_builtin =_bas_is_builtin_type(rt_expected.return_type);
   boolean candidate_is_builtin=_bas_is_builtin_type(rt_candidate.return_type);
   ...
   if (rt_expected.taginfo!='') {
      tag_tree_decompose_tag(rt_expected.taginfo,expected_tag_name,
                             expected_class,expected_tag_type,expected_flags);
   }
   ...
   if (rt_candidate.taginfo!='') {
      tag_tree_decompose_tag(rt_candidate.taginfo,candidate_tag_name,
                             candidate_class,candidate_tag_type,candidate_flags);
   }
 ...
 */
int _bas_MaybeBuildTagFile(int &tfindex)
{
   // maybe we can recycle tag file(s)
   _str ext='bas';
   _str tagfilename='';
   boolean force_rebuild_bas_vtg=false;
   if (!ext_MaybeRecycleTagFile(tfindex,tagfilename,ext,"dotnet")) {
      ...
      _str dotnet_path=frameworks[0].maketags_args;
      if (dotnet_path=='') {
         return PATH_NOT_FOUND_RC;
      }
      ...
      int status = ext_BuildTagFile(tfindex,tagfilename,ext,".NET Framework",
   ...
   if (!ext_MaybeRecycleTagFile(tfindex,tagfilename,ext,"bas") || force_rebuild_bas_vtg) {
      ...
      _str extra_file=ext_builtins_path(ext,'basic');
