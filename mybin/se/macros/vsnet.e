...
static void format_line()
{
   ...
   typeless Noflines=0, cur_line=0, first_word=0, last_word=0, rest=0;
   typeless non_blank_col=0, semi=0, prev_semi=0;
   typeless status=c_get_info(Noflines,cur_line,first_word,last_word,rest,
                     non_blank_col,semi,prev_semi);
   if (status) return;
   typeless col=c_indent_col(non_blank_col,false);
#if 1
   if (col==1) {
...
_command void select_prev_condition() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_CMDLINE|VSARG2_MARK)
{
   save_pos(auto p);
   cua_select_char();
   typeless status=prev_condition();
   if (status) {
      restore_pos(p);
      _deselect();
...
_command void select_next_condition() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_CMDLINE|VSARG2_MARK)
{
   save_pos(auto p);
   cua_select_char();
   typeless status=next_condition();
   if (status) {
      restore_pos(p);
      //_deselect();
      message('No enclosing conditional statement found.');
...
static void cua_select_char(_str markid="")
{
   ...
   typeless status=_select_char(markid,mstyle);
   if (status) {
...
_command void select_matching_brace() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_CMDLINE)
{
   ...
   save_pos(auto p);
   typeless status=find_matching_paren(true);
   if (status) {
      restore_pos(p);
      _deselect();
      return;
...
//{
//   return activate_toolbar('_tbprojects_form','');
//}

//_command activate_output_toolbar() name_info(','VSARG2_EDITORCTL)
//{
//   return activate_toolbar('_tboutputwin_form','');
//}
...
static void set_control(_str control_name,boolean on_off)
{
   int wid=_find_formobj('_goto_form','N');
   int control_id=wid._find_control(control_name);
