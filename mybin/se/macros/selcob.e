 ...
 */
static boolean isCommentLine(_str & line)
{
 ...
 */
static boolean isEmptyLine(_str & line)
{
   // Line is too short.
   int len = length(line);
   ...
   int firstNonSpace = pos('[~ \t]', line, 1, 'ri');
 ...
 */
static boolean isVerb(_str & word)
{
...
static void verbInfo(_str & verb, _str & endVerb,
                     boolean & canNest, _str & imperativeKey)
{
   _str canNestText;
 ...
 */
static boolean verbUnderCursor(_str & verb,
                               _str & endVerb,
                               boolean & canNest,
                               _str & imperativeKey
                               )
 ...
 */
static boolean hasEndPeriod(_str & line)
{
...
static _str firstWordInLine(_str & line)
{
   ...
   int startPos = pos('['word_chars']', line, 8, 'ri');
   if (!startPos || startPos >= 73) return("");

   // Find the end of word.
   int endPos = pos('[~'word_chars'$]', line, startPos, 'ri');
 ...
 */
static void skipWHENClause(boolean & reachedEOF)
{
                              ...
                              )
{
   ...
   boolean reachedEndStatement = false;
                                ...
                                )
{
   ...
   */
   ...
   boolean keyMaybeOnSameLine = true;
   ...
   _str line;
   _str word;
   boolean foundImperativeKey = false;
 ...
 */
static boolean isSpecialCasePERFORM()
{
   // The "PEFORM" verb has an optional following "UNTIL" which
   // causes the "PERFORM" block to have an optional "END-PERFORM".
 ...
 */
static boolean isParagraphName(_str & line)
{
   // Line too short.
   ...
   word_chars := _clex_identifier_chars();
   int position = pos('['word_chars']', line, 8, 'ri');
 ...
 */
static boolean isOnParagraphName()
{
   get_line(auto line);
...
int selectCodeBlock_cob()
{
   ...
   boolean reachedEndStatement, reachedEOF;
   if (verb == "PERFORM") {
      // The "PEFORM" verb has an optional following "UNTIL" which
      // causes the "PERFORM" block to have an optional "END-PERFORM".
      if (isSpecialCasePERFORM()) {
