...
_command int vi_restart_insertion() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   ...
   if ( def_vi_insertion_pos=='' ) {
      vi_message('No insertion information available');
      return(1);
   }
   parse value def_vi_insertion_pos with line col;
 ...
 */
_command int vi_restart_word() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   if ( command_state() ) {
      _str key=last_event();
      if ( length(key):==1 && isnormal_char(key) ) {
 ...
 */
_command int vi_space () name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   _str key='';
   if ( command_state() ) {
      key=last_event();
 ...
 */
_command int vi_ptab(...) name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   if ( command_state() ) {
      _str key=last_event();
      if ( length(key):==1 && isnormal_char(key) ) {
 ...
 */
_command int vi_pbacktab() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   return(vi_ptab('-'));
}
 ...
 */
_command int vi_move_text_tab() name_info(','VSARG2_REQUIRES_EDITORCTL)
{
