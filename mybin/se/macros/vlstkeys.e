...
defmain()
{
  if ( arg(1)!='' ) {
    ...
    _str temp_name=absolute('keydefs'_macro_ext);
    rc=edit('+b 'maybe_quote_filename(temp_name));
    if ( ! rc ) {
       clear_message();
       temp_name='keydefs..';
    }
    int status=edit('+t 'maybe_quote_filename(temp_name));
