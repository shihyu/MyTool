...
#define UPCHECK_QE_FIRST    1
#define UPCHECK_QE_START    (UPCHECK_QE_FIRST+0)
#define UPCHECK_QE_VERSION  (UPCHECK_QE_FIRST+1)
#define UPCHECK_QE_MANIFEST (UPCHECK_QE_FIRST+2)
#define UPCHECK_QE_QUIT     (UPCHECK_QE_FIRST+3)
#define UPCHECK_QE_END      (UPCHECK_QE_FIRST+4)
#define UPCHECK_QE_ONIDLE   (UPCHECK_QE_FIRST+5)
#define UPCHECK_QE_LAST     UPCHECK_QE_ONIDLE

   // Event to be processed
   int event;

#define UPCHECK_QS_FIRST             1
#define UPCHECK_QS_BEGIN             (UPCHECK_QS_FIRST+0)
#define UPCHECK_QS_WAITING_FOR_REPLY (UPCHECK_QS_FIRST+1)
#define UPCHECK_QS_ERROR             (UPCHECK_QS_FIRST+2)
#define UPCHECK_QS_ABORT             (UPCHECK_QS_FIRST+3)
#define UPCHECK_QS_END               (UPCHECK_QS_FIRST+4)
#define UPCHECK_QS_LAST              UPCHECK_QS_END
 ...
 */
#define UPCHECKQTIMER_INTERVAL (100)

/**
 * Timer interval used when there are no upcheck events to be processed.
 * 10 minutes (in milliseconds).
 */
#define UPCHECKQTIMER_SLOW_INTERVAL (600000)
...
#define MSMINUTES(m) (m*60*1000)
/** Convert hours to milliseconds */
#define MSHOURS(h) (h*60*60*1000)
/** Convert days to milliseconds */
#define MSDAYS(d) (d*24*60*60*1000)

/** Update interval = 1 day = 86400000 */
#define UPCHECK_INTERVAL_1DAY MSDAYS(1)

/** Update interval = 1 week = 604800000 */
#define UPCHECK_INTERVAL_1WEEK MSDAYS(7)
...
/** upcheck.exe version supported. */
#define UPC_VER_STRING      "1.0.0.0"
/** upcheck.exe version supported. */
#define UPC_VER_HEX         0x01000000

/** upcheck.exe major version supported. */
#define UPC_VER_MAJOR       1
/** upcheck.exe minor version supported. */
#define UPC_VER_MINOR       0
/** upcheck.exe revision version supported. */
#define UPC_VER_SUB         0
/** upcheck.exe build version supported. */
#define UPC_VER_BUILD       0

// Return codes used by _upcheckNotify form
#define UPCHECKNOTIFYDLG_STATUS_DISPLAYNOW  0
#define UPCHECKNOTIFYDLG_STATUS_REMINDLATER 1

#define UPCHECK_DEBUGFLAG_TIME_STAMP  0x1
#define UPCHECK_DEBUGFLAG_SAVE_LOG    0x2
#define UPCHECK_DEBUGFLAG_EXTRA_DEBUG 0x4
#define UPCHECK_DEBUGFLAG_SAY_DEBUG   0x8
int _upcheckdebug;
void upcheckDebug(_str line) {
...
#define upcheckInfo(line) upcheckLog(&_upcheckCurrentConnection,"!!! INFO: "line);
#define upcheckError(line) upcheckLog(&_upcheckCurrentConnection,"!!! ERROR: "line);
#define upcheckWarn(line) upcheckLog(&_upcheckCurrentConnection,"!!! WARNING: "line);
...
static boolean _timedOut(double start, double max)
{
   if( max<=0 ) {
      return false;
   }
...
static boolean _upcheckTimedOut(double start, double max)
{
   if( max<=0 ) {
      return false;
   }
 ...
 *                   updates to user.
 */
void upcheckUpdateCheck(boolean idleNotify=false)
{
   upcheckConnection_t ucc;
   ...
   int captionLabel = formWid._find_control("ctl_caption");
   int gaugeWid = formWid._find_control("ctl_gauge");
   int cancelButton = formWid._find_control("ctl_cancel");
...
void upcheckDisplayUpdates(_str filename, upcheckUpdate_t (&list)[]=null)
{
   ...
   if( list._length()<1 ) {
      ...
      _str msg = "No new updates available.";
   ...
   // displayed.
   boolean wasRunning = _upcheckQKillTimer();
...
void _upcheckQTimerCallback()
{
   ...
   default:
      ...
      _str msg = "Unknown update checker queue event : ":+event.event;
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return;
   }
...
static void _upcheckQSmartStartTimer()
{
   ...
   if( events ) {
      // Need fast interval
      boolean slow = (pfnupcheckQTimerCallback!=_upcheckQTimerCallback);
      ...
      // Need slow interval
      boolean fast = (pfnupcheckQTimerCallback!=_upcheckQSlowTimerCallback);
 ...
 * false otherwise.
 */
static boolean _upcheckQKillTimer()
{
   boolean wasRunning = (_upcheckQTimer>=0);
   if (wasRunning) {
...
#if __UNIX__
   #define UPCHECK_COMMAND "SlickEditUpdateMgr"
#else
   #define UPCHECK_COMMAND "SlickEditUpdateMgr.exe"
#endif

_str def_upcheck_updatebase = "update.slickedit.com/updates/";
...
static _str _upcheckCmdline()
{
   ...
   if( _upcheckPath=="" ) {
      ...
      path=path:+UPCHECK_COMMAND;
      if( file_match('-p 'maybe_quote_filename(path),1)=="" ) {
   ...
   cmdline=maybe_quote_filename(_upcheckPath);
      ...
      if( browser=="" ) {
         ...
         if( proxy!="" ) {
            ...
            if( host!="" && isinteger(port) && port>0 ) {
               cmdline=cmdline' -proxy 'host':'port;
            }
         }
      }
   } else {
#if __NT__
      cmdline=cmdline' -noproxy';
#endif
   }

   _str updatebase = def_upcheck_updatebase;
   if( updatebase!="" ) {
      cmdline=cmdline' -updatebase 'updatebase;
   }
...
static int _upcheckCommand(_str line, upcheckConnection_t* ucc_p)
{
   ...
   int status=_PipeWrite(ucc_p->hout,line);
...
{
   int hin = ucc_p->hin;
   int status = _PipeReadLine(response,hin);
   if( !status ) {
...
#define UPCHECK_LOG_BUFNAME ".upchecklog"
...
static _str _upcheckCreateLogBuffer()
{
   ...
   _str log_buf_name = _upcheckMkLogName();
   if( log_buf_name=="" ) {
      _str msg = "UPCHECK: "get_message(VSRC_UPCHECK_ERROR_CREATING_LOG);
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return("");
   }
...
void upcheckLog(upcheckConnection_t *ucc_p, _str buf, boolean saveLog=false)
{
   _str log_buf_name = UPCHECK_LOG_BUFNAME;
   ...
   if( saveLog || _upcheckdebug&UPCHECK_DEBUGFLAG_SAVE_LOG ) {
      ...
      _str log_filename = temp_path:+p_buf_name;
      _save_file('+o 'maybe_quote_filename(log_filename));
   }
   p_window_id=orig_view_id;
}
...
void upcheckCloseConnection()
{
   if( !_upcheckCurrentConnection._isempty() ) {
      if( _upcheckQ._length()>0 ) {
         ...
         boolean old_IdleCheck = _upcheckSetIdleCheck(false);
         // Cycle the queue
         _upcheckQTimerCallback();
...
static boolean gUpcheckIdleCheck = true;
/**
 * Set idle-check for _upcheckQTimerCallback and return old value.
 */
static boolean _upcheckSetIdleCheck(boolean value)
{
   boolean old_value = gUpcheckIdleCheck;
   gUpcheckIdleCheck=value;
   return old_value;
}
static boolean _upcheckGetIdleCheck()
{
...
static void _upcheckQEHandler_Start(upcheckQEvent_t *e_p)
{
   ...
   default:
      // Should never get here
      _str msg = "Unknown update checker queue event state: ":+event.state:+" for event ":+event.event;
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      break;
   }
...
static void _upcheckQEHandler_Version(upcheckQEvent_t *e_p)
{
   ...
   case UPCHECK_QS_WAITING_FOR_REPLY: {
      ...
      if( status ) {
         _str msg = "VERSION: "get_message(status);
         _upcheckEnQ(event.event,UPCHECK_QS_ERROR,0,&event.ucc,msg);
         return;
      }
      if( line=="" ) {
         ...
         if( _timedOut(event.start,UPCHECK_TIMEOUT) ) {
            _str msg = "VERSION: "get_message(VSRC_UPCHECK_TIMED_OUT);
            _upcheckEnQ(event.event,UPCHECK_QS_ERROR,0,&event.ucc,msg);
            return;
         }
      ...
      if( major!=UPC_VER_MAJOR || minor!=UPC_VER_MINOR ) {
         _str msg = "VERSION: "get_message(VSRC_UPCHECK_VERSION_NOT_SUPPORTED,version);
         _upcheckEnQ(event.event,UPCHECK_QS_ERROR,0,&event.ucc,msg);
         return;
      }
...
#define UPCHECK_MANIFEST_DOCTYPE_TAG "Manifest"
#define UPCHECK_MANIFEST_DTD_PATH "http://www.slickedit.com/dtd/vse/upcheck/1.0/manifest.dtd"
#define UPCHECK_MANIFEST_VERSION "1.0"
...
static boolean _upcheckUpdateIsValidAttribute_Type(_str& value, boolean fix=false)
{
   return (value=="message");
}

static boolean _upcheckUpdateIsValidAttribute_TimeStamp(_str& value, boolean fix=false)
{
...
static boolean _upcheckUpdateIsValidAttribute_PackageName(_str& value, boolean fix=false)
{
   return (value!=null && value!="");
}

static boolean _upcheckUpdateIsValidAttribute_DisplayName(_str& value, boolean fix=false)
{
   return (value!=null && value!="");
}

static boolean _upcheckUpdateIsValidAttribute_Description(_str& value, boolean fix=false)
{
   return (value!=null && value!="");
}

static boolean _upcheckUpdateIsValidAttribute_Summary(_str& value, boolean fix=false)
{
   return (value!=null && value!="");
}

static boolean _upcheckUpdateIsValidAttribute_Version(_str& value, boolean fix=false)
{
...
static boolean _upcheckUpdateIsValidAttribute_Options(_str& value, boolean fix=false)
{
   if( value!=null ) {
      return true;
   }
...
static boolean _upcheckUpdateIsValidAttribute_Status(_str& value, boolean fix=false)
{
...
static boolean _upcheckUpdateIsValidAttribute_RemindTime(_str& value, boolean fix=false)
{
   if( isinteger(value) && value>=0 ) {
      return true;
   }
 ...
 */
static boolean _upcheckUpdateValidateAttribute(_str name, _str& value=null,
                                               boolean fix=false,
 ...
 * @return true if valid, false if not valid.
 */
static boolean _upcheckValidateUpdate(_str (&ht):[], boolean fix, boolean& modified)
{
   _str name;
   boolean modifiedAttribute;
 ...
 * @return true if valid, false if not valid.
 */
static boolean _upcheckxmlGetAndValidateUpdate(int handle, int node, _str (&ht):[], boolean fix, boolean& modified)
{
   ...
   boolean valid = _upcheckValidateUpdate(ht,fix,modified);
   return valid;
}
...
static boolean _upcheckUpdateSetIsValidAttribute_ProductId(_str& value, boolean fix=false)
{
   return (value=="vse" || value=="ep" || value=="stu" || value=="*");
}

static boolean _upcheckUpdateSetIsValidAttribute_PlatformId(_str& value, boolean fix=false)
{
...
static boolean _upcheckUpdateSetIsValidAttribute_ForProductVersion(_str& value, boolean fix=false)
{
   return (value!=null && value!="");
}

static boolean _upcheckUpdateSetIsValidAttribute_SetName(_str& value, boolean fix=false)
{
   if( value!=null ) {
      return true;
   }
 ...
 * @return true if valid, false if not valid.
 */
static boolean _upcheckUpdateSetValidateAttribute(_str name, _str& value=null,
                                                  boolean fix=false,
                                                  boolean& modified=null,
                                                  _str (&ht):[]=null)
{
 ...
 * @return true if valid, false if not valid.
 */
static boolean _upcheckValidateUpdateSet(_str (&ht):[], boolean fix, boolean& modified)
{
   _str name;
   boolean modifiedAttribute;
 ...
 * @return true if valid, false if not valid.
 */
static boolean _upcheckxmlGetAndValidateUpdateSet(int handle, int node,
                                                  _str (&ht):[],
                                                  boolean fix, boolean& modified)
{
   int status = _xmlcfg_get_attribute_ht(handle,node,ht);
   if( status ) {
   ...
   boolean valid = _upcheckValidateUpdateSet(ht,fix,modified);
   return valid;
}
...
{
   _str filename = _ConfigPath();
   if( last_char(filename)!=FILESEP ) filename=filename:+FILESEP;
   filename=filename:+"manifest.xml";
   return filename;
}
...
static boolean _upcheckIsOption(_str option,_str list)
{
...
static _str _upcheckFixupVersion(_str version)
{
   ...
   _str newVersion = major'.'minor'.'revision'.'build;
   return newVersion;
}
...
static int _upcheckGetRelevantUpdates(_str filename, upcheckUpdate_t (&list)[],
                                      int& status,
                                      boolean close=true, boolean append=false)
{
   status = 0;
   ...
   do {
      ...
      _str PlatformId;
      _upcheckGetPlatformId(PlatformId);
      _str UpdateSet_xpath = "/UpdateSet[@ProductId='"ProductId"'][@PlatformId='"PlatformId"'][@ForProductVersion='"ForProductVersion"']";
...
static void _upcheckFilterUpdatesOnStatus(upcheckUpdate_t (&list)[],
                                          _str statusValue, boolean remove=true)
{
   int i;
   for( i=0;i<list._length();++i ) {
 ...
 */
static boolean _upcheckMatchWithWildcard(_str stringA, _str stringB)
{
 ...
 */
static boolean _upcheckVersionsMatch(_str version1, _str version2)
{
   ...
   boolean majorMatch = _upcheckMatchWithWildcard(major1,major2);
   boolean minorMatch = _upcheckMatchWithWildcard(minor1,minor2);
   boolean revisionMatch = _upcheckMatchWithWildcard(revision1,revision2);
   boolean buildMatch = _upcheckMatchWithWildcard(build1,build2);
...
{
   _str fileName = "c:/development/vslick10/win/config/manifest.xml";
   int h, status;
   h = _xmlcfg_open(fileName, status);
   if (h < 0) {
...
static void _upcheckQEHandler_Manifest(upcheckQEvent_t *e_p)
{
   ...
   case UPCHECK_QS_WAITING_FOR_REPLY: {
      ...
      status=_upcheckReadResponse(&event.ucc,line);
      if( status ) {
         _str msg = "MANIFEST: "get_message(status);
         _upcheckEnQ(event.event,UPCHECK_QS_ERROR,0,&event.ucc,msg);
         return;
      }
      if( line=="" ) {
         // Check for timeout
         if( _timedOut(event.start,UPCHECK_TIMEOUT) ) {
            _str msg = "MANIFEST: "get_message(VSRC_UPCHECK_TIMED_OUT);
            _upcheckEnQ(event.event,UPCHECK_QS_ERROR,0,&event.ucc,msg);
            return;
         }
         // No response yet, so re-queue
...
{
   double buildDate = 0;
   _str line = _getProductBuildDate();
   _str month, day, year;
...
static int onNext_ListAvailable(boolean validate);
static int onFinish_ListAvailable(boolean validate);

typedef void (*pfnOnCreate_tp)(upcheckUpdate_t list[]);
typedef void (*pfnOnResize_tp)();
typedef int (*pfnOnNext_tp)(boolean);
typedef int (*pfnOnFinish_tp)(boolean);
   ...
   if( list!=null ) {
      ...
      for( i=0;i<list._length();++i ) {
         if( list[i].Status!="installed" ) {
            _str caption = list[i].DisplayName"\t"list[i].Version"\t"list[i].Status"\t"list[i].PackageName;
...
static void onResize_ListAvailable()
{
   ...
   int vgap_update_list = ctl_ignore_selected.p_y - (ctl_update_list.p_y + ctl_update_list.p_height);
   // Preserve original gap between ctl_ignore_selected and ctl_description_caption
   int vgap_ignore_selected = ctl_description_caption.p_y - (ctl_ignore_selected.p_y + ctl_ignore_selected.p_height);
   // Preserve original gap between ctl_description_caption and ctl_description
   int vgap_description_caption = ctl_description.p_y - (ctl_description_caption.p_y + ctl_description_caption.p_height);
   ...
   //
   // ctl_update_list
   ctl_update_list.p_width = containerWidth - ctl_update_list.p_x - hgap_right;
   // ctl_ignore_selected
   ctl_ignore_selected.p_y = ctl_update_list.p_y + ctl_update_list.p_height + vgap_update_list;
   // ctl_description_caption
   ctl_description_caption.p_y = ctl_ignore_selected.p_y + ctl_ignore_selected.p_height + vgap_ignore_selected;
   // ctl_description
   ctl_description.p_y = ctl_description_caption.p_y + ctl_description_caption.p_height + vgap_description_caption;
   ...
   while( index>=0 ) {
      _str caption = ctl_update_list._TreeGetCaption(index);
...
{
   int index = ctl_update_list._TreeCurIndex();
   if( index >=0 ) {
...
void _upcheck_form.on_resize()
{
   ...
   gSlides[gCurrentSlide].wid.p_y=ctl_header.p_y+ctl_header.p_height;
   ...
   int finish_gap = ctl_cancel.p_x - (ctl_finish.p_x + ctl_finish.p_width);
   // Preserve original gap between Next and Finish
   int next_gap = ctl_finish.p_x - (ctl_next.p_x + ctl_next.p_width);
   // Preserve original gap between Back and Next
   int back_gap = ctl_next.p_x - (ctl_back.p_x + ctl_back.p_width);
   // Re-position the navigation buttons
   ctl_cancel.p_x = ctl_navigation.p_x + ctl_navigation.p_width - ctl_cancel.p_width - UPCHECK_GAP_BORDER_X;
...
void ctl_finish.on_create()
{
   ...
   for( i=0;i<gSlides._length();++i ) {
      int wid = p_active_form._find_control(gSlides[i].name);
      wid.p_visible=false;
      gSlides[i].wid=wid;
...
{
   _str caption = "New update(s) available.";
   // Count new updates
   int nofupdates = 0;
   if( list!=null ) {
      int i;
...
static void _upcheckdlgFillFetchIntervals()
{
   ...
   if( isinteger(def_upcheck_fetch_interval) ) {
      ...
      if( current_interval==0 ) {
         ...
         current_interval = UPCHECK_DEFAULT_FETCH_INTERVAL;
      } else if( abs(current_interval-UPCHECK_INTERVAL_1DAY)<=MSMINUTES(59) ) {
         // Close enough, so call it 1 day
         current_interval=UPCHECK_INTERVAL_1DAY;
      } else if( abs(current_interval-UPCHECK_INTERVAL_1WEEK)<=MSHOURS(23) ) {
   ...
   _str current_caption = _upcheckIntervalValueToCaption(current_interval);
   if( current_caption=="" ) {
      // Should never get here
      _str msg = "Invalid interval";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return;
   }
...
void ctl_ok.lbutton_up()
{
   ...
   _str interval_caption = ctl_fetch_interval.p_text;
