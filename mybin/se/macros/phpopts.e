...
static boolean isAppTypeWeb(_str appType)
{
   return ( substr(appType,1,length("web-")) == "web-" );
}

static boolean isAppTypeCli(_str appType)
{
   return ( substr(appType,1,length("cli-")) == "cli-" );
}

static boolean isAppTypeLocal(_str appType)
{
   return ( length(appType) > length("-local") &&
            substr(appType,length(appType)-length("-local")+1) == "-local" );
}

static boolean isAppTypeRemote(_str appType)
{
...
static void setProjectOptionsForConfig(int projectHandle, _str config, PhpOptions& php_opts, XdebugOptions& xdebug_opts)
{
   ...
   if( isAppTypeWeb(php_opts.appType) ) {
      ...
      _str cmdline = '"%(SLICKEDIT_PHP_EXE)"';
      if( php_opts.interpreterArgs != "" ) {
         cmdline = cmdline' 'php_opts.interpreterArgs;
      }
      if( php_opts.defaultFile != "" ) {
         cmdline = cmdline' 'maybe_quote_filename(php_opts.defaultFile)' %~other';
      } else {
...
{
   _str result = "";
   _str longestLocalRoot = "";
   _str longestRemoteUrl = "";
   ...
   foreach( auto ii=>map in fileMap ) {
      ...
      if( length(localRoot) < length(local_filename) &&
          file_eq(substr(local_filename,1,length(localRoot)),localRoot) ) {
                         ...
                         boolean quiet=false, _str& error_hint=null)
{
   ...
   boolean isPhpProject = ( strieq("php",_ProjectGet_Type(projectHandle,config)) );
   if( isPhpProject ) {
      PhpOptions php_opts;
   ...
   _str phpDir = _strip_filename(phpExePath,'N');
 ...
 * @return 0 on success, <0 on error.
 */
_command int set_php_environment()
{
                                ...
                                _str& error_hint)
{
   ...
   do {
      ...
      if( xdebug_opts.stayInDebugger ) {
         debugger_args = debugger_args' -stay-in-debugger';
      }
      ...
      // DBGp features
      debugger_args = debugger_args' 'se.debug.dbgp.dbgp_make_debugger_args(xdebug_opts.dbgp_features);
   
      // Attempt to start debug session
      status = debug_begin("xdebug","","","",def_debug_timeout,null,debugger_args);
 ...
 * @return 0 on success, <0 on error.
 */
_command int php_execute(_str args="")
{
   if( _project_name == "" ) {
      // What are we doing here?
      _str msg = "No project. Cannot execute.";
   ...
   if( local_filename == "" ) {
      // Error
      _str msg = "No file. Cannot execute.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return INVALID_ARGUMENT_RC;
...
_command int php_debug()
{
   ...
   if( _project_DebugCallbackName != "xdebug" ) {
      // What are we doing here?
      _str msg = "Project does not support Xdebug. Cannot debug.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return INVALID_ARGUMENT_RC;
   }
   ...
   if( xdebug_opts.serverHost == "" || xdebug_opts.serverPort == "" ) {
      _str msg = "Invalid Xdebug parameters. Cannot debug.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return INVALID_ARGUMENT_RC;
   }
   ...
   do {
      ...
      boolean already_listening = xdebug_is_listening(xdebug_opts.serverHost,xdebug_opts.serverPort);
...
#define PHPOPTS_FORM_MINWIDTH  8175
#define PHPOPTS_FORM_MINHEIGHT 7950
...
static boolean changingRunAs(int value=-1)
{
...
static boolean changingAcceptConnections(int value=-1)
{
...
static boolean changingBreakInSession(int value=-1)
{
                                     ...
                                     XdebugOptions& default_xdebug_opts)
{
   ...
   for( i=0; i < n; ++i ) {
      ...
      _str mapline = localRoot"\t"remoteRoot"\t"remoteUrl;
...
void ctl_run_as.on_change(int reason)
{
   ...
   boolean is_web = isAppTypeWeb(appType);
...
{
   _str wildcards = "PHP Files (*.php;*.php3)";
   _str format_list = "";
...
_command void ctlinsertPhpInterpreterArg(_str text="", _str delim='%')
{
   _str opt;
   ...
   if( opt == '-c' || opt == '-d' ) {
      ...
      if( opt == '-c' ) {
         ...
         if( result != "" ) {
            ...
            result = '-c 'maybe_quote_filename(result);
         }
      } else {
         // -d foo[=bar]
         _str name, value;
...
void ctl_map_add.lbutton_up()
{
   ...
   boolean is_apptype_local = isAppTypeLocal(appType);
...
void ctl_mappings.on_change(int reason, int index=-1, int col=-1, _str text="")
{
   ...
   case CHANGE_EDIT_CLOSE:
      ...
      if( col == 0 && isAppTypeLocal(mapRunModeCaption2Value(ctl_run_as.p_text)) ) {
         ...
         _str mapline = _TreeGetCaption(index);
         _str remoteUrl;
...
void ctl_browse_php_exe.lbutton_up()
{
   ...
   _str init_dir = "";
   _str init_filename = ctl_php_exe_path.p_text;
   if( init_filename == "" ) {
...
void _php_options_form.on_load()
{
   ...
   if( setting_environment ) {
      _str phpExePath = ctl_php_exe_path.p_text;
      _str msg = "";
      if( phpExePath != "" ) {
         ...
         msg = msg :+ "Please set the PHP interpreter on the Options dialog that follows (\"Build\", \"PHP Options\").";
      }
...
_command void phpoptions(_str options="")
{
   if( _project_name == "" ) {
      // What are we doing here?
   ...
   typeless result = ctlbutton_wid.call_event('_php_options_form 'options,ctlbutton_wid,LBUTTON_UP,'w');
 ...
 */
static boolean skipSlideByNumber(int num, int new_value=-1)
{
 ...
 * @return True if slide will be skipped.
 */
static boolean skipUrlSlide(int new_value=-1)
{
 ...
 * @return True if slide will be skipped.
 */
static boolean skipLocalSlide(int new_value=-1)
{
 ...
 * @return True if slide will be skipped.
 */
static boolean skipRemoteSlide(int new_value=-1)
{
...
static int phpwiz_slide2_next()
{
   ...
   PhpOptions* php_opts = _GetDialogInfoHtPtr("PhpOptions",ctlslide0.p_active_form);
   XdebugOptions* xdebug_opts = _GetDialogInfoHtPtr("XdebugOptions",ctlslide0.p_active_form);
   boolean* add_local_root_files = _GetDialogInfoHtPtr("add_local_root_files",ctlslide0.p_active_form);

   _str localRoot = ctl_local_root.p_text;
   if( localRoot == "" ) {
      _str msg = "Invalid local root directory.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      ...
      _str msg = "Local root directory does not exist.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      p_window_id = ctl_local_root;
      _set_sel(1,length(p_text)+1);
      _set_focus();
...
static int phpwiz_slide3_next()
{
   ...
   _str remoteRoot = ctl_remote_root.p_text;
   if( remoteRoot == "" ) {
      _str msg = "Invalid remote root directory.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
...
static int phpwiz_slide4_shown()
{
   ...
   _str text = '';
   text = text :+ '<p style="font-family:Default Dialog Font; font-size:10">';
   if( isAppTypeWeb(php_opts->appType) ) {
      if( isAppTypeLocal(php_opts->appType) ) {
         text = text :+ '<dt><b>Application type:</b> <dd>Local web server</dd></dt>';
      } else {
         // Remote
         text = text :+ '<dt><b>Application type:</b> <dd>Remote web server</dd></dt>';
      }
      text = text :+ nls('<dt><b>Local directory:</b> <dd>%s</dd></dt>',php_opts->urlMap[0].localRoot);
      text = text :+ nls('<dt><b>URL:</b> <dd>%s</dd></dt>',php_opts->urlMap[0].remoteUrl);
      if( php_opts->urlMap[0].localRoot != xdebug_opts->remoteFileMap[0].remoteRoot ) {
         text = text :+ nls('<dt><b>Remote directory:</b> <dd>%s</dd></dt>',xdebug_opts->remoteFileMap[0].remoteRoot);
      }
   } else {
      text = text :+ '<dt><b>Application type:</b> <dd>Local script</dd></dt>';
   }
   text = text :+ nls('<dt><b>Debug listener:</b> <dd>%s on port %s</dd></dt>',xdebug_opts->serverHost,xdebug_opts->serverPort);
   text = text :+ '';
   text = text :+ '</p>';
   text = text :+ '<p style="font-family:Default Dialog Font; font-size:10">';
   if( *add_local_root_files ) {
      text = text :+ nls('<dt><b>Files will be added to project from:</b> <dd>%s</dd></dt>',php_opts->urlMap[0].localRoot);
   } else {
      text = text :+ '<dt><b>Files will not be added to project.</b> <dd>You can add files from Project>Project Properties menu</dd></dt>';
   }
   text = text :+ '</p>';
...
_command int php_project_wizard()
{
   WIZARD_INFO info;
