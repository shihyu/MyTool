...
#define COFFEESCRIPT_MODE_NAME   'CoffeeScript'
#define COFFEESCRIPT_LANGUAGE_ID 'coffeescript'
#define COFFEESCRIPT_LEXERNAME   'CoffeeScript'
#define COFFEESCRIPT_EXTENSION   'coffee'
#define COFFEESCRIPT_WORD_CHARS  'a-zA-Z0-9_$'
...
defload()
{
   _str setup_info='MN='COFFEESCRIPT_MODE_NAME',TABS=+4,MA=1 74 1,':+
                   'KEYTAB='COFFEESCRIPT_LANGUAGE_ID'-keys,WW=1,IWT=0,ST='DEFAULT_SPECIAL_CHARS',IN=2,WC='COFFEESCRIPT_WORD_CHARS',LN='COFFEESCRIPT_LEXERNAME',CF=1,';
   _str compile_info='';
   _str syntax_info='4 1 ':+   // <Syntax indent amount>  <expansion on/off>
                    '1 0 4 ':+ // <min abbrev> <not used> <not used>
                    '1 1';     //notused> <notused>
   _str be_info='';
   _CreateLanguage(COFFEESCRIPT_LANGUAGE_ID, COFFEESCRIPT_MODE_NAME, setup_info, compile_info, syntax_info, be_info);
   _CreateExtension(COFFEESCRIPT_EXTENSION, COFFEESCRIPT_LANGUAGE_ID);
   LanguageSettings.setAutoBracket(COFFEESCRIPT_LANGUAGE_ID, AUTO_BRACKET_ENABLE|AUTO_BRACKET_DEFAULT);
}
...
//
//   _str aliasfilename='';
//   _str word=min_abbrev2(orig_word,coffeescript_space_words,name_info(p_index),aliasfilename);
//
...
_str coffeescript_proc_search(_str &proc_name,boolean find_first)
{
   _str searchOptions = '@riXc';
   // allowed characters in function name
   _str procSearchName = '[a-zA-Z0-9\-]#';
   _str rePart1 = '^( |\t)@{#0(';
   _str rePart2 = ')}(\:| =)( (\({#1?#}\))|) \->';

   // are we looking for a specific function?
   if ( proc_name != '' ) {
   ...
   if (!status) {
      // Pick out the name of this function from the tagged expression #0
      int groupStart = match_length('S0');
      int groupLen = match_length('0');
      _str tempFoundProc = get_text(groupLen, groupStart);

      // the arguments come from the tagged expression #1
      _str arguments = '';
//    groupStart = match_length('S1');
//    groupLen = match_length('1');
//    _str arguments = get_text(groupLen, groupStart);
//      _str type_name = '';
      int tag_flags = 0;
      type_name := 'function';
...
_command void coffeescript_enter() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
...
#define COFFEESCRIPT_DEBUG_INDENT      0
#define COFFEESCRIPT_BLOCK_WORDS       ' catch class else finally for if switch try unless when while '
#define COFFEESCRIPT_POSSIBLE_POSTFIX  ' if unless while '
#define COFFEESCRIPT_PAREN_CHARS       "{}[]()"
#define COFFEESCRIPT_INDENT_SYMBOLS    ' -> = : => '
...
//static boolean coffeescript_lineEndsWithBackslash(int truncate_linenum=-1,int truncate_col=-1)
//{
//   linecont := false;
//   _str line;
...
static boolean coffeescript_thisLineNeedsContinuing()
{
...
static boolean coffeescript_isLineContinuedFromPrevious() 
{
   // save where we were
   save_pos(auto p);
 ...
 */
int coffeescript_smartpaste(boolean char_cbtype, int first_col,int Noflines,boolean allow_col_1=false)
{
   //_begin_select();up();
   //_end_line();
   //save_pos(auto p4);
