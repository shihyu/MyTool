...
#define EJB_SESSION_STATEFUL  0
#define EJB_SESSION_STATELESS 1
#define EJB_ENTITY_BMP        2
#define EJB_ENTITY_CMP        3

#define DEPLOYMENT_DESCRIPTOR_FILENAME 'ejb-jar.xml'

static int ejb_slide1add();
static int ejb_slide3add(_str CapData="\t\t",boolean checked=false);
...
static boolean ispkg_valid(_str name)
{
   ...
   if (isinteger(ch) || ch=='.' || last_char(name)=='.') {
      return(false);
   }
   return(true);
}
...
static int ejb_slide1add()
{
   int status=ejb_slide3add("ejbCreate\t",0);
   return(status);
}
...
#define EJB_FBPK_INDEX ctls3_tree.p_user
...
static int ejb_slide3shown()
{
   if (EJB_FBPK_INDEX!='') {
      ...
      int wid=p_window_id;
      p_window_id=ctls3_tree;
      _str cap=_TreeGetCaption(fbpk_index);
      _str keyinfo='';
...
static void GetSlide3Info()
{
   ...
   for (;;) {
      ...
      if (cur!='') {
         ...
         _str line=returnCollection"\t"name"\t"args;
   ...
   for (i._makeempty();;) {
      ...
      if (i._isempty()) break;
      int len=gEJBInfo.FinderMethods._length();
...
static void GetSlide4Info()
{
   ...
   for (i._makeempty();;) {
      ...
      int len=gEJBInfo.BusinessMethods._length();
   ...
   for (;;) {
      ...
      if (cur!='') {
         ...
         if (type!='' && name!='') {
            ...
            int len=gEJBInfo.EnvProperties._length();
...
static void GetSlide6Info()
{
   ...
   for (;;) {
      ...
      if (cur!='') {
         ...
         _str name='';
         parse cur with "\t" type "\t" name;
         int len=gEJBInfo.ConvStateFields._length();
...
static boolean NeedToImportCollection(EJB_INFO *pinfo)
{
   int i;
...
{
   _str filename=Path:+pinfo->Filenames.HomeFilename;
   ...
   if (pinfo->Type==EJB_ENTITY_BMP ||
      ...
      for (i=0;i<pinfo->FinderMethods._length();++i) {
         ...
         _str args=strip(pinfo->FinderMethods[i].Args,'B','(');
         args=strip(args,'B',')');
...
{
   _str filename=Path:+pinfo->Filenames.RemoteFilename;
   int status=WarnIfFileExists(filename);
   if (status) {
      return(status);
   }
   ...
   boolean needToImportCollection=false;
   int i;
   for (i=0;i<pinfo->FinderMethods._length();++i) {
      if (pinfo->FinderMethods[i].ReturnsCollection) {
   ...
   for (i=0;i<pinfo->BusinessMethods._length();++i) {
      _str args=strip(pinfo->BusinessMethods[i].Args,'B','(');
      args=strip(args,'B',')');
...
{
   _str filename=Path:+pinfo->Filenames.BeanFilename;
   int status=WarnIfFileExists(filename);
   if (status) {
      return(status);
   }
...
{
   _str filename=Path:+pinfo->Filenames.ClientFilename;
   int status=WarnIfFileExists(filename);
   if (status) {
      return(status);
   }
...
static boolean SuffixFileMatches(_str String,_str Suffix)
{
   if ( file_eq(Suffix,substr(String,length(String)-length(Suffix)) ) ) {
      return(true);
   }
   return(false);
}
...
static int GeneratePKClassFile(EJB_INFO *pinfo,_str Path,_str &FileList)
{
   ...
   int temp_view_id=0;
   int orig_view_id=_create_temp_view(temp_view_id);
   _str varname=lowcase(pinfo->Name)'ID';
   _SetEditorLanguage('java');
   indentAmount := LanguageSettings.getSyntaxIndent(p_LangId);
   _str indentstr=indent_string(indentAmount);
...
{
   _str ejbPath=get_env('VSROOT'):+'wizards':+FILESEP:+'java':+FILESEP:+'ejb':+FILESEP;
   _str Files[]=null;
   int ff;
   for (ff=1;;ff=0) {
      _str cur_filename=file_match(maybe_quote_filename(ejbPath:+'*.xml'),ff);
...
{
   _str destFilename=DestPath:+_strip_filename(SourceFilename,'P');
   ...
   int temp_view_id,orig_view_id=0;
   ...
   _str pkgName;
   if (file_eq(_strip_filename(SourceFilename,'P'),DEPLOYMENT_DESCRIPTOR_FILENAME)) {
   ...
   p_window_id=orig_view_id;
   _delete_temp_view(temp_view_id);
   FileList=FileList' 'maybe_quote_filename(destFilename);
   return(status);
}
...
_command int ejb_wizard()
{
   ...
   if (status=='') {
      ...
      _maybe_append_filesep(PackagePath);
      status=edit(maybe_quote_filename(PackagePath:+gEJBInfo.Filenames.BeanFilename));
   }
   return(status);
}
...
static void EnableDeleteButton()
{
   int index=_TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   if (index<0) {
      p_next.p_next.p_enabled=0;
   }else{
      p_next.p_next.p_enabled=1;
   }
}
...
{
   if (_TreeCurIndex()<0) {
      int wid=p_active_form._find_control('ctlnext');
      if (wid) {
...
{
   if (ctls2_pkexisting.p_value) {
      ctls2_pkcombo.p_enabled=1;
      ctls2_pktext.p_enabled=0;
   }else{
      ctls2_pkcombo.p_enabled=0;
      ctls2_pktext.p_enabled=1;
   }
}
