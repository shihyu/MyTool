...
struct CALIBInfo
{
   ...
   boolean genJCLOnly; // Flag: true to generate JCL only and not submit
   boolean waitForCompletion; // Flag: true to wait for JCL to complete
};
      ...
      while (1) {
         ...
         if (pos("%LIBMASTER",line)) {
            ...
            if (info.password != "") {
               ...
               newtext = newtext :+ "PSWD="info.password;
            }
            if (info.archive != "") {
               if (newtext != "") newtext = newtext :+ ",";
               newtext = newtext :+ "ARC="info.archive;
            }
            if (info.sequence != "") {
               if (newtext != "") newtext = newtext :+ ",";
               newtext = newtext :+ "SEQ="info.sequence;
            }
            if (info.misc != "") {
               if (newtext != "") newtext = newtext :+ ",";
               newtext = newtext :+ info.misc;
            }
            ...
            _str dsname = "//"info.dsname;
            if (info.member != "") dsname = dsname"/"info.member;
            status = get(dsname);
            if (status) {
...
static int calib_op(_str op, _str arg1, int maxTimeOut, _str prefix, _str owner, _str queue)
{
   ...
   _str userName;
   _userName(userName);
   _str tmpJCL = "/tmp/"userName".calib.jcl";
   ...
   if (last_char(prefix) != '*' && length(prefix) < 8) prefix = prefix"*";
   if (prefix == "") prefix = "*";
 ...
 */
_command calib_add(_str arg1="", int maxTimeOut=120, _str prefix="", _str owner="", _str queue="O")
{
 ...
 * @return 0 for OK, !0 error
 */
_command calib_checkin(_str arg1="", int maxTimeOut=120, _str prefix="", _str owner="", _str queue="O")
{
 ...
 * @return 0 for OK, !0 error
 */
_command calib_checkout(_str arg1="", int maxTimeOut=120, _str prefix="", _str owner="", _str queue="O")
{
 ...
 * @return
 */
_command calib_noop(_str arg1="", int maxTimeOut=120, _str prefix="", _str owner="", _str queue="O")
{
}
 ...
 */
static int checkAndLimit(int maxLength, boolean allowSpace=false, boolean upcasetext=false)
{
...
void ctlOK.on_create(_str sourceds="", _str lang="", _str mode="add")
{
   ...
   _str dsname = ctlSourceDS.p_text;
   if (dsname != "") {
      _str member = "";
   ...
   _str today = _date();
   int start = 1;
   _str month = substr(today, start, pos("/",today,1) - start);
   start = length(month) + 2;
   _str day = substr(today, start, pos("/", today, start) - start);
...
void ctlOK.lbutton_up()
{
   ...
   _str sourceds = ctlSourceDS.p_text;
   _str sourcemem = "";
   ...
   // Check master file
   _str libmaster = ctlMaster.p_text;
   _str libmember = "";
   ...
   // Check archive
   _str archive = ctlArchive.p_text;
   if (archive != "") {
      _str fc = substr(archive,1,1);
      if (isdigit(fc)) {
   ...
   _str cc = substr(info.sequence,1,1);
   if (cc == '/') {
      int p;
      _str sPart="",lPart="",iPart="",vPart="";
      int start = 2;
      ...
      if (p) {
         ...
         if (p) {
            ...
            if (p) {
               ...
               if (last_char(vPart) == '/') {
                  vPart = substr(vPart,1,length(vPart)-1);
               }
            } else {
      ...
      if (last_char(info.sequence) != '/') info.sequence = info.sequence"/";
