...
class HTMLAdaptiveFormattingScanner : se.adapt.AdaptiveFormattingScannerBase {
   ...
   protected void examineTags(int cap,int orig_flags,_str embedLex) {
      ...
      boolean doCasing=flags && (m_available & (AFF_TAG_CASING|AFF_ATTRIBUTE_CASING|AFF_VALUE_CASING|AFF_HEX_VALUE_CASING));
      if (!doCasing) return;
      ...
      int status=search(searchString,'r@xcs');
      int col;
      typeless prev_pt='';
      int prev_col=0;
      while (flags && !status) {
   ...
   protected void scanIndent(int flags, int cap,_str embedLex)
   {
      ...
      _str idChars=stranslate(p_identifier_chars,'',' ');
      ...
      flags=flags & (AFF_INDENT_WITH_TABS|AFF_SYNTAX_INDENT|AFF_TABS);
