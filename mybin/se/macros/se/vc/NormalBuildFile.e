...
class NormalBuildFile : IBuildOriginalFile {
...
_command void test_normalbuildfile() name_info(',')
{
   status := _open_temp_view('/tmp/diffScript',auto scriptWID,auto origWID);
   if ( status ) {
      _message_box("Could not open script status="status);
      return;
   }
   p_window_id=origWID;
   NormalBuildFile buildFile;
   buildFileWID := buildFile.buildOriginalFile("/home/dhenry/perforceTest1/Workspace1/perforceTest/fwindow.h",scriptWID);
   if ( buildFileWID<0 ) {
      _message_box('did not generate built file');
   } else {
      buildFileWID._save_file('/tmp/savedFile');
