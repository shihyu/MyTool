...
class GitBuildFile : IBuildOriginalFile {
   ...
   private int processNextChange(int scriptWID,int &lastMatchLine,int localFileWID,int builtFileWID,int numIn) {
      ...
      if ( substr(scriptLine,1,2)!='@@' || !pos('@@',scriptLine,3) ) {
         return -1;
      }
...
_command void test_subversionbuildfile() name_info(',')
{
   status := _open_temp_view('c:\temp\diffscript.txt',auto scriptWID,auto origWID);
   if ( status ) {
      _message_box("Could not open script status="status);
      return;
   }
   p_window_id=origWID;
   GitBuildFile buildFile;
   buildFileWID := buildFile.buildOriginalFile("c:\\src\\VCTests\\subversion\\trunk\\appnamepatch.cpp",scriptWID);
   if ( buildFileWID<0 ) {
      _message_box('did not generate built file');
   } else {
      buildFileWID._save_file('+o c:\temp\savedFile.txt');
      _delete_temp_view(buildFileWID);
      diff("c:\\src\\VCTests\\subversion\\trunk\\appnamepatch.cpp c:\\temp\\savedFile.txt");
   }
