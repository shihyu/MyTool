...
#import "se/vc/VCRepositoryCache.e"
 ...
 *              values.
 */
_command void DumpDatabaseContents(_str svnURL="", int flags=255)
{
   // get the cache for slickedit for now
   VCRepositoryCache cache = g_svnCacheManager.getSvnCache(svnURL);
 ...
 */
_command void GetFilesForRevision(_str svnURL="", int revisionID=0)
{
   VCFile files[];
 ...
 * @param fileName : the full path of the file to get revision history for.
 */
_command void GetRevisionsForFile(_str fileName="") name_info(FILE_ARG'*')
{
   VCBranch root;
   VCLabel labels[];
   _str url = "";
 ...
 */
boolean isSvnVersionOkForVCCache()
{
   ...
   int tokenPosBegin = pos('version', outdata);
   if (tokenPosBegin) {
      // add 8 (the length of 'version ')
      tokenPosBegin += 8;
      // search for the next whitespace after 'version ' (8 chars)
      int tokenPosEnd = pos(' ', outdata, tokenPosBegin);
      if (tokenPosEnd) {
         // alright, let's get the version number
         _str versionNumber = substr(outdata, tokenPosBegin, tokenPosEnd - tokenPosBegin);
 ...
 */
_command void repair_version_cache(_str svnURL="")
{
   // get the cache for slickedit for now
   VCRepositoryCache cache = g_svnCacheManager.getSvnCache(svnURL);
   // repair the version cache
   boolean success = cache.repairCacheFile();
   if (success == true) {
 ...
 * is in a bad state and needs to be rebuilt. 
 */
_command void is_branch_history_available(_str svnURL="")
{
   // get the cache for slickedit for now
   VCRepositoryCache cache = g_svnCacheManager.getSvnCache(svnURL);
   // repair the version cache
   boolean success = cache.isSvnCapable();
   if (success == true) {
