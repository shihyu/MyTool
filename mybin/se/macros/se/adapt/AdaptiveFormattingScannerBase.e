...
class AdaptiveFormattingScannerBase {
    ...
    * 
    * @return boolean      whether setting is available
    */
   public boolean isSettingAvailable(AdaptiveFormattingFlags f)
   {
...
#region Calculate Results Methods
   ...
   protected int calculateBraceStyle(int &braceStyle)
   {
      ...
      int incidence=most;
      if ( !total || (incidence*100 / total)<def_adaptive_formatting_min_confidence) {
   ...
   protected int calculateIndentSettings(AFIndentSettings &is)
   {
      ...
      if (total<def_adaptive_formatting_min_indent_total) {
         ...
         indent=AF_RC_ERROR;
      } else if ( !total || (incidence*100 / total)<def_adaptive_formatting_min_confidence) {
         //_message_box('not confident incidence='incidence' total='total' r='((incidence / total)*100));
         indent=AF_RC_ERROR;
      }
      ...
      */
      ...
      if (langOptions != null ) {
         parse langOptions.szTabs with auto n1 auto n2 auto rest;
         if (!(rest=='' && isinteger(n1) && isinteger(n2))) {
            parse langOptions.szTabs with n1 '+' n2 rest;
      ...
      if (m_tally_Indent_Tabs || m_tally_Indent_Spaces) {
         ...
         } else if ( !tab_space_total || (tab_space_incidence*100 / tab_space_total)<def_adaptive_formatting_min_confidence) {
            tab_space_error=true;
         }
         if (tab_space_error) {
   ...
   protected int calculateParenSettings(AFParenSettings &ps)
   {
      ...
      error := (!total || (incidence*100 / total)<def_adaptive_formatting_min_confidence);
      if (!error) {
      ...
      boolean error2=(!total || (incidence*100 / total)<def_adaptive_formatting_min_confidence);
      if (!error2) {
   ...
   protected int calculateKeywordCasing(int &keywordCasing, _str wordType = 'K')
   {
      ...
      int total=lower+upper+preserve;
      int incidence=most;
      if ( !total || (incidence*100 / total)<def_adaptive_formatting_min_confidence) {
         //_message_box('not confident incidence='incidence' total='total' r='((incidence / total)*100));
         return(AF_RC_ERROR);
...
#region Get Setting Methods
    ...
    */
   public int getBraceStyle(int &braceStyle, boolean doScan = true)
   {
    ...
    *                      available for extension
    */
   public int getIndentSettings(AFIndentSettings &is, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getParenSettings(AFParenSettings &ps, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getIndentCaseFromSwitch(int &indentCase, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getKeywordCasing(int &casing, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getTagCasing(int &casing, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getAttributeCasing(int &casing, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getValueCasing(int &casing, boolean doScan = true)
   {
      // make sure this setting is available
    ...
    *                      available for extension
    */
   public int getHexValueCasing(int &casing, boolean doScan = true)
   {
      // make sure this setting is available
...
#region Statistics
   ...
   private _str compileStat(double value, int total, _str caption)
   {
      ...
      // format to #.## 
      _str strPer = percent;
      int decPos = pos('.', strPer);
      if (!decPos) {
         strPer = strPer :+ '.' :+ substr('', 1, 2, '0');
      } else if (strPer._length() - decPos < 2) {
         strPer = strPer :+ substr('', 1, strPer._length() - decPos, '0');
      }
