...
class ColorInfo {
    ...
    */
   boolean matchesColor(int colorId, IColorCollection *cc=null) {
      ColorInfo colorInfo;
    ...
    */
   void insertMacroCode(_str cfgName, boolean macroRecording) {
      fontFlagsString := fontFlagsToString(m_fontFlags);
      if (fontFlagsString == "") fontFlagsString=0;
      if (macroRecording) {
         _macro_append('_default_color('cfgName','dec2hex(m_foreground)','dec2hex(m_background)','fontFlagsString');');
      } else {
         insert_line('  _default_color('cfgName','dec2hex(m_foreground)','dec2hex(m_background)','fontFlagsString');');
      }
   }
