...
class MessageBrowser : se.lineinfo.LineInfoBrowser {
   ...
   void refreshMessages()
   {
      ...
      if (mCollection) {
         ...
         for (i = 0; i < m_messages._length(); ++i) {
            ...
            m_types:[m_messages[i]->m_type][m_types:[m_messages[i]->m_type]._length()] = i;
         }
      }

      //Update the toolwindow.
      int wid = _find_formobj('_tbmessages_browser_form', 'n');
      if ((wid > 0) && _iswindow_valid(wid)) {
         wid.refreshMessageBrowser(this);
      }
   }
    ...
    * 
    */
   public boolean goToMessageCodeLocation(int index)
   {
      ...
      if (m_messages[index]->m_sourceFile != '') {
         ...
         sourceFile = maybe_quote_filename(m_messages[index]->m_sourceFile);
...
{
   if (_no_child_windows() || _mdi.p_child != _get_focus()) {
      return;
   }
   int wid = _find_formobj('_tbmessages_browser_form', 'n');
   if (wid <= 0 || !_iswindow_valid(wid)) {
   ...
   se.messages.MessageBrowser* messageBrowser = _GetDialogInfoHtPtr("messageBrowser", wid);
   if (!messageBrowser) {
      return;
   }
   ...
   */
   ...
   _str sLineOffset;
   parse _mdi.p_child.point() with sLineOffset .;
   long lineOffset = (long)sLineOffset;
   int lineLen=_mdi.p_child._line_length(true);
   _StreamMarkerFindList(sMarkers, _mdi.p_child, lineOffset, lineLen,
                         VSNULLSEEK, 0);
   se.messages.Message* msgPtr;
   int msgIndex;
   for (i = 0; i < sMarkers._length(); ++i) {
      if (mCollection->m_sMarkers._indexin(sMarkers[i])) {
         msgIndex = mCollection->m_sMarkers:[sMarkers[i]];
         mCollection->getMessage(msgPtr, msgIndex);
         if (msgPtr) {
            if (_mdi.p_child._lineflags() & MODIFY_LF) {
   ...
   _StreamMarkerFindList(scMarkers, _mdi.p_child, _mdi.p_child._nrseek(), 1,
                         VSNULLSEEK, 0);
   ...
   _str lastMessageMarker = _GetDialogInfoHt("lastMessage", p_active_form);
   if (lastMessageMarker != messageMarker) {
      wid.selectMessage(*messageBrowser, messageMarker);
      lastMessageMarker = messageMarker;
   }
}
...
void _tbmessages_browser_form.on_create()
{
   ...
   _creator_label.p_y = _creator_combo.p_y +
                        (_creator_combo.p_height - _creator_label.p_height)/2;
   _creator_combo.p_x = _creator_label.p_x + _creator_label.p_width + 60;
   _type_label.p_y = _creator_label.p_y;
   _type_combo.p_y = _creator_combo.p_y;

   _message_tree.p_x = _creator_label.p_x;
   _message_tree.p_y = _creator_combo.p_y + _creator_combo.p_height + 60;

   _message_tree.p_AlwaysColorCurrent = true;
   _message_tree.p_Gridlines = TREE_GRID_BOTH;
   _message_tree._TreeSetColButtonInfo(0, 1000, TREE_BUTTON_PUSHBUTTON, 0, "");
   _message_tree._TreeSetColButtonInfo(1, 500, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT_NUMBERS|
                                       TREE_BUTTON_SORT_COLUMN_ONLY, 0, "");
   _message_tree._TreeSetColButtonInfo(2, 500, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT|
                                       TREE_BUTTON_SORT_COLUMN_ONLY, 0, "Type");
   _message_tree._TreeSetColButtonInfo(3, 2000, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT_FILENAME|
                                       TREE_BUTTON_SORT_COLUMN_ONLY|
                                       TREE_BUTTON_IS_FILENAME, 0,
                                       "Source File");
   _message_tree._TreeSetColButtonInfo(4, 500, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT_NUMBERS|
                                       TREE_BUTTON_SORT_COLUMN_ONLY|
                                       TREE_BUTTON_AL_RIGHT, 0, "Line");
   _message_tree._TreeSetColButtonInfo(5, 2000, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT_COLUMN_ONLY|
                                       TREE_BUTTON_SORT, 0, "Description");
   _message_tree._TreeSetColButtonInfo(6, 500, TREE_BUTTON_PUSHBUTTON|
                                       TREE_BUTTON_SORT_COLUMN_ONLY|
...
void _tbmessages_browser_form.on_load()
{
   ...
   if (messageBrowser) {
      ...
      messageBrowser->refreshMessages();
   }
}
...
// See _tbSaveState__tbproctree_form and friends.
// 
void _tbSaveState__tbmessages_browser_form(typeless& state, boolean closing)
{
   _message_tree._TreeAppendColButtonInfo();
}

void _tbRestoreState__tbmessages_browser_form(typeless& state, boolean opening)
{
   _message_tree._TreeRetrieveColButtonInfo();
}
...
void _tbmessages_browser_form.on_resize()
{
   ...
   _type_label.p_x = _creator_combo.p_x + _creator_combo.p_width + 60;
   _type_combo.p_x = _type_label.p_x + _type_label.p_width + 60;
   _clear_button.p_x = _type_combo.p_x + _type_combo.p_width + 60;

   _message_tree.p_y = _clear_button.p_y + _clear_button.p_height + 60;
   _message_tree.p_width = width - 120;
   _message_tree.p_height = height - _message_tree.p_y - 60;
}
...
static void refreshMessageBrowser(se.messages.MessageBrowser& messageBrowser)
{
   ...
   typeless creator;
   for (creator._makeempty(); ; ) {
      messageBrowser.m_creators._nextel(creator);
      if (creator._isempty()) {
         break;
      }
   ...
   typeless type;
   for (type._makeempty(); ; ) {
      messageBrowser.m_types._nextel(type);
      if (type._isempty()) {
         break;
      }
...
static void maybeFilterMessages()
{
   ...
   _str creator = _creator_combo.p_text;
   _str type = _type_combo.p_text;
...
{
   _str menuName = '_message_tree_menu';
   ...
   int treeIDX = _message_tree._TreeCurIndex();
...
static void filterMessages(_str creator='', _str type='')
{
   ...
   while (index > 0) {
      ...
      parse messageText with . "\t" . "\t" typeText "\t" . "\t" . "\t" . "\t"
         creatorText "\t" .;
      ...
      _TreeGetInfo(index, showChildren, bm1, bm2, origFlags, lineNumber, TREENODE_HIDDEN);
...
static int getMessageIDX(int treeIDX)
{
   ...
   int treeWID = _find_object('_tbmessages_browser_form._message_tree', 'n');
...
static _str makeMessageCaption(se.messages.Message* msg, int i)
{
   _str row = " \t "i"\t ";
   row = row:+msg->m_type"\t ";
   row = row:+msg->m_sourceFile"\t ";
   row = row:+msg->m_lineNumber"\t ";
   row = row:+msg->m_description"\t ";
   row = row:+msg->m_creator;
...
{
   boolean isRowColorSet = false;
   if (messageBrowser.m_messages[msgIDX]->m_lineModified) {
      windowID._TreeSetRowColor(treeIDX, hex2dec(def_message_modified_color), 0,
                                F_INHERIT_BG_COLOR);
      isRowColorSet = true;
   }
   if (!isRowColorSet && messageBrowser.m_messages[msgIDX]->m_lineVisited) {
      windowID._TreeSetRowColor(treeIDX, hex2dec(def_message_visited_color), 0,
                                F_INHERIT_BG_COLOR);
      isRowColorSet = true;
   }
   if (!isRowColorSet) {
