...
class ServerConnectionObserverDialog : ServerConnectionObserver {
   ...
   private int create() {
      ...
      do {
         ...
         x = (m_formwid.p_client_width*_twips_per_pixel_x() - 1125) / 2;
         y = (m_formwid.p_client_height*_twips_per_pixel_y() - 345 - message_wid.p_height - 180) / 2;
         width = 1125;
         height = 345;
   ...
   protected void printMessage(_str msg) {
      if( isStarted() ) {
         int msg_wid = m_formwid._find_control("ctl_message");
         if( msg_wid > 0 ) {
