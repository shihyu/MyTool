...
#region Imports
#include "slick.sh"
#include "se/debug/xdebug/xdebugattach.sh"
 ...
 */
boolean xdebug_is_pending(_str host, _str port)
{
 ...
 */
int xdebug_wait(_str host, _str port, int timeout, ServerConnectionObserver* observer, boolean close_on_error)
{
 ...
 */
int xdebug_wait_and_accept(_str host, _str port, int timeout, ServerConnectionObserver* observer, boolean close_if_first)
{
 ...
 */
boolean xdebug_is_listening(_str host, _str port)
{
 ...
 */
boolean xdebug_get_server_address(_str& host, _str& port)
{
 ...
 * @return 0 on success, <0 on error.
 */
_command int xdebug_project_watch(_str projectName=_project_name, boolean honorListenInBackground=false)
{
   ...
   if( status != 0 ) {
      if( status != COMMAND_CANCELLED_RC ) {
         _str msg = "Invalid Xdebug host/port. Cannot listen for connection.";
 ...
 *                     name.
 */
_command void xdebug_project_shutdown(_str projectName=_project_name)
{
   _str id_found = null;
   _str prefix = "";
   if( projectName != "" ) {
...
_command void xdebug_project_watch_toggle(_str projectName=_project_name)
{
   _str id_found = null;
   _str prefix = "";
   if( projectName != "" ) {
 ...
 */
void _project_close_xdebug(_str projectName)
{
   if( _project_DebugCallbackName != "xdebug" ) {
      return;
   }
