...
/**
 * Quick and dirty way of peeking the next packet from a perl5db 
 ...
 */
int perl5db_peek_packet(int socket)
{
   // For now this is just a pass-thru to dbgp_peek_packet()
   // since there is nothing perl5db-specific we need to know.
   // That could change later, hence this function.
   return se.debug.dbgp.dbgp_peek_packet(socket);
}

/**
 * Are we in the middle of starting a perl5db debug session?
 * 
 * @param new_value  Set to 1 (true) to indicate that a perl5db 
 *                   debug session is starting. Set to 0 (false)
 *                   to indicate a debug session is not
 *                   starting. Set to -1 to retrieve the current
 *                   state. Defaults to -1.
 * 
 * @return Current state of perl5db debug session start.
 */
boolean perl5db_almost_active(int new_value=-1)
{
