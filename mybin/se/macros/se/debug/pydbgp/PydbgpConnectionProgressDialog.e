...
class PydbgpConnectionProgressDialog : ServerConnectionObserverDialog {

   /**
    * Constructor.
    */
   PydbgpConnectionProgressDialog() {
   }
   ...
   private void onStatusListen(IServerConnection* server) {
      ...
      printMessage(nls("Waiting for pydbgp connection on %s:%s...%s seconds",host,port,remain));
   }

   private void onStatusPending(IServerConnection* server) {
      printMessage("pydbgp connection pending");
   }
