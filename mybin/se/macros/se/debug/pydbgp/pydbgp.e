...
#include "se/debug/pydbgp/PydbgpOptions.e"
#import "debug.e"
#import "main.e"
#import "stdprocs.e"
...
PydbgpOptions pydbgp_make_default_options(PydbgpOptions& pydbgp_opts=null)
{
 ...
 */
void pydbgp_project_get_options_for_config(int projectHandle, _str config, PydbgpOptions& pydbgp_opts, PydbgpOptions& default_pydbgp_opts=null)
{
   ...
   boolean listenInBackground = default_pydbgp_opts.listenInBackground;
   PydbgpRemoteFileMapping remoteFileMap[] = default_pydbgp_opts.remoteFileMap;
 ...
 *                             options present for config.
 */
void pydbgp_project_get_options(int projectHandle, _str (&configList)[], PydbgpOptions (&pydbgp_opts_list):[], PydbgpOptions& default_pydbgp_opts=null)
{
 ...
 */
boolean pydbgp_is_pending(_str host, _str port)
{
 ...
 */
int pydbgp_wait(_str host, _str port, int timeout, ServerConnectionObserver* observer, boolean close_on_error)
{
 ...
 */
int pydbgp_wait_and_accept(_str host, _str port, int timeout, ServerConnectionObserver* observer, boolean close_if_first)
{
 ...
 */
boolean pydbgp_is_listening(_str host, _str port)
{
 ...
 */
boolean pydbgp_get_server_address(_str& host, _str& port)
{
...
static int pydbgp_get_options(_str projectName, PydbgpOptions& pydbgp_opts)
{
   if( projectName != "" ) {
 ...
 * @return 0 on success, <0 on error.
 */
_command int pydbgp_project_watch(_str projectName=_project_name, boolean honorListenInBackground=false)
{
   ...
   if( status != 0 ) {
      if( status != COMMAND_CANCELLED_RC ) {
         _str msg = "Invalid pydbgp host/port. Cannot listen for connection.";
 ...
 *                     name.
 */
_command void pydbgp_project_shutdown(_str projectName=_project_name)
{
   _str id_found = null;
   _str prefix = "";
   if( projectName != "" ) {
...
_command void pydbgp_project_watch_toggle(_str projectName=_project_name)
{
   _str id_found = null;
   _str prefix = "";
   if( projectName != "" ) {
 ...
 * existing pydbgp servers for this project and start the server
 * for the active config. 
 */
void _prjconfig_pydbgp()
{
   ...
   pydbgp_project_shutdown(_project_name);

   // Start up servers for new active config.
   _str DebugCallbackName = _ProjectGet_DebugCallbackName(_ProjectHandle(_project_name));
   //say('_prjconfig_pydbgp : DebugCallbackName='DebugCallbackName);
   if( DebugCallbackName == "pydbgp" ) {
      pydbgp_project_watch(_project_name,true);
   }
}

/**
 * Called when a project is opened. Start up any pydbgp servers 
 * for this project. 
 */
void _prjopen_pydbgp()
{
   if( _project_DebugCallbackName != "pydbgp" ) {
      return;
   }
 ...
 */
void _project_close_pydbgp(_str projectName)
{
   if( _project_DebugCallbackName != "pydbgp" ) {
      return;
   }
