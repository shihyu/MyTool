...
class PydbgpConnectionMonitor : ServerConnectionObserver {
   ...
   private void onStatusListen(IServerConnection* server) {
      ...
      _str msg = nls("Listening for pydbgp connection on %s:%s...",host,port);
      if( msg != m_message ) {
   ...
   private void onStatusPending(IServerConnection* server) {
      ...
      if( debug_active() ) {
         ...
         _str msg = "Another process/thread is attempting to initiate a debug session.\n\nAllow?";
      ...
      sticky_message("pydbgp connection pending. Starting debugger...");
      // Start debugging
      if( !debug_active() && _project_DebugCallbackName == "pydbgp" ) {
         pydbgp_almost_active(1);
         int status = project_debug();
         pydbgp_almost_active(0);
         if( status != 0 ) {
            // Aborted
            // Accept and close
            int socket = server->accept();
         ...
         if( socket < 0 ) {
            // Error
            _str msg = nls("Failed to accept an pydbgp connection. %s",socket);
            sticky_message(msg);
         ...
         int handle = pydbgp_peek_packet(socket);
         if( handle < 0 ) {
            // Error
         ...
         pydbgp_almost_active(1);
         int status = debug_remote("pydbgp",attach_info);
         pydbgp_almost_active(0);
         if( status != 0 ) {
            // Error
            server->shutdown();
   ...
   private void onStatusError(IServerConnection* server) {
      ...
      _str msg = "Error waiting for pydbgp connection: ":+get_message(error_rc);
