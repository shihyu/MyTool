...
class PathMapperIterator : IPathMapperIterator {
   ...
   private boolean m_use_default_keys = false;
...
const PMI_FIELD_PATH_MAPPER = 0;
const PMI_FIELD_KEY = 1;
const PMI_FIELD_PATH = 2;
const PMI_FIELD_PATH_TO_MATCH = 3;
const PMI_FIELD_USE_DEFAULT_KEYS = 4;
...
class PathMapper : IPathMapper {
   ...
   private static boolean match(_str wildcard, _str path, _str search_opts, int& strength);
   private IPathMapperIterator make_iter(_str key, _str path, _str path_to_match, boolean use_default_keys);
   ...
   public void setCaseSensitive(int caseSensitive) {
      ...
      if( caseSensitive < 0 ) {
         ...
         m_search_options = m_search_options:+_fpos_case;
      } else if( caseSensitive == 0 ) {
         m_search_options = m_search_options:+'i';
      }
   }
    ...
    *                            to false.
    */
   public void removeByPath(_str path, boolean removeDefaultPaths=false) {
    ...
    *                            to false.
    */
   public void removeByPathSpec(_str path_spec, boolean removeDefaultPaths=false) {
    ...
    *         match found.
    */
   public typeless getByPath(_str path, boolean useDefaultPaths=false) {
      if( useDefaultPaths ) {
    ...
    */
   private static boolean match(_str pattern, _str path, _str search_opts, int& strength) {
      ...
      for( i=0; i < n; ++i ) {
         ...
         if( verify(search_for,'*?') > 0 ) {
         ...
         } else if( last_char(search_for) == '*' && (i+1) < n ) {
            ...
            search_for = search_for:+pat_array[++i];
    ...
    *         retrieve the key of the match.
    */
   public IPathMapperIterator mapNext(IPathMapperIterator where, boolean fail_fast=false) {
      ...
      boolean use_default_keys = where._getfield(PMI_FIELD_USE_DEFAULT_KEYS);
      Map* pkeys;
      if( use_default_keys ) {
