...
#define OPTIONS_IMPORT_ERROR_MESSAGE_SOURCE        'Import Options'
#define OPTIONS_IMPORT_ERROR_MESSAGE_TYPE          'Import Options Error Message';

#define OPTIONS_IMPORT_ALLOWED_VERSION_DIFFERENCE  2
...
class OptionsImportTree : OptionsTree {
      ...
      if (OptionsTree.init(treeID, frameID, helpID, file)) {
         ...
         if (m_parser.checkCrossPlatformImport(auto curPlat, auto importPlat)) {
            ...
            result := _message_box('The options you have selected to import were exported from '_cap_word(importPlat)'.  ':+
                                   'You are currently using '_cap_word(curPlat)'.  Because of differences between platforms, ':+
                                   'this is not supported.  Continue with import anyway?',
         ...
         se.options.ExportImportGroupManager temp(EIIS_TRUE, &m_parser);
         m_imports = temp;
    ...
    */
   private boolean checkExportFileVersion()
   {
      ...
      if (diff == 0) {
         goAhead = true;
      } else if (diff <= OPTIONS_IMPORT_ALLOWED_VERSION_DIFFERENCE) {
         ...
         result := _message_box('The options you have selected to import were exported from a different version of ':+
                      'SlickEdit ('version'). Because of possible changes between versions, this is not supported.  ':+
         ...
         result := _message_box('The options you have selected to import were exported from a different version of ':+
                      'SlickEdit ('version'). Because of possible changes, exporting from a version ':+
                      'more than two versions removed from the current version is not allowed.',
   ...
   public boolean apply(boolean restoreCurrent = true, boolean applyCurrent = true)
   {
      ...
      progressWid := show_cancel_form("Backing up configuration files...", null, false, true);
      progressWid.refresh();
      backupConfig();
                  ...
                  if (properties:[pIndex] == EIIS_TRUE) {
                     ...
                     if (importValue != null) {
                        ...
                        if ((p -> getPropertyType() == FILE_PATH_PROPERTY || 
                           ...
                           if (error != '') {
                              logErrors(mCollection, caption' > 'p -> getCaption(), error);
                              applySuccess = false;
                           }
                        ...
                        if (doImportProperty) {
                           if (PropertyGetterSetter.setSetting(info, importValue)) {
                              ...
                              if (propChangeEvents & OCEF_WRITE_COMMENT_BLOCKS) commentLangs:[p -> getLanguage()] = p -> getLanguage();
                           } else {
                              logErrors(mCollection, caption' > 'p -> getCaption(), 'Error applying property.');
                              applySuccess = false;
                           }
                        }
                     }
      ...
      _config_modify_flags(CFGMODIFY_DEFVAR|CFGMODIFY_OPTION|CFGMODIFY_KEYS);

      handleChangeEvents(changeEvents, commentLangs);

      // we ONLY do these things if everything was hunky dory
      if (!applySuccess) {
         _message_box(nls("There were errors importing your options.  Please see the Message List for detailed information."));
         activate_messages();
      }
    ...
    *                      otherwise.
    */
   private boolean warnAboutProtections()
   {
   ...
   private se.messages.MessageCollection * setupErrorLog()
   {
      ...
      mCollection -> removeMessages(OPTIONS_IMPORT_ERROR_MESSAGE_SOURCE);

      // create a message type
      Message newMsgType;
      newMsgType.m_creator = OPTIONS_IMPORT_ERROR_MESSAGE_SOURCE;
      newMsgType.m_type = OPTIONS_IMPORT_ERROR_MESSAGE_TYPE;
   ...
   private void logErrors(se.messages.MessageCollection * mCollection, _str caption, _str log)
   {
      ...
      foreach (auto errorMsg in errors) {
         ...
         if (strip(errorMsg)) {
            ...
            Message newMsg;
            newMsg.m_creator = OPTIONS_IMPORT_ERROR_MESSAGE_SOURCE;
            newMsg.m_type = OPTIONS_IMPORT_ERROR_MESSAGE_TYPE;
            newMsg.m_description = 'Error importing 'caption':  'errorMsg;
    ...
    */
   public boolean areAnyImportedItemsProtected()
   {
