...
class Condition : IPropertyDependency
{
   ...
   private boolean m_equals;                // whether this is a condition or not condition
    ...
    * 
    */
   Condition(ConditionType type = 0)
   {
      m_type = type;
      m_info = "";
      m_value = "";
    ...
    */
   private boolean checkFlagType(typeless returnVal)
   {
      // this better be an int...
      return ((returnVal & (int)m_value) != 0);
   }
   
   private boolean checkStringType(typeless returnVal)
   {
   ...
   private boolean doesCallbackExist()
   {
   ...
   private boolean doesLanguageInheritFromParent()
   {
      // make sure we even have a language
      if (m_language != '') {
