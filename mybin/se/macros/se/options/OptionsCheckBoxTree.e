...
class OptionsCheckboxTree : CheckboxTree {
    ...
    */
   public void registerPropertySheet(int xmlIndex, int psHandle, boolean changeable)
   {
      // save this handle for later
      m_propertySheets:[xmlIndex] = psHandle;

      // make sure the property sheet has a handle to us
      _SetDialogInfoHt(CHECKBOXTREE, &this, psHandle, true);
    ...
    */
   public void refreshPropertySheet(int xmlIndex, boolean changeable)
   {
    ...
    */
   private boolean propertySheetHasProtections()
   {
    ...
    */
   private int getProtectionStatusForDescription(_str desc)
   {
      status := -1;
   ...
   protected void changeNodeStatus(int treeIndex, int status)
   {
      ...
      if (xmlIndex) {
         m_statusData -> setItemStatus(xmlIndex, status);
   ...
   protected void changeNodeChildrenStatus(int treeIndex, int status)
   {
      ...
      if (child > 0) {
         ...
         xmlIndex := m_relations -> getXMLIndex(treeIndex);
         m_statusData -> changeXMLNodeChildrenStatus(xmlIndex, status);
      }
   ...
   protected int getNextSibling(int index)
   {
      ...
      while (sib > 0) {
         // see if this is colored like a property group
         m_treeHandle._TreeGetColor(sib, 1, auto fg, auto bg, auto flags);

         // no, break out
         if (bg != PROPERTY_GROUP_COLOR) break;
