...
#define EMULATION_FORM_NAME   '_emulation_form'
...
class OptionsConfigTree : OptionsTree {
   ...
   private boolean m_doProcessPanels = true;
    ...
    * 
    * 
    * @return boolean      whether options have been modified
    */
   public boolean areOptionsModified(boolean checkCurrent = true, boolean cancelling = false)
   {
   ...
   private void setTreeNodeNotModified(int index)
   {
      ...
      // update the caption, remove the *
      _str caption = m_treeHandle._TreeGetCaption(index);
    ...
    *                   otherwise
    */
   private boolean isNodeModified(int index)
   {
                               ...
                               int &flags, int startNode, boolean &recurseThis)
   {
      ...
      if (m_searching) {
         ...
         if (!m_parser.isSearchResultsNode(xmlIndex) && (selfFound)) {
            flags = TREENODE_BOLD;
         }
      }
    ...
    */
   public boolean showSearchNode(_str path, _str node)
   {
      // show the first part
      if (path == '') {
         path = node;
...
//          }
    ...
    */
   public boolean isNodeFavorited(int treeIndex)
   {
