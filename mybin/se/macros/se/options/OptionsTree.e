...
#define MIN_DIALOG_WIDTH 4500
#define FRAME_PADDING 210
...
class OptionsTree {
   ...
   protected boolean m_optionsChangeDelay = true;

   // whether we display dialogs as summaries (property sheets) when possible
   protected boolean m_dialogsAsSummaries = true;
   ...
   private boolean m_growFrame = true;
    ...
    */
   public boolean isOptionsChangeDelayed()
   {
   ...
   protected void handleChangeEvents(int changeEvents, _str (&commentLangs):[])
   {
      ...
      // update user if restart is required for settings to take effect
      if (changeEvents & OCEF_RESTART) {
         _message_box(nls("Some of your changes will not take effect until you restart SlickEdit."));
      }
      if (changeEvents & OCEF_TAGGING_RESTART) {
         _message_box(nls("The tagging cache options will not take effect until you restart SlickEdit."));
      }
      if (changeEvents & OCEF_THREAD_RESTART) {
         _message_box(nls("The thread options will not take effect until you restart SlickEdit."));
      }
      if (changeEvents & OCEF_DIALOG_FONT_RESTART) {
    ...
    */
   protected boolean selectXMLIndexInTree(int index, boolean selectParentIfUnavailable = false, boolean turnOffDelay = false)
   {
    ...
    */
   private void selectTreeIndex(int index, boolean turnOffDelay = false)
   {
      if (index > 0) {
   ...
   {
      index := m_treeHandle._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
    ...
    *                   path.
    */
   protected void buildNextLayer(int treeIndex = TREE_ROOT_INDEX, boolean recurse = false, int startNode = -1)
   {
      ...
      while (xmlIndex > 0) {
         ...
         caption := '';
         int children = 0, picIndex = 0, flags = 0;
    ...
    */
   private boolean getNodeInfo(int xmlIndex, _str &caption, int &picIndex, int &children, int &flags, int startNode, boolean &recurseThis)
   {
    ...
    */
   public boolean showDialogNode(_str form, _str arguments, int errorBehavior = ISEB_RETURN)
   {
    ...
    */
   public boolean showLanguageNode(_str modeName, _str options = '', _str arguments = '', int errorBehavior = ISEB_RETURN)
   {
      // find the node
    ...
    * @return              whether we were able to show the node specified
    */
   public boolean showVersionControlNode(_str vcName, _str options = '', _str arguments = '', int errorBehavior = ISEB_RETURN)
   {
      // find the node
    ...
    */
   public boolean showNode(_str caption, boolean doReload = false, 
    ...
    *                   there was an error
    */
   private boolean showXMLIndex(int index, int errorBehavior = ISEB_RETURN)
   {
    ...
    */
   private int loadDialogTemplate(_str form, _str arguments = null, boolean sizeToFrame = false)
   {
      int formWid = -1;
   ...
   {
      formWid := loadDialogTemplate(dt.getFormName());
   ...
   private void resizePanel()
   {
      ...
      if (!changed) m_currentPanelWid.call_event(m_currentPanelWid, ON_RESIZE);
   }
   ...
   public _str getTreeNodePath(int index)
   {
      ...
      while (index > 0) {
         path = m_treeHandle._TreeGetCaption(index) :+ " > " :+ path;
    ...
    */
   public _str goToTreeNode(int treeIndex)
   {
      // these are the same, so don't change!
      if (m_currentTreeIndex == treeIndex) {
      ...
      if (!processCurrentPanelBeforeSwitching(m_currentTreeIndex)){
         ...
         if (treeIndex != newTreeIndex || treeIndex != curTreeIndex) {
            ...
            m_currentCaption = getTreeNodePath(m_currentTreeIndex);
    ...
    *                            not switch
    */
   private boolean processCurrentPanelBeforeSwitching(int treeIndex = -1, int action = OPTIONS_SWITCHING)
   {
      return true;
   }
   ...
   {
      int helpIdx = getCurrentXMLIndex();
      if(helpIdx) {
