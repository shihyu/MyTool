...
#define AWK_MODE_NAME   'Awk'
#define AWK_LANGUAGE_ID 'awk'
...
defload()
{
   _str setup_info='MN='AWK_MODE_NAME',TABS=+3,MA=1 74 1,':+
               'KEYTAB='AWK_MODE_NAME'-keys,WW=0,IWT=0,ST='DEFAULT_SPECIAL_CHARS',':+
               'IN=2,WC=A-Za-z0-9_$,LN=AWK,CF=1,';
   _str compile_info='';
   _str syntax_info='3 1 1 0 4 1 1 0';
   _str be_info='';

   _CreateLanguage(AWK_LANGUAGE_ID, AWK_MODE_NAME, setup_info, compile_info, syntax_info, be_info);
   _CreateExtension('awk', AWK_LANGUAGE_ID);
}
...
_command void awk_enter() name_info(','VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   generic_enter_handler(_awk_expand_enter);
}
_command void awk_space() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
...
_command void awk_begin() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_CMDLINE)
{
...
_command void awk_endbrace() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL)
{
   keyin('}');
...
int awk_indent_col(int non_blank_col,boolean pasting_open_block = false)
{
   int Noflines=0;
   typeless cur_line='';
   _str first_word='';
   ...
   int is_structure=pos(' 'first_word' ',' if do while switch for ');
   boolean level1_brace=substr(cur_line,1,1)=='{';
...
static _str awk_expand_space()
{
   ...
   int col=0;
   _str aliasfilename='';
   _str word=min_abbrev2(orig_word,awk_space_words,name_info(p_index),aliasfilename);
...
static awk_expand_begin()
{
   ...
   _str last_word='';
   if ( pcol-2>0 ) {
      int i=lastpos('[~ ]',rline,pcol-2,p_rawpos'r');
...
static boolean stat_has_semi(_str option='')
{
   _str line='';
   get_line(line);
...
int awk_proc_search(_str &proc_name,int find_first)
{
   ...
   int status=0;
   if (find_first) {
      status=_nocomment_search('^(:b|)function:b'fn,'@rh>');
   }else{
      status=_nocomment_search('^(:b|)function:b'fn,'@rh>');
   }
   if (status) {
   ...
   _str line='';
   get_line(line);
   int fp=lastpos('function',line)+length('function');
   int p=pos('(',line);
   if (!p) {
      return(awk_proc_search(proc_name,0));
   }
   proc_name=strip(substr(line,fp,p-fp));
   return(0);
}
