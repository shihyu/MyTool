...
#define BROWSE_INFO_POINTER_INDEX 0
...
#endif
         if (x_pos>=ctltree1.p_x&&x_pos<=ctlfile_tree.p_x+ctlfile_tree.p_width) {
            ctldivider.p_x=x_pos;
            ctlfile_tree.p_x=ctldivider.p_x+ctldivider.p_width;
            ctltree1.p_width=ctldivider.p_x-ctltree1.p_x;
...
_subversion_browser_form.on_resize()
{
   ...
   ctladd_to_list.p_y=ctltree1.p_y+ctltree1.p_height+ybuffer;
...
int ctlcheckout.lbutton_up()
{
   ...
   if ( !status ) {
      ...
      if ( status==COMMAND_CANCELLED_RC ) {
         _str del_path=co_info.created_path!=''?co_info.created_path:co_info.local_path;
         int result=_message_box(nls("Delete path '%s' and all files under it?",del_path),'',MB_OKCANCEL);
         if ( result==IDOK ) {
            status=_DelTree(del_path,1);
         }
      }else if ( status ) {
...
_command int svn_browser_add_repository()
{
...
static _str SVNBrowseBuildURL(int index)
{
   _str URL='';
   for (;;) {
      _str cur_caption=_TreeGetCaption(index);
...
void _subversion_browser_form.on_destroy()
{
   ...
   if ( open_repositories_index>-1 ) {
      ...
      for (;;) {
         ...
         _str cur_repository_name=_TreeGetCaption(tree_index);
...
static _str SVNGetCheckoutPath(_str local_path,_str URL,boolean preserveURL)
{
...
static int SVNVerifyCheckoutPath(_str path,_str &created_path)
{
   ...
   if ( !isdirectory(parent_dir) ) {
      ...
      svn_subdir=svn_subdir:+SUBVERSION_CHILD_DIR_NAME;
      ...
      cvs_subdir=cvs_subdir:+CVS_CHILD_DIR_NAME;
...
static void _subversion_checkout_form_initial_alignment()
{
   ...
   sizeBrowseButtonToTextBox(ctlpath.p_window_id, _browsedir1.p_window_id, 0, ctlURL.p_x + ctlURL.p_width);
}
