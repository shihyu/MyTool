...
// Regex used for identifying a junit.jar or junit-VERSION.jar file
_str junit_regex="junit(|-(:d|:d.:d|:d.:d.:d)).jar";
...
_str _utCreateJUnitBeginTestBatchFile(_str testName)
{
   ...
   status = _save_file("+O "maybe_quote_filename(fileName));
   if (status) {
      return "";
   }
...
int _utExtractJUnit4TestsForContext()
{
   ...
   _str curClass = '';
   _str classes[], annotations[];
   boolean importsJunit = false;
   boolean has_suite_annotations = false;

   for (tagID = 0; tagID <= numContextTags; tagID++) {
      packageName = '';
      if (tag_get_context(tagID, tagName, tagType, fileName, startLine, 
         startPos, scopeLine, scopePos, endLine, endPos, classPkgName, 
         tagFlags, arguments, returnType) != 0) {
         continue;
      }
      // Skip abstract classes and interfaces
      if(tagFlags & VS_TAGFLAG_abstract || tagType == 'interface') {
         return 1;
      }
      ...
      if (tagType == 'class') {
         ...
         if (packageName == "") {
            curClass = VS_UNITTEST_HIERSEPARATOR :+ tagName;
         } else {
            curClass = packageName :+ VS_UNITTEST_HIERSEPARATOR :+ tagName;
         }
         if (has_suite_annotations) {
            _utAddSuiteItemToCurrentTests(curClass);
         }
      } else if (tagType == 'import' && pos("^org\\.junit\\.", tagName, 1, 'U')) {
         importsJunit = true;
      } else if (tagType == 'annotation' && importsJunit) { 
         if (pos(tagName, VS_UNITTEST_JUNITANNOTATIONS) > 0) {
            _utAddClassItemToCurrentTests(curClass);
         } else if (pos(tagName, VS_UNITTEST_JUNITSUITEANNOTATIONS) > 0) {
 ...
 * @return 0 on success
 */
int _utExtractJUnitTestsForContext()
{
   _UpdateContext(true);
   ...
   boolean importsJUnit = true;
// boolean importsJUnit = _utImportsJUnitPackage(bins:["import"]);

   // Figure out which classes extend TestCase, and which classes extend TestSuite
   int testCases:[];
   _utPareJUnitTestCases(bins:["class"], testCases, importsJUnit);
   int testSuites:[];
 ...
 */
int _utExtractJUnitTestsForFile(_str fileName)
{
   ...
   status = _utExtractJUnitTestsForContext();
   _delete_temp_view(tempViewID);
...
int _utExtractJUnitTestsForFiles(_str (&fileNames)[])
{
   ...
   for (i = 0; i < fileNames._length(); i++) {
      status = _utExtractJUnitTestsForFile(fileNames[i]);
      if (status) {
...
_str _utJavaEXE()
{
   ...
   _str javaEXE = JDKPath :+ FILESEP :+ "bin" :+ FILESEP :+ "java" :+ EXTENSION_EXE;

   return javaEXE;
}

/**
 * Return the full path to the JUnit jar file
 * 
 * @return Full path to JUnit jar
 */
_str _utJUnitJar()
{
   _str jar = get_env("VSROOT") :+ FILESEP :+ "junit" :+ FILESEP :+ "junit.jar";
   return jar;
}
...
int _utBuildJUnitProject(_str projectName)
{
   ...
   workspace_set_active(projectName, true);
   project_build('build', false, true, 0, true);
   workspace_set_active(oldProject, true);
...
_str _utModifyDebugCmdLineForJUnit(_str testName)
{
   ...
   if (mainClass == "") {
      ...
      int mainClassPos = pos(mainClass, cmdLine);
      newCmdLine = substr(cmdLine, 1, mainClassPos-1) :+ ' 'VS_UNITTEST_JUNITCORE' ';
   }
   newCmdLine = newCmdLine :+ testName;
...
void _utParseJUnitOutputBlock(int startLine, int endLine)
{
   ...
   int runs = 0, failures = 0, errors = 0;
   ...
   while (_utSearchForTestMethod() == 0) {
      ...
      _utAddMethodItemToCurrentTests(methodName, VS_UNITTEST_LANGUAGE_JAVA);
...
void _utParseJUnitDefects(int startLine, int endLine)
{
   ...
   _begin_select('');
   p_col = 1;
   _str regex2 = "^There was|were \\:i failure";
   int failuresStartLine = 0, failuresEndLine = 0;
...
/**
 * Add junit.jar to the class path if it does not already exist
 * 
 * @param projectName Name of project
 * @param config Configuration whose classpath we want to modify
 */
void _utMaybeAddJUnitJar(_str projectName, _str config)
{
   _str junitJarFile = _utGetJUnitJarFile();
   if (junitJarFile != "") {
      _str foundItem = _utFindItemInClassPath("\\"FILESEP""junit_regex, projectName, config);
      // if we did not find any junit.jar in the cp for the project, set it to junitJarFile 
      if (foundItem == "") {
         _utAddItemToClassPath(junitJarFile, projectName, config);
      } else if (foundItem != junitJarFile) {
         // if we did find a junit.jar in the cp for the project, and its not the same was what was returned from _utGetJUnitJarFile, lets set the def var
         def_unittest_junitjar = foundItem;
      }
   }
}
...
_str _utFixupClassPath(_str classPath)
{
   ...
   _str result = '"' :+ _utJoinStrings(classPaths, PATHSEP) :+ '"';
...
{
   int pos1 = pos("-classpath", cmdLine, 1, "I");
   int pos2 = pos(VS_UNITTEST_JUNITCORE, cmdLine, 1, "I");
   if (pos1 < 1) {
      return cmdLine;
   }
   ...
   _str result = substr(cmdLine, 1, pos1-1) :+ classPath :+ substr(cmdLine, pos2+1);
   //_message_box("result="result);
   return result;
}
...
void _utMaybePrependVSJUnitJar(_str projectName, _str config)
{
   ...
   _str jarFile = vsroot"vsjunit"FILESEP"vsjunit.jar";
   int index = -1;
...
/**
 * Search for junit.jar in the project's class path. If it exists, return
 * it; else prompt the user for the path to junit.jar
 * 
 * @return Full path to junit.jar on success; else ""
 */
_str _utGetJUnitJarFile()
{
   // First try getting it from the def var; maybe it's already been set
   if (def_unittest_junitjar != "") {
      return def_unittest_junitjar;
   }

   // Then try finding it in the class path of the current project
   _str junitJarFile = _utFindItemInClassPath("\\"FILESEP""junit_regex, _project_name, "*");
   if (junitJarFile == "" || !file_exists(junitJarFile)) {
      // If it's still not present, prompt for it
      junitJarFile = _OpenDialog('-modal', "Select junit.jar...", "", "Jar Files (*.jar)", OFN_FILEMUSTEXIST, "", "", "", "jar");
   }

   def_unittest_junitjar = junitJarFile;
   return junitJarFile;
}
...
{
   _str vsbin = get_env("VSLICKBIN1");
   _str vsroot = get_env("VSROOT");
   _str projectName = vsroot"vsjunit"FILESEP"vsjunit.vpj";
   ...
   _ProjectSet_ClassPathList(projectHandle, junitJarFile, "Release");
   _ProjectSave(projectHandle);
   _str buildCmd = vsbin"vsbuild -t \"make jar\" \""projectName"\"";
   shell(buildCmd);
}
...
{
   int projectHandle = _ProjectHandle(projectName);
   _str currentConfig = GetCurrentConfigName(projectName);
   if (strieq(_ProjectGet_Type(projectHandle, currentConfig), "java")) {
      if (_ProjectGet_TargetNode(projectHandle, "UnitTest", currentConfig) >= 0) {
         // Have to make sure vsjunit.jar is built
         _utCompileJUnitRunner();
         if (!file_exists(get_env("VSROOT") :+ "vsjunit"FILESEP"vsjunit.jar")) {
         ...
         _utMaybeAddJUnitJar(projectName, GetCurrentConfigName(projectName));
         _utMaybePrependVSJUnitJar(projectName, GetCurrentConfigName(projectName));
      }
   }
}
...
int _utPipeAndParseJUnitOutput(_str cmdLine, int outputWindowID)
{
   ...
   outputWindowID.bottom();
   int progressLineNum = outputWindowID.p_line;
...
int _utRunJUnitTestSet(_str (&classPkgNames)[], _str cmdLine)
{
   ...
   int startLine = buildWindowID.p_line;
 ...
 */
void _utPareJUnitTestCases(int (&inputClasses):[], int (&outputClasses):[], boolean importsJUnit)
{
   ...
   for (i._makeempty(); ; ) {
      ...
      if (outputClasses._indexin(i)) {
         _utAddClassItemToCurrentTests(i);
      }
   }
}
...
void _utPareJUnit4Tests(int (&inputClasses):[], int (&inputAnnotations):[])
{
   ...
   for (i._makeempty(); ; ) {
      ...
      _str fullname = packageName :+ VS_UNITTEST_HIERSEPARATOR :+ className;
 ...
 * @param importsJUnit Does the context import the JUnit package?
 */
void _utPareJUnitTestSuites(int (&inputFuncs):[], int (&inputClasses):[], int (&outputClasses):[], boolean importsJUnit)
{
   outputClasses._makeempty();
   typeless i;
   ...
   for (i._makeempty(); ; ) {
      ...
      if (importsJUnit && methodName == "suite") {
         ...
         _str fullClass = packageName :+ VS_UNITTEST_HIERSEPARATOR :+ className;
         typeless j;
         for (j._makeempty(); ; ) {
 ...
 * false otherwise
 */
boolean _utFileImportsJUnitPackage(_str fileName)
{
 ...
 */
boolean _utIsJavaSourceFile(_str fileName)
{
   return _utIsSourceFileOfSpecificType(fileName, JAVA_SOURCE_FILE_EXT);
}
...
{
   return _utIsProjectOfSpecificType(name, config, "java");
}
 ...
 */
boolean _utImportsJUnitPackage(int (&imports):[])
{
   int status;
 ...
 */
_command int junit(_str testName="")
{
 ...
 */
_command int junit_debug(_str key="")
{
   _utPrepForJUnit(_project_name);
