...
defload()
{
   _str setup_info='MN='TCL_MODE_NAME',TABS=+8,MA=1 74 1,':+
               'KEYTAB='TCL_MODE_NAME'-keys,WW=1,IWT=0,ST='DEFAULT_SPECIAL_CHARS',':+
   ...
   LanguageSettings.setAutoBracket(TCL_LANGUAGE_ID, AUTO_BRACKET_ENABLE|AUTO_BRACKET_PAREN|AUTO_BRACKET_BRACKET|AUTO_BRACKET_DOUBLE_QUOTE|AUTO_BRACKET_BRACE);
}
...
#define BLOCK_WORDS ' for foreach if elseif method proc switch while '
...
static boolean _tcl_split_brace_block(int syntax_indent)
{
 ...
 */
int tcl_smartpaste(boolean char_cbtype, int first_col)
{
   updateAdaptiveFormattingSettings(AFF_SYNTAX_INDENT);
   syntax_indent := p_SyntaxIndent;
 ...
 * @since 11.0
 */
int _tcl_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                          ...
                          VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   if (otherinfo == '$') {
     if (cur_context != '' && cur_type_id == VS_TAGTYPE_function) {
         filter_flags = VS_TAGFILTER_LVAR|VS_TAGFILTER_VAR;

      } else {
         filter_flags = VS_TAGFILTER_GVAR|VS_TAGFILTER_VAR;
      }
   }
   ...
   if (prefixexp == '') {
      ...
      if (find_parents) context_flags |= VS_TAGCONTEXT_FIND_parents;
      _str template_args:[];

      context_flags |= VS_TAGCONTEXT_ALLOW_any_tag_type|VS_TAGCONTEXT_NO_globals|VS_TAGCONTEXT_ALLOW_package;
      tag_list_symbols_in_context(lastid, prefixexp, 0, 0, 
                                  tag_files, '',
                                  num_matches, max_matches,
                                  filter_flags, context_flags,
                                  exact_match, case_sensitive, 
                                  visited, depth, template_args);
      if (_chdebug) {
...
boolean _tcl_autocomplete_after_replace(AUTO_COMPLETE_INFO &word,
