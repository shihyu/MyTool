 ...
 * @categories XMLCFG_Functions
 */
boolean _xmlcfg_buffer_is_valid()
{
 ...
 * @categories XMLCFG_Functions
 */
void _xmlcfg_copy_children_as_siblings(int dest_handle,int dest_index,int src_handle,int src_index, boolean copyBefore = false)
{
   first := true;
...
void _xmlcfg_copy_children_with_name(int handle,int ParentNode,int NewParentNode,_str name,boolean doMove=false)
{
   int child;
...
int _xmlcfg_get_xpath_from_node(int handle, int elementNode, _str& queryStr)
{
   ...
   do {
      ...
      _str name = _xmlcfg_get_name(handle,elementNode);
      if( name==null ) {
         // Error
      ...
      while( iattr>=0 ) {
         _str attr_name = _xmlcfg_get_name(handle,iattr);
         if( attr_name==null ) {
            // Error
         ...
         result=result"[@"attr_name"='"attr_val"']";
 ...
 * @categories XMLCFG_Functions
 */
int _xmlcfg_get_attribute_ht(int handle, int elementNode, _str (&ht):[], boolean append=false)
{
   int status = 0;
