...
{
   _macro_delete_line();
   boolean append_option=doAppend!='';
...
_ok.lbutton_up()
{
   ...
   _str result=_lffiles.p_text;
   boolean blankOption = false;
   if (result=='') {
      blankOption = true;
      result=ALLFILES_RE;
   }
...
_str _fm_subcopy(_str dest_path, _str doPause='')
{
   ...
   _str source_path=_strip_filename(absolute(source),'N');
   _str add_slash='';
   for (;;) {
   ...
   _str name_part=_strip_filename(source,'P');
   if ( ! __UNIX__ && (_Substr(dest_path,2,1)==':' || substr(dest_path,1,2):=='\\') ) {
      add_slash='';
   }
   if ( dest_path=='' ) {
      add_slash='';
   }
   _str destfilename=add_slash:+dest_path:+_strip_filename(source_path,'d'):+name_part;
 ...
 *
 * (<i>operation</i>" "maybe_quote_filename(<i>source</i>)" "maybe_quote_filename(<i>destfilename</i>) )
 * 
                     ...
                     _str source, _str destfilename,typeless doPause='')
{
   ...
   _str line=operation " "maybe_quote_filename(source) " "maybe_quote_filename(destfilename);
   if ( doPause!='' ) {
   ...
   int index=find_index(proc_name,PROC_TYPE|COMMAND_TYPE);
...
static _str file_move_or_copy(_str cmdline,_str doPause, _str operation)
{
   ...
   _str destfilename=dest_path:+_strip_filename(absolute(source),'P');
...
{
   filename=strip(filename,'B','"');
   typeless status = edit(maybe_quote_filename(filename), EDIT_DEFAULT_FLAGS);
   if ( status==FILE_NOT_FOUND_RC ) status=0;
   return(status);
}
...
{
   _macro_delete_line();
   _str permissions=strip(parse_file(files),'B','"');
   for (;;) {
      _str p1='';
                           ...
                           _str caption, _str prompt, _str help_item)
{
   ...
   _macro('m',_macro('s'));
   _str param1=append_to_prompt' ':+cmdline;
 ...
 * 
 */ 
_command unlist_search(_str search_string='', _str arg2='', _str search_options='') name_info(','VSARG2_REQUIRES_MDI_EDITORCTL)
{
   if (search_options=='') search_options='I';
   if (search_string=='') {
      _macro_delete_line();
      ...
      _macro('m',_macro('s'));
      _macro_call('process_list','psearch',search_string,search_options,'prepeat-search','zap-line');
   }
   typeless status=process_list('psearch',search_string,search_options,'prepeat-search','zap-line');
   return(status);
}
/**
...
{
   int search_index       = find_index(search_fun,PROC_TYPE|COMMAND_TYPE);
   int repeat_search_index= find_index(repeat_search_fun,PROC_TYPE|COMMAND_TYPE);
   int action_index       = find_index(action_fun,PROC_TYPE|COMMAND_TYPE);
   ...
   for (;;) {
     ...
     if ( go ) {
       ...
       message(msg);
       for (;;) {
          _str k=upcase(get_event());
          int select=pos(k,nls_chars);
          if ( select==1 ) {
            status=call_index(action_index);
            break;
          } else if ( select==2 ) {
                         ...
                         _str search_string, _str action_fun, _str search_opts='')
{
   ...
   _str repeat_search_fun='prepeat-search';
...
_replaceok.lbutton_up()
{
   ...
   if (_findre.p_value) {
      if (def_re_search==UNIXRE_SEARCH) {
         ...
         if (def_re_search==BRIEFRE_SEARCH) {
            options=options:+'B';
         }else{
            options=options:+'R';
         }
...
_command int fileman_replace(_str search_string='', _str replace_string='', _str options='') name_info(','VSARG2_REQUIRES_FILEMAN_MODE|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   typeless atbuflist=_lbmulti_select_result('','','1');
   if (atbuflist=='') {
...
_command fileman_list_buffers(_str wildcard = ALLFILES_RE) name_info(','VSARG2_REQUIRES_MDI)
{
   ...
   while (!rc) {
      if (name != '') {
        _str line = file_match('-p +VRHS 'maybe_quote_filename(name), 1);
