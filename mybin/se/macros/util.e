...
boolean def_do_block_mode_key=true;
boolean def_do_block_mode_delete=true;
boolean def_do_block_mode_backspace=true;
boolean def_do_block_mode_del_key=true;
...
_command void scroll_up() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   bottom_of_window();
   int bottom_cursor_y=p_cursor_y;
   p_cursor_y=cursor_y;
...
_str _scroll_when(_str value = '')
{
   ...
   if (value == '') {
      ...
      if (num != value) {
         style = style :+ ' ' :+ value;
         _scroll_style(style);
         _macro_call('_scroll_style('style')');
      }
   }
   return value;
}
 ...
 *
 */
_command int sort_on_selection(_str cmdline='',_str markid='') name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   _str hiddenOption='';
   _str string=cmdline;
   _str option='';
   ...
   if ( _select_type(markid)=='BLOCK' ) {
      ...
      _begin_select();_select_line(markid2);_end_select();_select_line(markid2);
   ...
   if (hiddenOption!='') {
      ...
      down();_select_line(markid3);
      p_line=p_line+Noflines-1;_select_line(markid3);
 ...
 *
 */
_command sort_within_selection(_str cmdline='') name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   if (p_TruncateLength && last_col>p_TruncateLength) {
...
_command void asc(_str ch='')
{
  ...
  _str msg='';
  if (doUTF8) {
     msg='asc 'ch'=U+'substr(dec2hex(_UTF8Asc(ch)),3);
 ...
 */
_command case_indirect(_str key='') name_info(','VSARG2_EDITORCTL|VSARG2_CMDLINE)
{
   _macro_delete_line();
   _str orig_key=key;
   if ( key:=='' ) {
...
_command man(_str param='') name_info(','VSARG2_REQUIRES_MDI)
{
...
#if __UNIX__
   _str program_name=file_match('-p /usr/bin/man',1);
   if ( program_name=='' ) {
   ...
   _str alternate_shell=file_match('-p /bin/sh',1);
   if ( alternate_shell=='' ) {
      alternate_shell=path_search('sh');
   }
#else
   // Probably Cygwin
   _str program_name=path_search('man','','P');
   if ( program_name=='' ) {
      message(nls('Program "%s" not found','man'));
      return(1);
   }
...
#if __UNIX__
   _str line=program_name' 'param' </dev/null >'temp_file' 2>&1';
#else
   _str line=program_name' 'param' >'temp_file' 2>&1';
#endif
   mou_hour_glass(1);
   int status=shell(line,'pq',alternate_shell);
   mou_hour_glass(0);
   // man return code is unreliable on some UNIX's like LINUX
   boolean unreliable=(machine()=='LINUX') /*|| (machine()=='???')*/;
   ...
   int orig_buf_id=0;
   int old_max_filehist=def_max_filehist;
   def_max_filehist=0;
   if (p_mdi_child) {
...
{
   if ( _rsprocessbug() ) {
      _str filename=get_env('RSPROCESSBUG')'/kill.slk';
      ...
      if ( ! rc ) {
         ...
         _save_file('+o');
         _chmod('+x 'maybe_quote_filename(filename));
         _delete_buffer();
      }
   }
}
 ...
 */
_command void '-','_'(_str NofLines='') name_info(','VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories CursorMovement_Functions
 */
_command void '+'(_str NofLines='') name_info(','VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @see gui_goto_line
 */
_command void '0'(_str LineNumber='') name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @see gui_goto_line
 */
_command int goto_line(_str lineNumber='') name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
...
_command int goto_col(_str colNo='') name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
...
_command int gui_goto_line() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   typeless result=show('-modal _textbox_form',
        'Go to line',          // Caption
         ...
         '-r 0,2147483647 Line Number (1 - 'p_RNoflines'):'p_line
         );
   if (result=='') {
      return(COMMAND_CANCELLED_RC);
   }
...
static void maybe_add_mode(_str modename,int callback_index)
{
   ...
   if ( callback_index && index_callable(callback_index) ) {
      int status=call_index(modename,callback_index);
      if ( status ) {
         return;
      }
   }
...
#define AUTOMATIC_ITEM 'Automatic'
#define LIST_MODES_CALLBACK_NAME '_list_modes_callback'
void _list_modes(_str cur_lang,_str cur_mode_name,boolean insert_automatic=true,boolean list_all_modes=false)
{
   int index=1;
   int ff=1;

   int callback_index=find_index(LIST_MODES_CALLBACK_NAME,PROC_TYPE);
   ...
   _str lang='';
   while (index) {
      index=name_match('def-language-',ff,MISC_TYPE);
      ni=name_info(index);
      if (ni!='' && substr(ni,1,1)!='@') {
         typeless junk;
         parse ni with junk'MN='modename',';
         if (modename=='') {
            parse name_name(index) with 'def-language-'lang;
                  ...
                  !_ModenameEQ(modename,'grep')))) 
            {
               ...
               AlreadyAdded:[lowcase(modename)]=1;
            }
         }
      }
      ff=0;
   ...
   for (i._makeempty();;) {
      ...
      if (!modename._isempty() && modename!='' && !AlreadyAdded._indexin(lowcase(modename))) {
         maybe_add_mode(modename,callback_index);
      }
   }
...
_command void select_mode(_str mode_name='') name_info(MODENAME_ARG','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   int was_recording=_macro();
   _macro_delete_line();
   _str origLangId=p_LangId;
   _str cur_mode_name=p_mode_name;
   _str cur_lang='';
   _str lang='';
   _str result='';
   if (mode_name != '') {
      ...
      if (cur_lang!='') {
         ...
         cur_mode_name = _LangId2Modename(_Ext2LangId(_get_extension(p_buf_name)));
      }
   }
   if (cur_lang=='') {
      if(p_LangId == FUNDAMENTAL_LANG_ID) {
         cur_lang=_file_case(_get_extension(p_buf_name,1));
      } else {
         cur_lang=p_LangId;
      }
      if (_DataSetIsMember(p_buf_name) && def_record_dataset_mode) {
         ...
         _list_modes(cur_lang,cur_mode_name,true);
   ...
   if (result==AUTOMATIC_ITEM) {
      ...
      check_and_load_mode_support(result);
   ...
   if (ext != '' && lang != FUNDAMENTAL_LANG_ID && pos(" ", ext) == 0) {
      ...
      if (index <= 0) {
         answer := _message_box(nls("Would you like to map all files with the extension '.%s1' to '%s2'?",ext,_LangId2Modename(lang)), "File Extension Manager", MB_YESNO);
         if (answer == IDYES) {
...
_ok.on_create(int wid)
{
  ...
  /*Selection sorts can only be performed on Block and Line marks*/
          ...
          _ctl_within_selection.p_enabled = 0;
...
_command install_update(_str cmdline='')
{
   ...
   _str output_dir=strip(parse_file(cmdline),'B','"');
   //parse arg(1) with output_dir . ;
   typeless status=install_update2(output_dir);
   _str semaphore_dir=output_dir:+'semaphor';
...
static install_update2(_str output_dir)
{
   ...
   _str filename=output_dir:+USERDEFS_FILE;
   ...
   _str old_value=get_env(_SLICKCONFIG);
   _str old_value2=get_env(_SLICKPATH);
...
_command int format_columns() name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
{
   ...
   typeless cols;
   cols._makeempty();
   _str line=strip(_expand_tabsc_raw(),'T');
   int nofcols=1;
   ...
   for( ;; ) {
      ...
      if( line!='' ) {
         ...
         new_line=new_line:+_expand_tabsc_raw(end_col+1,-1,'S');
                           ...
                           _str valuename, boolean versionSpecific)
{
   ...
   if (versionSpecific) {
      ...
      prefixPath = prefixPath :+ suffixPath;
 ...
 *
 */
_command transpose_words() name_info(','VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   int state=command_state();
   init_command_op();
   boolean no_word_after_cursor=p_col>_text_colc();
   ...
   typeless bp=point();
   int bcol=p_col;
   p_col=p_col+length(before_word);
 ...
 *
 */
_command void transpose_chars() name_info(','VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   init_command_op();
 ...
 * 
 */ 
_command prev_paragraph() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   int col=p_col;
   first_non_blank();
   if ( col<=p_col ) {
 ...
 * 
 */
_command next_paragraph() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   _begin_line();
   /* skip paragraph separator lines */
   search(SKIP_PARAGRAPH_SEP_RE,'@rh');
 ...
 * 
 */ 
_command select_paragraph() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   prev_paragraph();
   emacs_select_char();
   /* Skip paragraph separator lines */
 ...
 *
 */
_command void next_sentence() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   typeless p=point();
   int col=p_col;
   int status=0;
 ...
 *
 */
_command void prev_sentence() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   typeless p=point();
   int col=p_col;
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void cut_sentence() name_info(','VSARG2_REQUIRES_EDITORCTL)
{
   int lindex=last_index('','C');
   boolean push=name_name(prev_index('','C'))!='cut-sentence';
...
{
   _str re='\\x{#0:h}{#1}';                // match \xH+
   re=re:+'|':+'\\x\{{#0:h}{#1}\}';    // match \x{H+}
   re=re:+'|':+'\\u{#0[0-9a-fA-F]:4,4}{#1}';      // match \uHHHH
   re=re:+'|':+'\\U{#0[0-9a-fA-F]:8,8}{#1}';      // match \UHHHHHHHH
   re=+re:+'|':+'&\#x{#0:h}{#1};';       // match &#xH+;
   re=re:+'|':+'&\#{#1:i}{#0};';        // match &#D+;

   // \x123%%\x123%%
   _str result='';
   boolean lastUTF16WasStartOfSurrogate=false;
   int lastUTF16WasStartOfSurrogate_number=0;
   int i,j;
   int match_len=0;
   _str textdigits='';
   _str text='';
   for (i=1;;) {
      ...
      if (!j) {
         if (lastUTF16WasStartOfSurrogate) {
            result=result:+_UTF8Chr(lastUTF16WasStartOfSurrogate_number);
            lastUTF16WasStartOfSurrogate=false;
         }
         j=length(s)+1;
         result=result:+substr(s,i,j-i);
         break;
      }
      //say('result='result);
      //say('j='j);
      ...
      if (textdigits=='') {
         ...
         typeless number=hex2dec('0x'textdigits);
 ...
 */
_command void copy_ucn_as_unicode(boolean doCopy=false) name_info(','VSARG2_TEXT_BOX|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION|VSARG2_REQUIRES_UNICODE_BUFFER)
{
   ucn_to_unicode();
}
static void ucn_to_unicode(boolean doCopy=true)
{
...
struct VSUNICODE_TO_UCN {
   _str prefix;
   _str suffix;
#define VSNUMERICFORMAT_DECIMAL  0
#define VSNUMERICFORMAT_UTF16    1
#define VSNUMERICFORMAT_UTF32    2
   int numericFormat;
   boolean case_of_last_letter_of_prefix_indicates_16_or_32_bit;
};
static VSUNICODE_TO_UCN gU2UCNHashTable:[]={
   ...
   static boolean gU2UCNcase_of_last_letter_of_prefix_indicates_16_or_32_bit;

static _str filter_unicode_to_ucn(_str s)
{
   int slen=length(s);
   _str result='';
   boolean isDecimal=gU2UCNnumericFormat==VSNUMERICFORMAT_DECIMAL;
   boolean isUTF16=gU2UCNnumericFormat==VSNUMERICFORMAT_UTF16;
   int i,len=0;
   int charLen=0;
   typeless number;
   _str hex;
   for (i=1;i<=slen;) {
      ...
      //say('number='number);
      if (isDecimal) {
         result=result:+gU2UCNprefix:+number:+gU2UCNsuffix;
      } else {
         hex=substr(dec2hex(number),3);
         len=length(hex);
         if (len<=4) {
            ...
            // output surrogates
            number-= 0x10000;
            hex=substr(dec2hex((number>>10)|0xD800),3);
            hex=hex:+gU2UCNsuffix:+gU2UCNprefix:+substr(dec2hex((number&0x3ff)|0xDC00),3);
         } else {
            hex=substr('',1,8-len,'0'):+hex;
            if (gU2UCNcase_of_last_letter_of_prefix_indicates_16_or_32_bit) {
               result=result:+upcase(gU2UCNprefix):+hex:+gU2UCNsuffix;
               i+=charLen;
               continue;
            }
         }
         result=result:+gU2UCNprefix:+hex:+gU2UCNsuffix;
      }
      i+=charLen;
   }
   return(result);
...
static void unicode_to_ucn(_str UCNType='',boolean doCopy=true)
{
   int was_command_state=command_state();
   if (was_command_state) {
      init_command_op();
...
boolean doBlockModeKey(_str key,_str keya,boolean InsertingText)
{
   // DJB 05-02-2007
   // initially false, keep track of if this is our first time in
   static boolean EditedSomeTextAlready; 
   _str select_style = _select_type('','S');
   _str persistent_mark = _select_type('','U');
   boolean userLockedSelection = select_style=='E' && persistent_mark=='P';
   ...
   se_mark := _alloc_selection();
   cur_mark := _alloc_selection('B');
   save_pos(auto p);
   _begin_select(markid); _select_char(se_mark);
   _end_select(markid); _select_char(se_mark);
   restore_pos(p); _select_char(cur_mark);
   cur_col_no := p_col;
   int start_col = 0, end_col = 0, buf_id = 0;
   _get_selinfo(start_col, end_col, buf_id, markid);
   if (def_do_block_mode_delete && InsertingText && end_col > start_col && _insert_state()) {
      _delete_selection(markid);
   }
   _deselect(markid);
   ...
   if (key :== ESC && end_col == start_col && EditedSomeTextAlready) {
      _deselect();
      restore_pos(p);
      _free_selection(markid);
      _free_selection(se_mark);
      _free_selection(cur_mark);
      EditedSomeTextAlready = false;
      return(true);
   }
   ...
   // text
   if (!InsertingText && key:==BACKSPACE && def_do_block_mode_backspace && start_col:==end_col) {
      InsertingText=true;
      --start_col; // adjust selection one char to right if doing backspace
   }
   if (!InsertingText && key:==DEL && def_do_block_mode_del_key && start_col:==end_col) {
      InsertingText=true;
   }
   ...
   if (InsertingText) {
      _end_select(se_mark); p_col = start_col; _select_block(markid);
      _begin_select(se_mark); p_col = start_col; _select_block(markid);
      _begin_select(cur_mark); p_col = start_col; 

      // DJB 11-15-2005
      // add special handling for backspace
      // could use _delete_selection() instead here
      // does not support irregular backspace options
      // like hack-tabs backspace or pull-chars backspace.
      if (key :== BACKSPACE) {
         _delete_selection(markid);
      } else if (key :== DEL) {
         _delete_selection(markid);
         ...
         for (i = 1; i <= length(event); ++i) {
            ...
            _fill_selection(substr(event, length(event)-i+1, 1), markid);
         }
         _str raw_line = substr('', 1, p_col-1):+event;
   ...
   if (done) {
      _deselect();
      cur_col_no = p_col;
      _end_select(se_mark); p_col = cur_col_no; _select_block();
      _begin_select(se_mark); p_col = cur_col_no; _select_block();
      restore_pos(p);
   }
   _free_selection(se_mark);
   return(done);
...
_command void block_insert_mode() name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
{
   ...
   for (;;) {
      ...
      if (event:==BACKSPACE || event:==LEFT) {
         ...
         int orig_col=p_col;
         left();
         p_line=end_line_no;_select_block();
         p_line=start_line_no;_select_block();
         p_line=cur_line_no;
         if (event:==BACKSPACE) {
      ...
      if (event:==DEL) {
         _str cur_ch=get_text(-2);
         for (i=1;i<=length(cur_ch);++i) {
            _shift_selection_left();
         }
         continue;
      }
      if (event:==PGDN || event:==PGUP) {
         _deselect();
         call_key(event);
         diff := cur_line_no - p_line;
         cur_line_no = p_line;
         p_line=end_line_no-=diff;_select_block();
         p_line=start_line_no-=diff;_select_block();
         p_line=cur_line_no;
         continue;
      }
...
{
   if( v != null && v != '' ) {
      int index = find_index(v,VAR_TYPE);
      if( index > 0 ) {
...
_command void findJarForClass(_str cmdLineArg="", _str clsName="", _str jarFileSpecs="")
{
   ...
   _str fileSpecsAndOptions = "+T "jarFileSpecs;
   _str jarpath = file_match(fileSpecsAndOptions, 1);
   while (jarpath != "") {
...
_command int filter_command(_str cmd='') name_info(','VSARG2_REQUIRES_AB_SELECTION|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   for(;;) {
      ...
      if( cmd=="" ) {
         ...
         //
         if( def_gui ) {
            _str helpmsg="?Specifiy the command to run against the selected text. The selected text will be used as input to the command, and the output from the command will replace the selected text.";
      ...
      // Only work with line selections
      if( upcase(_select_type(mark,'T'))!='LINE' ) {
 ...
 */
void _SoftWrapGetSettings(_str lang,boolean &SoftWrap,boolean &SoftWrapOnWord)
{
...
int _MFUndoCallbackCopyFile(_str DestFilename,_str SrcFilename)
{
   ...
   boolean orig_AllowSave=p_AllowSave;
   p_AllowSave=true;
   // Retag the file and
   status=save(maybe_quote_filename(DestFilename),SV_OVERWRITE|SV_RETURNSTATUS);
...
_command void mfundo(_str strRedo='',boolean CheckCurrent=true) name_info(','VSARG2_EDITORCTL)
{
   _tbSetRefreshBy(VSTBREFRESHBY_UNDO);
   boolean redo=strRedo=='R';
...
#define MFREDO ctlRestoreFiles.p_user
defeventtab _mfundo_date_mismatch_form;
void ctlViewChanges.lbutton_up()
{
   boolean redo= MFREDO;
   _str filename=ctllist1._lbget_text();
   if (filename=='') {
      return;
   }
   ...
   _DiffModal('-r1 -r2 -file2title "Expected contents" -viewid2 'maybe_quote_filename(file1)' 'temp_view_id);
...
void ctlRestoreFiles.on_create(boolean redo=false)
{
   MFREDO=redo;
   // Check if any of the files that will be restored
   // have been modified since a multi-file operation
   ...
   for (i=0;i<count;++i) {
      ...
      if ((ModifyFlags!='' && (ModifyFlags&1)) || CurFileDate!=_file_date(filename,'C')) {
...
#define MFREDO2 ctlTryAgain.p_user
defeventtab _mfundo_failed_form;

void ctlTryAgain.on_create(boolean redo=false)
{
   //say('**********************');
   MFREDO2=redo;
...
boolean gForwardBackNoUpdate=false;
 ...
 * @see back
 */
boolean def_forwardback_enabled=true;
/**
...
static boolean _ForwardBack_delete(int i,boolean doJoin=false)
{
      ...
      if (i>0 && i<gForwardBackStack._length()) {
         ...
         if (_select_type(mark_id)!='' && _select_type(mark_id2)!='') {
            ...
            if (_select_type(mark_id)=='' && _select_type(mark_id2)=='') {
               ...
               same=file_eq(gForwardBackStack[i].buf_name,
...
void _ForwardBack_update(boolean BufferQuit=false,_str buf_name='')
{
...
static int _ForwardBack_GoTo(int i)
{
   ...
   if (buf_name!='' && file_exists(buf_name)) {
      ...
      status=edit(maybe_quote_filename(buf_name),EDIT_DEFAULT_FLAGS);
 ...
 *
 * @return boolean         true if same buffer, false if not
 */
static boolean _ForwardBack_on_same_buffer()
{
   ...
   say('   'p_buf_name);
#endif
      return(file_eq(gForwardBackStack[gForwardBackItemPos].buf_name,p_buf_name) );
   }
...
{
   #define RULER_LINE   "----|----1----|----2----|----3----|----4----|----5----|----6----|----7----|----8----|----9----|----0"
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void remove_trailing_spaces() name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   int hasselection = 0;
   int direction = -1;
   if (select_active()) {
   ...
   _str search_options = "rh@" :+ (hasselection ? "m" : "") :+ ((direction < 0) ? "-" : "");
   _str line;
   for (;;) {
      if (direction > 0) {
         _begin_line();
      ...
      int status=search('[ \t]@$', search_options);
      if (p_line != orig_line) {
         p_line = orig_line;
      } else {
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void highlight_selection() name_info(','VSARG2_READ_ONLY | VSARG2_REQUIRES_EDITORCTL | VSARG2_REQUIRES_AB_SELECTION)
{
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void remove_highlight_in_selection(_str markid = '') name_info(','VSARG2_READ_ONLY | VSARG2_REQUIRES_EDITORCTL | VSARG2_REQUIRES_AB_SELECTION)
{
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void remove_highlight() name_info(','VSARG2_READ_ONLY | VSARG2_REQUIRES_EDITORCTL)
{
   int i;
   int list[];
   int highlight_marker = _GetTextColorMarkerType();
