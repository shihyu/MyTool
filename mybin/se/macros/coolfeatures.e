 ...
 */
boolean def_show_tips_on_startup = true;
...
#define CURRENT_STATE ctl_cool_html.p_user
...
static void showCoolFeature(int i)
{
   ...
   _str text = "<h3>"cfi[i].title:+newFeature"</h3>":+"<p>"cfi[i].text;
...
void ctl_feature_html.on_create(_str autoshow="")
{
   ...
   if (isEclipsePlugin()) {
      ...
      p_active_form.ctl_slickedit_pic.p_picture = _update_picture(-1, "vse_profile_eclipse_256.bmp");
   }
...
void ctl_feature_html.on_destroy()
{
   ...
   boolean show_tips = (ctl_startup_btn.p_value != 0);
...
void _coolfeatures_form.on_resize()
{
   ...
   if (!_minimum_width()) {
      int min_width = 5*border_x +
                      ctl_configure_btn.p_width +
                      ctl_help_btn.p_width +
                      ctl_demo_btn.p_width;
      int min_height = 6*border_y + button_height*2 + 2*logo_height;
   ...
   ctl_help_btn.p_x = ctl_configure_btn.p_x + ctl_configure_btn.p_width + border_x;
   ctl_demo_btn.p_x = ctl_help_btn.p_x + ctl_help_btn.p_width + border_x;

   // set second row of button's x positions
   ctl_topics_btn.p_x = ctl_close_btn.p_x + ctl_close_btn.p_width + border_x;
   ctl_prev_btn.p_x = ctl_topics_btn.p_x + ctl_topics_btn.p_width + border_x;
   ctl_next_btn.p_x = ctl_prev_btn.p_x + ctl_prev_btn.p_width + border_x;
...
{
   return get_env("VSROOT"):+"sysconfig":+FILESEP:+"coolfeatures":+FILESEP:+"coolfeatures.xml";
}
