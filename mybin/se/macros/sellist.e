...
#define BUFFER_COL 5
#define SORT_BUFLIST_FLAG         0x1
#define SEPARATE_PATH_FLAG        0x2
#define SHOW_SYSTEM_BUFFERS_FLAG  0x4
#define SELECT_ACTIVE_BUFFER      0x8
#define FORCE_OLD_BUFLIST        0x10

static _str lb_nls_chars;
static int gstart_buf_id;

int def_buflist=SORT_BUFLIST_FLAG+SEPARATE_PATH_FLAG;
boolean def_isearch_buflist=true;
...
static _str _list_modified_ft(int reason,var result,_str key)
{
   ...
   if (reason==SL_ONINIT || reason==SL_ONSELECT) {
      ...
      int diff_selected_wid=btndiff_selected;
      if (_sellist.p_Nofselected > 0) {
         if (!save_selected_wid.p_enabled) {
             save_selected_wid.p_enabled = 1;
         }
         if (_sellist.p_Nofselected==1 && !_sellist._lbfind_selected(1)) {
            _sellist. get_line(line);
            diff_selected_wid.p_enabled = file_exists(_buflist_name(line));

            doc_name=_buflist_name(line);
            // IF editor does not have this buffer (this could be a gui builder buffer)
            if (buf_match(doc_name,1,'hx')=='') {
               diff_selected_wid.p_enabled=0;
            }
         } else  {
            diff_selected_wid.p_enabled = 0;
         }
   ...
   if ( key==5) {  /* Diff Selected */
      ...
      result=_DiffModal('-r2 -b1 -d2 'maybe_quote_filename(doc_name)' 'maybe_quote_filename(doc_name));
      p_window_id=orig_wid;
      return('');
   }
   if ( key==6) { /* Invert. */
...
static _str _list_buffers_callback(int reason, _str& result, _str key)
{
   ...
   if( reason == SL_ONDEFAULT ) {
      ...
      if( def_buflist & SELECT_ACTIVE_BUFFER ) {
   ...
   if( buffer_list ) {
      if( !user_button && isalpha(key) ) {
         if( upcase(key) == 'Q' ) {
            ...
            int i = pos(key,lb_nls_chars,1,'i');
            if( i != 0 ) {
               key = i + 2;
            }
         }
      ...
      if( key == 6) {
         ...
         //get_line line
         _str indicators = stranslate(substr('',1,BUFFER_COL-1),'','*');
      ...
      if( key == 5 ) {
         ...
         def_one_file = '';
         boolean old_process_tab_output = def_process_tab_output;
 ...
 * @return Returns 0 if switched buffers successfully.
 */
_command int list_buffers(_str option='', boolean hide_current=false) name_info(','VSARG2_EDITORCTL|VSARG2_REQUIRES_MDI)
{
   if(isEclipsePlugin()){
      eclipse_list_buffers();
      return(0);
   }
   if ( (def_buflist&FORCE_OLD_BUFLIST) || option!="" || hide_current!=false ) {
      return old_list_buffers(option,hide_current);
   }
   activate_files_files();
   if ( def_buflist&SELECT_ACTIVE_BUFFER ) {
      _FilelistSelectCurrentBuffer();
   }
   return 0;
}
 ...
 * @return Returns 0 if switched buffers successfully.
 */
static int old_list_buffers(_str option='', boolean hide_current=false)
{
   boolean link_window=false;
   boolean show_system_buffers=false;
   ...
   do {
      ...
      if ( show_system_buffers ) {
         def_buflist|=SHOW_SYSTEM_BUFFERS_FLAG;
      }
...
void _build_buf_list(int &width, 
                     ...
                     int start_buf_id=-1,
                     boolean hide_current=false,
                     int (*bufIdList)[]=null)
{
   // build hash table of buffer's to check
   boolean bufferIdsToCheck:[];
   if (bufIdList != null) {
   ...
   boolean checkForDelphi = (index_callable(find_index("delphiIsRunning",PROC_TYPE)) &&
                             list_modified && delphiIsRunning());

   _safe_hidden_window();
   int allow_system_buffers=(def_buflist&SHOW_SYSTEM_BUFFERS_FLAG);
   width=0;
   if (start_buf_id<0) {
   ...
   for (i=1; i<=p_Noflines ; ++i) {
      ...
      _str new_modify=substr(stranslate(modify,'','H'),1,BUFFER_COL-1);
...
int _save_non_active(_str &buf_name,boolean quit_option=false,int save_flags=SV_RETURNSTATUS)
{
   typeless status=0;
   typeless buf_id=0;
   int temp_view_id=0;
   ...
   if ( !quit_option ||
      ...
      if (result==IDYES) {
         _str option = ""; // option to pass to save()
         boolean is_noname = _isno_name(buf_name);
         boolean is_dir = (buf_name != '' && (isdirectory(buf_name) != '0'));
         if (is_noname || is_dir) {
            fname := '';
            if (is_noname) {
            parse buf_name with '<' buf_id'>';
   ...
   if (quit_option) {
      if (def_one_file!='') {
         ...
         int count=0;
         buf_id=p_buf_id;
         int wid=window_match(p_buf_name,1,'xn');
         for (;;) {
            if (!wid) break;
 ...
 *
 */
_command int list_modified(_str title='',boolean quiet=false, boolean hideCurrent=false,int (*bufIdList)[]=null,boolean show_using_current_wid=false) name_info(','VSARG2_REQUIRES_MDI)
{
   ...
   int width=0;
   _build_buf_list(width,p_buf_id,1,gstart_buf_id,hideCurrent,bufIdList);
   ...
   _str buttons=nls('Save &All,&Save Selected:btnsave_selected,&Diff Selected:btndiff_selected,&Invert,Save &None');
   lb_nls_chars=nls_selection_chars(buttons);
   typeless buflist=def_buflist;
   ...
   int orig_buf_id=0;
   int mdi_view_id=0;
   boolean was_hidden_window=false;
   switch (result) {
   case 1:
      p_window_id=_mdi.p_child;
      return(save_all());
...
*/
boolean _need_to_save2(_str buf_name="", _str doTestModify="")
{
   boolean TestModify=doTestModify!='' && doTestModify;
   if (buf_name!="") {
