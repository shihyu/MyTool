...
_command j2me_app_wizard()
{
   ...
   _str mf_name=_strip_filename(_project_name,'E'):+'.mf';
   _str jar_name=_strip_filename(_project_name,'E'):+'.jar';
   _str jad_name=_strip_filename(_project_name,'E'):+'.jad';
   ...
   for (index=0;index<config_names._length();++index) {
      ...
      // set classpath
      _str classpath=location:+'lib':+FILESEP:+'midpapi20.jar':+PATHSEP:+location:+'lib':+FILESEP:+'cldcapi10.jar';
      ...
      _ProjectSet_TargetCmdLine(handle,execute_node,location:+'bin':+FILESEP:+'emulator':+ext:+' -Xdevice:':+phone_type:+' -Xdescriptor:'_RelativeToProject(jad_name));

      // use the emulator in the "Debug" command
      int debug_node=_ProjectGet_TargetNode(handle,'Debug',config_names[index]);
      _ProjectSet_TargetCmdLine(handle,debug_node,location:+'bin':+FILESEP:+'emulator':+ext:+' -Xdebug -Xnoagent -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=8000 -Xdevice:':+phone_type:+' -Xdescriptor:':+_RelativeToProject(jad_name));

      // call preverify as a post build step
      _ProjectSet_PostBuildCommandsList(handle,location:+'bin':+FILESEP:+'preverify':+ext:+' -cldc %cp -d classes tmpclasses',config_names[index]);
   ...
   _str package_name='';
   int last_dot_pos=lastpos('.',class_name);
   ...
   _str filename=_strip_filename(_project_name,'N'):+stranslate(package_name,FILESEP,'.');
 ...
 * be created.
 */
_command void make_jad(_str prj_name='')
{
   if (prj_name:=='') {
   ...
   _str jad_name=_strip_filename(prj_name,'E'):+'.jad';
   _str jar_name=_strip_filename(prj_name,'E'):+'.jar';
   _str mf_name=_strip_filename(prj_name,'E'):+'.mf';
