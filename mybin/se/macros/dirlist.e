 ...
 *                 Defaults to true.
 */
void _dlSetPath(_str newPath, boolean cdToDir=true)
{
   DirListObject_t dlo = _dlGetDirListObject();
   ...
   if( newPath!="" ) {
      ...
      // absolute() converts "/datasets" to the standard "//" format and
      // so we need to convert the "//" format back to "/datasets".
      // For the directory listboxes to work properly, the dataset name
      // need to be in the mount file format (ie. "/datasets/ds.name".
      if( pos("//", path) == 1 ) {
         path = DATASET_MOUNT_NAMESL :+ substr(path,3);
      }
#endif
      if( path!="") {
         new_path=path;
      }
      if( last_char(new_path)!=FILESEP ) {
         new_path=new_path:+FILESEP;
      }
      if( file_eq(orig_path,new_path) && !cdToDir ) {
      ...
      if( substr(new_path,1,2)!='\\' ) {
         ...
         if( last_char(temppath)!=FILESEP ) {
            temppath=temppath:+FILESEP;
         }
         if( !file_eq(new_path,temppath) && !_DataSetIsFile(new_path) ) {
      ...
      dlo.path=getcwd();
      if( last_char(dlo.path)!=FILESEP ) {
         dlo.path=dlo.path:+FILESEP;
      }
   }
 ...
 *
 */
void _dlpathChildren(_str parentPath, boolean showDotFiles=true)
{
   DirListObject_t dlo = _dlGetDirListObject();
             ...
             boolean doCd=true)
{
   ...
   if( last_char(currentPath)==FILESEP ) {
      currentPath=substr(currentPath,1,length(currentPath)-1);
   }
   (*dlo.pfnViewClear)();
   ...
   while( path!="" ) {
      ...
      item=dir;
      currentPath=currentPath:+FILESEP:+dir;
 ...
 */
boolean _dlGetTreeData(_str key, typeless& value)
{
   DirListObject_t dlo = _dlGetDirListObject();
 ...
 */
boolean _dlInOnChange(int onoff=-1)
{
   DirListObject_t dlo = _dlGetDirListObject();
   boolean old_onoff = dlo.inOnChange;
   if( onoff > -1 ) {
