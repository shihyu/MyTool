...
#define OI2TYPENAME (' 'OI_FORM'=_form':+\
...
_command list_objects(_str args='', ...) name_info(','VSARG2_REQUIRES_MDI)
{
   ...
   boolean quiet = arg(2)!='';
   boolean eraseifempty = !arg(3);   // Not used if INSERT option specified
 
   boolean system_only=false;
   int exclude_mask=FF_SYSTEM;
   boolean modified_only=false;
   ...
   if ( option!='' ) {
     ...
     if (UseTempViewNSave) {
        ...
        status=edit('+q 'maybe_quote_filename(temp_name),EDIT_NOADDHIST|EDIT_NOSETFOCUS);
