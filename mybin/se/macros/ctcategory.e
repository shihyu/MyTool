...
static boolean isVersionControlDirectory(_str dir)
{
   _maybe_strip_filesep(dir);
   _str foldername = substr(dir,pathlen(dir)+1);
...
static boolean haveAtleastOneChildDirectory(_str path)
{
   ...
   while( result!="" ) {
      ...
      result = file_match(maybe_quote_filename(path:+ALLFILES_RE)' +X +D +S -P -V',0);
   }
 ...
 */
static boolean isSysCategoryPath(_str path, _str& relativePath=null)
{
 ...
 */
static boolean isUserCategoryPath(_str path, _str& relativePath=null)
{
   path=translate(path,FILESEP,FILESEP2);
   if( first_char(path)==FILESEP ) {
      path=substr(path,2);
...
_str _ctCategoryToTemplatePath(_str categoryPath=null)
{
   ...
   if( first_char(categoryPath)==FILESEP ) {
      categoryPath=substr(categoryPath,2);
   }
 ...
 *                  Default to false.
 */
void _ctCategoryTree_etab.on_change(int reason, int nodeIndex, boolean force=false)
{
 ...
 */
boolean _ctCategorySetCategoryPath(_str path, boolean refreshTree=true)
{
   boolean bstatus = false;
   boolean old_InOnChange = _dlInOnChange();
   if( !refreshTree ) {
      _dlInOnChange(1);
   }
   do {
