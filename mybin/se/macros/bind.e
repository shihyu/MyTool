...
_command where_is(_str commandName='', _str quiet='',_str separatorChar=',') name_info(COMMAND_ARG',')
{
  ...
  int index= find_index(arg1,COMMAND_TYPE);
  if ( ! index ) {
    if (quiet=='' ) {
  ...
  append_key_bindings(index,key_binding_list,'',_default_keys,p_mode_eventtab,quiet==3,separatorChar);
...
void append_key_bindings(int index,_str &key_binding_list,_str prefix_keys,
                         int _root_keys,int _mode_keys,boolean ReturnFirstOnly=false,
                         _str separatorChar=',')
{
   ...
   for (i=0;i<list._length();++i) {
      ...
      int name_index_found=eventtab_index(_root_keys,_mode_keys,key_index);
      ...
      if ( name_type(name_index_found)&EVENTTAB_TYPE ) {
         ...
         */
         ...
         for (j=key_index;j<=keyend_index;++j) {
            ...
            if ( key_binding_list!='' ) key_binding_list=key_binding_list:+separatorChar;
            key_binding_list=key_binding_list:+prefix_keys " "key_name;
         }
...
_command void insert_keydef() name_info(','VSARG2_REQUIRES_EDITORCTL)
{
   ...
   keyname=stranslate(keyname,"'-'","._.");
   _str param='def 'keyname' = ':+translate(name_name(index),'_','-');
...
_command what_is()
{
   ...
   if ( index && (name_type(index)&(COMMAND_TYPE|EVENTTAB_TYPE))) {
     ...
     if ( type & (COMMAND_TYPE) ) {
       if (h_match_exact(name_name(index))!='') {
          status=help(name_name(index));
       }
     }
     if ( status ) {
...
_str prompt_for_key(_str prompt,
                    ...
                    _str maxcount="", _str mustbe_mouse_event="")
{
   int mode=(p_HasBuffer)?p_mode_eventtab: _edit_window().p_mode_eventtab;
   int count=1;     /* Count key presses. */
   ...
   if ( ! status ) {
      ...
      for (;;) {
         ...
         keyname=keyname:+_key_for_display(k)' ';
...
static void parse_keytab_options(var a,var keytab_option,var a7,typeless arg4='')
{
   _str re='';
   a7='';
   if ( arg4 ) {  /* '-' required. */
   ...
   _str after='';
   parse a with a ('(^| )'re':i( |$)'),'r' +0 after ;
   if ( after!='' ) {
      if ( a7 ) {  /* 0 command not being bound. */
         parse after with a7 after ;
         if ( substr(a7,1,1)=='-' ) {
            a7=substr(a7,2);
         }
      }
...
_command bind_to_key(_str commandName='') name_info(COMMAND_ARG',')
{
   ...
   _str a=prompt(commandName);
   _macro('m',was_recording);
   typeless keytab_option='';
   typeless a7='';
   _str name='';
   _str keyindex_list='';
   parse_keytab_options(a,keytab_option,a7);
   parse a with name keyindex_list ;
   int command_index=find_index(name,COMMAND_TYPE);
   if ( ! command_index ) {
   ...
   if ( keyindex_list!='' ) {
     ...
     typeless status=prompt_for_key(nls('To key:')' ',keytab_used,k,keyname,keytab_option,'',a7,1);
 ...
 * 
 */ 
_command unbind_key(_str keyName='')
{
   /*if (_isEditorCtl()) {
      _maybeMakeModeEventTab();
      keytab_used=p_mode_eventtab;
   ...
   */
   ...
   typeless k='';
   typeless a7='';
   parse_keytab_options(a,keytab_option,a7,0);
   typeless status=prompt_for_key(nls('Unbind key:')' ',keytab_used,k,keyname,keytab_option,'',a7,1);
   if ( status ) { return(COMMAND_CANCELLED_RC); }
   typeless mode_etab=(p_HasBuffer)?p_mode_eventtab: _edit_window().p_mode_eventtab;
   ...
   _config_modify_flags(CFGMODIFY_KEYS);
   clear_message();
   _str line='set_eventtab_index('string_keytab_used(keytab_used)',';
   _str line2='event2index(name2event('_quote(event2name(k))')),0);';
   _macro_append(line:+line2);
   _macro_append('_config_modify_flags(CFGMODIFY_KEYS);');
   _UncacheTagKeyInfo();
   return(0);
...
_command create_prefix_key()
{
   ...
   _config_modify_flags(CFGMODIFY_KEYS);
   _str line='set_eventtab_index('string_keytab_used(keytab_used)',';
   _str line2='event2index(name2event('_quote(event2name(k))')),'_quote(name_name(command_index))');';
   _macro_append(line:+line2);
   _macro_append('_config_modify_flags(CFGMODIFY_KEYS);');

}
_str me_match(_str name,boolean find_first)
{
...
boolean _issysmenu_key(_str key)
{
...
_update_sysmenu_bindings()
{
   _str mdichild_close='';
   _str mdichild_next='';
   _str mdichild_restore='';
   _str mdichild_move='';
   _str mdichild_size='';
   _str mdichild_minimize='';
   _str mdichild_maximize='';
   _str mdi_close='';
   _str mdi_restore='';
   _str mdi_move='';
   _str mdi_size='';
...
#if 0
/**
 * Allows you to move the MDI window with the keyboard.  Not supported by 
 * UNIX version.
 * 
 * @categories Window_Functions
 * 
 */
_command void move_mdi() name_info(','VSARG2_REQUIRES_MDI|VSARG2_REQUIRES_MINMAXRESTOREICONIZE_WINDOW)
{
   if(isEclipsePlugin()) {
      return;
   }
...
#if __VERSION__>=1.62
      _mdi._sysmenu_command(SC_MOVE);
#endif
   }
}
/**
 * Allows you to size the MDI window with the keyboard.  Not 
 * supported by UNIX version.
 * 
 * @categories Window_Functions
 * 
 */ 
_command void size_mdi() name_info(','VSARG2_REQUIRES_MDI|VSARG2_REQUIRES_MINMAXRESTOREICONIZE_WINDOW)
{
   if (_mdi.p_window_state!='M') {
#if __VERSION__>=1.62
      _mdi._sysmenu_command(SC_SIZE);
#endif
   }
}
#endif 
