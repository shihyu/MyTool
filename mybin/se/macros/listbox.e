...
void _lbadd_item_list(_str (&text)[], _str indent="", _str picture_index="")
{
#ifdef not_finished
   // rb - TODO
   // Refactor to get rid of indent which is no longer supported is listbox?
#endif
   switch (p_multi_select) {
   case MS_NONE:
      if (_lbisline_selected()) {
   ...
   _str prefix = ' ';
   if (indent != '') {
      prefix = prefix :+ substr(indent,1,BMINDENT_WIDTH)' 'substr(picture_index,1,5)':';
   }
   int i;
...
_ul2_listbox.s_home,"s-home"()
{
   _lbcommand('s-home');
}
_ul2_listbox.s_end,"s-end"()
{
   _lbcommand('s-end');
}
...
static void set_scroll_directions(boolean &past_bottom, boolean &past_top, int &new_y)
{
   new_y=mou_last_y();
...
static void select_to_cursor(boolean do_select, typeless specific_value="")
{
   if (!do_select) return;
...
void _cbset_text(_str text, int pic_index=0)
{
   ...
   if (p_text == text) return;
   if (_lbfind_item(text) < 0) _lbadd_item(text,0,pic_index);
   p_text = text;
}
 ...
 * <dt>R</dt><dd>Interpret <i>text</i> as a regular expression.</dd>
 * <dt>U</dt><dd>Interpret <i>text</i> as a UNIX regular expression.</dd>
 * <dt>B</dt><dd>Interpret <i>text</i> as a Brief regular expression.</dd>
 * </dl>
 *
...
_str _lbsearch2(_str text, _str options="")
{
   ...
   int status=search(text,'@'case_sense);
   if (status) {
      return(status);
   }
   for (;;) {
...
static _str _lbi_search2(_str &last_data, _str text, _str options="")
{
   ...
   if (last_text!='') {
      if (last_status) {
         boolean prefix_not_found=substr(text,1,length(last_text)):==last_text &&
      ...
      if (!add_char && length(last_text)-1==length(text) &&
         ...
         int ignore_case=pos('i',options,1,'i');
   ...
   int lineNum=_lbsearch_index(p_line,retext,'@r'options);
 ...
 * 
 */
int _lbfind_selected(boolean find_first)
{
   if (find_first) {
      p_line=0;
   }
   int lineNum=_lbfind_next_selected_index(p_line);
   if (lineNum<0) {
      p_line=lineNum;
      return(STRING_NOT_FOUND_RC);
   } else {
...
static boolean maybeChangeCaseSensitive(_str &options)
{
	newCS := false;
	if (options != '') {
...
static void maybeRestoreCaseSensitive(_str options, boolean oldCS)
{
	if (options != '') {
 ...
 *
 */
int _lbfind_and_select_item(_str item, _str options = '', boolean selectTopIfNotFound = false)
{
	// maybe change the case sensitivity
 ...
 *
 */
int _lbadd_item_no_dupe(_str item, _str options = '', int addOption = 0, boolean selectItem = false)
{
	// maybe change the case sensitivity
	oldCS := maybeChangeCaseSensitive(options);
