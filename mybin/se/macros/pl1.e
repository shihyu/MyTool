...
#define PL1_LANGUAGE_ID  'pl1'
#define PL1_MODE_NAME    'PL/I'
#define PL1_VLXLEXERNAME 'PL/I'
#define PL1_IDENTIFIER_CHARS  'A-Za-z0-9_@$#'

/**
 * @deprecated Use {@link LanguageSettings_API}
 */
boolean def_pl1_autocase=1;
...
defload()
{
   _str setup_info='MN='PL1_MODE_NAME',TABS=+4,MA=1 74 1,':+
               'KEYTAB='PL1_LANGUAGE_ID'-keys,WW=1,IWT=0,ST='DEFAULT_SPECIAL_CHARS',IN=2,WC='PL1_IDENTIFIER_CHARS',LN='PL1_VLXLEXERNAME',CF=1,';
   _str compile_info='';
   ...
   _str syntax_info='4 1 ':+   // <Syntax indent amount>  <expansion on/off>
               '1 1 4 ':+ // <min abbrev> <word_case> <be_style|doend_flags>
               '1 0 1 ':+ // <notused> <notused> <indent WHEN from SELECT>
               '0';       // <UseContOnParameters>

   _str be_info='(procedure),(proc),(select),(do),(begin)|(end);i';

   _CreateLanguage(PL1_LANGUAGE_ID, PL1_MODE_NAME,
                   setup_info, compile_info, syntax_info, be_info);
   _CreateExtension('pl1', PL1_LANGUAGE_ID);
   _CreateExtension('pli', PL1_LANGUAGE_ID);
   _CreateExtension('plm', PL1_LANGUAGE_ID);

   kt_index := find_index('pl1-keys',EVENTTAB_TYPE);
   if (kt_index) {
      set_eventtab_index(kt_index,event2index(ENTER),find_index('pl1-enter',COMMAND_TYPE));
      set_eventtab_index(kt_index,event2index(' '),find_index('pl1-space',COMMAND_TYPE));
   }
}

static _str pl1_next_sym(boolean getword=false)
{
   if (p_col>_text_colc()) {
      if(down()) {
         gtk=gtkinfo='';
...
static _str pl1_prev_sym(boolean getword=false)
{
   typeless status=0;
   _str ch=get_text();
...
_command void pl1_enter() name_info(','VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   generic_enter_handler(_pl1_expand_enter);
}
_command void pl1_semi() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL)
{
   if (command_state()) {
      keyin(';');
      return;
...
_command pl1_space() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
...
static _str pl1_expand_space()
{
   ...
   _str aliasfilename='';
   _str word=min_abbrev2(orig_word,pl1_space_words,name_info(p_index),aliasfilename);
   ...
   if ( word=='if' || if_special_case) {
      if (be_style == BES_BEGIN_END_STYLE_3) {
         ...
         maybe_insert_doend(syntax_indent,be_style,width,word,_word_case('then do;'));
      ...
      replace_line(line:+maybespace:+_word_case('()':+e1));
      maybe_insert_doend(syntax_indent,be_style,width,word,_word_case('do;'),1);
   } else if (word=='other' || word=='otherwise') {
      newLine := line:+maybespace:+strip(_word_case(e1));
...
/*&int pl1_proc_search(_str &proc_name,int find_first)
{
   ...
   if ( find_first ) {
      ...
      if (name != '') {
         have_vars:[name] = true;
         name=tag_tree_compose_tag(name, '', 'var');
         if (proc_name:=='') {
            proc_name=name;
            return(0);
         }
   ...
   for (;;) {
      ...
      name=tag_tree_compose_tag(name, '', type, tag_flags, args);
      ...
      // and no longer used. 
      name=tag_tree_compose_tag(name, '', type);
      if (proc_name==name) {
         return(0);
      }
...
*/
int _pl1_find_block_col(_str &block_info/* currently just block word */,boolean restoreCursor,boolean returnFirstNonBlank)
{
   typeless orig_pos;
   save_pos(orig_pos);
   int nesting;
   nesting=1;
   int status=search('select|begin|do|end|procedure|proc|entry','h@-wirxcs');
   ...
   for (;;) {
      ...
      _str word=lowcase(get_match_text());
      //messageNwait(word);
      switch (word) {
      case 'begin':
      case 'do':
...
*/
static int pl1_begin_stat_col(boolean RestorePos,boolean SkipFirstHit,boolean ReturnFirstNonBlank,
...
*/
static boolean _SemicolonTerminatesProc(int &col)
{
   save_pos(auto p);
   if (p_col==1) {
      up();_end_line();
...
static boolean _SemicolonTerminatesBlock(int &col,_str &blockinfo,boolean &semi_follows)
{
...
*/
int pl1_indent_col(int non_blank_col, boolean pasting_open_block = false)
{
   int orig_col=p_col;
   int orig_linenum=p_line;
   save_pos(auto p);
   ...
   for (;;) {
      ...
      case 'declare':
         ...
         save_search(s1,s2,s3,s4,s5);
         _clex_skip_blanks('-');
         boolean isContinuation=get_text()!=',';
...
int pl1_smartpaste(boolean char_cbtype,int first_col,int Noflines,boolean allow_col_1=false)
{
   _str comment_col='';
...
static boolean pl1_enter_col2(int &enter_col)
{
...
_command void pl1_maybe_case_word() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
   _maybe_case_word(LanguageSettings.getAutoCaseKeywords(PL1_LANGUAGE_ID),gWord,gWordEndOffset);
}

_command void pl1_maybe_case_backspace() name_info(','VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
...
int _pl1_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                               VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
