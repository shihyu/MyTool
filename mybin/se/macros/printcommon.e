      ...
      if( upcase(substr(string,j+1,1)) :== 'F' ) {
         ...
         case "FD":
            ...
            if( upcase(substr(string,j+3,1)) :== 'E' ) {
               ...
               result = result :+ dd'/'mm'/'yyyy :+ substr(result,2);
               i = j+4;
            } else {
               result = result :+ _file_date(buf_name,'L');
               i = j+3;
            }
            break;
         case "FT":
            // FTE=European format
            if( upcase(substr(string,j+3,1)) :== 'E' ) {
               result = result :+ _file_time(buf_name,'M');
               i = j+4;
            } else {
               result = result :+ _file_time(buf_name,'L');
               i = j+3;
            }
            break;
         case "FP":
            ...
            if( doc_name != "" ) {
               result = result :+ doc_name;
            } else {
               result = result :+ buf_name;
            }
            i = j+3;
            break;
         case "FN":
            ...
            if( doc_name != "" ) {
               result = result :+ createFilename(buf_name,doc_name);
            } else {
               result = result :+ _strip_filename(buf_name,'P');
            }
            i = j+3;
            break;
         default:
