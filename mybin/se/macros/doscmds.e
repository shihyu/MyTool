...
_str get_xdesktop_session_name()
{
   ...
   _str temp = get_env('WINDOWMANAGER');
   ...
   _str session_name = get_env('DESKTOP_SESSION');
...
_command void launch_os_shell() name_info(',')
{
   ...
   _str cmd = '';
    if (_isMac()) {
        _str scriptPath = get_env("VSDIR");
        _maybe_append_filesep(scriptPath);
        scriptPath :+= "terminalPath.applescript";
        _str scriptArg = maybe_quote_filename(scriptPath);
        _str directoryArg = maybe_quote_filename(directory);
        cmd = "osascript " :+ scriptArg :+ " " :+ directoryArg;
    } else {
        _str session = get_xdesktop_session_name();
        switch (session) {
        case 'gnome':
 ...
 * @categories Miscellaneous_Functions
 */
_command dos(_str command='', _str unused='', _str errorFileName='') name_info(FILE_ARG' 'WORD_ARG'*,'VSARG2_EDITORCTL)
{
   _str async='a';
   _str shell_wait='';
   _str memory='';
   _str echo_dos_output='';
   _str cmdline=strip(command,'L');
   _str number='';
   _str rest='';
   _str compname='';
   _str winapp_file='';
   ...
   boolean AccumlateErrors=false;
   boolean NoNextError=false;
   boolean AddVslickErrorInfo=false;
   for (;;) {
...
#endif
         if ( ! pos('2>&1',cmdline) ) {
            redirect_args=redirect_args' 2>&1';
         }
#if __UNIX__
   ...
   if ( cmdline=='' ) {
      ...
      if ( pos(' ',temp) ) {
         temp=maybe_quote_filename(temp);
      }
                 ...
                 _str shell_options, _str exact_options)
{
   ...
   if (first_char(params)=='"' && !pos('"', params, 2)) {
      params = params :+ '"';
   }
...
int _compiler_redirect(_str prog_name,_str &output_file)
{
   ...
   if(!(file_eq('cm68k',prog_name)||file_eq('himake',prog_name))) return(0);
 ...
 */
static boolean containsXterm(_str directory, _str term, _str & xterm, boolean setEnvVar=true)
{
   xterm = directory;
   if (last_char(xterm) != FILESEP) xterm = xterm :+ FILESEP;
   xterm = xterm :+ term;
...
_str _XtermGetCommandPrefix(_str title,boolean includeShell=true)
{
