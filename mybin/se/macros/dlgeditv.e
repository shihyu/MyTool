...
#define SMALLEST_LIST_WIDTH 2500
#define SMALLEST_COL1_WIDTH 1000
#define LARGEST_COL1_WIDTH 1500
#define SMALLEST_COL2_WIDTH 1000
#define RIGHT_PADDING 100
#define BOTTOM_PADDING 50
#define PAD_AFTER_BITMAPS 100
...
ctllist.on_create()
{
   ...
   int bitmap_padx = _sarrow.p_x;
   int bitmap_pady = _sarrow.p_y;
   int dlge_bwidth = _sarrow.p_width+bitmap_padx;
   int dlge_bheight= _sarrow.p_height+bitmap_pady;
   ...
   for (i=0;;++i) {
      ...
      int r_wid=_find_control(rbitmap);
...
_dlge_form.on_resize(boolean move)
{
   if (!move) {
...
void _deselect_tool(_str tool)
{
   ...
   for (;;) {
      if (wid.p_object==OI_IMAGE ||wid.p_object==OI_PICTURE_BOX) {
         if (file_eq(tool,name_name(wid.p_picture))) {
            if (!wid.p_value) {
               wid.p_value=1;
            }
   ...
   _str tool_noext=_strip_filename(tool,'E');
   if (file_eq(tool_noext,DE_ARROW)) {
      _decreate_mode(0);
   } else {
      _decreate_mode(1);
   }
...
void _arrow.lbutton_double_click()
{
   ...
   if (file_eq(_strip_filename(name_name(p_picture),'E'),DE_ARROW)) {
      return;
   }
   int selected_wid=_display_wid;
   int Nofselected=_deNofselected;
   int form_wid=_get_form(_display_wid);
   _reset_selected();
   if (Nofselected==1 &&
...
lcombo.lbutton_down()
{
   ...
   if (p_cb_active==p_cb_picture) {
      ...
      case 'leafpicture':
         ...
         reinit='-hideondel';
#endif
         typeless result=_OpenDialog('-modal 'reinit,
#if __UNIX__
              'Open picture', '*.xpm;*.bmp;*.ico',
              "Images (*.xpm;*.bmp;*.ico),Pixmaps (*.xpm),Bitmaps (*.bmp),Icons (*.ico),All Files ("ALLFILES_RE")",
#else
...
#if 0
static boolean isDlgEditComboTextBoxMultilinePasteAllowed(int wid)
{
 ...
 */
boolean _ProcessCommandPaste_DlgEditComboPasteMultiline(int targetWid, int firstLineNumber, int lastLineNumber)
{
 ...
 * 
 */ 
_command save_form() name_info(FORM_ARG',')
{
   if (p_mdi_child) {
      if (!_display_wid) {
 ...
 * 
 */
_command load_all(_str eventtab_name="") name_info(','VSARG2_ICON|VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   for (;;) {
     ...
     if ( ! (p_buf_flags & VSBUFFLAG_HIDDEN) &&
          (file_eq('.'extension,_macro_ext) ||file_eq(extension,'cmd'))) {
        if (eventtab_name!='') {
           save_pos(auto p);
           top();
...
_str _set_tab_index(int index)
{
   ...
   // to p_tab_index
   int orig_view_id=0;
   int temp_view_id=_list_tab_index(orig_view_id);
...
static int _set_selected2(int wid, boolean val)
{
   if (wid.p_selected) {
      wid.p_selected=val;
      _selected_list=_selected_list' 'wid;
   }
   return(0);
}
   ...
   for (;;) {
      ...
      if (child.p_selected && child!=wid) {
         if (length(_selected_list)<MAX_LINE-5) {
            _selected_list=_selected_list' 'child;
         }
         child.p_selected=0;
      }
      child=child.p_next;
...
void _set_selected(boolean val)
{
   _selected_list='';
 ...
 * 
 */
_command insert_object(_str result="") name_info(OBJECT_ARG','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (result=='') {
   ...
   int index=find_index(result,OBJECT_TYPE);
   if (!index) {
...
static _str _open_form_callback(int reason,var result,_str key)
{
   ...
   if (reason==SL_ONDEFAULT) {  // Enter key
      _str form_caption=lowcase(p_active_form.p_caption);
 ...
 * 
 */
_command open_form(_str cmdline="") name_info(FORM_ARG'*,')
{
   _str names='';
   if (cmdline=="") {
   ...
   boolean new_form_if_not_found=false;
   boolean reset_modify=false;
   for (;;) {
      _str name='';
      ...
      if (reset_modify) {
         ...
         if (wid.p_x>=(screen_x+screen_width)*_twips_per_pixel_x()-720 ||
             wid.p_x+wid.p_width-720<screen_x*_twips_per_pixel_x() ||
             wid.p_y>=(screen_y+screen_height)*_twips_per_pixel_y()-720 ||
             wid.p_y+wid.p_height-720<screen_y*_twips_per_pixel_y()
             ) {
             wid._center_window(_desktop);
         }
         wid.p_visible=1;
         wid.p_object_modify=0;
      }
      first_time=0;
   }
 ...
 * @categories Form_Functions
 */
_command new_form(_str form_name="")
{
   show_properties(1);
   ...
   wid.p_caption=wid.p_name;
   boolean object_modify=wid.p_object_modify;
   wid.p_selected=1;
   //wid.p_visible=1;p_window_id=wid.p_window_id
   /* _fill_list_boxes() */
   wid.p_object_modify=0;
   if (_display_wid) {
      _reset_selected();
 ...
 * 
 */ 
_command typeless show(_str cmdline="", ...) name_info(OBJECT_ARG','VSARG2_EDITORCTL)
{
   int orig_wid=p_window_id;
   _str arg1=prompt(cmdline,nls('Show form'));
   _str name=strip(arg1);
   _str options='';
   ...
   if (!new_instance) {
      ...
      if (wid && wid.p_name==index.p_name){
         ...
         if (hideondel && !wid.p_visible) {
            hidden=hidden:+'W':+((hideondel==IS_REINIT)?'R':'');
            index=wid;
         } else {
            wid.p_enabled=1;
            wid.p_visible=1;
   ...
   */    
   ...
   wid=_load_template(index,parent,hidden'A',2); /* Hidden */
#if __UNIX__
   mou_hour_glass(0);
#endif
   if (wid<0) {
   ...
   if (do_span) {
      ...
      wid.getScreenInfo(screen_x+screen_width, screen_y+(screen_height intdiv 2), screenInfo);
      if (screenInfo.x == screen_x+screen_width) {
         screen_width += screenInfo.width;
      }
      wid.getScreenInfo(screen_y+screen_height, screen_x+(screen_width intdiv 2), screenInfo);
   ...
   if (do_center || wid.p_x>=(screen_x+screen_width)*tpx-ppx ||
       wid.p_x+wid.p_width<screen_x*tpx+ppx ||
       wid.p_y>=(screen_y+screen_height)*tpy-ppy ||
       wid.p_y+wid.p_height<screen_y*tpy-ppy
       ) {
       if (parent==VSWID_HIDDEN /*|| parent.p_mdi_child || parent==_cmdline*/) {
          parent=_mdi;
       }
   ...
   if (!wid.p_enabled) wid.p_enabled=1;
   if (showmodal || modal) {
      wid.p_ShowModal=1;
   }
   if (!keep_hidden) {
...
_str _set_undo_visible2(int wid, boolean val)
{
   if (wid.p_selected) {
...
void _set_undo_visible(boolean val)
{
   ...
   if (wid.p_selected) {
      ...
      case 'sx':
         if (_display_wid.p_width>_twips_per_pixel_x()*8) {
            wid.p_width=wid.p_width+add;
         }
         break;
      case 'sy':
         if (_display_wid.p_height>_twips_per_pixel_y()*8) {
            wid.p_height=wid.p_height+add;
         }
         break;
      }
   }
...
_command void vdlgedit_save()
{
   if (!p_active_form.p_edit) return;
   save_form();
}
_command void vdlgedit_load(_str option='')
{
   if (!p_active_form.p_edit) return;
   if (upcase(option)=='R') {
      _on_run();
...
void _ainh_dlg_edit.on_got_focus()
{
   ...
   int new_form=_get_form(p_window_id);
...
{
   _get_form(p_window_id).p_object_modify=1;
   typeless wid=_defind_selected();
...
_ainh_dlg_edit.lbutton_down()
{
   ...
   boolean sstabClickProcessed = false;
   int mx, my;
   mou_get_xy(mx,my);
   ...
   boolean form_modify=_get_form(p_window_id).p_object_modify;
   boolean parent_modify=p_object_modify;
   _str old_mouse='';
   _str event='';
   boolean done=0;
   ...
   if (orig_wid.p_x==old_x && orig_wid.p_y==old_y && !form_modify) {
      _get_form(p_window_id).p_object_modify=0;
...
{
   boolean child_modify=selected_wid.p_object_modify;
   int orig_wid=p_window_id;
   ...
   if (event==ESC || (new_x==orig_x && new_y==orig_y &&
      ...
      selected_wid.p_object_modify=1;
   }
   p_window_id=orig_wid;
   return(0);
}
...
{
   boolean old_object_modify=p_object_modify;
   /* Limit the range of movement of the mouse to form or frame. */
   mou_mode(1);
   mou_capture();
...
static boolean _rects_intersect(int wx1, int wy1, int wx2, int wy2,
...
void _deinit_object(_str object_name)
{
   ...
   if (p_edit) {
      ...
      index=find_index('_ul2'object_name,EVENTTAB_TYPE);
      p_eventtab2=index;
      if (index) {
 ...
 * 
 */ 
_command void show_properties(_str showSelected='')
{
   if (showSelected=='') {
      show_selected(1);
   }
 ...
 * 
 */ 
_command void show_selected(_str showProperties='')
{
   if (_display_wid) {
      if (showProperties=='') {
         show_properties(1);
...
static void qrebind_shift_space()
{
   ...
   if (result==IDYES) {
      ...
      int command_index=find_index('keyin-space',COMMAND_TYPE);
      if (!command_index) {
 ...
 * 
 */
_command void grid(_str cmdline='')
{
   if (cmdline=='') {
