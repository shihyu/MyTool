...
#define VSAUTOCODEINFO_SABL_IS_PROC   VSAUTOCODEINFO_ALLOW_SPACE_IN_LIST_MEMBERS
#define VSAUTOCODEINFO_SABL_IS_FUNC   0x1000000
#define VSAUTOCODEINFO_SABL_IS_LABEL  0x2000000
/*
   This is turned on if there is only one
   possible match.
*/
#define VSAUTOCODEINFO_SABL_EXACT_MATCH 0x4000000
#define VSAUTOCODEINFO_SABL_LASTID_FOLLOWED_BY_SPACE  0x8000000

#define SEQ_LANGUAGE_ID    'seq'
#define SEQ_MODE_NAME      'SABL'
#define SEQ_VLXLEXERNAME   'SABL'
#define SEQ_IDENTIFIER_CHARS  'A-Za-z0-9%$_'

boolean def_seq_autocase;

static int gWordEndOffset=-1;
static _str gWord;

#define BESTYLE_FLAG    1
//#define DOEND_INSERT_FLAG 4
//#define DOEND_INSERT_LINE_FLAG 8
//#define NO_SPACE_BEFORE_PAREN 16
...
defload()
{
   if (!index_callable(find_index('pro_proc_search',PROC_TYPE))) {
      load(get_env('VSROOT'):+'macros':+FILESEP:+'pro.e');
   }
   _str setup_info='MN='SEQ_MODE_NAME',TABS=+3,MA=1 74 1,':+
               'KEYTAB='SEQ_LANGUAGE_ID'-keys,WW=1,IWT=0,ST='DEFAULT_SPECIAL_CHARS',IN=2,WC='SEQ_IDENTIFIER_CHARS',LN='SEQ_VLXLEXERNAME',CF=1,';
   _str compile_info='';
   ...
   _str syntax_info='4 1 ':+   // <Syntax indent amount>  <expansion on/off>
               '1 1 0 ':+ // <min abbrev> <word_case> <be_style|doend_flags>
               '1 0 1 ':+ // <notused> <notused> <notused>
               '0';       // <notused>

   _str be_info='';
   _CreateLanguage(SEQ_LANGUAGE_ID, SEQ_MODE_NAME,
                   setup_info, compile_info, syntax_info, be_info);
   _CreateExtension('seq', SEQ_LANGUAGE_ID);

   kt_index := find_index('seq-keys', EVENTTAB_TYPE);
   if (kt_index) {
      set_eventtab_index(kt_index,event2index(ENTER),find_index('seq-enter',COMMAND_TYPE));
      set_eventtab_index(kt_index,event2index(' '),find_index('seq-space',COMMAND_TYPE));
   }
}

// The auto keyword casing code in the module is not
// yet complete, so we are not using it.
boolean def_seq_autocase=1;

static _str seq_next_sym(boolean getword=false)
{
   if (p_col>_text_colc()) {
      if(down()) {
         gtk=gtkinfo='';
...
static _str seq_prev_sym(boolean getword=false)
{
   typeless status=0;
   _str ch=get_text();
...
_command seq_space() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
   if ( command_state() || ! doExpandSpace(p_LangId) || p_SyntaxIndent<0 ||
      ...
      if ( command_state() ) {
         ...
         left();left();
         int cfg=_clex_find(0,'g');
         if (cfg==CFG_KEYWORD && def_seq_autocase) {
            int word_pcol=0;
...
#define SEQ_EXPAND_WORDS ' #define #elif #else #endif #error #if #ifdef #ifndef #include #pragma #undef '

/*
    Returns true if nothing is done
*/
boolean _seq_expand_enter()
{
   int col=_seq_indent_col(0);
   indent_on_enter(0,col);
   return(false);
}
static void maybe_insert_doend(int syntax_indent,int be_style,int width,_str word,_str begin_word,int adjust_col=0,boolean putCursorInsideDoBlock=false)
{
...
static _str seq_expand_space()
{
   ...
   _str aliasfilename='';
   _str word=min_abbrev2(orig_word,seq_space_words,name_info(p_index),aliasfilename);
   ...
   if ( word=='if' || if_special_case) {
      ...
      maybe_insert_doend(syntax_indent,be_style,width,word,_word_case('begin'));
   } else if (word=='begin') {
      set_surround_mode_start_line();
...
*/
int _seq_find_block_col(_str &block_info/* currently just block word */,boolean restoreCursor,boolean returnFirstNonBlank)
{
   typeless orig_pos;
   save_pos(orig_pos);
   int nesting;
...
*/
static int seq_prev_stat_col(_str &blockinfo,boolean RestorePos)
{
...
int seq_smartpaste(boolean char_cbtype,int first_col)
{
...
static boolean seq_enter_col2(int &enter_col)
{
...
{
   _maybe_case_word(def_seq_autocase,gWord,gWordEndOffset);
}

_command void seq_maybe_case_backspace() name_info(','VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   _maybe_case_backspace(def_seq_autocase,gWord,gWordEndOffset);
}
...
_ok.on_create()
{
   ...
   int acindex=find_index('def_'ext'_autocase',VAR_TYPE);
...
_ok.lbutton_up()
{
   ...
   int acindex=find_index('def_'lang'_autocase',VAR_TYPE);
   _set_var(acindex,(ctlautocase.p_value!=0));

   p_active_form._delete_window(0);
}

int _seq_MaybeBuildTagFile(int &tfindex)
{
   _str ext='seq';

   // IF the user does not have an extension specific tag file for Slick-C
   typeless status=0;
   _str tagfilename=absolute(_ConfigPath():+'seq.vtg');
   if (LanguageSettings.getTagFileList(ext) == '' || tag_read_db(tagfilename)==FILE_NOT_FOUND_RC) {
      ...
      if (slickc_filename!='') {
         ...
         _str extra_file=get_env('VSROOT'):+'sabl.pro';
         status=shell('maketags -t -n "SABL Libraries" -o 'maybe_quote_filename(tagfilename)' 'maybe_quote_filename(extra_file));
...
static int _seq_get_proc_offset2(int &FunctionNameOffset,boolean StartFromCursor)
{
   if (StartFromCursor) {
...
static int _seq_get_proc_offset(_str &tagNamePrefix,int &FunctionNameOffset,boolean &doPrefixMatch,boolean StartFromCursor=false,boolean AllowAssignmentStatement=false)
{
   save_pos(auto p);
   ...
   _str tagfilename=absolute(_tagfiles_path():+'seq.vtg');
...
outer:
   for (;;) {
      ...
      for (;;) {
         ...
         if (tag_type=='proc' && substr(tag_name,length(new_tag_name)+1,1):==' ') {
            break;
         }
   ...
   if (doPrefixMatch) {
      status=tag_find_prefix(tagNamePrefix);
   } else {
      status=tag_find_equal(tagNamePrefix);
   }
   int count=0;
   for (;;) {
      if (status) {
      ...
      if (doPrefixMatch) {
         status=tag_next_prefix(tagNamePrefix);
      } else {
         status=tag_next_equal();
      }
   }
   int flags=VSAUTOCODEINFO_SABL_IS_PROC;
   if (count==1) {
      flags|=VSAUTOCODEINFO_SABL_EXACT_MATCH;
   }
   tag_close_db(null,true);
   return(0);
#endif
                     ...
                     VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   _str search_string='[,()[]|goto|gosub|count|$';
   int status=search(search_string,'irh@');
   int linenum=p_line;
   for (;;) {
      if (status) {
         break;
   ...
   for (;;--stack_top) {
      ...
      if (offset_stack[stack_top]==procNameOffset) {
         ...
         status=_seq_get_expression_info(true,idexp_info,visited,depth);
      }
      idexp_info.errorArgs[1] = idexp_info.lastid;

      if (_chdebug) {
         tag_idexp_info_dump(idexp_info,"_seq_fcthelp_get");
         say('status='status);
      }
      if (!status) {
         ...
         if (offset_stack[stack_top]==procNameOffset) {
            ...
            for (;;) {
               ...
               if (type_name=='proc') {
                  ++count;
                  _str file_name,proc_name,signature, class_name, return_type;
                  ...
                  tag_get_detail(VS_TAGDETAIL_name,proc_name);
                  ...
                  match_list[match_list._length()] = proc_name "\t" type_name "\t" signature "\t"file_name"\t"line_no"\t"taginfo;
               }
               if (doPrefixMatch) {
            ...
            for (i=1; i<=num_matches; ++i) {
               _str tag_file,proc_name,type_name,file_name,class_name, signature, return_type, match_tag_name;
               int line_no, tag_flags;
               ...
               match_tag_name = idexp_info.lastid;
               if (type_name:!='func') {
                  continue;
               }
               //match_list[match_list._length()] = proc_name "\t" signature "\t" ;
         ...
         if (match_list._length()>0) {
            ...
            for (i=0; i<match_list._length(); i++) {
               ...
               _str match_tag_name,match_type_name,signature,match_file_name;
               _str  match_line_no, match_taginfo;
               parse match_list[i] with match_tag_name "\t" match_type_name "\t" signature "\t" match_file_name"\t"match_line_no"\t"match_taginfo;
               if (isproc) {
...
static boolean seq_need_label(int seekpos)
{
   save_pos(auto p);
   _nrseek(seekpos);
   seq_prev_sym_same_line();
 ...
 * @since 11.0
 */
int _seq_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                             VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   int cfg;
   if (PossibleOperator) {
                           ...
                           VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   int tag_type=VS_TAGTYPE_function;
   // tag_type is VS_TAGTYPE_function, we really
   // want vars,defines, and functions
   if (info_flags & VSAUTOCODEINFO_SABL_IS_PROC) {
      tag_type=VS_TAGTYPE_proc;
      // tag_type is VS_TAGTYPE_proc, we really
      // want procs,and vars
   } else if (info_flags & VSAUTOCODEINFO_SABL_IS_LABEL) {
      tag_type=VS_TAGTYPE_label;
   }

   tag_files := tags_filenamea(p_LangId);
   num_matches := 0;
   if ((context_flags & VS_TAGCONTEXT_ONLY_locals) || (context_flags & VS_TAGCONTEXT_ONLY_this_file)) {
