...
static _str _sftpSSHClientCmdline(FtpConnProfile *fcp_p)
{
   ...
   if( ssh_exe=="" ) {
      ...
      if( file_match('-p 'path,1)=="" ) {
         ...
         if( path!="" ) {
            ...
            _ini_set_value(FTPUserIniFilename(),'options','sshexe',path);
         }
      }
      ssh_exe=path;
   }
   ...
   _str PreferredAuthentications_option="";
   ...
   cmdline=maybe_quote_filename(ssh_exe);
   if( _ftpdebug&FTPDEBUG_SSH_VERBOSE ) {
      cmdline=cmdline:+" -v -v -v";
   }
   cmdline=cmdline:+' "-oStrictHostKeyChecking no" "-oFallBackToRsh no" "-oForwardX11 no" "-oForwardAgent no" "-oClearAllForwardings yes" 'PreferredAuthentications_option' 'ipVersion_option' 'port_option' -l'user' -s "-oProtocol 2" 'host' 'subsystem;

   return(cmdline);
}

boolean _sftpQTimedOut(FtpQEvent *e_p)
{
...
#if __EBCDIC__
      int p=pos('\13|\21',buf,1,'er');
#else
      int p=pos('\13|\10',buf,1,'er');
#endif
...
void _sftpQEHandler_SSHStart(FtpQEvent *e_p)
{
   ...
   case QS_BEGIN:
      ...
      if( file_match('-p 'maybe_quote_filename(askpass_path),1)=="" ) {
      ...
      _str old_display=get_env("DISPLAY");
      if( old_display=="" ) {
         set_env("DISPLAY",":0.0");
      }
      _str old_ssh_askpass=get_env("SSH_ASKPASS");
      ...
      int x = _mdi.p_x + (_mdi.p_width - 400) / 2;
      int y = _mdi.p_y + (_mdi.p_height - 100) / 2;
      vs_ssh_askpass_args = vs_ssh_askpass_args' --geometry 'x','y;
      ...
      if( plain!="" ) {
         vs_ssh_askpass_args = vs_ssh_askpass_args' --password "'plain'"';
      }
      set_env(VS_SSH_ASKPASS_ARGS_ENVVAR,vs_ssh_askpass_args);

      int hin=0, hout=0, herr=0;
...
void _sftpQEHandler_EndConnProfile(FtpQEvent *e_p)
{
   ...
   default:
      // Should never get here
      _str msg="Unknown FTP queue event state: ":+event.state:+" for event ":+event.event;
      _message_box(msg,FTP_ERRORBOX_TITLE,MB_OK|MB_ICONEXCLAMATION);
      break;
   }
