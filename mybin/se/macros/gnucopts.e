...
#define GNUCOPTS_FORM_HEIGHT     5730
#define GNUCOPTS_FORM_WIDTH      6950
#define ALL_CONFIGS              "All Configurations"
#define COMPILE_AS_DEFAULT       "default"
#define COMPILE_AS_CPP           "c++"
#define COMPILE_AS_C             "c"
#define COMPILE_AS_OBJECTIVE_C   "objective-c"
#define OPTIMIZATION_LEVEL_NONE  "None (-O0)"
#define OPTIMIZATION_LEVEL_LOW   "Low (-O1)"
#define OPTIMIZATION_LEVEL_MED   "Med (-O2)"
#define OPTIMIZATION_LEVEL_HIGH  "High (-O3)"
#define OPTIMIZATION_LEVEL_NA    "Default"
#define DEBUG_LEVEL_MINIMAL      "Minimal (-g1)"
#define DEBUG_LEVEL_DEFAULT      "Default (-g2)"
#define DEBUG_LEVEL_MAXIMAL      "Maximal (-g3)"
#define DEBUG_LEVEL_NA           "Default"
#define OUTPUT_TYPE_EXECUTABLE   "Executable (gcc/g++)"
#define OUTPUT_TYPE_STATIC       "Object Library (*.a built with ar -rs)"
#if __MACOSX__
   #define OUTPUT_TYPE_SHARED       "Shared Library (*.dylib built with gcc/g++ -shared -fPIC)"
#else
   #define OUTPUT_TYPE_SHARED       "Shared Library (*.so built with gcc/g++ -shared -fPIC)"
#endif
#define GNU_BLANK_TREE_NODE_MSG  "<double click here to add another entry>"
#define ADV_CODEGEN_CAPTION      "Code Generation"
#define ADV_DEBUG_CAPTION        "Debug"
#define ADV_LANGUAGE_CAPTION     "Language"
#define ADV_OPT_CAPTION          "Optimization"
#define ADV_PREPROC_CAPTION      "Preprocessor"
#define ADV_MACHINE_CAPTION      "Machine"

#define GNUC_PROJECT_NAME        ctlCancel.p_user
#define GNUC_CHANGING_CONFIG     ctlCurConfig.p_user
#define GNUC_GPROJECT_HANDLE     ctlok.p_user
#define GNUC_INFO                ctlVerbose.p_user
#define GNUC_COMPILE_INDEX       ctlUsePipes.p_user
#define GNUC_LINK_INDEX          ctlSaveTemps.p_user
#define GNUC_DEBUG_INDEX         ctlUseGDB.p_user
#define GNUC_EXECUTE_INDEX       ctlUseOther.p_user
#define ADV_CODEGEN_ROOT_NODE    ctlNoDefaultLibs.p_user
#define ADV_DEBUG_ROOT_NODE      ctlNoStdLibs.p_user
#define ADV_LANGUAGE_ROOT_NODE   ctlStatic.p_user
#define ADV_OPT_ROOT_NODE        ctlSymbolic.p_user
#define ADV_PREPROC_ROOT_NODE    ctlOutputTypeLabel.p_user
#define ADV_MACHINE_ROOT_NODE    ctlOutputFileLabel.p_user
...
struct GNUC_OPTIONS
{
   ...
   boolean verbose; // -v
   boolean usePipes; // -pipe
   boolean saveTemps; // -save-temps
   boolean ansi; // -ansi
   boolean traditional; // -traditional
   ...
   boolean debugInfo; // -g
   boolean ggdbExtensions; //-ggdb
   boolean profilerInfo; // -p
   boolean gprofInfo; // -pg
   _str    debugLevel; // -g#
   boolean gasInfo;    // -mgas

   // link tab
   _str    linker; // gcc, g++, ar, etc
   boolean noDefaultLibs; // -nodefaultlibs
   boolean noStdLibs; // -nostdlibs
   boolean linkStatic; // -static
   boolean linkSymbolic; // -symbolic
   ...
   // warnings tab
   boolean showAllWarnings; // -Wall
   boolean pedantic; // -pedantic
   boolean warningsAsErrors; // -Werror
   boolean inhibitWarnings; // -w
...
{
   boolean debugging_on=ctlUseGDB.p_value? true:false;
   ctldbgDebugger.p_enabled=!debugging_on;
   ctldbgDebuggerLabel.p_enabled=!debugging_on;
   ctldbgOtherDebuggerOptions.p_enabled=!debugging_on;
   ctldbgOtherDebuggerButton.p_enabled=!debugging_on;
   ctldbgDebuggerOtherLabel.p_enabled=!debugging_on;
   ctldbgFindApp.p_enabled=!debugging_on;
   if (ctldbgDebugger.p_text=="" && !debugging_on) {
      ctldbgDebugger.p_text="gdb";
   }
   ...
   _str debuggerName = (debugging_on? "vsdebugio":ctldbgDebugger.p_text);
   if(ctlCurConfig.p_text == ALL_CONFIGS) {
      ...
      for(i._makeempty();;) {
         ...
         allGNUOptions:[i].debuggerName = debuggerName;
      }
   } else {
      allGNUOptions:[ctlCurConfig.p_text].debuggerName = debuggerName;
   }
                     ...
                     _str projectFilename = _project_name, boolean isProjectTemplate = false)
{
   ...
   // load the options
   loadGNUOptionsFromXML(get_env("VSROOT") "sysconfig" FILESEP "gnucopts.xml");

   // load the configurations list
   int wid = p_window_id;
   p_window_id = ctlCurConfig;
   ...
   for(j = 0; j < gConfigList._length(); j++) {
      ...
      if(configName == ALL_CONFIGS) continue;
...
static void _TreeSetListFromArray(GCC_OPTION_INFO options[], int parentNode = TREE_ROOT_INDEX,
                                  boolean allowDuplicates = false, int (&indexHash):[] = null)
{
   int i = 0;
 ...
 * @return
 */
static boolean getGNUOptions(int projectHandle, _str config, int targetNode, GNUC_OPTIONS& options)
{
   if(targetNode < 0) return false;

   // get target name.  if there is no name, it must be a compile rule
   _str targetName = _ProjectGet_TargetName(projectHandle, targetNode);
   if(targetName == "") {
      targetName = "compile";
   }
 ...
 */
static boolean usesGNUDialog(int projectHandle, int targetNode, GNUC_OPTIONS& options)
{
   if(targetNode < 0) return false;
 ...
 */
static boolean isEnabled(int optionHash:[], GCC_OPTION_INFO optionList[], int index)
{
 ...
 */
_str parse_next_option(_str& command, boolean returnQuotes = true,
                       boolean allowCommaDelimiter = false)
{
   int offset = 1;
   int delimiterLength = 0;

   while(offset <= length(command)) {
      _str ch = substr(command, offset, 1);

      if(ch == "\"") {
         int endQuote = pos(ch, command, offset + 1);
         if(!endQuote) {
   ...
   _str option = substr(command, 1, offset - 1 - delimiterLength);
...
static void setGNUCommand(int projectHandle, _str config, int targetNode, GNUC_OPTIONS& options)
{
   ...
   int ppindex = find_index('_GNUPreprocessTargetCmdLine',PROC_TYPE);
...
static void setGCCCommand(int projectHandle, _str config, int targetNode, GNUC_OPTIONS& options, int preprocessCmdLineIndex=0)
{
   ...
   boolean isLinkCommand = strieq(targetName, "link");

   // start with the appropriate base command for the target type (gcc, g++, etc)
   _str command = "";
   if(isLinkCommand) {
      command = maybe_quote_filename(options.linker);
   } else {
      command = maybe_quote_filename(options.compiler);
      command = command " -c";
   }
   ...
   if(!isLinkCommand) {
      ...
      if(options.traditional) {
         command = command " -traditional";
      }
      if(options.compileAs != COMPILE_AS_DEFAULT && options.compileAs != "") {
         command = command " -x " options.compileAs;
      }
   }

   // defines
   if(!isLinkCommand) {
      command = command " %defd";
   ...
   if(options.gasInfo) {
      command = command " -mgas";
   }

   // link options
   if(isLinkCommand) {
      if(options.noDefaultLibs) {
         command = command " -nodefaultlibs";
      }
      if(options.noStdLibs) {
         command = command " -nostdlibs";
      }
   ...
   if(options.pedantic) {
      command = command " -pedantic";
   }
   if(options.warningsAsErrors) {
      command = command " -Werror";
   }
   ...
   if(!isLinkCommand) {
      ...
      for(i = 0; i < GCC_WARNING_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedWarnings, GCC_WARNING_OPTIONS, i)) {
            command = command " " GCC_WARNING_OPTIONS[i].option;
         }
      }
      ...
      for(i = 0; i < GCC_CODEGENERATION_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedCodeGen, GCC_CODEGENERATION_OPTIONS, i)) {
            command = command " " GCC_CODEGENERATION_OPTIONS[i].option;
         }
      }
      ...
      for(i = 0; i < GCC_DEBUG_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedDebug, GCC_DEBUG_OPTIONS, i)) {
            command = command " " GCC_DEBUG_OPTIONS[i].option;
         }
      }
      ...
      for(i = 0; i < GCC_LANGUAGE_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedLanguage, GCC_LANGUAGE_OPTIONS, i)) {
            command = command " " GCC_LANGUAGE_OPTIONS[i].option;
         }
      }
      ...
      for(i = 0; i < GCC_OPTIMIZATION_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedOptimization, GCC_OPTIMIZATION_OPTIONS, i)) {
            command = command " " GCC_OPTIMIZATION_OPTIONS[i].option;
         }
      }
      ...
      for(i = 0; i < GCC_PREPROCESSOR_OPTIONS._length(); i++) {
         if(isEnabled(options.advancedPreprocessor, GCC_PREPROCESSOR_OPTIONS, i)) {
            command = command " " GCC_PREPROCESSOR_OPTIONS[i].option;
         }
      }

      // advanced machine tree (maintain hash to prevent duplicates)
      boolean machineOptions:[];
      for(i = 0; i < GCC_MACHINE_OPTIONS._length(); i++) {
         for(j = 0; j < GCC_MACHINE_OPTIONS[i].options._length(); j++) {
            if(isEnabled(options.advancedMachine, GCC_MACHINE_OPTIONS[i].options, j)) {
               ...
               if(machineOptions:[GCC_MACHINE_OPTIONS[i].options[j].option] != true) {
                  command = command " " GCC_MACHINE_OPTIONS[i].options[j].option;
...
//      command = command " %libs";
      command = command " %objs";
   } else {
      command = command " -o \"%bd%n%oe\"";
      command = command " %i";
      if (options.inputFilename != '') {
         command = command " " options.inputFilename;
      } else {
         command = command " \"%f\"";
      }
   }
      ...
      for(;;) {
         ...
         if (!pos('[\\/]', lib, 1, 'R')) {
            if(pos("^([Ll][Ii][Bb])(.+)([.]a)$", lib, 1, "U") > 0) {
               newLibsList = newLibsList " -l" substr(lib, pos("S2"), pos("2"));
            } else {
               newLibsList = newLibsList " " lib;
            }
         } else {
            newLibsList = newLibsList " " lib;
         }
...
static void setGDBCommand(int projectHandle, _str config, int targetNode, GNUC_OPTIONS& options, int preprocessCmdLineIndex=0)
{
   ...
   if(strieq(command, "vsdebugio")) {
      ...
      if(options.debuggerOptions != "") {
         command = command " " options.debuggerOptions;
      }

      if (otherOptions != "") {
         command = command " --args \"%o\" %~other";
      } else {
         command = command " \"%o\"";
      }
   }
      ...
      switch(parameter) {
         ...
         default:
            ...
            if(prefix == "-D") {
               ...
               _str defineValue = substr(parameter, 3);
               if(defineValue != "") {
                  if(options.defines == "") {
...
static void setGNUControls(GNUC_OPTIONS& options)
{
   ...
   for(i = 0; i < GCC_WARNING_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_WARNING_OPTIONS[i].option "\t" GCC_WARNING_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_CODEGENERATION_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_CODEGENERATION_OPTIONS[i].option "\t" GCC_CODEGENERATION_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_DEBUG_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_DEBUG_OPTIONS[i].option "\t" GCC_DEBUG_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_LANGUAGE_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_LANGUAGE_OPTIONS[i].option "\t" GCC_LANGUAGE_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_OPTIMIZATION_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_OPTIMIZATION_OPTIONS[i].option "\t" GCC_OPTIMIZATION_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_PREPROCESSOR_OPTIONS._length(); i++) {
      ...
      _str fullCaption = GCC_PREPROCESSOR_OPTIONS[i].option "\t" GCC_PREPROCESSOR_OPTIONS[i].description;
   ...
   for(i = 0; i < GCC_MACHINE_OPTIONS._length(); i++) {
      ...
      for(j = 0; j < GCC_MACHINE_OPTIONS[i].options._length(); j++) {
         ...
         _str fullCaption = GCC_MACHINE_OPTIONS[i].options[j].option "\t" GCC_MACHINE_OPTIONS[i].options[j].description;
...
void ctlCurConfig.on_change(int reason)
{
   ...
   if(configName == ALL_CONFIGS) {
      ...
      if(usesGNUDialog(projectHandle, allGNUOptions:[configName].compileTargetNode, allGNUOptions:[configName])) {
         ...
         _str compileTargetName = _ProjectGet_TargetName(projectHandle, allGNUOptions:[configName].compileTargetNode);
         if(strieq(compileTargetName, "compile")) {
            ...
            if(inputExts != "") {
               _str caption = p_active_form.p_caption;
               if(!pos(" - " allGNUOptions:[configName].compileInputExt, caption)) {
                  caption = caption " - compile(" inputExts ")";
...
void _gnuc_options_form.on_resize()
{
   ...
   widthDiff := p_width - (ctlCurConfig.p_x + ctlCurConfig.p_width + padding);
   heightDiff := p_height - (ctlok.p_y + ctlok.p_height + padding);
   ...
   xChange := (ctlIncDirLabel.p_x + ctlIncDirLabel.p_width + padding) - ctlUserIncludesList.p_x;
   ctlUserIncludesList.p_x += xChange;
   ctlUserIncludesList.p_width += (widthDiff - xChange);
   ctlMoveUserIncludesDown.p_x = ctlUserIncludesList.p_x + ctlUserIncludesList.p_width + 20;
...
static void _gnuc_options_form_initial_alignment()
{
   ...
   // compile tab
   rightAlign := ctlLanguageOptionsFrame.p_x + ctlLanguageOptionsFrame.p_width;
   ...
   // link tab
   rightAlign = ctlOutputType.p_x + ctlOutputType.p_width;
   ...
   alignUpDownListButtons(ctlUserIncludesList, rightAlign, ctlBrowseUserIncludes, ctlMoveUserIncludesUp, ctlMoveUserIncludesDown);

   // run/debug
   rightAlign = ctlDebuggerFrame.p_x + ctlDebuggerFrame.p_width;
   sizeBrowseButtonToTextBox(ctlProgramArgs, ctlRunFromButton.p_window_id, 0, rightAlign);
   rightAlign = ctlDebuggerFrame.p_width - padding;
   sizeBrowseButtonToTextBox(ctldbgDebugger.p_window_id, ctldbgFindApp.p_window_id, 0, rightAlign);
   sizeBrowseButtonToTextBox(ctldbgOtherDebuggerOptions.p_window_id, ctldbgOtherDebuggerButton.p_window_id, 0, rightAlign);
...
void ctldbgOtherDebuggerOptions.on_change()
{
   ...
   if(ctlCurConfig.p_text == ALL_CONFIGS) {
      ...
      for(i._makeempty();;) {
         ...
         allGNUOptions:[i].debuggerOptions = p_text;
      }
   } else {
      allGNUOptions:[ctlCurConfig.p_text].debuggerOptions = p_text;
   }
...
void ctldbgDebugger.on_change()
{
   ...
   if(ctlCurConfig.p_text == ALL_CONFIGS) {
      ...
      for(i._makeempty();;) {
         ...
         allGNUOptions:[i].debuggerName = p_text;
      }
   } else {
      allGNUOptions:[ctlCurConfig.p_text].debuggerName = p_text;
   }
...
void ctlProgramArgs.on_change()
{
   ...
   if(ctlCurConfig.p_text == ALL_CONFIGS) {
      ...
      for(i._makeempty();;) {
         ...
         allGNUOptions:[i].programArguments = p_text;
      }
   } else {
      allGNUOptions:[ctlCurConfig.p_text].programArguments = p_text;
   }
...
static void setAllCheckBoxStyles(int style)
{
   ...
   for(;;) {
      ...
      if(wid.p_object == OI_CHECK_BOX) {
         ...
         int firstcontainer = wid.p_child;
         int container = firstcontainer;
         for(;;) {
         ...
         wid.setAllCheckBoxStyles(style);
      }
 ...
 */
static void enableLinkOptions(boolean enable, boolean complete = false)
{
...
typeless ctlUserIncludesList.on_change(int reason,int index,int col=-1,_str value="",int wid=0)
{
   ...
   if(reason == CHANGE_EDIT_CLOSE) {
      ...
      boolean wasNewEntryNode = strieq(_TreeGetCaption(index), GNU_BLANK_TREE_NODE_MSG);
...
boolean _gdb_ConfigNeedsDebugMenu(_str debug_command)
{
   return(pos('vsdebugio',debug_command,1,'i')!=0);
}
boolean _gdb_DebugCommandCaptureOutputRequiresConcurrentProcess(_str debug_command)
{
   return(debug_command=='' || pos('vsdebugio',debug_command,1,'i')!=0);
}
boolean _gdb_ToolbarSupported(_str FormName)
{
   switch (FormName) {
...
_command void gnucoptions()
{
   ...
   typeless result = ctlbutton_wid.call_event('_gnuc_options_form',ctlbutton_wid,LBUTTON_UP,'W');
...
// wizard functions ================================================================================
#define GNUCWIZ_PROJ_EXECUTABLE        "Executable"
#define GNUCWIZ_PROJ_SHARED_LIBRARY    "Shared library"
#define GNUCWIZ_PROJ_STATIC_LIBRARY    "Static library"

#define GNUCWIZ_APP_EMPTY_PROJECT      "An empty project"
#define GNUCWIZ_APP_WITH_MAIN          "An application with a main() function"
#define GNUCWIZ_APP_HELLO_WORLD        "A \"Hello World\" application"

#define GNUCWIZ_BUILD_VSBUILD          "Build without a makefile (dependencies automatically checked)"
#define GNUCWIZ_BUILD_AUTO_MAKEFILE    "Build with an auto-generated, auto-maintained makefile"
#define GNUCWIZ_BUILD_MAKEFILE         "Build with a user-maintained makefile or custom build command"

#define GNUCWIZ_LANGUAGE_C             "C"
#define GNUCWIZ_LANGUAGE_ANSIC         "ANSI C"
#define GNUCWIZ_LANGUAGE_CPP           "C++"

#define GNUCWIZ_DNAOFE_BASE            "Do not append output file extension"
...
static int gnuc_slide0create()
{
   ...
   int doNotModifyOutputExtValue = 0;
   int i = 0;
...
#define GCC_VERSIONS 5
static _str gnuc_check_cygwin_name(boolean useCPP)
{
#if __UNIX__
   return("");
#else
...
{ 
   ...
   command = stranslate(command, gccName, 'g++');
...
static int gnuc_finish()
{
   ...
   int origViewID = p_window_id;
   boolean useCOnly = !strieq(gGNUCWizardInfo.language, GNUCWIZ_LANGUAGE_CPP);
   ...
   for(i = 0; i < configList._length(); i++) {
...
#if __UNIX__
   #if __MACOSX__
               _str sharedLibExt = "dylib";
   #else
               _str sharedLibExt = "so";
   #endif
#else
               _str sharedLibExt = "dll";
#endif
               _str outputfile = _ProjectGet_OutputFile(projectHandle, configList[i]);
               if(pos("[.]" sharedLibExt "$", outputfile, 1, "U") == 0) {
                  outputfile = outputfile "." sharedLibExt;
               }
      ...
      switch(gGNUCWizardInfo.buildSystem) {
         case GNUCWIZ_BUILD_AUTO_MAKEFILE: {
            ...
            //       is added
            _AddFileToProject(_parse_project_command(gGNUCWizardInfo.makefile, "", _project_name, ""));
            _ProjectSet_BuildMakeFile(projectHandle, gGNUCWizardInfo.makefile);

            // replace build command with "make makefilename" and clear the dialog
            _str makeCommand = _findGNUMake() " -f \"" gGNUCWizardInfo.makefile "\" CFG=%b";
            ...
            _str rebuildCommand = _findGNUMake() " -f \"" gGNUCWizardInfo.makefile "\" rebuild CFG=%b";
...
static int gnuc_show_new_project_info()
{
   ...
   status = show("-modal _new_project_info_form",
                 "GNU C wizard will create a skeleton project for you with the following\nspecifications:",
                 line);
   if(status == "") {
      return COMMAND_CANCELLED_RC;
   }
...
#if __UNIX__
   #if __MACOSX__
   ctls0_DoNotModifyOutputExt.p_caption = GNUCWIZ_DNAOFE_BASE " (.dylib)";
   #else
   ctls0_DoNotModifyOutputExt.p_caption = GNUCWIZ_DNAOFE_BASE " (.so)";
   #endif
#else
   ctls0_DoNotModifyOutputExt.p_caption = GNUCWIZ_DNAOFE_BASE " (.dll)";
#endif
}
