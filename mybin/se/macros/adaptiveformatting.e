...
boolean adaptive_format_is_adaptive_on(_str lang='')
{
   if (lang=='') {
      if (_isEditorCtl()) {
         lang = p_LangId;
...
void adaptive_format_set_adaptive_on(boolean value, _str lang = '')
{
   if (lang=='') {
      if (_isEditorCtl()) {
         lang = p_LangId;
   ...
   if (lang != '') {
      ...
      if (!value) {
         adaptive_format_reset_buffers(-1, lang);
      }
   }
}
 ...
 * 
 * @return boolean         whether adaptive formatting is on for 
 *                         that setting in that language
 */
boolean adaptive_format_is_flag_on_for_buffer(int flags, _str lang, int adaptiveFlags = null) 
{
...
boolean adaptive_format_is_flag_on_for_language(int flags, _str lang, int adaptiveFlags = null)
{
   if (adaptiveFlags == null) {
...
void revertCurrentBuffer(int flags)
{
   ...
   if (options != null) {
      if (flags & AFF_SYNTAX_INDENT) {
         bufId.p_SyntaxIndent = options.SyntaxIndent;
      }
      if (flags & AFF_TABS) {
         bufId.p_tabs = options.szTabs;
      }
      if (flags & AFF_INDENT_WITH_TABS) {
         bufId.p_indent_with_tabs = options.IndentWithTabs;
      }
      if (flags & AFF_BEGIN_END_STYLE) {
         bufId.p_begin_end_style = options.BeginEndStyle;
      }
      if (flags & AFF_INDENT_CASE) {
         bufId.p_indent_case_from_switch = (options.IndentCaseFromSwitch != 0);
      }
      if (flags & AFF_NO_SPACE_BEFORE_PAREN) {
         bufId.p_no_space_before_paren = (options.NoSpaceBeforeParen != 0);
      }
      if (flags & AFF_PAD_PARENS) {
         bufId.p_pad_parens = (options.PadParens != 0);
      }
      if (flags & AFF_KEYWORD_CASING) {
         bufId.p_keyword_casing = options.KeywordCasing;
      }
      if (flags & AFF_TAG_CASING) {
         bufId.p_tag_casing = options.TagCasing;
      }
      if (flags & AFF_ATTRIBUTE_CASING) {
         bufId.p_attribute_casing = options.AttributeCasing;
      }
      if (flags & AFF_VALUE_CASING) {
         bufId.p_value_casing = options.ValueCasing;
      }
      if (flags & AFF_HEX_VALUE_CASING) { 
         bufId.p_hex_value_casing = options.HexValueCasing;
      }
   }
...
_str compileUpdateString(int flags, _str lang)
{
   ...
   // check for tabs, syntax indent, indent with tabs
   if (flags & AFF_SYNTAX_INDENT) {
      ustr = ustr :+ SYNTAX_INDENT_UPDATE_KEY'='options.SyntaxIndent',';
   }
   if (flags & AFF_TABS) {
      ustr = ustr :+ TABS_UPDATE_KEY'='options.szTabs',';
   }
   if (flags & AFF_INDENT_WITH_TABS) {
      ustr = ustr :+ INDENT_WITH_TABS_UPDATE_KEY'='options.IndentWithTabs',';
   }
   if (flags & AFF_BEGIN_END_STYLE) {
      ustr = ustr :+ BEGIN_END_STYLE_UPDATE_KEY'='options.BeginEndStyle',';
   }
   if (flags & AFF_INDENT_CASE) {
      ustr = ustr :+ INDENT_CASE_FROM_SWITCH_UPDATE_KEY'='options.IndentCaseFromSwitch',';
   }
   if (flags & AFF_NO_SPACE_BEFORE_PAREN) {
      ustr = ustr :+ NO_SPACE_BEFORE_PAREN_UPDATE_KEY'='options.NoSpaceBeforeParen',';
   }
   if (flags & AFF_PAD_PARENS) {
      ustr = ustr :+ PAD_PARENS_UPDATE_KEY'='options.PadParens',';
   }
   if (flags & AFF_KEYWORD_CASING) {
      ustr = ustr :+ KEYWORD_CASING_UPDATE_KEY'='options.KeywordCasing',';
   }
   if (flags & AFF_TAG_CASING) {
      ustr = ustr :+ TAG_CASING_UPDATE_KEY'='options.TagCasing',';
   }
   if (flags & AFF_ATTRIBUTE_CASING) {
      ustr = ustr :+ ATTRIBUTE_CASING_UPDATE_KEY'='options.AttributeCasing',';
   }
   if (flags & AFF_VALUE_CASING) {
      ustr = ustr :+ VALUE_CASING_UPDATE_KEY'='options.ValueCasing',';
   }
   if (flags & AFF_HEX_VALUE_CASING) { 
      ustr = ustr :+ HEX_VALUE_CASING_UPDATE_KEY'='options.HexValueCasing',';
   }

   ustr = ustr :+ ADAPTIVE_FORMATTING_FLAGS_UPDATE_KEY'='adaptive_format_get_buffer_flags(lang);
 ...
 *         adaptive formatting. 
 */
boolean areAdaptiveFormattingTabSettingsRequiredImmediately(int flag)
{
 ...
 */
void updateAdaptiveFormattingSettings(int flag, boolean confirm = true, int* updatedFlags = null)
{
   ...
   if (getNotificationMethod(NF_ADAPTIVE_FORMATTING) == NL_DIALOG && p_mdi_child && confirm) {
...
_command void adaptive_format_stats() name_info(','VSARG2_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   if (scanner != null) {
      ...
      message("Adaptive Formatting not available for "_LangId2Modename(lang)".");
   }
}
...
static boolean matchesBufferSetting(_str value, int flag)
{
   bufId := _edit_window();
...
positionAndSetIndentSettings(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                             AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
...
positionAndSetKeywordCasing(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                            AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetTagCasing(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                        AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetAttributeCasing(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                              AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetValueCasing(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                          AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetHexValueCasing(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                             AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetIndentCase(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                         AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   int value;
...
positionAndSetParenSettings(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                            AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething)
{
   se.adapt.AFParenSettings ps;
...
positionAndSetBeginEndStyle(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags, boolean automaticScan, 
                            AdaptiveFormattingSettings &afs, int &shift, int &hiddenFlags, boolean &showSomething, 
                            _str beStyles[])
{
   int value;
...
void _ctl_ok.on_create(se.adapt.AdaptiveFormattingScannerBase * scanner, int flags = -1, 
                       _str beStyles[] = null, boolean automaticScan = false)
{
   p_active_form.p_caption=_LangId2Modename(scanner->getLangId())' 'p_active_form.p_caption;
...
void realignBEStyles(int width, int styles)
{
   ...
   if (styles == 2) {
      ...
      _lbl_bs2.p_x = _lbl_bes2.p_x = _lbl_bs1.p_x + _lbl_bs1.p_width + (width * 2);;
...
void gatherSyntaxIndentSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherIndentWithTabsSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherTabsSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherKeywordCasingSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
   if (_cb_use_keyword_casing.p_visible) {
...
void gatherTagCasingSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{   
   if (_cb_use_tag_casing.p_visible) { 
...
void gatherAttributeCasingSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
   if (_cb_use_attribute_casing.p_visible) { 
...
void gatherValueCasingSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
   if (_cb_use_value_casing.p_visible) { 
...
void gatherHexValueCasingSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
   if (_cb_use_hex_value_casing.p_visible) {
...
void gatherIndentCaseSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherNoSpaceBeforeParensSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherPadParensSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void gatherBeginEndStyleSettings(AdaptiveFormattingSettings &afs, int &flags, int bufId, boolean automaticScan)
{
...
void _ctl_ok.lbutton_up()
{
   ...
   if (automaticScan) {
      if (_cb_confirm.p_value) {
         setNotificationMethod(NF_ADAPTIVE_FORMATTING, NL_MESSAGE);
      } else {
         setNotificationMethod(NF_ADAPTIVE_FORMATTING, NL_DIALOG);
      }
   }
...
void setFormattingOptionsForExtension(AdaptiveFormattingSettings afs)
{
   ...
   if (afs.SyntaxIndent >= 0) {
      ...
      updateString = updateString :+ SYNTAX_INDENT_UPDATE_KEY'='afs.SyntaxIndent;
   }
   ...
   if (_LanguageInheritsFrom('html') || _LanguageInheritsFrom('xml')) {
      ...
      if (afs.TagCasing > AF_RC_ERROR) {
         ...
         updateString = updateString :+ TAG_CASING_UPDATE_KEY'='afs.TagCasing;
      }

      // attribute casing
      if (afs.AttributeCasing > AF_RC_ERROR) {
         LanguageSettings.setAttributeCase(langID, afs.AttributeCasing);
         updateString = updateString :+ ATTRIBUTE_CASING_UPDATE_KEY'='afs.AttributeCasing;
      }

      // value casing
      if (afs.ValueCasing > AF_RC_ERROR) {
         LanguageSettings.setValueCase(langID, afs.ValueCasing);
         updateString = updateString :+ VALUE_CASING_UPDATE_KEY'='afs.ValueCasing;
      }

      // hex value casing
      if (afs.HexValueCasing > AF_RC_ERROR) {
         LanguageSettings.setHexValueCase(langID, afs.HexValueCasing);
         updateString = updateString :+ HEX_VALUE_CASING_UPDATE_KEY'='afs.HexValueCasing;
      }
      ...
      if (afs.KeywordCasing > AF_RC_ERROR) {
         ...
         updateString = updateString :+ KEYWORD_CASING_UPDATE_KEY'='afs.KeywordCasing;
      }

      // brace style
      if (afs.BeginEndStyle >= 0) {
         LanguageSettings.setBeginEndStyle(langID, afs.BeginEndStyle);
         updateString = updateString :+ BEGIN_END_STYLE_UPDATE_KEY'='afs.BeginEndStyle;
      }

      // padding between parens
      if (afs.PadParens >= 0) {
         LanguageSettings.setPadParens(langID, (afs.PadParens != 0));
         updateString = updateString :+ PAD_PARENS_UPDATE_KEY'='afs.PadParens;
      }

      // no space before paren
      if (afs.NoSpaceBeforeParens >= 0) {
         LanguageSettings.setNoSpaceBeforeParen(langID, (afs.NoSpaceBeforeParens != 0));
         updateString = updateString :+ NO_SPACE_BEFORE_PAREN_UPDATE_KEY'='afs.NoSpaceBeforeParens;
      }

      // indent case from switch
      if (afs.IndentCaseFromSwitch >= 0) {
         LanguageSettings.setIndentCaseFromSwitch(langID, (afs.IndentCaseFromSwitch != 0));
         updateString = updateString :+ INDENT_CASE_FROM_SWITCH_UPDATE_KEY'='afs.IndentCaseFromSwitch;
      }
   }
   ...
   if (afs.Tabs != '') {
      ...
      updateString = updateString :+ TABS_UPDATE_KEY'='afs.Tabs;
   }

   // indent with tabs
   if (afs.IndentWithTabs >= 0) {
      LanguageSettings.setIndentWithTabs(langID, (afs.IndentWithTabs != 0));
      updateString = updateString :+ INDENT_WITH_TABS_UPDATE_KEY'='afs.IndentWithTabs;
   }
