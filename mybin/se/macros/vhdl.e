...
#define VHDL_LEXER "VHDL"
#define VHDL_LANGUAGE_ID        'vhd'
#define VHDL_MODE_NAME          'VHDL'
#define VHDL_TABS               '+4'           // Tab stops every 4 characters
#define VHDL_MARGINS            '1 74 1'       // Margin column for left, right, new paragraph
#define VHDL_KEYTAB_NAME        'vhd-keys'     // Eventtab name
#define VHDL_WORD_WRAP_STYLE    '0'            // 1=ON, 0=OFF
#define VHDL_INDENT_WITH_TABS   '0'            // 1=ON
#define VHDL_SHOW_TABS          DEFAULT_SPECIAL_CHARS
#define VHDL_INDENT_STYLE       '2'            // 0=None, 1=Auto, 2=Syntax
#define VHDL_WORD_CHARS         'A-Za-z0-9_$'
#define VHDL_LEXER_NAME         'VHDL'
#define VHDL_COLOR_FLAGS        '1'            // 1=Use Language color flags
#define VHDL_WORD_SEP           '([~a-zA-Z0-9_]|^|$)'
#define VHDL_IDENTIFIER         '[a-zA-Z0-9_]'
...
// ****************************************************************************
// VHDL File Extension Setup (*.vhd)
// ****************************************************************************
defload()
{
   // defload will only run this block of code ONCE when this macro is loaded

   _str setup_info='MN='VHDL_MODE_NAME',TABS=+4,MA=1 74 1,KEYTAB='VHDL_KEYTAB_NAME',WW='VHDL_WORD_WRAP_STYLE',IWT='VHDL_INDENT_WITH_TABS',ST='VHDL_SHOW_TABS',IN='VHDL_INDENT_STYLE',WC=A-Za-z0-9_$,LN='VHDL_LEXER_NAME',CF='VHDL_COLOR_FLAGS',LNL=0,';
   _str compile_info="";

   // DEVELOPER NOTE: 'YOU' DEFINE THIS STRING: The first 2 are pre-defined,
   // the remaining n subelements you can define.
   // we are simply storing away a string which contains a bunch of switch settings
   // that we want to maintain and use whenever a buffer displaying a file of this
   // extension is active. name_info(_edit_window().p_index) will return this
   // identical string. You can then use "parse" to break it back into fieldnames.
   _str syntax_info='4 ':+   // SyntaxIndent: 4 spaces
                        //
               '1 ':+   // SyntaxExpand: ON
                        //
               '1 ':+   // MinimumAbbreviation : 1 (default)
                        // Specify large value to avoid abbreviation expansion.
                        //
               '-1 ':+  // KeywordCase:   -1=Preserve; 0=lowercase; 1=UPPERCASE; 2=Capitalized
                        //
               '4 ':+   // BeginEndStyle (not implemented) :
                        //
                        //       Style 0:
                        //           block
                        //           begin
                        ...
                        // during expansion.  (defaults to on).
                        //
               '0 ':+   // AutoInsertLabel: OFF
               '0 ':+   // Unused_6
               '0';     // Unused_7:

   
   _CreateLanguage(VHDL_LANGUAGE_ID, VHDL_MODE_NAME,
                   setup_info, compile_info, syntax_info, VHDLBeginEndPairs);
   _CreateExtension('vhd',VHDL_LANGUAGE_ID);
   _CreateExtension('vhdl',VHDL_LANGUAGE_ID);
} // defload
 ...
 */
_str _vhd_keyword_case(_str s, boolean confirm = true)
{
...
{
   int block_count=0;
   boolean expecting_declaration=0;
   typeless result="";
...
static boolean _vhdl_check_component_instantiation(_str kw='')
{
...
static _str _vhd_expand_space()
{
   ...
   VHDLword=min_abbrev2(PartialWord,_Keywords,name_info(p_index),aliasfilename);
   ...
   line=substr(line,1,length(line)-length(PartialWord)):+_word_case(VHDLword);
   ...
   if( label!="" && pos(' 'VHDLword' ',_KeywordsPreceededByLabels) ) {
      ...
      line=before:+_word_case(label):+after;
   } else {
   ...
   if( VHDLword=='assert' ) {
      ...
      if( insertBE ) {
         if( label!="" ) {
            // Label exists
            insert_line(indent_string(width):+_word_case('end')' '_word_case(label)';');
      ...
      if( insertBE ) {
         ...
         if( label=="" ) {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('block')';');
         } else {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('block')' '_word_case(label)';');
         }
         set_surround_mode_end_line();
         up(2);
         _end_line();
      ...
      //   end case;
      new_line=line:+' ':+_word_case(' is');
      replace_line(new_line);
      if( LanguageSettings.getInsertBeginEndImmediately(p_LangId) ) {
         insert_line(indent_string(width+SyntaxIndent):+_word_case('when')' => ;');
         if( label=="" ) {
            insert_line(indent_string(width)_word_case('end')' '_word_case('case')';');
         } else {
            insert_line(indent_string(width)_word_case('end')' '_word_case('case')' '_word_case(label)';');
         }
         set_surround_mode_end_line();
         up(2);
      ...
      if( AutoInsertLabel && label=="" ) {
         ...
         new_line=_word_case(label)' : 'line'  '_word_case('in')'  '_word_case('loop');
      } else {
      ...
      if( LanguageSettings.getInsertBeginEndImmediately(p_LangId) ) {
         if( label=="" ) {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('loop;'));
         } else {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('loop')' '_word_case(label)';');
         }
         set_surround_mode_end_line();
         up(1);
      ...
      if( LanguageSettings.getInsertBeginEndImmediately(p_LangId) ) {
         insert_line(indent_string(width):+_word_case('begin'));
         if( label=="" ) {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('process')';');
         } else {
            insert_line(indent_string(width):+_word_case('end')' '_word_case('process')' '_word_case(label)';');
         }
         up(2);
         _end_line();
         ++p_col;
      ...
      // Strip away leading and trailing spaces
      line=strip(line,'B');
      replace_line(indent_string(width):+line'  '_word_case('of')'  '_word_case('is'));
      if( LanguageSettings.getInsertBeginEndImmediately(p_LangId) ) {
         insert_line(indent_string(width):+_word_case('begin'));
         insert_line(indent_string(width):+_word_case('end')' '_word_case(VHDLword)';');
         up(2);
      }
      //insert_mode();        // Goto insertion mode
      ...
      if (!_vhdl_check_component_instantiation(VHDLword)) {
         line=strip(line,'B');
         replace_line(indent_string(width):+line'  '_word_case('is'));
         if( LanguageSettings.getInsertBeginEndImmediately(p_LangId) ) {
            insert_line(indent_string(width):+_word_case('end')' '_word_case(VHDLword)';');
            up();
         }
      ...
      if( AutoInsertLabel && label=="" ) {
         VHDLLabelAutoCounter++;
         label='loop_n'VHDLLabelAutoCounter;
         new_line=_word_case(label)' : 'line'  '_word_case('loop');
      } else {
 ...
 * @return
 */
_command vhd_space() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
...
static int declarative_part()
{
   ...
   while( status==0 ) {
      get_line(auto prev_line);
      _str prev_first_word="", prev_second_word="", rest="";
 ...
 * @return
 */
_command void vhd_enter() name_info(','VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL)
{
...
//
#define ModelTechErr_VHDL '[* ]*Error\: {#0:p}\({#1:i}\)\: {#3?@}'
...
//
#define AlteraErr_VHDL 'Error\: Line {#1:i}\: File {#0:p}\: {#3?@}'
...
//
#define VantageErrLine1_VHDL '?*[ \t]+(Compiling)[ \t]{#0:q}'
#define VantageErrLine2_VHDL '\*\*Error\: LINE {#1:i}{#0} \*\*\* {#3?@}'
...
// @END
//
#define SynplifyErr_VHDL '\@E\:\"{#0:p}\"\:{#1:i}\::i\::i\::i\|{#3?@}'

#define _ErrParseLine1_VHDL '^'VantageErrLine1_VHDL
#define _ErrParseLine2_VHDL '^('ModelTechErr_VHDL')|('VantageErrLine2_VHDL')|('AlteraErr_VHDL')'
...
static void get_filename_onprevline(_str &filename)
{
   ...
   int status=search('^'VantageErrLine1_VHDL,'@rhi-');
...
boolean _get_error_info_vhdl(_str &filename,_str &linenum, _str &col, _str &err_msg)
{
   get_line(auto line);
   if( !pos(_ErrParseLine2_VHDL,line,1,'ri')) {
      return(false);
   }
   _str found_filename=substr(line,pos('S0'),pos('0'));
   _str found_linenum=substr(line,pos('S1'),pos('1'));
   _str found_col=substr(line,pos('S2'),pos('2'));
   _str found_err_msg=substr(line,pos('S3'),pos('3'));
...
int _vhd_MaybeBuildTagFile(int &tfindex)
{
   return ext_MaybeBuildTagFile(tfindex,'vhd','vhdl',"VHDL Builtins");
}
 ...
 * @since 11.0
 */
int _vhd_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
 ...
 */
int _vhd_find_matching_word(boolean quiet)
{
   return _ada_find_matching_word(quiet);
}
