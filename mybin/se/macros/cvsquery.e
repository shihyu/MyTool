...
#define CVS_QUERY_CAPTION_PREFIX 'CVS query for '
#define CVS_QUERY_OUTPUT_CAPTION_PREFIX 'CVS query output for '

#define STOP_CAPTION  '&Stop'
#define CLOSE_CAPTION 'Close'

_command int cvs_query(_str filename='') name_info(FILE_ARG'*,'VSARG2_CMDLINE|VSARG2_REQUIRES_MDI)
{
...
static _str GetBranch(_str RevisionNumber)
{
   ...
   _str Branch=substr(RevisionNumber,1,lastp);
   return(Branch);
}
...
{
   _str last_branch='';
   int xml_last_index=-1;
   int version_list_len=VersionList._length();
...
static _str GetNamedBranch(_str BranchNumber,CVS_VERSION_INFO (&NamedBranches)[])
{
   ...
   for (i=0;i<len;++i) {
      if ( BranchNumber==ConvertedBranchNumber(NamedBranches[i].RevisionNumber) ) {
                         ...
                         CVS_VERSION_INFO (&NamedBranches)[])
{
   ...
   for (; xml_index>-1 ;) {
      ...
      _str cap=_xmlcfg_get_name(xml_handle,xml_index);
      if (xml_index!=TREE_ROOT_INDEX) {
         if (last_char(cap)!='.') {
            ...
            _str end_version=substr(cap,length(cap)-1,2);
            if ( end_version=='.1' ) {
               ...
               if ( xml_parent_index!=TREE_ROOT_INDEX ) {
                  ...
                  for (;;) {
                     ...
                     _str cur_cap=_xmlcfg_get_name(xml_handle,xml_parent_index);
                     if (last_char(cur_cap)=='.') continue;
...
static _str GetHTMLFriendlyTagCaption(_str tag_caption)
{
   ...
   if ( sig!='' ) html_friendly_tag_caption=html_friendly_tag_caption'(...)';
...
static void InitMatchedVersions(boolean (&MatchedVersions):[],CVS_VERSION_INFO (&VersionList)[])
{
...
#define QUERY_VERSION_INFO ctlstop.p_user
#define LOCAL_FILENAME ctlinfo.p_user

void ctlok.lbutton_up()
{
   int query_form_wid=p_active_form;
   ...
   boolean symbol_changed=ctlsymbol_changed.p_value!=0;
   boolean by_user=ctlby_user.p_value!=0;
   boolean by_date=ctlby_date.p_value!=0;
   ...
   _str start_date=ctlstart_date.p_text;
   _str end_date=ctlend_date.p_text;
   typeless start_month,start_day,start_year;
   typeless end_month,end_day,end_year;
   int start_int_day=0,end_int_day=0;

   boolean MatchedVersions:[]=null;
   ...
   if ( by_date ) {
      int len=info.VersionList._length();
      int i;
      for (i=0;i<len;++i) {
         ...
         boolean start_date_ok=!start_int_day || ! (cur_int_day<start_int_day);
         boolean end_date_ok=!end_int_day || ! (cur_int_day>end_int_day);
   ...
   remote_filename=remote_filename:+_strip_filename(filename,'P');
   ...
   _str tag_caption=ctlsymbol_name.p_text;
   query_form_wid.p_active_form._delete_window();
   if ( symbol_changed ) {
      int startline=0,endline=0,commentline=0;
      _str type='';
      _str tag_name='';
...
{
   return(maybe_quote_filename(cur_version)' 'maybe_quote_filename(cur_tempfile)' 'maybe_quote_filename(cur_start_line_number)' 'maybe_quote_filename(cur_end_line_number));
}
                                  ...
                                  _str RemoteFilename)
{
   ...
   boolean tip_will_be_compared=pos("\n"branches_to_compare"\n","\nTip\n")!=0;

   boolean SaveVersions:[];

   typeless i;
   _str orig_dir=getcwd();
   boolean version_changed=false;
   OLD_VERSION_SYMBOL_INFO version_info:[];
   boolean existed_in_last_version=false;
   outerloop:
   for (i._makeempty();;) {
      branches._nextel(i);
      if (i._isempty()) break;
      if ( pos("\n"i"\n","\n"branches_to_compare"\n") ) {
         ...
         int last_start_line_number=0,last_end_line_number=0,last_comment_line_number=0;
         _str last_tag_type='';
         _str last_tempfile=last_tempdir:+FILESEP:+just_name;
         ...
         for (j=1;j<len;++j) {
            ...
            boolean compare_these_versions=(MatchedVersions==null) || (MatchedVersions:[last_version]==true) || (MatchedVersions:[cur_version]==true);
            ...
            int cur_start_line_number=0,cur_end_line_number=0,cur_comment_line_number=0;
            _str cur_tag_type='';
            _str cur_tempfile=cur_tempdir:+FILESEP:+just_name;
                           ...
                           boolean &sections_matched)
{
   ...
   int OutputBufId;
   status=Diff(temp_view_id1,
               temp_view_id2,
               def_diff_options|DIFF_NO_BUFFER_SETUP,
               0,
               0,
               0,
               def_load_options,
...
static void SetInfoPane(boolean by_date,boolean by_user,boolean symbol_changed,
                        _str start_date,_str end_date,_str user_name,_str html_friendly_tag_caption)
{
   ...
   if ( by_date ) {
      ...
      if ( end_date!='' ) {
         if ( info_text!='' ) info_text=info_text' AND ';
         info_text=info_text'Versions commited on or before <B>'end_date'</B>';
      }
   }
   if ( by_user ) {
      if ( info_text!='' ) info_text=info_text'<P><B>AND</B><P>';
      info_text=info_text'Versions commited by user <B>'user_name'</B>';
   }
   if ( symbol_changed ) {
      if ( info_text!='' ) info_text=info_text'<P><B>AND</B><P>';
      info_text=info_text'Versions where the symbol <B>'html_friendly_tag_caption'</B> changed';
   }
...
void ctlstop.on_create()
{
   ...
   if (isinteger(font_size)) {
      ctlminihtml1._minihtml_SetProportionalFontSize(3,(int)font_size*10);
   }
...
void _cvs_query_output_form.on_resize()
{
   ...
   ctlinfo.p_y=ctlminihtml1.p_y+ctlminihtml1.p_height+ybuffer;
   ctlinfo.p_width=ctlminihtml1.p_width;
   ctlinfo.p_height=(height_available intdiv 4);

   ctlstop.p_y=ctlinfo.p_y+ctlinfo.p_height+ybuffer;
...
void ctlminihtml1.on_change(int reason,_str hrefText)
{
   ...
   if ( reason==CHANGE_CLICKED_ON_HTML_LINK ) {
      ...
      if (!whole_file) {
         diff_command_line=diff_command_line' -range1:'pversion1_info->StartLine','pversion1_info->EndLine' -range2:'pversion2_info->StartLine','pversion2_info->EndLine;
      }
      diff_command_line=diff_command_line' 'maybe_quote_filename(pversion1_info->filename)' 'maybe_quote_filename(pversion2_info->filename);
...
void ctlstop.on_destroy()
{
   ...
   for (i._makeempty();;) {
      ...
      if ( cur_filename!='' && cur_filename!=null) {
         _DelTree(_file_path(version_info:[i].filename),1);
      }
   }
}
