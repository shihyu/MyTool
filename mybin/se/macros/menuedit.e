...
//#define DEBUG 1
#define MENUEDIT_PIC_LFIRST_INDENT_X 100
#define MENUEDIT_PIC_LSPACE_Y 60     // Extra line spacing for list box.
#define MENUEDIT_PIC_LINDENT_X 250    // Indent before for list box bitmap.
   //STRIP_KEYSTRING=1;

/*
   list1.p_user (TEMP_INDICES)         Space delimited tindex's. stack of menu nest level.
                                       First number is top tindex of stack.  There is
                                       always at least one tindex.
   _ok.p_user (LINE_NUM_STACK)         Stack of space delimited numbers.

   _menu_name.p_user (MENU_OPTIONS)    space delimited string of following:

       new-bool original-menu-name original-menu-index temp-menu-index

   _cancel.p_user (EDITING_MDI)        Non-zero if editing mdi menu
   _item_help.p_user (TREE_SELECTED_INDEX)    Line number of old selected line.

*/
//static _str dcount=0;
...
_command void reset_menu(_str menuNameArg = '') name_info(',')
{
   ...
   if (installVerIndex > 0) {
      ...
      if (curVerIndex > 0) delete_name(curVerIndex);
...
#define MENU_OPTIONS          _menu_name.p_user
#define TEMP_INDICES          list1.p_user
#define EDITING_MDI           _cancel.p_user
#define LINE_NUM_STACK        _ok.p_user
#define TREE_SELECTED_INDEX          _item_help.p_user
...
_ok.on_create(_str menu_name="")
{
   ...
   if (!tindex) {
      ...
      if (menu_name=='') {
         ...
         for (;;) {
            if (name=='') break;
            _str prefix=substr(name,1,4);
            _str number=substr(name,5);
   ...
   if (!new_option) {
      ...
      if (tindex<=0) {
         _message_box(nls("Unable to temp menu")"\n\n"get_message(tindex));
         p_active_form._delete_window();
         return('');
      }
...
void _ok.lbutton_up()
{
   ...
   typeless tindex="";
   parse MENU_OPTIONS with new_option menu_name orig_tindex tindex . ;
   _str new_name=strip(_menu_name.p_text);
...
#define MENU_EDITOR_MIN_WIDTH       6200
#define MENU_EDITOR_MIN_HEIGHT      5800
...
void _menu_editor_form.on_resize()
{
   ...
   widthDiff := width - (_insert_child.p_x + _insert_child.p_width + padding);
   heightDiff := height - (list1.p_y + list1.p_height + padding);
...
void _down.lbutton_up()
{
   ...
   _str caption=list1._TreeGetCaption(index);
   int add_parent=list1._TreeGetNextSiblingIndex(index);
   int add_flags;
...
void _up.lbutton_up()
{
   ...
   _str caption=list1._TreeGetCaption(index);
   int add_parent=list1._TreeGetPrevSiblingIndex(index);
   int add_flags;
   int move_to_position;
   if (add_parent<0) {
...
static void update_up_down_enable() {
   ...
   int next=list1._TreeGetNextSiblingIndex(index);
   int prev=list1._TreeGetPrevSiblingIndex(index);
   if (next>=0 || prev>=0) {
...
static void insert_new_item(int add_parent,int add_flags,int tindex, int position)
{
   ...
   _item_short_cut.p_enabled=_item_scutlabel.p_enabled=1;
   _item_command.p_enabled=_alias.p_enabled=1;_item_cmdlabel.p_enabled=1;
   _item_popup.p_enabled=1;
...
static void update_item_props(int child)
{
   ...
   if (child.p_object==OI_MENU) {
      ...
      _item_short_cut.p_enabled=_item_scutlabel.p_enabled=0;
      _item_command.p_enabled=_alias.p_enabled=0;_item_cmdlabel.p_enabled=0;
      _item_popup.p_value=1;
      _item_popup.p_enabled=0;
   } else {
      _str before="";
...
static boolean maybe_commit_changes()
{
...
static boolean maybe_delete_blank_line()
{
   if (TREE_SELECTED_INDEX=='') {
      return true;
   }
...
#define AUTO_ENABLE_LIST 'ab-sel=1 block=2 clipboard=3 fileman=4 mto-buffer=5 mto-window=6 ncw=7 nicon=8 nrdonly=9 sel=10 undo=11'
defeventtab _menucat_form;
_ok.lbutton_up()
{

   _str list=strip(_user_cat.p_text);
   int wid=_find_control('check1');
   for (;;) {
      if (wid.p_value) {
...
_ok.on_create(_str list="")
{
   ...
   for (;;) {
      ...
      if (value!='') {
         int wid=_find_control('check'value);
         wid.p_value=1;
      } else {
         if (user_list=='') {
            user_list=cat;
         } else {
            user_list=user_list'|'cat;
         }
      }
   }
