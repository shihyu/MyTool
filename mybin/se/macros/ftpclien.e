...
int _ftpclientQFormWid()
{
   int formWid;
   if (isEclipsePlugin()) {
      formWid = _find_object(ECLIPSE_FTPCLIENT_CONTAINERFORM_NAME,'n');
      if (formWid > 0) {
         formWid = formWid.p_child;
      }
   } else {
      formWid=_find_object(TBFTPCLIENT_FORM_NAME,'n');
   }
   return formWid;
}

_command void ftpClient() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   int formWid=_ftpclientQFormWid();
   tbShow(TBFTPCLIENT_FORM_NAME);
   if( formWid ) {
      _str msg="";
...
boolean _ftpclientChangeProfileOnOff(_str onoff="")
{
   if( onoff != "" ) {
...
boolean _ftpclientChangeDrvListOnOff(_str onoff="")
{
   if( onoff != "" ) {
...
boolean _ftpclientChangeLocalCwdOnOff(_str onoff="")
{
   if( onoff != "" ) {
...
static boolean gchangelocaldir_allowed=true;
boolean _ftpclientChangeLocalDirOnOff(_str onoff="")
{
   if( onoff!="" ) {
...
boolean _ftpclientChangeRemoteCwdOnOff(_str onoff="")
{
   if( onoff!="" ) {
...
boolean _ftpclientChangeRemoteDirOnOff(_str onoff="")
{
   if( onoff!="" ) {
...
void _ftpclientFillProfiles(boolean set_textbox)
{
   boolean oldchangeprofile_allowed=_ftpclientChangeProfileOnOff();
...
int _ftpclientChangeProfile(_str profile_name="")
{
   ...
   boolean oldchangeprofile_allowed;
...
// This expects the active window to be a tree view.
static int _RefreshLocalDir(FtpConnProfile *fcp_p, boolean quiet=false)
{
   if( !fcp_p ) return(1);

   // Generate a temp view with the local file listing
   int tree_view_id=p_window_id;
   int temp_view_id=0;
   ...
   _str filespec=cwd:+ALLFILES_RE;
   int status=insert_file_list(maybe_quote_filename(filespec):+' +ADV');
   if( status && status!=FILE_NOT_FOUND_RC ) {
      // Failed, so try the current working directory
      _str curcwd=getcwd();
      if( last_char(curcwd)!=FILESEP ) curcwd=curcwd:+FILESEP;
      if( cwd!=curcwd ) {
         cwd=curcwd;
         fcp_p->localCwd=cwd;
         if( !quiet ) {
   ...
   for(;;) {
      ...
      _str line='';
      get_line(line);
      typeless size, date, time, attribs, filename;
      ...
      //say('caption='caption);
      // Directory name vs. File name
      _str dirInfo = (pos('d',lowcase(attribs),1,'e'))? "D":"F";
      int idx=_TreeAddItem(TREE_ROOT_INDEX,caption,TREE_ADD_AS_CHILD,picidx,picidx,-1,0,dirInfo);
   }
   _delete_temp_view(temp_view_id);
   p_window_id=tree_view_id;
   int nidx=0;
...
static void _RefreshRemoteDirFields()
{
   ...
   for(;;) {
      ...
      typeless filename, size, date, time, attribs;
...
void _ftpclientProgressCB(_str operation, typeless nofbytes, typeless total_nofbytes)
{
   ...
   int label1Wid = formWid._find_control('_ctl_progress_label1');
   if( !label1Wid ) {
      return;
   }
   int label2Wid = formWid._find_control('_ctl_progress_label2');
   if( !label2Wid ) {
      return;
   }
   int gaugeWid = formWid._find_control('_ctl_progress');
   if( !gaugeWid ) {
      return;
   }
   int groupWid = formWid._find_control('_ctl_group1');
   if( !groupWid ) {
      return;
   }
   int buttonWid = formWid._find_control('_ctl_abort');
   if( !buttonWid ) {
      return;
   }
   ...
   _str label1_text = strip(operation);
   _str label2_text = '';
...
void _ftpclientUpdateRemoteSession(boolean force)
{
   int formWid;
...
void _ftpclientUpdateSession(boolean force)
{
   int formWid;
 ...
 */
static void _ftpclientConnect(FtpConnProfile* fcp_p, boolean sync,
 ...
 */
_command void ftpclientConnect()
{
   FtpConnProfile fcp;
 ...
 *                     a connection for further processing.
 */
static void _ftpclientDisconnect(FtpConnProfile* fcp_p, boolean sync,
                                 boolean reconnecting=false, FtpQEvent& lastEvent=null)
{
 ...
 */
_command void ftpclientDisconnect()
{
...
_command void ftpclientAutoRefresh() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   int formWid;
...
void _ftpclientUpDownLoadRefresh(FtpConnProfile *fcp_p, _str option="")
{
   ...
   if( formwid ) {
      ...
      if( option!='R' ) {
         ...
         if( localcwd_wid ) {
            boolean old_gchangelocalcwd_allowed=_ftpclientChangeLocalCwdOnOff();
            _ftpclientChangeLocalCwdOnOff(0);
      ...
      if( option!='L' ) {
         ...
         if( remotecwd_wid ) {
            boolean old_gchangeremotecwd_allowed=_ftpclientChangeRemoteCwdOnOff();
            _ftpclientChangeRemoteCwdOnOff(0);
      ...
      while( idx>=0 ) {
         ...
         if( filename=="." || filename==".." ) {
            ...
            if( !(file.type&FTPFILETYPE_DIR) ) {
               ...
               _str line=file_match('-P +V ':+maybe_quote_filename(src),1);
               if( line=="" ) {
...
_command void ftpclientUpload() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   while( idx>=0 ) {
      ...
      parse caption with filename "\t" size "\t" date "\t" time "\t" attribs;
      if( filename!=".." ) {
         userinfo=localWid._TreeGetUserInfo(idx);
   ...
   for(;;) {
      ...
      userinfo=localWid._TreeGetUserInfo(index);
      userinfo=lowcase(userinfo);
      file.type=0;
      if( pos("d",userinfo) ) {
         file.type |= FTPFILETYPE_DIR;
...
_command void ftpclientManualUpload() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   FtpConnProfile fcp;
   FtpSendCmd scmd;
   ...
   _str filename="";
   _str path="";
   _str remote_path=strip(_param2);
   ...
   if( remote_path=="" ) {
      ...
      case FTPSYST_OS400:
         ...
         if( substr(remote_path,1,1)=='/' || substr(fcp.remoteCwd,1,1)=='/' ) {
            ...
            if( member!="" ) {
               remote_path=remote_path:+'.':+_ftpUploadCase(&fcp,member);
            }
         }
         break;
      case FTPSYST_WINNT:
...
_command void ftpclientOpenLocalFile() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   mou_hour_glass(1);
   int idx=localWid._TreeGetNextSelectedIndex(1,auto treeSelectInfo);
   while( idx>=0 ) {
      _str caption=localWid._TreeGetCaption(idx);
      _str filename="";
      parse caption with filename "\t" .;
      if( filename==".." ) {
         idx=localWid._TreeGetNextSelectedIndex(0,treeSelectInfo);
         ...
         _str info=localWid._TreeGetUserInfo(idx);
         info=lowcase(info);
         if( pos("f",info) ) {
            // We have a file so open it
            _str cwd=fcp_p->localCwd;
            if( last_char(cwd)!=FILESEP ) cwd=cwd:+FILESEP;
            _str path=cwd:+filename;
            int status=edit(maybe_quote_filename(path),EDIT_DEFAULT_FLAGS);
            if( status ) {
               _str msg='Error opening file "':+filename:+'".  ':+_ftpGetMessage(status);
               ftpDisplayError(msg);
               break;
            }
         } else if( pos("d",info) ) {
   ...
   while( idx>=0 ) {
      ...
      if( filename==".." ) {
         ...
         if( pos("f",info) ) {
            ...
            if( last_char(cwd)!=FILESEP ) cwd=cwd:+FILESEP;
            _str path=cwd:+filename;
            int status=NTShellExecute("open",path,"","");
            if( status<=32 ) {
               _str msg='Error viewing file "':+filename;
               ftpDisplayError(msg);
               break;
            }
         } else if( pos("d",info) ) {
...
_command void ftpclientChangeLocalDir(_str cwd="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   ...
   #endif
   _str orig_cwd=cwd;
   cwd=isdirectory(maybe_quote_filename(cwd));   // Resolve
   if( (cwd=="" || cwd=="0") && !isuncdirectory(orig_cwd) ) {
      _str msg='Unable to change to the local working directory "'orig_cwd'"';
      ftpDisplayError(msg);
      return;
   }
   // Append FILESEP
...
_command void ftpclientMkLocalDir(_str path="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   FtpConnProfile fake;   // Used when there is no connection
   int formWid;
...
_command void ftpclientDelLocalFile() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   while( idx>=0 ) {
      ...
      parse caption with filename "\t" size "\t" date "\t" time "\t" attribs;
      if( filename!=".." ) {
         _str userinfo=localWid._TreeGetUserInfo(idx);
         userinfo=lowcase(userinfo);
         // Filename OR directory
         if( pos("d",userinfo) ) {
            dirs=true;
...
_command void ftpclientRenameLocalFile() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   int idx=localWid._TreeCurIndex();
   if( idx<0 ) return;
   _str caption=localWid._TreeGetCaption(idx);
   _str rnfr="";
   ...
   if( rnfr==".." ) {
      ...
      _str local_path=fcp_p->localCwd;
      ...
      if( substr(rnto,1,1)==FILESEP ) {
         ...
         _str drive=_ctl_local_drvlist.p_text;
         if( drive=="" ) {
            // This should never happen
      ...
      #endif
      _str rnto_dir=_strip_filename(rnto,'N');
      _str rnto_filename=_strip_filename(rnto,'P');
      _str orig_rnto_dir=rnto_dir;
...
_command void ftpclientLocalSort(_str line="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   int formWid;
   int sortflags;
   FtpConnProfile *fcp_p;
...
_command void ftpclientDownload(typeless downloadLinks = "") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   FtpConnProfile fcp;
   FtpQEvent event;
   ...
   for(;;) {
      int index=remoteWid._TreeGetNextSelectedIndex(ff, treeSelectInfo);
      ff=0;
      if( index<0 ) break;
...
_command void ftpclientManualDownload() name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   ...
   if( localpath=="" ) {
      ...
      if( last_char(localpath)!=FILESEP ) localpath=localpath:+FILESEP;
...
_command void ftpclientChangeRemoteDir(_str cwd="", 
                                       typeless isOS400LFS="", 
                                       typeless isLink="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   FtpConnProfile fcp;
   int formWid;
...
_command void ftpclientChangeRemoteDirLink(_str filename="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   // Get rid of the link part
   parse filename with filename '->' .;
   filename=strip(filename);
...
_command void ftpclientMkRemoteDir(_str path="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY)
{
   FtpConnProfile *fcp_p;
   FtpConnProfile fcp;
   int formWid;
 ...
 * directory listing.
 */
void _ctl_local_dir.on_change(int reason)
{
   if( !_ftpclientChangeLocalDirOnOff() ) return;
...
void _ctl_local_dir.rbutton_up()
{
   ...
   _str menu_name="_FTPClient_localdir_menu";
   ...
   formWid=_ftpclientQFormWid();
   if( !formWid ) return;
   int treeWid=formWid._find_control("_ctl_local_dir");
...
{
   if( !_ftpclientChangeRemoteDirOnOff() ) return;
...
void _ctl_remote_dir.rbutton_up()
{
   ...
   _str menu_name="_FTPClient_remotedir_menu";
   int idx=find_index(menu_name,oi2type(OI_MENU));
   if( !idx ) {
      return;
   }
...
void _ctl_local_drvlist.on_change(int reason)
{
   ...
   typeless isdir=isdirectory(maybe_quote_filename(cwd),1);
   if( (isdir=="" || isdir=="0") && file_match('+d 'maybe_quote_filename(temp),1)=="" ) {
   #else
   _str temp=cwd;
   if( last_char(temp)!=FILESEP ) temp=temp:+FILESEP;
   temp=temp:+ALLFILES_RE;
...
// This expects the active window to be a combo box
void _ftpclientFillCwdHistory(boolean set_textbox)
{
...
static void onresizeTabControl(typeless &ht)
{
   ...
   _ctl_group1.p_width=_ctl_binary.p_x+_ctl_binary.p_width+_dx2lx(SM_TWIP,4);

   // Position the tab control
   int tab_y = _ctl_connect.p_y+_ctl_connect.p_height+_dy2ly(SM_TWIP,4);
   int prog_y = _ctl_progress_label2.p_y + _ctl_progress_label2.p_height+_dy2ly(SM_TWIP,2);
   ...
   // Byte count label
   int new_width= _ctl_progress.p_x-(_ctl_group1.p_x+_ctl_group1.p_width)-2*_dx2lx(SM_TWIP,4);
...
static void onresizeDirTab(typeless &ht)
{
   ...
   new_width=containerW/2-lgap-mgap/2;
   ...
   // Resize height
   int vgap=_ctl_local_dir.p_y-(_ctl_local_drvlist.p_y+_ctl_local_drvlist.p_height-1);
   ...
   new_y= (containerH-_ctl_no_connection.p_height)/2;
...
static void onresizeLogTab(typeless &ht)
{
   ...
   new_x= (containerW-_ctl_no_log.p_width)/2;
   _ctl_no_log.p_x=new_x;
   new_y= (containerH-_ctl_no_log.p_height)/2;
...
static void _enable_ftpclient(boolean enable)
{
   ...
   int sstabWid=formWid._find_control("_ctl_ftp_sstab");
