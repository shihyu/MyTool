 ...
 */
static _str nextSortedEl(typeless & idHash,_str index,boolean returnHidden)
{
   _str next_index='';
   _str test_index;
   _str comp_index;
   boolean next_has_quotes=false;
   boolean comp_has_quotes=false;
   ...
   for (test_index._makeempty();;) {
      ...
      if ((!returnHidden)&&(first_char(test_index):=='#')) continue;
...
{
   _str output=_strip_filename(xcode_project_file,'N');

   if (last_char(output):==FILESEP) {
...
static _str xcode_get_output_dir(_str target_name)
{
   ...
   _str output_dir=strip(*symRoot,'B','"');
   if (output_dir:!='') {
...
//            break; The result should be relative to the project directory
         ...
         default:
            ...
            if (first_char(path_string):==FILESEP || !parent_id || parent_id->_varformat()!=VF_LSTR || (*parent_id):=='') {
...
static _str xcode_get_text(_str terminating_char)
{
   ...
   boolean foundEnd=(last_char(output):==terminating_char);
   if (foundEnd) {
   ...
   while (!foundEnd) {
      ...
      foundEnd=(last_char(line):==terminating_char);
      if (foundEnd) {
 ...
 */
static boolean xcode_check_archive_version()
{
 ...
 * @return true on error
 */
static boolean xcode_check_object_version()
{
   boolean ret_value=false;
   int version=100;
...
static _str xcode_strip42 (_str lineIn)
{
   ...
   int commentClosePos = 0;
   _str tempLine1 = "";
   _str tempLine2 = lineIn;
   do {
      commentOpenPos = pos("/*", lineIn);
      tempLine1 = tempLine1:+substr(lineIn, 1, commentOpenPos - 1);
 ...
 */
static boolean xcode_parse_object(typeless &idHash)
{
   _str line;
   _str id;
   ...
   boolean needs_archive_validation=(&idHash==&xcode_idHash);
   boolean needs_object_validation=false;
...
static void xcode_recreate_object(typeless & idHash,_str indent="\t")
{
   ...
   boolean isObjects=(xcode_objects==&idHash);
...
static _str make_new_id(_str id)
{
   ...
   int id_length=length(id);
   int result_length=length(result);
 ...
 */
static boolean xcode_is_header_ext(_str ext)
{
   return (ext:=='pch' || first_char(ext):=='h');
}
...
static typeless * xcode_find_file_in_phase(_str file_name,_str target_name,typeless * phase)
{
   ...
   for (file_index=0;file_index<files->_length();++file_index) {
      ...
      if (file_eq(file_name,idHashTranslateAbsoluteFile(file_ref_id,target_name,file_id))) {
         break;
      }
   }
...
static int xcode_add_file(_str filename,_str target_name)
{
   ...
   _str ext=lowcase(_get_extension(filename));
   if (ext:=='mm') {
...
static int xcode_remove_file(_str filename, _str target_name)
{
   ...
   for (file_index=0;file_index<files->_length();++file_index) {
      file_ref_id=(*files)[file_index];
      if (file_eq(filename,idHashTranslateAbsoluteFile(file_ref_id,target_name))) {
         break;
      }
   }
 ...
 * and group nodes
 */
static boolean xcode_set_parents()
{
   _str main_group_id;
...
{
   if (file_eq(filename,xcode_project_file)) {
      return 0;
   }
   ...
   if (last_char(xcode_project_path):==FILESEP) {
      xcode_project_path=substr(xcode_project_path,1,length(xcode_project_path)-1);
   }
...
{
   // Create the name for the .vpj file by looking in the .pbxproj file
   // This should look the same as it does in Xcode, where the top-level
   // tree node reflects the name of the project
   _str projectfile = absolute("project.pbxproj", XcodeProjBundleDir);
   _str projectName;
   if(_GetXcodeProjectName(projectfile, projectName) != 0){
      return 1;
   }
   ...
   _str vpjFullName;
   if (projectName:!='') {
      vpjName=VSEProjectFilename(projectName);
      ...
      vpjFullName=_strip_filename(XcodeProjBundleDir,'N'):+vpjName;

      // Set the .vpj if it already exists
      if (file_exists(vpjFullName)) {
          VendorProjectNames[VendorProjectNames._length()]=vpjFullName;
      } else {
         // Some voodoo magic here...
         // Temporiarly set the Vendor Project file to the Xcode project bundle directory
         // full path. The .vpj will then be created later from the template. Take a peek
         // at _WorkspaceAssociate, where workspace_new_project2 is called.
...
int _xcode_workspace_get_vpj_names(_str XcodeWorkspaceBundleDir, _str (&ProjectNames)[],_str (&VendorProjectNames)[])
{
   ...
   _str workspaceData = absolute("contents.xcworkspacedata", XcodeWorkspaceBundleDir);
   int stat = 0;
   ...
   _str workspaceRootDir = _strip_filename(XcodeWorkspaceBundleDir, "N");
   ...
   for (i=0;i<projLocations._length();++i) {
      ...
      if(pos("group:", loc) == 1)
      {
         ...
         _str relativeLoc = substr(loc, 7);
         absoluteLoc = absolute(relativeLoc, workspaceRootDir);
      } else if(pos("container:", loc) == 1) {
          _str relativeLoc = substr(loc, 11);
         absoluteLoc = absolute(relativeLoc, workspaceRootDir);
      }
      // TODO: What other prefixes besides "container:" and "group:" are possible?
      
      if(absoluteLoc != '') {
         // Get the parent directory of the .xcodeproj bundle. This is where
         // the .vpj will be located
         _str xcodeProjBaseDir = _strip_filename(absoluteLoc, 'N');

         // Call _xcode_project_get_vpj_names
         _str ProjectNamesTemp[]; ProjectNamesTemp._makeempty();
         _str VendorProjectTemp[]; VendorProjectTemp._makeempty();
         _xcode_project_get_vpj_names(absoluteLoc, ProjectNamesTemp, VendorProjectTemp);

         int j;
         for(j=0;j<ProjectNamesTemp._length();j++) {
            // Convert the ProjectNamesTemps to relative-to-the-workspace paths
            _str projNameT = ProjectNamesTemp[j];
            _str assocXcode = VendorProjectTemp[j];
            _str projNameAbsolute = absolute(projNameT, xcodeProjBaseDir);
            _str projNameRel = relative(projNameAbsolute, workspaceRootDir);
            ProjectNames[ProjectNames._length()] = projNameRel;
            VendorProjectNames[VendorProjectNames._length()] = assocXcode;
         }
      }
   ...
   for (build_option_name._makeempty();;) {
      ...
      case 'FRAMEWORK_SEARCH_PATHS':
         ...
         while (build_option_value!='') {
            parse build_option_value with path (PATHSEP) build_option_value;
            strappend(compile_command,' ':+maybe_quote_filename('-F':+path));
         }
         break;
      ...
      case 'HEADER_SEARCH_PATHS':
         ...
         while (build_option_value!='') {
            parse build_option_value with path (PATHSEP) build_option_value;
            strappend(compile_command,' ':+maybe_quote_filename('-I':+path));
         }
         break;
      case 'LIBRARY_SEARCH_PATHS':
         ...
         while (build_option_value!='') {
            parse build_option_value with path (PATHSEP) build_option_value;
            strappend(compile_command,' ':+maybe_quote_filename('-L':+path));
         }
         break;
...
static boolean xcode_get_build_settings(_str target_name,_str vpj_filename,typeless & all_buildSettings,_str style_name='')
{
   typeless * root_object=xcode_find_root_object();
   if (!root_object) {
      return true;
...
_str _xcode_make_compile_command(_str buf_name, _str vpj_filename=_project_name)
{
   ...
   _str projectfile = absolute("project.pbxproj",VendorWorkspaceFilename);
   _str target_name = _strip_filename(vpj_filename, "PE");
   ...
   _str compile_command='/usr/bin/gcc-3.3 -pipe -fmessage-length=0 -x ';
   ...
   while (pos('$(',compile_command)) {
      ...
      if (last_char(leading):=='\') {
         leading=substr(leading,1,length(leading)-1);
      }
...
_str _xcode_get_output_file(_str vpj_file, _str xcodeBundleDirName, _str config, _str & output_dir, _str sdkName='')
{
   ...
   _str vpjDir = _strip_filename(vpj_file,'N');
   _str xcodeBundleAbsoluteDir = absolute(xcodeBundleDirName, vpjDir);

   // Get absolute path to the project.pbxproj file
   _str projectfile = absolute("project.pbxproj",xcodeBundleAbsoluteDir);
   _str outputPath;
...
#define XCMENU_BUILDWORKSPACE_SCHEME_CATEGORY 'build xcworkspace scheme'
#define XCMENU_BUILDWORKSPACE_SCHEME_CAPTION 'Build Scheme...'
#define XCMENU_BUILDWORKSPACE_SCHEME_COMMAND 'xcode-build-scheme'

_command void xcode_build_scheme(_str SchemeName='', _str WorkspaceFile=_workspace_filename) name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
   if (SchemeName=='') {
      return;
   }
   if (_IsWorkspaceAssociated(_workspace_filename, auto associateWkspace)) {
      ...
      if (isXcodeWorkspace || isXcodeProject) {
         _str commandLine = 'xcodebuild';
         if (isXcodeWorkspace) {
            commandLine :+= ' -workspace ':+ maybe_quote_filename(WorkspaceFile);
         } else {
            commandLine :+= ' -project ':+ maybe_quote_filename(WorkspaceFile);
         }
      
         // add scheme name
         commandLine :+= ' -scheme ' :+ maybe_quote_filename(SchemeName);
         ...
         // set build dir
         build_dir := 'build/';
         //commandLine :+= ' BUILD_DIR=' :+ maybe_quote_filename(build_dir);
            ...
            if (allSchemes._length() == 0) {
                ...
                for (i := 0; i < allSchemes._length(); ++i) {
                    status = _menu_insert(schemes_submenu_handle, i,
                                          ...
                                          'xcode_build_scheme 'maybe_quote_filename(allSchemes[i]),
                                          "scheme", // category
                                          "",  // help command
                                          ""   // help message
                                         );
...
#define SET_TARGET_DESTINATION 'Set Target Destination'
...
_str GetCurrentTargetSDK(_str ProjectName=_project_name)
{
   ...
   if (file_eq(ProjectName, _project_name) && gActiveTargetDestination!='') {
...
_command void xcode_set_target_sdk(_str TargetName='', _str ProjectFilename=_project_name) name_info(','VSARG2_REQUIRES_PROJECT_SUPPORT)
{
...
void initMenuTargetDestinations(int menu_handle, _str ProjectFilename)
{
   ...
   boolean no_targets = (_workspace_filename == "" || _project_name == "" || (targetDestinations._length() == 0));
   int tdmenu, tdpos;
   ...
   for (i := 0; i < targetDestinations._length(); ++i) {
      ...
      status = _menu_insert(tdsubmenu_handle, i,
                            ...
                            'xcode_set_target_sdk -p 'maybe_quote_filename(ProjectFilename)' 'maybe_quote_filename(sdkTargetName[i]),
                            "file", // category
                            "",  // help command
                            ""   // help message
                            );
...
{
   _str configStrings[];
   _str projectfile = absolute("project.pbxproj", ProjectFilename);
   _GetXcodeProjectConfigurations(projectfile, configStrings);
...
static _str xcode_ios_simulator_command(_str outputFile = "", _str debug = "")
{
   ...
   cmd := maybe_quote_filename(get_env("VSLICKBIN1"):+"se_simlaunch");
   cmd :+= " ":+maybe_quote_filename(outputFile);
   if (debug == "debug") {
      cmd :+= " -debugwait";
   }
   if (targetSDK != "") {
...
_command void xcode_project_launch_sim(_str outputFile = "", _str debug = "")
{
...
_command void xcode_project_debug_sim()
{
   associatedProject := _ProjectGet_AssociatedFile(_ProjectHandle(_project_name));
   if (associatedProject == "" || !_IsXcodeProjectFilename(associatedProject)) {
      // not xcode project
...
_command xcode_activate_project_toolbar() name_info(','VSARG2_EDITORCTL)
{
   return activate_project_files();
}

_command xcode_activate_output_toolbar() name_info(','VSARG2_EDITORCTL)
{
   return activate_output();
}

_command macos_activate_build_output_toolbar() name_info(','VSARG2_EDITORCTL)
{
   return activate_build();
}

_command void xcode_activate_tag_properties_toolbar() name_info(','VSARG2_EDITORCTL)
{
