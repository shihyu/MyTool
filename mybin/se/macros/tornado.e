...
*/
#define TORNADO_EXE_NAME  "tornado.exe"
// tornado\host\x86-win32\bin\
// tornado\host\resource\help\windmanw.hlp
// Help filename relative to executable path
#define TORNADO_RELHELP_NAME  '..\..\resource\help\windmanw.hlp'
#define TORNADO_STARTUP_TIMEOUT 30   // Seconds
#define TORNADO_TITLE_PREFIX 'Tornado'


#define APPMENU_DISABLED_ERROR 1
#define APPMENU_GENERAL_ERROR  2
...
void ctlok.lbutton_up()
{
   ...
   if (last_char(_tornado_bin_path)!=FILESEP) {
      _tornado_bin_path=_tornado_bin_path:+FILESEP;
   }
...
static boolean tornado_exe_found()
{
   return(file_match("-p "maybe_quote_filename(get_tornado_path():+TORNADO_EXE_NAME),1)!="");
}
// Returns true if tornado exectuable not found
static boolean check_tornado_path()
{
   if (!tornado_exe_found()) {
...
static boolean maybe_start_tornado()
{
   if (!TornadoIsUp()) {
      path=maybe_quote_filename(_tornado_bin_path:+TORNADO_EXE_NAME);
      shell(path,'A');
