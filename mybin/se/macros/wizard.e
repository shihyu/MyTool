...
#define SLIDE_WIDTH  6360
#define SLIDE_HEIGHT 3390

#define DIALOG_WIDTH  6480
#define DIALOG_HEIGHT 4395

#define CUR_SLIDE_NAME ctlnext.p_user
#define gpWizardInfo ctlback.p_user

#define NEXT_BUTTON_CAPTION   "&Next >"
/*
...
int _Wizard(WIZARD_INFO *pWizardInfo)
{
   ...
   typeless status=_modal_wait(wid);
   return(status);
}
...
void _WizardMaybeEnableButtons()
{
   ...
   int tempNum = curNum;
   tempNum++;
   _str nextSlideName='ctlslide'tempNum;
   ...
   int nextWid=_find_control(nextSlideName);
   if (nextWid) {
      TurnOnNext();
   }else{
      TurnOffNext();
...
static void arrangeDialog(WIZARD_INFO *pWizardInfo)
{
   ...
   for (i=1;;++i) {
      _str otherFrameName='ctlslide'i;
...
static void TurnOnFinish()
{
   //ctlnext.p_enabled=0;
   //ctlnext.p_default=0;
   ctlfinish.p_enabled=1;
   ctlfinish.p_default=1;
}

static void TurnOffFinish()
{
   ctlfinish.p_enabled=0;
   ctlfinish.p_default=0;
}
...
{
   ctlcancel.p_enabled=1;
}

static void TurnOffCancel()
{
   ctlcancel.p_enabled=0;
}
...
void ctlnext.lbutton_up()
{
   ...
   ctlnext.p_enabled = true;
   int curWid=_find_control(curSlideName);
   typeless curNum='';
   parse curSlideName with 'ctlslide' curNum;
   if (!isinteger(curNum)) return;

   ++curNum;
   _str nextSlideName='ctlslide'curNum;
   while (pWizardInfo->callbackTable:[nextSlideName'.skip']==1) {
      ++curNum;
