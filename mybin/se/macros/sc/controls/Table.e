...
class Table : sc.lang.IControlID {
   ...
   private void getRowFromID(int rowID,STRARRAY &cellInfo) {
      ...
      numCols := _TreeGetNumColButtons();
      _str wholeCaption = _TreeGetCaption(rowID);
      for ( ;; ) {
   ...
   private void setRowFromID(int rowID,STRARRAY &cellInfo) {
      ...
      caption := "";
      int len=cellInfo._length();
      for ( i:=0;i<len;++i ) {
         caption=caption:+"\t":+cellInfo[i];
      }
   ...
   public int deleteColumn(int colX) {
      ...
      do {
         ...
         int childIndex = _TreeGetFirstChildIndex(TREE_ROOT_INDEX);
         for (;;) {
   ...
   public void addFilenameColumn(_str caption
                                 ...
                                 ,boolean sortCaseInsensitive = true
                                 ,boolean isPushButton = true
                                 ) {
      int extraFlags = 0;
      if ( isPushButton ) extraFlags|=TREE_BUTTON_PUSHBUTTON;
    ...
    *         false
    */
   public int getNodeIndexFromRow(int rowY,boolean addRowsIfNecessary=true) {
      int wid=p_window_id;
      p_window_id=m_treeWID;
    ...
    */
   private int getRowIDs(INTARRAY &rowIDs,int &lastRowID,boolean countOnly=false) {
      int wid=p_window_id;
      p_window_id=m_treeWID;
