...
class ToolbarCustomizationHandler : CustomizationHandler {
   ...
   protected UserModification compareNonIdentifiers(UserControl &origItem, UserControl &curItem)
   {
      ...
      if (origTC.getPicture() != curTC.getPicture()) {
    ...
    */
   private boolean checkToolbarForChanges(int tbIndex, _str tbName, ToolbarModification (&mods)[], Separator (&curSeps)[], ToolbarModification (&dupes)[])
   {
      ...
      for (i := 0; i < tempList._length(); i++) {
         ...
         if (type == TBCT_SEPARATOR) {
            ...
            case TBCT_PIC_BUTTON:
               ...
               tbc.setPicture(tempList[i].name);
               break;
         ...
         if (tbIndex > 0) {
            ...
            if ((tbFlags & FF_SYSTEM) && _tbIsCustomizeableToolbar(tbIndex)) {
               ...
               if (checkToolbarForChanges(tbIndex, tbName, mods, seps, dupes)) {
                  ...
                  modified = true;
               } else if (removeMods(xmlHandle, toolbarsNode, tbName)) modified = true;
            }
         }
      }
   ...
   protected void readMod(int xmlHandle, int modNode, UserModification &mod)
   {
      if (modNode > 0) {
         ...
         if (attr._indexin('Picture')) tm.setPicture(attr:['Picture']);
   ...
   void applyUserControlList(_str formName, UserControl (&list)[])
   {
      ...
      for (i := 0; i < list._length(); ++i) {
         ...
         if (tbc.getType() == TBCT_PIC_BUTTON) {
            ...
            int picIndex = find_index(_strip_filename(tbc.getPicture(), 'p'), PICTURE_TYPE);
            if (!picIndex) {
               // maybe we can try and load up this picture?
               picFile := '';
               if (_strip_filename(tbc.getPicture(), 'n') == '') {
                  // no path, try adding the bitmaps dir
                  picFile = get_env("VSROOT") :+ "bitmaps" :+ FILESEP :+ tbc.getPicture();
               } else picFile = tbc.getPicture();

               if (file_exists(picFile)) {
                  picIndex = _update_picture(-1, picFile);
               }
            }
    ...
    *                                  return -1
    */
   private int insertItemIntoList(ToolbarControl tbc, _str commandBefore, _str commandAfter, UserControl (&list)[], boolean strictPositioning = false)
   {
