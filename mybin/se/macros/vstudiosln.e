...
{
   if(_pic_website == 0)
      _pic_website = load_picture(-1,'_website.ico');
   if(_pic_website_dir == 0)
      _pic_website_dir = load_picture(-1,'_website.ico');
   ...
   int firstChildNode = treeViewId._TreeGetFirstChildIndex(rootIndex);
...
static void InsertWebsiteFolderContents(int treeViewId, int parentNode, _str websiteDirectory)
{
   ...
   _str childDir = file_match(websiteDirectory' +D +X', 1);
   ...
   _str childItem = file_match(websiteDirectory'*.* +P -D', 1);
   ...
   if (subDirectories._length() > 0)
   {
      ...
      for (subdirIdx = 0; subdirIdx < subDirectories._length(); subdirIdx++)
      {
         ...
         if (!pos('\\\.{1,2}\\$', subDirName, 1, 'U') && isdirectory(subDirName))
         {
            ...
            _str subDirPlainName = strip(relative(subDirName, websiteDirectory, false), 'B', '\');
...
static void InsertSolutionItems2005(_str WorkspaceName)
{
   ...
   _str userFolderRegex = '^Project\(\"\{2150E333-8FDC-42A3-9474-1A3956D46DE8\}\"\):b\=:b\"{#1[^\"]+}\",:b\"{#2[^\"]+}\",:b\"\{{#3[A-Fa-f0-9-]?+}\}\"$';
   _str searchOptions = '@Rh';
   int foundFolder = search(userFolderRegex, searchOptions);
   while (foundFolder == 0)
   {
      // Get the start and length for group #1, which is the folder name
      _str folderName = '';
      ...
      int sidx = solutionItems._length();
      ...
      while (!status) {
         ...
         if (pos('ProjectSection(SolutionItems)', nextLine)) {
            ...
            while (!status) {
               ...
               // Regex: ^:b*{#0:p}:b\=:b\g0$
               _str fileRegex = '^{#0?#}:b\=:b\g0$';
               if (pos(fileRegex, nextLine, 1, 'R') > 0) {
                  _str fileRelPath = substr(nextLine,pos('S0'),pos('0'));
   ...
   _str nestedProjectRegex = '\{{#0:h:8-(:h:4-):3:h:12}\} = \{{#1:h:8-(:h:4-):3:h:12}\}';
   _str line = '';
   while (!status) {
      down();
      get_line(line);
      ...
      if (pos(nestedProjectRegex, line, 1, 'R')) {
         _str childGuid = substr(line,pos('S0'),pos('0'));
         _str parentGuid = substr(line,pos('S1'),pos('1'));
   ...
   for (idx=0; idx < solutionItems._length(); ++idx)
   {
      ...
      _str parentGuidRegex = '^:b*\{' :+ childGuid :+ '\}:b\=:b\{{#0:h:8-(:h:4-):3:h:12}\}';

      // If we found a matching line, this virtual folder has a parent folder
      if (search(parentGuidRegex, '@RhI') == 0)
      {
         int parentGuidStart = match_length('S0');
         int parentGuidLen = match_length('0');
         if (parentGuidStart >= 0 && parentGuidLen > 0)
         {
            _str parentGuid = get_text(parentGuidLen, parentGuidStart);
            solutionItems[idx].ParentGuid = parentGuid;
         }
      }
   }
...
{
   _str caption=_TreeGetCaption(treeIndex);
   _str filename;
...
static void RemoveSolutionItem2005(int treeIndex)
{
   ...
   int parentFolder = _TreeGetParentIndex(treeIndex);
   if(parentFolder < 1)
      return;
...
void RemoveSolutionItemFolder2005(_str folderGuid)
{
   ...
   _str findChildrenRE = '\{{#1:h:8-(:h:4-):3:h:12}\}:b=:b\{'folderGuid'\}';
   // childGuid = parentGuid
   // \{{#0:h:8-(:h:4-):3:h:12}\}:b=:b\{275F73CF-B80B-4612-8F08-C2FBA7C03CCB\}
   int foundChild = search(findChildrenRE, '@RhI');
   while(foundChild == 0)
   {
      ...
      if(groupStart >= 0 && groupLen > 0)
      {
         _str childFolderGuid = get_text(groupLen, groupStart);
...
static void _RemoveSolutionItemFolder2005(_str folderGuid)
{
   ...
   _str findSolutionFolderRE = '^Project\(\"\{{#1:h:8-(:h:4-):3:h:12}\}\"\)?+"\{'folderGuid'\}\"$';
   int foundFolder = search(findSolutionFolderRE, '@RhI');
   if(foundFolder == 0)
   {
      // Delete lines until we find the 'EndProject' line
      // (NOT the EndProjectSection) line
      boolean lastLineToDelete = false;
      do
      {
         _delete_line();
         _str nextLine = '';
      ...
      if(foundFolder == 0)
      {
         ...
         _str findChildrenRE = '\{'folderGuid'\}:b=:b\{{#1:h:8-(:h:4-):3:h:12}\}';
...
static void InsertSolutionItemTree(int treeViewId, int treeNode, _str parentGuid, VS2005SolutionItems (&solutionItems)[])
{
   ...
   for (; fidx >= 0; --fidx) {
      ...
      {
         _str toInsert = solutionItems[fidx].FolderName :+ '@@' :+ fidx;
   ...
   for (; ffix < foldersAtThisLevel._length(); ffix++) {
      ...
      if(isnumber(indexVal))
      {
         ...
         int siblingNode = _TreeGetFirstChildIndex(treeNode);
         if(siblingNode > 0)
         {
...
{
   boolean existingItems:[];

   int solutionItemsNode=_TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   int fileNode=_TreeGetFirstChildIndex(solutionItemsNode);
   _str caption;
         ...
         if (pos('ProjectSection(SolutionItems)', nextLine)) {
            ...
            while (!status) {
               ...
               if (pos(fileRegex, nextLine, 1, 'RI')) {
                  ...
                  _str existingFile = substr(nextLine, pos('S0'), pos('0'));
...
static void ReparentProjects2005(_str WorkspaceName)
{
   ...
   _str solutionItemGuid = '2150E333-8FDC-42A3-9474-1A3956D46DE8';
   _str userFolderRegex = '^Project\(\"\{{#0[A-Fa-f0-9-]?+}\}\"\):b\=:b\"{#1[^\"]+}\",:b\"{#2[^\"]+}\",:b\"\{{#3[A-Fa-f0-9-]?+}\}\"$';
   _str searchOptions = '@Rh';
   int foundFolder = search(userFolderRegex, searchOptions);
   while (foundFolder == 0)
   {
      ...
      int groupStart = match_length('S0');
      int groupLen = match_length('0');
      _str projTypeGuid = get_text(groupLen, groupStart);
      if(projTypeGuid != solutionItemGuid)
      {
         // Get the name for this project
         groupStart = match_length('S1');
         groupLen = match_length('1');
         _str projName = get_text(groupLen, groupStart);

         // Geth the relative path to the project file
         groupStart = match_length('S2');
         groupLen = match_length('2');
         _str projRelPath = get_text(groupLen, groupStart);

         // Get the GUID for this project
         groupStart = match_length('S3');
         groupLen = match_length('3');
         _str projIdGuid = get_text(groupLen, groupStart);
   ...
   if (solutionHierarchyStart >= 0) {
      ...
      foreach (projectGuid => projectRelativePath in projectGuids) {
         ...
         // childGuid = parentGuid
         _str findParentRE = '\{' :+ projectGuid :+ '\}:b=:b\{{#0[A-Fa-f0-9-]?+}\}';
         if(search(findParentRE, '@RhI') == 0)
         {
            int groupStart = match_length('S0');
            int groupLen = match_length('0');
            _str parentGuid = get_text(groupLen, groupStart);
                                           ...
                                           _str solutionFolderGuid)
{
   ...
   _str projectAbsPath = _AbsoluteToWorkspace(projectRelPath, WorkspacePath);
   _str capname=_strip_filename(projectRelPath,'p')"\t"projectAbsPath;
...
void InsertVSWorkspaceFolder(_str WorkspaceName, _str folderName = '')
{
   ...
   p_indent_with_tabs=1;

   // Search for all project that *aren't* Solution Item folders
   // Solution Item guid = 2150E333-8FDC-42A3-9474-1A3956D46DE8
   _str solutionItemGuid = '2150E333-8FDC-42A3-9474-1A3956D46DE8';
   _str folderGuid = guid_create_string('G');
   _str folderGuidWithBraces = '{'folderGuid'}';
   int foundprojectItem = search('Global', '@h');
   ...
   int curTreeNode = orig_wid._TreeCurIndex();
   ...
   boolean isWorkspaceNode = orig_wid._projecttbIsWorkspaceNode();
...
void InsertCsharpProjectFolder(_str ProjectName, _str folderName)
{
   ...
   _str projectFileName = _TreeGetCaption(projectNode);
   _str folderPath = projectPath :+ folderRelPath;
   _str relPath = '';
   parse projectFileName with projectFileName "\t" relPath;
   _str vsProjectFileName = projectPath :+ projectFileName;
   ...
   int foundFolderSection = search('<Folder Include="' :+ folderRelPath :+ '" />', '@');
...
void InsertVCXProjFile(_str ProjectName, _str fullPath)
{
   ...
   _str filename = relative(fullPath, _strip_filename(ProjectName,'N'));
   if (filename == '') {
      return;
   }
...
void DeleteVCXProjFile(_str ProjectName, _str fullPath)
{
   _str filename = relative(fullPath,_strip_filename(ProjectName,'N'));
   if (filename == '') {
      return;
   ...
   _VCXProjectDeleteFile(ProjectName, filename);
}
...
void DeleteVCXProjFolder(_str ProjectName)
{
   ...
   _str folderName = _TreeGetCaption(folderNode);
   int node = _TreeGetParentIndex(folderNode);
   _str folderPath = folderName;
   while (node > 0) {
      if (_projecttbIsProjectNode(node)) {
         break;
   ...
   if (node > 0) {
      _str msg = 'Delete "' :+ folderName :+ '" and all its contents?';
...
static boolean _IsTreeControl(int treeViewId)
{
   return ( _iswindow_valid(treeViewId) && (treeViewId.p_object == OI_TREE_VIEW));
}

_str vstudio_solution_file_version(_str SolutionFilePath)
{
   return GetVisualStudioSolutionVersion(SolutionFilePath);
}

_str vstudio_application_version(_str SolutionFilePath)
{
   _str solutionVer = GetVisualStudioSolutionVersion(SolutionFilePath);
...
static _str GetVisualStudioSolutionVersion(_str solutionFile)
{
   _str fileVersion = '';
   if (solutionFile:!='')
   {
      _str solutionFullPath = maybe_quote_filename(solutionFile);
      int temp_wid;
      int orig_wid;
...
// default generic msbuild projects
_str def_vs_msbuild_projects_ext = '.unityproj;.pyproj';

// set default msbuild project extensions
void _workspace_opened_vstudio_solution()
{
   if (_workspace_filename != '') {
      ext := _get_extension(_workspace_filename, 1);
      if (file_eq(ext, VISUAL_STUDIO_SOLUTION_EXT)) {
         _VisualStudioSetMSBuildProjectExtensions(def_vs_msbuild_projects_ext);
      }
   }
}
