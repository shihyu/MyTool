...
#define VBS_MODE_NAME   'VBScript'
#define VBS_LANGUAGE_ID 'vbs'
#define VBS_WORD_CHARS  'a-zA-Z0-9_$!%#&'
...
defload()
{
   // Set up VBScript (Visual Basic Scripting) extension (vbs)
   _str setup_info='MN='VBS_MODE_NAME',TABS=+3,MA=1 74 1,':+
                   'KEYTAB=vbscript-keys,WW=0,IWT=0,ST='DEFAULT_SPECIAL_CHARS',';
   _str compile_info='0 Wscript *;';
   _str syntax_info='3 1 1 0 0 3 0';
   _str be_info='(FOR)|(NEXT) (WHILE)|(WEND) (DO)|(LOOP);i';
   
   _CreateLanguage(VBS_LANGUAGE_ID, VBS_MODE_NAME,
                   setup_info, compile_info,
                   syntax_info, be_info, '', 
                   VBS_WORD_CHARS, VBS_MODE_NAME);
   _CreateExtension('vbscript', VBS_LANGUAGE_ID);
   _CreateExtension('vbs', VBS_LANGUAGE_ID);
   LanguageSettings.setAutoBracket(VBS_LANGUAGE_ID, AUTO_BRACKET_PAREN|AUTO_BRACKET_BRACKET|AUTO_BRACKET_DOUBLE_QUOTE);
}

_str _vbs_keyword_case(_str s, boolean confirm = true)
{
...
_command void vbscript_enter() name_info(','VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
...
_command vbscript_space() name_info(','VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
...
#define VBS_EXPAND_WORDS (' call class const dim do erase execute executeglobal exit for function':+\
...
#define FIRST_WORDS  ' class function private public sub '
...
static _str vbscript_expand_space()
{
   ...
   _str aliasfilename='';
   _str word=min_abbrev2(orig_word,vbs_space_words,name_info(p_index),aliasfilename);
   ...
   if( word=='class' ) {
      ...
      newLine := _vbs_keyword_case(line);
      replace_line(newLine);

      endInserted := maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('end')' '_vbs_keyword_case('class'),true);
      if( endInserted ) {
      ...
      endInserted := maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('Loop'),true);
      if( endInserted ) {
         up();up();_end_line();right();
      } else {
         _end_line();right();
      ...
      endInserted := maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('next'),true);
      if( endInserted ) {
         up();up();_end_line();right();
      } else {
         _end_line();right();
      ...
      endInserted := maybe_insert_end_clause(indent_string(width):+_vbs_keyword_case('end')' '_vbs_keyword_case('function'));
      if( endInserted ) {
      ...
      replace_line(_vbs_keyword_case(line):+_vbs_keyword_case('  then'));
      if( maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('end'):+_vbs_keyword_case(' if')) ) {
         up();p_col=width+4;
      } else {
         p_col=width+4;
      }
      ...
      endInserted := maybe_insert_end_clause(indent_string(width):+_vbs_keyword_case('end')' '_vbs_keyword_case('property'));
      if( endInserted ) {
         up(1);_end_line();right();
      } else {
         _end_line();right();
      ...
      replace_line(_vbs_keyword_case(line)' '_vbs_keyword_case('case'));
      if( maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('end')' '_vbs_keyword_case('select')) ) {
         up();_end_line();right();
      } else {
         _end_line();right();
      }
      ...
      endInserted := maybe_insert_end_clause(indent_string(width):+_vbs_keyword_case('end')' '_vbs_keyword_case('sub'));
      if( endInserted ) {
         up(1);_end_line();right();
      } else {
         _end_line();right();
      ...
      endInserted := maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('wend'));
      if( endInserted ) {
         up();p_col=width+7;
      } else {
         p_col=width+7;
      ...
      endInserted := maybe_insert_end_clause(indent_string(width)_vbs_keyword_case('end'):+' ':+_vbs_keyword_case('with'));
      if( endInserted ) {
         up();p_col=width+=6;
      } else {
         p_col=width+=6;
      ...
      if( word=='executeglobal' ) {
         finalword = _vbs_keyword_case('ExecuteGlobal');
      } else if( word=='redim' ) {
         finalword = _vbs_keyword_case('ReDim');
      } else
         finalword = _vbs_keyword_case(word);
...
static boolean maybe_insert_end_clause(_str line, boolean insert_blank_line=false)
{
   if( line=='' ) {
      insert_line(line);
      return true;
...
boolean _vbs_expand_enter()
{
   ...
   if( first_word=='for' && name_on_key(ENTER)=='nosplit-insert-line' ) {
      ...
      get_line(new_line);
      if ( rest=='' && new_line=='' ) {
         replace_line(indent_string(p_col-1)_vbs_keyword_case('case '));
         _end_line();
      ...
      newLine := substr(line,1,pos(word,line)-1)_vbs_keyword_case(first_word) " "rest;
      replace_line(newLine);
                    ...
                    int start_seekpos=0, int end_seekpos=0)
{
   ...
   int status=0;
   if( find_first ) {
      _str modifiers='(((overrides|mustoverride|public|private|protected)[ \t]#)*)';
      _str keywords='sub|function|property|const|attribute|class|namespace|imports';
      _str letget='[lg]et';
      if( proc_name:=='' ) {
         _str variable_re='([A-Za-z][.'VBS_WORD_CHARS']@)';
         ...
         _str proc_name_re=stranslate(proc_name,'\$','$');
   ...
   for( ;; ) {
      ...
      if( tag_tree_type_is_func(type_name) ) {
         ...
         while( last_char(cur_line)=='_' ) {
            //say("vbs_proc_search: cur_line="cur_line" args="arguments);
            if( down() ) break;
            get_line(cur_line);
            cur_line=strip(cur_line);
            if( last_char(cur_line)=='_' ) {
               arguments=arguments' 'substr(cur_line,1,length(cur_line)-1);
            } else if( lastpos(')',cur_line) ) {
               arguments=arguments' 'substr(cur_line,1,pos('s')-1);
      ...
      if( proc_name:=='' ) {
         proc_name=tag_tree_compose_tag(name, '', type_name, tag_flags, arguments, return_type);
         break;
      }
...
int _vbs_MaybeBuildTagFile(int &tfindex)
{
   return ext_MaybeBuildTagFile(tfindex,'vbs','vbscript',"VBScript Builtins");
}
...
#define VBS_COMMON_END_OF_STATEMENT_RE 'if|while|select|for|case|public|private|class|do|else|set|get|let|redim|property|option|on|exit'
#define VBS_NOT_FUNCTION_WORDS  ' do for if while for call class dim end erase error exit goto on stop set let get select case with while return resume redim public private '
                     ...
                     VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   _str common='[,#{};()]|'VBS_COMMON_END_OF_STATEMENT_RE;
      ...
      if (!status) {
         ...
         if (num_matches == 0) {
            if (pos('new ',idexp_info.prefixexp) == 1) {
               ...
               status = _vbs_get_type_of_prefix(idexp_info.errorArgs, idexp_info.prefixexp, rt, visited, depth);
            ...
            if (idexp_info.lastid == '' && rt.taginfo != '') {
               ...
               if (rt.return_type!='' && !pos('(',tr)) {
                  ...
                  status = _vbs_get_return_type_of(idexp_info.errorArgs,tag_files,'()',rt.return_type,
 ...
 * @since 11.0
 */
int _vbs_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                             VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   case '.':     // member access operator
      ...
      if (previous_id != '') {
         ...
         status = _vbs_get_return_type_of(errorArgs, tag_files,
                                          ...
                                          VS_TAGFILTER_ANYSTRUCT|VS_TAGFILTER_ANYDATA|VS_TAGFILTER_ENUM,
                                          true,rt,visited);
         if (status) {
            return status;
         }
                                   ...
                                   VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   int status = _vbs_match_return_type_of(errorArgs,tag_files,
...
static int _vbs_match_return_type_of(_str (&errorArgs)[], typeless tag_files,
                                     ...
                                     int pushtag_mask, int vbs_return_flags,
                                     struct VS_TAG_RETURN_TYPE &rt,
                                     VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   //say("_vbs_match_return_type_of("symbol","search_class_name")");
   // filter out mutual recursion
   _str input_args='match;'symbol';'search_class_name';'cur_class_name';'min_args';'maybe_class_name';'pushtag_mask';'vbs_return_flags;
   if (visited._indexin(input_args)) {
      if (visited:[input_args].return_type==null) {
                                    ...
                                    VS_TAG_RETURN_TYPE (&visited):[], int depth=0)
{
   ...
   for (;;) {
      for (i=1; i<=num_matches; i++) {
         ...
         if (tag_tree_type_is_class(type_name) || type_name=='enum') {
...
_command void vbscript_codehelp_key() name_info(','VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
   if( !command_state() ) {
      int cfg;
      left();cfg=_clex_find(0,'g');right();
