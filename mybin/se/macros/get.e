...
static boolean hit_actapp;
void _actapp_get_string()
{
   hit_actapp=true;
}

static boolean IsMouseButtonEvent(_str key)
{
   _str event_name=event2name(key);
   _str event_name_prefix=substr(event_name,1,8);
 ...
 * 
 */
_str get_string(_str &line,_str prompt, _str arg_type_info='', _str initial_value='')
{
   typeless arg_type='';
   typeless result=0;
   _str help_name='';
   ...
   if (!_default_option(VSOPTION_HAVECMDLINE)) {
      prompt=strip(prompt);
      if (last_char(prompt)!=':' && prompt != ':'EX_VISUAL_RANGE) {
         prompt=prompt:+':';
      }
      ...
      if ( isnormal_char(key) ) {
         ...
         if ( (key:=='?' || brief_complete) && completion_info!='') {
            if ( completion_info!='' ) {
               ...
               if ( brief_complete ) {
                  last_arg=maybe_list_matches(completion_info,'',true,'1');
               } else {
                  last_arg=maybe_list_matches(completion_info,'',false,'1');
               }
               hit_actapp=false;
               cursor_command();
