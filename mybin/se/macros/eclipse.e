...
#define ECLIPSE_MIN_IDLE_TIME 500

boolean CalledFromEclipseFlag = false;
boolean gEclipseInitialized = false;
...
{
   int index=find_index(varname,VAR_TYPE|BUFFER_TYPE);
   if (!index) {
...
_str eclipse_get_compatible_sc_schemes()
{
    ...
    scc.getSchemeNamesOnly(schemeNames, compatibleWith);

    haveCompatibleScheme := false;
    for (i = 0; i < schemeNames._length(); i++) {
        if (compatibleWith[i] != null && pos(";"def_color_scheme";", ";"compatibleWith[i]";") > 0) {
    ...
    for (i = 0; i < schemeNames._length(); i++) {
       ...
       if (!haveCompatibleScheme ||
           ...
           pos(";"def_color_scheme";", ";"compatibleWith[i]";") > 0) {
          ret = ret :+ name :+ ",";
       }
    }
    return(ret);
...
_command void eclipse_history_diff(_str args="") name_info(',')
{
   ...
   if (use_rev1 == "1") {
      ...
      diff_cmdline = diff_cmdline :+ " " :+ file1;
   }
   ...
   if (use_rev2 == "1") {
      ...
      diff_cmdline = diff_cmdline :+ " " :+ file2;
   }
...
_command int eclipse_get_xml_cw_enabled() name_info(',')
{
   currentState := _GetXMLWrapFlags(XW_ENABLE_CONTENTWRAP, 'xml');
   return(currentState ? 1 : 0);
}
_command int eclipse_get_xml_tl_enabled() name_info(',')
{
   currentState := _GetXMLWrapFlags(XW_ENABLE_TAGLAYOUT, 'xml');
   return(currentState ? 1 : 0);
}
_command int eclipse_get_html_cw_enabled() name_info(',')
{
   currentState := _GetXMLWrapFlags(XW_ENABLE_CONTENTWRAP, 'html');
   return(currentState ? 1 : 0);
}
_command int eclipse_get_html_tl_enabled() name_info(',')
{
   currentState := _GetXMLWrapFlags(XW_ENABLE_TAGLAYOUT, 'html');
   return(currentState ? 1 : 0);
}
_command int eclipse_get_xml_formatting_supported() name_info(',')
{
   return(XW_isSupportedLanguage() ? 1 : 0);
}
_command int eclipse_get_cw_enabled() name_info(',')
{
   boolean currentState = _GetCommentWrapFlags(CW_ENABLE_COMMENT_WRAP);
   return(currentState ? 1 : 0);
}
_command int eclipse_get_af_on() name_info(',')
{
   return(adaptive_format_is_adaptive_on() ? 1 : 0);
}
_command int eclipse_get_af_support() name_info(',')
{
   return(adaptive_format_get_available_for_language(p_LangId) ? 1 : 0);
}
_command int eclipse_get_cw_support() name_info(',')
{
   return(commentwrap_isSupportedLanguage() ? 1 : 0);
}
_command int eclipse_istagging_supported(_str ext = null)
{
...
//   refs_wid := _find_object('_tbtagrefs_form');
 ...
 * 
 **/
boolean isInternalCallFromEclipse()
{
   return CalledFromEclipseFlag;
}
_command int isDiffedEclipse(int wid = 0)
{
   if(wid == 0)
   {
      return -1;
...
_command boolean setInternalCallFromEclipse(boolean new_val = true)
{
...
/*#if __UNIX__
 ...
 // def_deselect_copy = 0; 
   ...
   for (;index;) {
     ...
     if (extensionList != "") extensionList = extensionList",";
...
_str eclipse_get_supported_extensions()
{
   ...
   for (;;) {
      ...
      boolean supported = _istagging_supported(_Ext2LangId(name));
      if (supported) {
         retval = retval :+ name :+ ',';
      }
...
_command int eclipseGetBitmapIndex(_str bitmapName="")
{
...
_command int eclipse_createOutput(typeless outputForm='')
{
   ...
   int editor_ctl = outputContainer._find_control("ctloutput");
...
_command int eclipse_createSearchOutput(...)
{
   ...
   int list1_wid = searchOutputContainer._find_control("_search_tab");
...
_command int eclipse_resizeOutput(...)
{
   ...
   int ctlOutput = outputContainer._find_control("ctloutput");
...
_command int eclipse_resizeProcTree(...)
{
   ...
   int ctlcurpath = proctreeContainer._find_control("ctlcurpath");
   int ctlproctree = proctreeContainer._find_control("_proc_tree");
   ...
   ctlproctree.p_height = formH - ctlproctree.p_y;
 ...
 * 
 **/ 
_command int eclipse_modified_on_disk()
{
   ...
   //   activate_window(orig_view_id);
      ...
      if (result==IDYES) {
        ...
        // activate_window(actapp_view_id);
         ...
         int status=load_files(build_load_options(buf_name):+' +q +d +r +l ':+options' ':+maybe_quote_filename(buf_name));
         if (status) {
 ...
 */
void eclipseDoIdleWork()
{
   if (isEclipsePlugin()) {
      if (_idle_time_elapsed() < ECLIPSE_MIN_IDLE_TIME) return;
...
_command eclipse_reposition_text(...)
{
   int wid = arg(1);
   if(wid > 0) {
      int col = wid.p_col;
...
/*_command void eclipse_list_keys()
{
   ...
   for (i=0; i<keyList._length(); i++) {
      ...
      if (length(keyListText)) {
         keyListText = keyListText"\n";
      }
      keyListText = keyListText""keyList[i];
   }
...
static void translateKeySequences(_str (&keyList)[])
{
   ...
   int i, j;
   boolean changed, expectingCharOrModifier, dontTake;
      ...
      while (length(keySequence)) {
         if (pos('C-', keySequence, 1, "I") == 1) {
            ...
            if (expectingCharOrModifier) {
               if (oneChar == ' ') {
                  newSequence = newSequence"Space";
               } else {
                  newSequence = newSequence""oneChar;
               }
               expectingCharOrModifier = false;
            } else {
