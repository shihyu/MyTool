...
#define TREE_INFO_FILENAME 'svcURL.xml'
...
void _svc_url_explorer_form.on_resize()
{
   ...
   ctltree1.p_y = ControlYExtent(labelWID) + bufferY;
   ...
   ctlclose.p_y = ctladd_url.p_y = ctlremove.p_y = ctlcheckout.p_y = ControlYExtent(ctltree1)+bufferY;
}
   ...
   case CHANGE_SELECTED:
      {
         if ( _TreeGetDepth(index)==1 ) {
            ctlremove.p_enabled = 1;
         } else {
            ctlremove.p_enabled = 0;
         }
      }
   }
}
...
static _str SVCGetCheckoutPath(_str local_path,_str URL,boolean preserveURL)
{
...
void ctlok.on_create(_str URL='',SVC_CHECKOUT_INFO *pco_info=null)
{
   ctlURL.p_text=URL;
