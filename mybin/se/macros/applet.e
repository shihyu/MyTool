...
#define OKBUTTON_CALLBACK ctlclassname_label.p_user
...
void ctlok.lbutton_up()
{
   ...
   if (!isid_valid(ctlclassname.p_text)) {
      ...
      _str captionName=ctlclassname_label.p_caption;
      _str ch1=_chr(1);
      parse captionName with (ch1) '<B>' captionName (ch1) '</B>';
      if (last_char(captionName)==':') {
         captionName=substr(captionName,1,length(captionName)-1);
      }
...
{
   _str className=_strip_filename(Filename,'PE');
...
static int GenerateAWTAppletFile(_str AppletClass)
{
   ...
   _str filename=projectDir:+AppletClass'.java';
   ...
   _str className=GetClassName(filename);

   _str classFilename=className'.class';
...
static int GenerateJFCAppletFile(_str AppletClass)
{
   ...
   _str classFilename=_strip_filename(filename,'E')'.class';
   classFilename=_RelativeToProject(classFilename);
...
_command int awt_applet_wizard()
{
   ...
   edit(maybe_quote_filename(_param1));
   return(0);
}

_command jfc_applet_wizard()
{
...
{
   _str projectDir=_file_path(_project_name);
   _str filename=projectDir:+ClassName'.java';
   if (file_exists(filename)) {
      int result=_message_box(nls("A file named '%s1' already exists.\n\nGenerate file anyway?",filename),'',MB_YESNOCANCEL);
      if (result==IDCANCEL) {
            return(COMMAND_CANCELLED_RC);
...
_command int java_with_main_wizard()
{
   _str mainClassName=GetClassName(_project_name);
   ...
   status=edit(maybe_quote_filename(_param1));
   if (!status) {
      top();down();down();
      end_line();
   }
...
#define AWTAPP_MAIN_FILENAME        'awtapp.java'
#define AWTAPP_ABOUTDIALOG_FILENAME 'AboutDialog.java'
#define AWTAPP_EXITDIALOG_FILENAME  'ExitDialog.java'

static int GenerateAWTApplicationFiles(_str ClassName)
{
   _str sourcePath=get_env('VSROOT'):+'wizards':+FILESEP'java'FILESEP'awtapp'FILESEP;
   _str destPath=_file_path(_project_name);

   //Get together the filenames of the files that we will be copying
   _str aboutFilename=destPath:+AWTAPP_ABOUTDIALOG_FILENAME;
   _str exitFilename=destPath:+AWTAPP_EXITDIALOG_FILENAME;
...
#define AWTAPP_JFC_FILENAME 'jfcapp.java'
#define IMAGES_DIR_NAME 'images'

static int GenerateJFCApplicationFiles(_str ClassName)
{
   _str sourcePath=get_env('VSROOT'):+'wizards':+FILESEP'java'FILESEP'jfcapp'FILESEP;
   _str destPath=_file_path(_project_name);
   _str destFilename=destPath:+ClassName'.java';

   // Build a path name where the image files are
   _str imageSourcePath=sourcePath:+IMAGES_DIR_NAME:+FILESEP;
   _str imageDestPath=destPath:+IMAGES_DIR_NAME:+FILESEP;
   ...
   int ff;
   for (ff=1;;ff=0) {
      _str curFilename=file_match('-d 'maybe_quote_filename(imageSourcePath:+ALLFILES_RE),ff);
      if (curFilename=='') break;
      if (last_char(curFilename)==FILESEP) continue;
   ...
   for (i=0;i<imageFilenames._length();++i) {
      _str curDestFilename=imageDestPath:+_strip_filename(imageFilenames[i],'P');
      ...
      if (status) {
         ...
         for (j=0;j<i;++j) {
            if (substr(imageFilenames[j],1,1)!=_chr(1)) {
               _str curDestFilenameJ=imageDestPath:+_strip_filename(imageFilenames[j],'P');
...
static int javabean_slide1create()
{
   ...
   for (i=0;i<taginfo._length();++i) {
      ...
      int CurWidth=_text_width(taginfo[i].class_name'::'taginfo[i].member_name);
...
static int GetBeanFilename2(_str NamePrefix,_str ProjectPath,_str &OutFilename,
                            boolean CheckForExistingFiles=true)
{
   _str Filename=absolute(NamePrefix,ProjectPath);
...
static int javabean_show_new_project_info()
{
   ...
   _str BeanFilename='',BeanInfoFilename='';
...
static int CreateJavaBeanFile(_str BeanFilename,JAVABEAN_INFO info)
{
   ...
   insert_line('');insert_line('');
   _str FirstLine='public class 'info.Name;
   if (info.ExtendingClass!='') {
      FirstLine=FirstLine' extends 'info.ExtendingClass;
      if (info.ImplementingClass!='') FirstLine=FirstLine' ';
   }
   if (info.ImplementingClass!='') {
      FirstLine=FirstLine'implements 'info.ImplementingClass' {';
   }
...

   {
      ...
      for (i=0;i<MatchList._length();++i) {
         VS_TAG_BROWSE_INFO cm;
         status=tag_get_match_info(MatchList[i]+1,cm);
...
{
   if (last_char(ProjectPath)!=FILESEP) {
      ProjectPath=ProjectPath:+FILESEP;
   }
   _str packagePath='';
   packagePath=ProjectPath:+stranslate(PackageName,FILESEP,'.');
   if (last_char(packagePath)!=FILESEP) {
      packagePath=packagePath:+FILESEP;
   }
   return(packagePath);
}
...
static int javabean_finish()
{
   ...
   //will be in
   _str classFilename=_strip_filename(_project_name,'E')'.class';
   classFilename=_RelativeToProject(classFilename);
   ...
   if (pkgName!='') {
      ...
      if (last_char(gBeanInfo.PackageName)!='.') {
   ...
   edit(maybe_quote_filename(BeanFilename));
   p_window_id=orig_view_id;
   return(0);
}
...
{
   _str PropName=ctlPropName.p_text;
   if (PropName=='') {
                     ...
                     boolean HideProjectInfo=false)
{
   ...
   if (HideProjectInfo) {
      ...
      int diff=(ctlproject_dir.p_y+ctlproject_dir.p_height)-(ctlminihtml1.p_y+ctlminihtml1.p_height);
   ...
   int wid=p_window_id;
   p_window_id=_control ctlminihtml1;
   _minihtml_SetProportionalFont(VSDEFAULT_DIALOG_FONT_NAME,-1);
   _minihtml_SetProportionalFontSize(3,80);
   p_backcolor=0x80000022;
   p_text='<nobr>'stranslate(HTMLInfo,'<br>',"\n")'</nobr>';
   p_window_id=wid;
}
