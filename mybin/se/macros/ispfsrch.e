...
static int convertSearchString(_str &searchString,_str &options,_str string,_str stringType,boolean doReplaceString=false)
{
   ...
   switch (stringType) {
   case 'T':
      if (!doReplaceString) options=options:+'I';
      searchString=string;
      return(0);
   case 'C':
      if (!doReplaceString) options=options:+'E';
      searchString=string;
      return(0);
   case 'P':
      if (doReplaceString) {
   ...
   case '*':
      ...
      if (!doReplaceString) options=options:+make_search_options(flags);
      return(0);
   }
   return(1);
}
   ...
   struct STACK {
      boolean isString; // if isString==false, this is integer or .label
      _str string;
      _str stringType;
   };
   ...
   boolean doExclude=false)
{
   ...
   for (;;) {
      ...
      _str string='';
      _str stringType='T';  // Type of string 'T', 'C', 'P', 'X', or '*'
      if (ch=='"' || ch=="'") {
         stringType='T';
         i=pos(ch,cmdline,2);
         if (!i) {
...
static void _HideMore(int Noflines,long startpoint)
{
   ...
   int NoflinesAfter=ispf_is_excluded_line();
   if (NoflinesAfter) {
 ...
 */
_command int ispf_rfind() name_info(','VSARG2_EDITORCTL)
{
   /*if (old_search_bounds!=null && old_search_bounds.result_doAll) {
 ...
 * @categories ISPF_Primary_Commands
 */
_command int ispf_rchange(_str cmdline='',boolean supportDoAll=false) name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   int status=search(old_search_string,'@'new_search_options);
   if (!status) {
      flags=_LCQFlags();
      _MaybeUnhideLine();
   ...
   if (_SearchQNofSkipped()) {
      ...
      msg=msg:+"Errors: "_SearchQNofSkipped();
   }
   clear_message();
   if (msg!='') {
      refresh();
...
static int find_next_excluded()
{
   save_pos(auto p);
   _end_line();
   int status=search('not displayed','@icv');
   if (status) {
      restore_pos(p);
      return(status);
   }
...
_command void ispf_locate,ispf_loc,ispf_l(_str cmdline="") name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   for (;i>=0 && i<_LCQNofLineCommands();i+=direction) {
      _str data=upcase(strip(_LCQDataAtIndex(i)));
      if (searchfor=='LABEL') {
         if (substr(data,1,1)=='.') {
