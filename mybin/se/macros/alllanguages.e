...
#define LANGS_DONT_MATCH_TB      ''
#define LANGS_DONT_MATCH_CB      'Languages Differ'

#define RADIO_SET_KEY            'RadioSet'
#define NEUTRAL_RADIO_SET        'NeutralRadioSet'
...
/**
 * Retrieves the "first" language id in the list of languages.  When looping 
 * through the languages, call this to retrieve the first one. 
 * 
 * @return              language id
 */
_str getFirstLangId()
{
   ...
   if (index) {
      ...
      // okay, we have a blank language id.  I don't know how this happens, 
...
/**
 * Retrieves the "next" language id in the list of languages.  When 
 * looping through the languages, call this to retrieve each language 
 * after the first one. 
 * 
 * @return              language id
 */
...
void _language_form_init_for_options(_str langId, typeless pfnGetValue, typeless pfnIsLangIncluded)
{
   ...
   _set_language_form_mode_name(_LangId2Modename(langId));
 ...
 *                                     enabled
 */
void _language_form_disable_controls(int firstWid, _str langId, typeless pfnIsLangIncluded, boolean parentEnabled)
{
   if (!firstWid) return;
...
void _language_form_set_controls(int firstWid, _str formName, typeless pfnGetValue, 
                                 typeless pfnIsLangIncluded, _str langId, boolean setAll)
{
   if (!firstWid) return;
...
static _str getControlExclusionsForAllLanguages(_str controlName, typeless pfnIsLangIncluded)
{
   ...
   while (langId != '') {
      ...
      if (!(*pfnIsLangIncluded)(controlName, langId, true)) {
         exclusions :+= langId',';
      }
 ...
 *                         otherwise
 */
boolean setControlValue(int wid, _str value)
{
   set := false;
   if (wid.p_visible) {
      switch (wid.p_object) {
...
void _language_form_apply(typeless pfnApplyControlCallback)
{
   ...
   if (langId == ALL_LANGUAGES_ID) {
      ...
      // some languages have beautifiers - we may need to update their settings, too
      updateTabs := (alsTable._indexin(TABS_UPDATE_KEY) ||
                     alsTable._indexin(INDENT_WITH_TABS_UPDATE_KEY) ||
                     alsTable._indexin(SYNTAX_INDENT_UPDATE_KEY));
      updateBraces := alsTable._indexin(BEGIN_END_STYLE_UPDATE_KEY) != 0;
      updateMisc := alsTable._indexin(CUDDLE_ELSE_UPDATE_KEY) != 0;

      all_lang_update_all_current_profiles(updateTabs, updateBraces, updateMisc);
   } else {
 ...
 */
boolean _language_form_control_needs_validation(int wid, _str &value)
{
   formName := getLanguageFormName();
   langId := _get_language_form_lang_id();
                                       ...
                                       AllLanguagesSetting (&alsTable):[], _str exclusions = '')
{
   ...
   while (langId != '') {
      ...
      if (!pos(','langId',', ','exclusions',')) {
         ...
         result := (*pfnApplyControlCallback)(controlName, langId, value);
         if (updateString == '') {
            updateString = result;
         }
      } 
 ...
 *                         otherwise
 */
boolean _language_form_is_modified()
{
   // retrieve the language
   langId := _get_language_form_lang_id();
   formName := getLanguageFormName();
 ...
 */
void compileCurrentSettings(int firstWid, _str (&settings):[], boolean includeDisabled)
{
   if (!firstWid) return;
 ...
 *                         otherwise
 */
static boolean isRadioButtonKey(_str key)
{
 ...
 * @param wid                    window id of control to set
 */
void neutralizeAllLanguageControl(int wid, boolean saveRadioSetting)
{
   if (wid.p_visible && wid.p_name != '') {
      set := false;
      switch (wid.p_object) {
 ...
 */
boolean isControlNeutralized(int wid)
{
   neutral := false;
 ...
 * 
 * @return boolean               true if we are to set this value for the 
 *                               buffer, false otherwise
 */
boolean doSetValue(AllLanguagesSetting (&settings):[], typeless (&langValues):[], _str key, _str langId, _str &value, boolean force = false)
{
   do {
 ...
 *                                           value
 */
static boolean doUpdateWithLangValue(_str key)
{
...
void _update_buffers_for_all_languages(AllLanguagesSetting (&settings):[])
{
   ...
   for (;;) {
      ...
      typeless value;
      if (doSetValue(settings, langValues, COLOR_FLAGS_UPDATE_KEY, currentLang, value)) p_color_flags = value;
      ...
      if ( ! read_format_line() ) {
         if (doSetValue(settings, langValues, TABS_UPDATE_KEY, currentLang, value)) {
            p_tabs = value;
         }
         ...
         if (doSetValue(settings, langValues, INDENT_WITH_TABS_UPDATE_KEY, currentLang, value)) p_indent_with_tabs = value;
      ...
      if (doSetValue(settings, langValues, BEGIN_END_STYLE_UPDATE_KEY, currentLang, value)) p_begin_end_style = value;
      if (doSetValue(settings, langValues, NO_SPACE_BEFORE_PAREN_UPDATE_KEY, currentLang, value)) p_no_space_before_paren = value;
      if (doSetValue(settings, langValues, INDENT_CASE_FROM_SWITCH_UPDATE_KEY, currentLang, value)) p_indent_case_from_switch = value;
      if (doSetValue(settings, langValues, PAD_PARENS_UPDATE_KEY, currentLang, value)) p_pad_parens = value;
      if (doSetValue(settings, langValues, POINTER_STYLE_UPDATE_KEY, currentLang, value)) p_pointer_style = value;
      if (doSetValue(settings, langValues, FUNCTION_BRACE_ON_NEW_LINE_UPDATE_KEY, currentLang, value)) p_function_brace_on_new_line = value;
      if (doSetValue(settings, langValues, KEYWORD_CASING_UPDATE_KEY, currentLang, value)) p_keyword_casing = value;
      if (doSetValue(settings, langValues, TAG_CASING_UPDATE_KEY, currentLang, value)) p_tag_casing = value;
      if (doSetValue(settings, langValues, ATTRIBUTE_CASING_UPDATE_KEY, currentLang, value)) p_attribute_casing = value;
      if (doSetValue(settings, langValues, VALUE_CASING_UPDATE_KEY, currentLang, value)) p_value_casing = value;
      if (doSetValue(settings, langValues, HEX_VALUE_CASING_UPDATE_KEY, currentLang, value)) p_hex_value_casing = value;
      ...
      if (doSetValue(settings, langValues, SYNTAX_INDENT_UPDATE_KEY, currentLang, value, true)) {
...
boolean isLangExcludedFromAllLangsBraceStyle(_str langId)
{
   return (_LanguageInheritsFrom('pas', langId) || _LanguageInheritsFrom('pl1', langId) || _LanguageInheritsFrom('ruby', langId));
}

boolean isLangExcludedFromAllLangsTabs(_str langId)
{
