...
/**
 * Displays the Seek dialog box which lets you specify a seek position to 
 * go to.
 * 
 ...
 * 
 * @example _str show('-modal _seek_form')
 * 
 * @categories Forms
 * 
 */ 
...
_seekok.lbutton_up()
{
   ...
   _str text=_seektext.p_text;
   text=strip(text);
   _str ch1=substr(text,1,1);
   typeless result='';
...
_command int seek(...) name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   sticky_message('Seek position dec='result' hex='sign:+dec2hex(result));
   return(0);
}
...
typeless _nrseek(...)
{
   ...
   if ( b!='' ) {   /* Null line? */
      int col=p_col;
      goto_point(a);
      typeless LineLen=_line_length(1);
      goto_point(a" "b);
      p_col=col;
      result=a+LineLen;
   } else {
