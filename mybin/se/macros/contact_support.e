...
#define CONTACTSUPPORT_MACRO_VERSION "1.0"
#define CONTACTSUPPORT_DEBUG "OFF"

_str def_support_baseURL = "http://www.slickedit.com/support/";
const DEF_SUPPORT_NOTICE = 'Click "Submit" to initiate a problem report with Product Support.  This will submit information essential in diagnosing this problem.  To view the information that will be submitted, click "View Info."';
...
void webmail_support()
{
   ...
   URL = def_support_baseURL;
   _maybe_append(URL, "/");
   _maybe_prepend(URL, "http://");
   URL = URL"mailto/?";
   URL = URL :+ join_tuples(tupleHash, "&");
   safe_goto_url(URL, false, 0);
}
...
static void get_support_tuples(_str (&tupleHash):[])
{
   ...
   _str installPath = get_env("VSROOT");
   ...
   if ((machine() != "WINDOWS") && (_isMac() == false)) {
                 ...
                 boolean encode=true)
{
   ...
   for (i._makeempty(); ; ) {
      ...
      j++;
      if (encode) {
         urlParams = urlParams :+ i :+ tupleJoinChar :+ urlencode(tupleHash._el(i));
      }
      else {
         urlParams = urlParams :+ i :+ tupleJoinChar :+ tupleHash._el(i);
      }
      if (j < numElements) {
         urlParams = urlParams :+ joinChar;
      }
   }
   return urlParams;
}
...
static _str getPathInfo(_str path)
{
   ...
   if (substr(path,1,2)=='\\') {
      ...
      diskinfo=diskinfo:+' (remote'FSInfo')';
   } else {
      ...
      if (dt==DRIVE_NOROOTDIR) {
         ...
         diskinfo=diskinfo:+' (non-removable drive'FSInfo')';
      } else if (dt==DRIVE_CDROM){
         diskinfo=diskinfo:+' (CD-ROM'FSInfo')';
      } else if (dt==DRIVE_REMOTE){
         diskinfo=diskinfo:+' (remote'FSInfo')';
      } else {
         diskinfo=diskinfo:+' (removable drive'FSInfo')';
      }
   }
#endif
   return diskinfo;
