...
{
   _str arg1=prompt(commandName,'',number2onoff(_default_option('s')&(RE_SEARCH|UNIXRE_SEARCH|BRIEFRE_SEARCH)));
   typeless re_on='';
   setonoff(re_on,arg1);
   _default_option('s',_default_option('s')&~(RE_SEARCH|UNIXRE_SEARCH|BRIEFRE_SEARCH));
   if (re_on) {
      _default_option('s',_default_option('s')|def_re_search);
   }
}
...
_command void re_toggle() name_info(','VSARG2_EDITORCTL)
{
   _macro_delete_line();
   _str msg='';
   if ((old_search_flags & (RE_SEARCH|UNIXRE_SEARCH|BRIEFRE_SEARCH)) || arg(1)!='') {
      old_search_flags&= ~(RE_SEARCH|UNIXRE_SEARCH|BRIEFRE_SEARCH);
      msg=nls('Regular expressions OFF');
      _default_option('s',_default_option('s')&~(RE_SEARCH|UNIXRE_SEARCH|BRIEFRE_SEARCH));
      _macro_call('re_toggle',1);
   } else {
      old_search_flags|= def_re_search;
