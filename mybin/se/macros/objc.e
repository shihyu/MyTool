...
int _m_MaybeBuildTagFile(int &tfindex)
{
...
// Starting from/inside @interface/@implemenation/@protocol or method decl, find next method
 ...
 *
 */
int _m_get_expression_info(boolean PossibleOperator, VS_TAG_IDEXP_INFO &info,
                           VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   save_pos(auto orig_pos);
   do {
      if (_chdebug) say("_m_get_expression_info: @"_QROffset()" ["p_line","p_col"]");
      cfg := _clex_find(0, 'g');
      ...
      in_msg := _objectivec_message_statement(bracket_count, auto indent_col, auto bracket_col, auto arg_col, auto arg_count, false, true);
      if (in_msg) {
         if (_chdebug > 9) say("_m_get_expression_info: Objective-C [message] @"_QROffset()" ["p_line","p_col"]");
         ...
         if (_chdebug > 9) say("_m_get_expression_info: Objective-C signature="signature" receiver="receiver" arg_count="arg_count);
         if (receiver != '') {
            if (arg_count < 0) {
         ...
         if (indent_col > 0) {
            ...
            if (ch == '-' || ch == '+') {
               ...
               if (prefix_offset < orig_point) {
                  ...
                  if (_chdebug > 9) say("_m_get_expression_info: Objective-C method signature="signature" arg_count="arg_count" in_selector="in_selector);
      ...
      if (pos('['word_chars']', ch, 1, 'r')) {
         ...
         // protocol decl
         if (ch == '<') {
            if (_chdebug > 9) say("_m_get_expression_info: Objective-C @protocol: "lastid);
            status := search('[;{}()\[\]]|\@(interface|implementation|protocol|end)','-@rhxcs');
            if (!status) {
               switch (get_match_text()) {
               case '@interface':
               case '@protocol':
   ...
   if (info_flags & VSAUTOCODEINFO_OBJECTIVEC_CONTEXT) {
      ...
      if (otherinfo == "@protocol") {
         tag_list_symbols_in_context(lastid, class_name, 0, 0,
                                     ...
                                     VS_TAGFILTER_INTERFACE, 0,
                                     exact_match, case_sensitive, visited, depth, template_args);

      } else {
         ch := first_char(otherinfo);
         boolean matchSelectorID = (ch == ':' || ch == '-');
         signature := "";
         selector_id := lastid;
         filter_flags = VS_TAGFILTER_PROC;
         int context_list_flags = VS_TAGCONTEXT_NO_globals|VS_TAGCONTEXT_ONLY_inclass;

         if (otherinfo == "@method" || ch == '-') {
            cur_tag_name := cur_type_name := "";
            cur_context := cur_class := cur_package := "";
            cur_flags := cur_type_id := 0;
            context_id := tag_get_current_context(cur_tag_name, cur_flags,
                                                  cur_type_name, cur_type_id,
                                                  cur_context, cur_class,
                                                  cur_package);

            if (cur_context != '' && cur_type_id == VS_TAGTYPE_selector) {
            ...
            if (prefixexp != '') {
               ...
               status = _c_get_type_of_prefix_recursive(errorArgs, tag_files, prefixexp, rt, visited, 0, VSCODEHELP_PREFIX_OBJECTIVEC_MESSAGE);
               if (status) {
                  tag_pop_matches();
                  return status;
               }
               ...
               if (rt.return_flags & VSCODEHELP_RETURN_TYPE_STATIC_ONLY) {
                  context_list_flags |= VS_TAGCONTEXT_ONLY_static;
               }
               if (rt.pointer_count > 0) {
                  context_list_flags |= VS_TAGCONTEXT_ONLY_non_static;
               }
            }
            tag_pop_matches();
         }
         ...
         boolean strict = (prefixexp != '');
         if (!strict)      context_list_flags |= VS_TAGCONTEXT_FIND_lenient;
                   ...
                   VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   if (_chdebug > 9) {
      isay(depth, "_m_fcthelp_get: fnoffset="FunctionNameStartOffset", flags="dec2hex(flags));
   }
   ...
   if (in_msg && receiver != '') {
      ...
      typeless tag_files = tags_filenamea(p_LangId);
      int filter_flags = VS_TAGFILTER_PROC;
      int context_flags = VS_TAGCONTEXT_NO_globals|VS_TAGCONTEXT_ONLY_inclass;
      _str template_args:[];
      class_name := "";
      ...
      if (receiver != '') {
         // evaluate the prefix expression to set up the return type 'rt'
         status = _c_get_type_of_prefix_recursive(errorArgs, tag_files, receiver, rt, visited, 0 , VSCODEHELP_PREFIX_OBJECTIVEC_MESSAGE);
         if (status) {
            tag_pop_matches();
            return(status);
         }
      ...
      if (num_matches > 0) {
         ...
         for (i = 1; i <= n; ++i) {
            ...
            FunctionHelp_list[k].prototype = cm.return_type' ':+lastid:+cm.arguments;

            int base_length = length(cm.return_type) + 1;
            ...
            FunctionHelp_list[k].tagList[0].filename = cm.file_name;
            FunctionHelp_list[k].tagList[0].linenum = cm.line_no;
            FunctionHelp_list[k].tagList[0].taginfo = tag_tree_compose_tag_info(cm);
...
{
   opts := get_options_for_language('m');
   pad1 := pad2 := pad3 := '';
   if (opts._length() > OBJC_METH_PARAM_RPAREN) {
      if (opts[OBJC_METH_PARAM_LPAREN] == 4) {
         pad1 = " ";
      }
      if (opts[OBJC_METH_PARAM_PADPAREN] == 4) {
         pad2 = " ";
      }
      if (opts[OBJC_METH_PARAM_RPAREN] == 4) {
         pad3 = " ";
      }
   }
   openparen = pad1:+"(":+pad2;
...
static _str objc_get_selector_colon_format(boolean decl)
{
   enum OBJCSettingIndex option = (decl) ? OBJC_SP_DECL_SELECTOR_COLON : OBJC_SP_CALL_SELECTOR_COLON;
   opts := get_options_for_language('m');
   if (opts._length() > option) {
      switch (opts[option]) {
...
static int _m_auto_insert_selector_signature(boolean decl)
{
   save_pos(auto p);
   status := 1;
   do {
 ...
 *
 */
boolean _m_autocomplete_after_replace(AUTO_COMPLETE_INFO &word,
                                      ...
                                      _str terminationKey="")
{
   ...
   ch := first_char(idexp_info.otherinfo);
   if ((ch == ':' || ch =='-') && (word.symbol.type_name == "param")) {
      ...
      if (ch == ':') {
         if (_GetCodehelpFlags() & VSCODEHELPFLAG_AUTO_FUNCTION_HELP) {
            _do_function_help(true,true,true);
         }
      }
      return(true);
   }

   if (word.symbol.type_name != "selector" || idexp_info.otherinfo == "@selector") {
      return(false);
   }

   boolean in_decl = (idexp_info.otherinfo == "@method");
...
int _m_get_expression_pos(int &lhs_start_offset,_str &expression_op,int &pointer_count)
{
 ...
 */
int _m_find_matching_word(boolean quiet)
{
   int status;
   save_pos(auto p);
   cfg := _clex_find(0, 'g');
   ...
   if (cfg == CFG_KEYWORD) {
      ...
      if (start_col > 0 && get_text() == '@') {
         if (pos(' 'word' ',' interface implementation protocol end ')) {
            dir := '';
            if (word == 'end') {
               dir = '-';
               if (p_col == 1) {
            ...
            status = search('\@(interface|implementation|protocol|end)', dir:+'@rhxcs');
            for (;;) {
               if (status) break;
               word = get_match_text();
               cfg = _clex_find(0, 'g');
...
int m_smartpaste(boolean char_cbtype, int first_col, int Noflines, boolean allow_col_1=false)
{
   ...
   if (Noflines == 1 && cfg != CFG_STRING && cfg != CFG_COMMENT) {
      ...
      if (pos('^':+id_re:+'[ \t]*\:', line, 1, 'r')) { // line starts with ID:
         _objectivec_message_statement(0, auto indent_col, auto bracket_col, auto arg_col, auto arg_count, true);
...
boolean _objc_in_class()
{
   ...
   save_pos(auto p);
   save_search(auto s1, auto s2, auto s3, auto s4, auto s5);
   int status=search('\@interface|\@implementation|\@protocol|\@end','@rh-xcs');
...
boolean _objectivec_index_operator()
{
...
static boolean _objectivec_in_method_decl()
{
   save_pos(auto p);
 ...
 */
boolean _objectivec_message_statement(int bracket_count, int& indent_col, int& bracket_col, int& arg_col, int& arg_count, boolean arg_align = false, boolean bracket_pos = false)
{
   orig_line := p_line;
   orig_col := p_col;
   orig_point := (int)point('s');
   ...
   int status = search('[?:;{}()\[\]]|\b(class|struct|default|case|public|private|protected)|\@(interface|implementation|protocol|end|package|private|protected|public|optional|dynamic|synthesize|required|property)','-@rhxcs');
   for (;;) {
      if (status) {
         arg_col = -1;
         arg_count = -1;
      ...
      case '@protocol':
         if (!_objectivec_find_next_class_decl(orig_point)) {
            ch := get_text();
            if (ch == '+'|| ch == '-') {
               indent_col = p_col + beaut_continuation_indent(p_LangId);
      ...
      case '@public':
         if (beaut_indent_members_from_access_spec(p_LangId)) {
            ...
            indent_col = p_col + syntax_indent - beaut_member_access_indent(p_LangId);
         }
         arg_col = -1;
         arg_count = -1;
         break;
      ...
      if (bracket_count > 0) {
         ...
         if (!_objectivec_index_operator() && !at_start) {
            ...
            if (LanguageSettings.getUseContinuationIndentOnFunctionParameters(p_LangId)) {
               // continuation indent
               first_non_blank();
               indent_col = p_col + syntax_indent;
            } else {
...
static boolean _at_unary_position(boolean in_brackets = false) {
   save_pos(auto p);
   if (in_brackets) {
      _clex_skip_blanks('-');
      left();
...
const OBJECTIVEC_CLASS_KEYWORDS_RE = '\@(implementation|public|protected|private|package|interface|protocol|optional|dynamic|synthesize|required|property)';

boolean objectivec_inside_dict_literal() {
   save_pos(auto p);
   save_search(auto s1, auto s2, auto s3, auto s4, auto s5);
   int nesting_seen = 0;
   int status=search('[{}]|'OBJECTIVEC_CLASS_KEYWORDS_RE'|\@end',"@RH-");
   int viable = 0;
...
boolean _m_surroundable_statement_end() {
