...
boolean def_prompt_extless_select_mode = true;
// whether the C-code copy of the file name mapper has 
// been initialized (reset every session)
boolean def_file_name_map_init_c_value = false;
// whether the file name mapper has been initialized 
// with our default values
boolean def_file_name_map_init_defaults = false;

FileNameMapper def_file_name_mapper;

#define DIRECTORY_WILDCARD       '**'FILESEP
#define FILENAME_WILDCARD        '*'

void _InitializeFileNameMapping(boolean forceRebuildRegex)
{
   if (!def_file_name_map_init_c_value || forceRebuildRegex) {
      def_file_name_mapper.init(forceRebuildRegex);

      def_file_name_map_init_c_value = true;
   }
}

/**
 * This function is meant to be called from the C-code, to make 
 * sure that we have initialized our mappings for extensionless 
 * files. 
 * 
 * @param vshPathMap 
 * @param vshPatternMap 
 */
void getFileNameMapperMaps(_str (&vshPathMap):[], FILELANGMAP_FILEPATTERN (&vshPatternMap)[])
{
   def_file_name_mapper.getSimpleMaps(vshPathMap, vshPatternMap);
   def_file_name_map_init_c_value = true;
}

/**
 * Migrates old hard-coded and C/C++-only stuff to the new extensionless file 
 * handling added in SlickEdit 2010. 
 */
void _UpgradeExtensionlessFiles(_str config_migrated_from_version)
{
   if (config_migrated_from_version != '') {
      ...
      // we introduced this bool a little late, so the values may already be initialized!
      if (!def_file_name_map_init_defaults) {
         _str absoluteFiles:[];
         FilePattern patterns[];
         def_file_name_mapper.getLists(absoluteFiles, patterns);
         if (absoluteFiles._length() || patterns._length()) {
   ...
   pattern.AntPattern = filenameToAntPattern('makefile');
   pattern.Language = 'mak';
   def_file_name_mapper.addPatternMap(pattern);
   ...
   pattern.AntPattern = filenameToAntPattern('Imakefile');
   ...
   // we've initialized the map now, too
   def_file_name_map_init_c_value = true;
}

defeventtab _manage_advanced_file_types_form;

#define ADVANCED_FILE_TYPES_MODIFIED         _ctl_files_label.p_user
...
boolean _manage_advanced_file_types_form_is_modified()
{
   return ADVANCED_FILE_TYPES_MODIFIED;
}

boolean _manage_advanced_file_types_form_apply()
{
   _str absoluteFiles:[];
   FilePattern patterns[];
   getListsFromTrees(absoluteFiles, patterns);

   def_file_name_mapper.setLists(absoluteFiles, patterns);
...
_str _manage_advanced_file_types_form_export_settings(_str &file)
{
   ...
   status := def_file_name_mapper.exportMap(file :+ filename);
...
_str _manage_advanced_file_types_form_import_settings(_str &file)
{
   ...
   status := def_file_name_mapper.importMap(file);
...
void _ctl_files_tree.on_create()
{
   // set up the trees!
   colSlice := _ctl_files_tree.p_width / 4;
   _ctl_files_tree._TreeSetColButtonInfo(0, colSlice * 3, TREE_BUTTON_PUSHBUTTON | TREE_BUTTON_SORT_FILENAME, -1, 'File');
   _ctl_files_tree._TreeSetColButtonInfo(1, colSlice, TREE_BUTTON_PUSHBUTTON | TREE_BUTTON_SORT, -1, 'Language');
   ...
   FilePattern patterns[];
   def_file_name_mapper.getLists(absoluteFiles, patterns);
   loadListsIntoTrees(absoluteFiles, patterns);
 ...
 *                               matters.
 */
static void loadListsIntoTrees(_str (&files):[], FilePattern (&patterns)[])
{
   // add these files to the files tree
   _str file, langId;
   foreach (file => langId in files) {
      caption := buildTreeCaption(file, _LangId2Modename(langId));
      _ctl_files_tree._TreeAddItem(TREE_ROOT_INDEX, caption, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   }
 ...
 */
static void getListsFromTrees(_str (&files):[], FilePattern (&patterns)[])
{
   _ctl_files_tree._TreeTop();
   index := _ctl_files_tree._TreeCurIndex();
   while (index > 0) {
      getTreeLineData(_ctl_files_tree, index, auto type, auto file, auto language);
      files:[file] = _Modename2LangId(language);

      if (_ctl_files_tree._TreeDown()) break;
      index = _ctl_files_tree._TreeCurIndex();
   }

   _ctl_patterns_tree._TreeTop();
   index = _ctl_patterns_tree._TreeCurIndex();
   while (index > 0) {
      FilePattern pattern;
      pattern.Regex = '';
      pattern.AllFiles = false;
      ...
      if (_ctl_patterns_tree._TreeDown()) break;
      index = _ctl_patterns_tree._TreeCurIndex();
   }
}
...
void _manage_advanced_file_types_form.on_resize()
{
   ...
   // determine padding
   padding := _ctl_files_label.p_x;

   widthDiff := width - (_ctl_edit_file_btn.p_x + _ctl_edit_file_btn.p_width + padding);
   heightDiff := height - (_ctl_patterns_tree.p_y + _ctl_patterns_tree.p_height + padding);

   if (widthDiff) {
      // move the files tree buttons over
      _ctl_edit_file_btn.p_x += widthDiff;
      _ctl_add_file_btn.p_x += widthDiff;
      _ctl_delete_file_btn.p_x += widthDiff;

      // move the pattern tree buttons over
      _ctl_edit_pattern_btn.p_x += widthDiff;
      _ctl_add_filename_pattern_btn.p_x += widthDiff;
      ...
      // widen the trees
      _ctl_files_tree.p_width += widthDiff;
   ...
   if (heightDiff) {
      // divide the height change between the two trees
      treeHeightDiff := heightDiff / 2;
      _ctl_files_tree.p_height += treeHeightDiff;
      _ctl_patterns_tree.p_height += treeHeightDiff;

      // move the pattern tree down
      _ctl_patterns_tree.p_y += treeHeightDiff;

      // move the pattern label down
      _ctl_patterns_label.p_y += treeHeightDiff;

      // move only the patterns tree buttons down
      _ctl_edit_pattern_btn.p_y += treeHeightDiff;
      _ctl_add_pattern_btn.p_y += treeHeightDiff;
      _ctl_add_filename_pattern_btn.p_y += treeHeightDiff;
      _ctl_add_path_pattern_btn.p_y += treeHeightDiff;
      _ctl_delete_pattern_btn.p_y += treeHeightDiff;
      _ctl_move_up_pattern_btn.p_y += treeHeightDiff;
      _ctl_move_down_pattern_btn.p_y += treeHeightDiff;
   }
}

void _ctl_files_tree.on_change(int reason)
{
   if (reason == CHANGE_SELECTED) {
      enableDisableButtons(AFTMT_FILE);
   }
}
...
{
   if (reason == CHANGE_SELECTED) {
      enableDisableButtons(AFTMT_PATTERN);
   }
}
 ...
 * @param type             
 */
void enableDisableButtons(int type = -1)
{
   if (type == -1 || type == AFTMT_FILE) {
      // check out the files tree
      int selected[];
      _ctl_files_tree._TreeGetSelectionIndices(selected);
      numSelected := selected._length();

      if (numSelected == 0) {
         _ctl_delete_file_btn.p_enabled = _ctl_edit_file_btn.p_enabled = false;
      } else if (selected._length() == 1) {
         _ctl_edit_file_btn.p_enabled = _ctl_delete_file_btn.p_enabled = true;//(curIndex > 0);
      } else {
         _ctl_delete_file_btn.p_enabled = true;
         _ctl_edit_file_btn.p_enabled = false;
      }
   } 
...
{
   treeWidth := _ctl_files_tree.p_width;

   colSlice := _ctl_files_tree.p_width / 4;
   _ctl_files_tree._TreeSetColButtonInfo(0, colSlice * 3);
   _ctl_files_tree._TreeSetColButtonInfo(1, colSlice);
 ...
 */
static _str pathToAntPattern(_str path, boolean recursive)
{
   antPattern := path;
   if (recursive) {
 ...
 */
static _str antPatternToPath(_str antPattern, boolean &recursive)
{
   path := '';
   recursive = false;
   ...
   if (last_char(antPattern) == FILENAME_WILDCARD) {
 ...
 *                                  nothing changed.
 */
static int maybeAddToTree(int treeWid, _str caption, int type, int index = -1, boolean silent = false)
{
...
void _ctl_edit_file_btn.lbutton_up()
{
   editExtensionlessFileSetting(AFTMT_FILE);
}

void _ctl_add_file_btn.lbutton_up()
{
   addNewExtensionlessFileSetting(AFTMT_FILE);
}

void _ctl_delete_file_btn.lbutton_up()
{
   deleteExtensionlessFileSetting(AFTMT_FILE);
}
...
void _ctl_add_filename_pattern_btn.lbutton_up()
{
...
{
   // AFTMT_FILE is in the top tree, the rest are in the patterns tree
   return (type == AFTMT_FILE) ? _ctl_files_tree : _ctl_patterns_tree;
}
...
#define CURRENT_TYPE             _ctl_ok.p_user
...
static void loadExampleTree()
{
   ...
   _ctl_example_tree._TreeAddItem(TREE_ROOT_INDEX, '**\foo\*'\t'files directly under a directory named "foo"', TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   _ctl_example_tree._TreeAddItem(TREE_ROOT_INDEX, '**\foo\**'\t'files or directories named "foo"', TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   _ctl_example_tree._TreeAddItem(TREE_ROOT_INDEX, '**\*foo\**'\t'files or directories ending with "foo"', TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   _ctl_example_tree._TreeAddItem(TREE_ROOT_INDEX, '**\foo\**\*'\t'files under a directory named "foo"', TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
   _ctl_example_tree._TreeAddItem(TREE_ROOT_INDEX, '**\foo\**\bar'\t'files named "bar" under a directory named "foo"', TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF);
}
...
#define MAP_FILE           _ctl_ok.p_user
#define MAP_LANGID         _ctl_options.p_user
...
void _ctl_ok.on_create(_str file, _str langId)
{
   ...
   dir = _ShrinkFilename(dir, leftoverWidth);
...
void _ctl_ok.lbutton_up()
{
   if (_ctl_rb_map_file.p_value) {
      ...
      def_file_name_mapper.addFileMap(MAP_FILE, MAP_LANGID);
...
void _map_files_like_this_form.ESC()
{
   def_prompt_extless_select_mode = (_ctl_do_not_ask.p_value == 0);
   p_active_form._delete_window();
}
