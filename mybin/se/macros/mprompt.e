...
void ctlbrowse.lbutton_up()
{
   ...
   boolean orig_val=_use_cd_tbopen();
   _use_cd_tbopen(0);
...
#define AFTER_LABEL_PAD     90
#define BOX_DEFAULT_WIDTH   3500
#define MINIMUM_BUTTON_PAD  180

static _str parse_option(_str &prompt, _str &p1, _str &p2)
{
   _str ch = substr(prompt, 1, 1);
   if (ch != '-') {
      return('');
   }
   _str option=upcase(parse_file(prompt));
...
static boolean in_irange(int p1, int p2, typeless num)
{
...
static boolean in_range(int p1, int p2, typeless num)
{
                   ...
                   _str prompt1="", _str promptN="") // prompt1 .. promptN
{
   ...
   if (ButtonList!='') {
      ...
      if (ButtonList!='') {
         _help.p_visible=_cancel.p_visible=0;
      }
      x=_ok.p_x;
      y=180;
      for (;;) {
         ...
         if (pos("-html ", CurCaption)==1) {
            ...
            wid.p_height= _dy2ly(SM_TWIP,_screen_height()/2) - y;
            wid._minihtml_ShrinkToFit(p_active_form.p_width-wid.p_x*2);
            y+=wid.p_height;
            FirstBox=wid.p_y+wid.p_height;
            if (!CaptionWID) {
               CaptionWID=wid;
            }
         } else {
            ...
            if (wid) {
               ...
               y+=wid.p_height+180;
               FirstBox=wid.p_y+wid.p_height+180;
               if (!CaptionWID) {
                  CaptionWID=wid;
               }else{
   ...
   _str init_value="";
   _str prompt="";
   int minimum_form_width=0;
   int height=0;
   int ul2_index=0;
   int ul1_index=0;
   boolean have_init_val=false;
   ...
   if( arg(FIRST_DATA_ARG_INDEX) != "" ) {
      ...
      minimum_form_width=_help.p_x+_help.p_width+_ok.p_x+x_border_width;
      ...
      for( ;; ) {
         ...
         completion = "";
         complete_option = current_line;
         boolean have_browse_button = false;
         int browse_button_eventtab = 0;
         boolean checkbox = false;
         ...
         for( ;; ) {
            ...
            } else if( strieq(option,'BD') ) {
               browse_button_eventtab=defeventtab _textbox_browse_directory.ctlbrowse;
               have_browse_button=true;
         ...
         if( !checkbox ) {
            ...
            if( retrieve ) {
               ...
               wid=_create_window(OI_COMBO_BOX,
                                  form_wid, // Parent
                                  "",       // Title
                                  label_wid.p_x + label_wid.p_width + AFTER_LABEL_PAD, // x
            ...
            if (have_browse_button) {
               ...
               if (wid2) {
                  ...
                  wid2.p_x=wid.p_x+wid.p_width + AFTER_LABEL_PAD;
   ...
   if( ButtonList!="" ) {
      ...
      if (ButtonList!='') {
         ...
         int SpaceBetweenButtons=180;//_cancel.p_x-(_ok.p_x+_ok.p_width);
         ...
         int NextX=LastWID.p_x+LastWID.p_width+SpaceBetweenButtons;
         int newwid=0;
         int ButtonCount=1;
         for (;;) {
            ...
            if (newwid) {
               ...
               NextX=newwid.p_x+newwid.p_width+SpaceBetweenButtons;
            ...
            if (newwid.p_x+newwid.p_width > p_active_form.p_width) {
               p_active_form.p_width=newwid.p_x+newwid.p_width+_ok.p_x/*SpaceBetweenButtons*/;
            }
            if (TextBoxWidth=='') {
               ...
               for (i=1;;++i) {
                  ...
                  if( wid.p_object != OI_CHECK_BOX ) {
                     int browse_button_width=0;
                     if (wid.p_next.p_object==OI_COMMAND_BUTTON && wid.p_next.p_caption=='...') {
                        browse_button_width=wid.p_next.p_width;
                     }
                     wid.p_width=max(BOX_DEFAULT_WIDTH-browse_button_width ,(newwid.p_x+newwid.p_width)-wid.p_x);
                  }
               }
            }
         }
   ...
   p_active_form.p_height = _ok.p_y + _ok.p_height + y_border_width +180;
   if (buffer_input) {
      get_window_id(view_id);
      activate_window(temp_view_id);
      _delete_buffer();
   ...
   if( CaptionWID ) {
      ...
      if( CaptionWID.p_x+CaptionWID.p_width>clientwidth ) {
         p_active_form.p_width+=(CaptionWID.p_x+CaptionWID.p_width+CaptionWID._text_width(' '))-clientwidth;
      }
   }
}
...
_ok.lbutton_up()
{
   ...
   for (;;) {
      ...
      if( wid.p_object!=OI_CHECK_BOX ) {
         ...
         case 'E' :
         case 'E1':
            int index = find_index(p1, PROC_TYPE|COMMAND_TYPE);
            if (_isfunptr(p1)) {
...
_str _ini_xlat_multiline(_str line)
{
   ...
   for (;;) {
      ...
      result=result:+substr(line,i,j-i);
      ch=substr(line,j+1,1);
      switch (ch) {
      case 'n':
         result=result:+_chr(13);
         break;
      default:
         result=result:+ch;
      }
...
*/
void ctlok.on_create(_str FormCaption,_str Captions[],int Flags=0,int DefaultRB=1,_str HelpItem='',_str RetrieveName='')
{
   if (RetrieveName!='') {
   ...
   if (wid) {
      ctlok.p_y=ctlok.p_next.p_y=ctlhelp.p_y=wid.p_y+wid.p_height+200;
   }
   p_active_form.p_height=ctlok.p_y+ctlok.p_height+diff_y+100;
   if (RetrieveName!='') {
      _retrieve_prev_form();
   }
}
...
int RadioButtons(_str DialogCaption,_str Captions[],int InitialOn=1,_str RetrieveName='')
{
   typeless result=show('-modal _rb_form',DialogCaption,Captions,0,InitialOn,'',RetrieveName);
   if (result=='') {
      return(COMMAND_CANCELLED_RC);
   }
   return(result);
...
int CheckBoxes(_str DialogCaption,_str Captions[],int BoxOnFlags=0,_str RetrieveName='')
{
   typeless result=show('-modal _rb_form',DialogCaption,Captions,RBFORM_CHECKBOXES,BoxOnFlags,'',RetrieveName);
   if (result=='') {
      return(0);
   }
   return(result);
...
#define COMBOBOX_FORM_RETRIEVE_NAME      _ctl_ok.p_user
...
#define RETRIEVE_NAME      _checkbox.p_user
...
_message.on_create(_str formTitle, _str label, _str checkBoxLabel, int buttons, int defaultValue, _str retrieveName = '', _str help = '')
{
   ...
   if (textWidth > maxLabelWidth) {
      ...
      lines := (textWidth / maxLabelWidth) + 1;
...
void _checkbox_form.on_resize()
{
   ...
   xDiff := p_width - (_cancel.p_x + _cancel.p_width + padding);
   _cancel.p_x += xDiff;
   _no.p_x += xDiff;
   _yes.p_x += xDiff;

   yDiff := p_height - (_cancel.p_y + _cancel.p_height + padding);
   _cancel.p_y += yDiff;
