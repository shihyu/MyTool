...
{
   typeless status=0;
   if (last_char(HomeDir)==FILESEP) HomeDir=substr(HomeDir,1,lastpos(FILESEP,HomeDir)-1);
   _str ExistingFilename=file_match(HomeDir:+FILESEP:+Filename' -p',1);
   if (ExistingFilename=='') {
      ...
      switch (Filename) {
      case '.fvwmrc':
         _str rfilename='/usr/lib/X11/fvwm/system'Filename;
...
static _str GetModifierKeyNames(_str ModStr,_str WindowManager)
{
   ...
   case 'fvwm':
      for (;;) {
         _str ch=substr(ModStr,1,1);
         if (ch=='') break;
         ModStr=substr(ModStr,2);
         if (KeyNameStr=='') {
            KeyNameStr=GetKeyName(ch,WindowManager);
         }else{
            KeyNameStr=KeyNameStr'-'GetKeyName(ch,WindowManager);
         }
      }
      break;
   case 'mwm':
...
{
   _str ValidAreaNameStr='';
   switch (lowcase(WindowManager)) {
   case 'fvwm':
      for (;;) {
         _str ch=substr(ValidAreaStr,1,1);
         if (ch=='') break;
         ValidAreaStr=substr(ValidAreaStr,2);
         if (ValidAreaNameStr=='') {
            ValidAreaNameStr=GetAreaName(ch,WindowManager);
         }else{
            ValidAreaNameStr=ValidAreaNameStr'|'GetAreaName(ch,WindowManager);
         }
      }
      break;
   case 'mwm':
...
static boolean IsValidKey( _str KeyName, _str WindowManager)
{
   KeyName=strip(KeyName);
   switch (lowcase(WindowManager)) {
   case 'fvwm':
   case 'mwm':
      if (pos('^(Left|Right|Up|Down)$',KeyName,'','ri')) return 1;
      if (pos('^F:i$',KeyName,'','ri')) return 1;
   }
   return(0);
}
static boolean IsValidModifier( _str KeyName, _str WindowManager)
{
   KeyName=strip(KeyName);
   switch (lowcase(WindowManager)) {
   case 'mwm':
      if (pos('^(Alt|Ctrl|Shift)$',KeyName,'','ri')) return 1;
   }
   return(0);
}
   ...
   if (!status) {
      ...
      while (!search(string,"@ri")) {
         ...
         if (Command=='') {
            ...
            if (ModNameStr!='') ModNameStr=ModNameStr'-';
...
static int MWMFillInListBoxes()
{
   ...
   if (!status) {
      ...
      _str string='((Shift|Meta|Extend|Alt|Ctrl|)( |\t|))@<Key>';
      ...
      while (!search(string,"@ri")) {
         ...
         _str Modifiers="", KeyName="", ValidArea="", Command="";
         ...
         int KeyIsInactive=pos('!VS',line,'','i');
         p_window_id=OrigViewId;
...
static void GetKeyNameAndModifiers(_str &KeyName, _str &Modifiers, _str WindowManager)
{
   ...
   for (;;) {
      ...
      if (!IsValidKey(KeyOrMod,WindowManager)) {
         ...
         switch (lowcase(WindowManager)) {
         case 'fvwm':
            Modifiers=Modifiers:+ModStr;
            break;
         case 'mwm':
            if (Modifiers=='') {
               Modifiers=GetModifierKeyAbbreviation(KeyOrMod,WindowManager);
            }else{
               Modifiers=Modifiers' 'GetModifierKeyAbbreviation(KeyOrMod,WindowManager);
            }
            break;
         }
      }else{
...
static _str GetValidAreas( _str ValidAreas, _str WindowManager)
{
   ...
   case 'fvwm':
      ...
      for (;;) {
         ...
         if (cur=='') break;
         str=str:+GetAreaAbbreviation(ValidAreas,WindowManager);
      }
      return str;
   case 'mwm':
...
static void ActivateOrDeactivate( boolean Activate, int RCFileViewId, _str WindowManager)
{
   switch (WindowManager) {
   case 'fvwm':
   case 'mwm':
      ...
      while (!_lbdown()) {
         ...
         _str KeyName="", ValidAreas="", Command="";
...
static int WindowManagerWriteFile( _str WindowManager)
{
   ...
   _ctlcommand_list.ActivateOrDeactivate(1,TempViewId,WindowManager);
   _ctlinactive_command_list.ActivateOrDeactivate(0,TempViewId,WindowManager);
   p_window_id=TempViewId;
...
{
   int wid=p_window_id;p_window_id=_control _ctlcommand_list;
...
static void RestartWindowManager(typeless WindowManager)
{
   ...
   if (result==IDYES) {
      ...
      p_window_id=temp_view_id;
      top();up();
      int status=search(WindowManager,'w=A-Za-z0-9_$');
      p_window_id=orig_view_id;
      _delete_temp_view(temp_view_id);
...
_ctlok.lbutton_up()
{
   ...
   _str homedir=get_env('HOME');
   if (last_char(homedir)!=FILESEP) homedir=homedir:+FILESEP;
