...
_command void template_manager() name_info(","VSARG2_READ_ONLY)
{
...
static void _ctTemplateManagerContextMenu(_str menuNameOrIndex, int x=-1, int y=-1)
{
   ...
   if( handle<0) {
      _str msg = "Unable to load menu \"":+menuNameOrIndex:+"\"";
...
#define CTM_DETAILS_TAB 0
#define CTM_FILES_TAB   1
#define CTM_CUSTOM_PARAMETERS_TAB 2
...
static int findCategoryTreeOrDie(int wid=0)
{
   ...
   if( wid==0 ) {
      _str msg = "Unable to locate Categories tree.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   }
   return wid;
}
...
static int findItemListOrDie(int wid=0)
{
   ...
   if( wid==0 ) {
      _str msg = "Unable to locate Templates list.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   }
   return wid;
}
 ...
 */
static boolean verifyFormInput(_str& templateFilename,
                               ctTemplateDetails_t& templateDetails,
                               ctTemplateContent_t& templateContent,
                               boolean quiet=false, boolean selectError=true,
...
{
   boolean was_ReadOnly = p_ReadOnly;
   p_ReadOnly=false;
 ...
 */
static boolean detailsEqual(ctTemplateDetails_t& details1, ctTemplateDetails_t& details2)
{
 ...
 */
static boolean contentEqual(ctTemplateContent_t& content1, ctTemplateContent_t& content2)
{
   _str Delimiter1 = strip(content1.Delimiter);
   _str Delimiter2 = strip(content2.Delimiter);
 ...
 */
static boolean checkForUnsavedFormInput(boolean& needToSave,
                                        ...
                                        _str& statusMessage=null)
{
   ...
   int sstabWid = p_active_form._find_control("ctl_template_sstab");
   int categoryWid = p_active_form._find_control("ctl_category");
   int itemWid = p_active_form._find_control("ctl_item");
...
static boolean saveTemplate(_str templateFilename,
                            ctTemplateDetails_t& details, ctTemplateContent_t& content,
                            boolean quiet=false, _str& msg=null)
{
 ...
 * @return true if no errors.
 */
static boolean maybeSaveFormInput(boolean quiet=false)
{
...
static void maybeReloadCurrentItem(boolean postCall=false)
{
...
static void _ctTemplateManager_form_initial_alignment()
{
   ...
   alignUpDownListButtons(ctl_files.p_window_id, rightAlign, ctl_file_add.p_window_id,
                          ctl_file_edit.p_window_id, ctl_file_up.p_window_id,
                          ctl_file_down.p_window_id, ctl_file_remove.p_window_id);

   alignUpDownListButtons(ctl_params.p_window_id, rightAlign, ctl_param_add.p_window_id,
                          ctl_param_edit.p_window_id, ctl_param_remove.p_window_id);
}
...
_command void ctTemplateManagerCategoryTreeNewTemplate(int categoryWid=0)
{
...
_command void ctTemplateManagerCategoryTreeNew(int categoryWid=0)
{
   ...
   if( _ctIsSysItemTemplatePath(templateDir) ) {
      _str msg = "Cannot create new categories under installed templates category. Create new categories under user templates category instead.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return;
   }
   ...
   do {
      ...
      // current category.
      if( path!="" && !file_eq( substr(absolute(path,templateDir),1,length(templateDir)) , templateDir ) ) {
...
_command void ctTemplateManagerCategoryTreeDelete(int categoryWid=0)
{
   categoryWid=findCategoryTreeOrDie(categoryWid);
   if( categoryWid==0 ) {
      return;
   ...
   if( _ctIsSysItemTemplatePath(templateDir) ) {
      _str msg = "Deleting installed categories not allowed.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return;
   }

   _str folder = substr(catPath,lastpos('/',catPath)+1);
   ...
   if( file_eq(substr(cwd,1,length(templateDir)),templateDir) ) {
      // Cannot delete the directory if we are in it
      _str new_cwd = strip(templateDir,'T',FILESEP);
   ...
   boolean isRootDir = file_eq(templateDir,userItemTemplatesRoot);
   ...
   if( lastTemplateFilename!="" && file_eq(substr(lastTemplateFilename,1,length(templateDir)),templateDir) ) {
 ...
 *                Defaults to "".
 */
_command void ctTemplateManagerCategoryTreeRefresh(int categoryWid=0, _str subPath="", ...)
{
   categoryWid=findCategoryTreeOrDie(categoryWid);
   if( categoryWid==0 ) {
      return;
...
static void onSelectItem()
{
   ...
   if( _ctItemListGetCurrentItem(templateFilename,itemName) ) {
      ...
      do {
         ...
         if( status!=0 ) {
            _str msg = "Error retrieving template data for \"":+templateFilename:+"\". ":+get_message(status);
            _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
            break;
         }
   ...
   int templateFilenameWid = p_active_form._find_control("ctl_template_filename");
   int sstabWid = p_active_form._find_control("ctl_template_sstab");
   int detailsViewWid = p_active_form._find_control("ctl_details_view");
   int filesViewWid = p_active_form._find_control("ctl_files_view");
   int paramsViewWid = p_active_form._find_control("ctl_params_view");
   int categoryWid = p_active_form._find_control("ctl_category");
   int itemWid = p_window_id;
   ...
   boolean enable = ( templateFilename != "" );
   ...
   if( enable ) {
      ...
      boolean readOnly = _ctIsSysItemTemplatePath(templateFilename);
 ...
 */
static boolean pointToCategory(int categoryWid, int mx, int my, _str& categoryPath, _str& fsPath, boolean refreshList)
{
   _str catPath = "";
...
static void dragdropFromItemListContextMenu(_str categoryDir, _str srcTemplateFilename, int x=-1, int y=-1)
{
   ...
   _str menuName = "_temp_TemplateManagerDragDrop_menu";
...
void _ctTemplateManager_ItemList_etab.lbutton_down(boolean rbuttonDown=false)
{
   ...
   boolean done = false;
   _str event = "";
#define MIN_CX ( 40 intdiv _twips_per_pixel_x() )
#define MIN_CY ( 40 intdiv _twips_per_pixel_y() )
   boolean notEnoughDrag = true;
      ...
      case MOUSE_MOVE:
         {
            ...
            boolean inside = _WinRectPointInside(r,ctl_category,x,y);
            if( inside ) {
               mx=ctl_category.mou_last_x();
               my=ctl_category.mou_last_y();
               _str catPath="", templatePath="";
   ...
   _str catPath="", dstPath="";
   boolean bstatus = pointToCategory(ctl_category,mx,my,catPath,dstPath,false);
   ...
   if( rbuttonDown ) {
      ...
      if( _ctIsSysItemTemplatePath(srcTemplateFilename) ) {
         ...
         _str msg = "You cannot move a template from a system category folder.";
         if( _isMac() ) {
...
_command void ctTemplateManagerItemListNew(int categoryWid=0, int itemWid=0)
{
   categoryWid=findCategoryTreeOrDie(categoryWid);
   if( categoryWid==0 ) {
      return;
   ...
   _maybe_append_filesep(templateDir);
   if( _ctIsSysItemTemplatePath(templateDir) ) {
      _str msg = "Cannot create new templates under installed templates category. Create new templates under user templates category instead.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return;
   }
   ...
   do {
      ...
      // current category.
      if( subdir!="" && !file_eq( substr(absolute(subdir,templateDir),1,length(templateDir)) , templateDir ) ) {
      ...
      templateFilename=templateFilename:+CT_EXT;
      if( subdir != "" ) {
   ...
   if( subdir!="" ) {
      location=location:+subdir:+FILESEP:+name;
   } else {
 ...
 * @return int
 */
static int deleteTemplate(_str templateFilename, boolean prompt=true)
{
   ...
   if( content.Files != null && content.Files._length() > 0 ) {
      ...
      for( i=0; i<content.Files._length(); ++i ) {
         ...
         if( file_eq(substr(Filename,1,length(templateDir)),templateDir) ) {
...
_command void ctTemplateManagerItemListDelete(int itemWid=0)
{
   itemWid=findItemListOrDie(itemWid);
   if( itemWid==0 ) {
      return;
   }
   _str templateFilename="", itemName="";
                                           ...
                                           boolean refreshList=true)
{
   ...
   if( !_ctIsUserItemTemplatePath(dstCategoryDir) ) {
      _str msg = "You cannot copy/move a template into a system category folder. Please choose a user category folder destination.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return COMMAND_CANCELLED_RC;
   }
   ...
   _str srcTemplateFilename="", srcItemName="";
   if( !itemWid._ctItemListGetCurrentItem(srcTemplateFilename,srcItemName) ) {
      return COMMAND_CANCELLED_RC;
   }
   _str srcTemplateDir = _strip_filename(srcTemplateFilename,'n');
   ...
   if( !file_eq(srcTemplateDir,userItemTemplatesDir) ) {
   ...
   _str dstTemplateFilename = absolute( relative(srcTemplateFilename,srcCategoryDir) , dstCategoryDir );
   _str dstTemplateDir = _strip_filename(dstTemplateFilename,'n');
   _maybe_append_filesep(dstTemplateDir);

   // Verify we are not copying source over itself
   if( file_eq(srcTemplateFilename,dstTemplateFilename) ) {
   ...
   do {
      ...
      for( i=0; i<content.Files._length(); ++i ) {
         ...
         _str srcFilename = absolute(content.Files[i].Filename,srcTemplateDir);
         _str dstFilename = absolute(content.Files[i].Filename,dstTemplateDir);
         if( file_eq(srcFilename,dstFilename) ) {
         ...
         _str dstDir = _strip_filename(dstFilename,'n');
   ...
   if( refreshList ) {
      ...
      if( file_eq(categoryDir,dstCategoryDir) ) {
                                           ...
                                           boolean refreshList=true)
{
   ...
   if( !_ctIsUserItemTemplatePath(srcTemplateFilename) ) {
      _str msg = "You cannot move a template from a system category folder. Consider copying the template instead.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return COMMAND_CANCELLED_RC;
   }
   ...
   _maybe_append_filesep(categoryDir);
   _maybe_append_filesep(dstCategoryDir);
   if( refreshList && file_eq(categoryDir,dstCategoryDir) ) {
...
_command void ctTemplateManagerItemListRefresh(int itemWid=0, int categoryWid=0)
{
   itemWid=findItemListOrDie(itemWid);
   if( itemWid==0 ) {
      return;
   ...
   _str templateDir = categoryWid._ctCategoryToTemplatePath();
   if( templateDir!="" ) {
      boolean old_inOnChange = itemWid._ctItemListInOnChange(1);
