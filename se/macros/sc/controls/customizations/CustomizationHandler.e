...
const LAST_ITEM_TEXT=        'LAST_ITEM';                // how we designate the last item in a list
const FIRST_ITEM_TEXT=       'FIRST_ITEM';               // how we designate the first item in a list
const LAST_ITEM_INDEX=       -1;                         // index of "last item"
const FIRST_ITEM_INDEX=      -2;                         // index of "first item"
const NOT_FOUND=             -3;                         // index of an item which is not found

static const MODIFICATION_LIST_ELEMENT=   'Modifications';
static const MODIFICATION_ELEMENT=        'Modification';
static const SEPARATOR_LIST_ELEMENT=      'Separators';
static const SEPARATOR_ELEMENT=           'Separator';
static const DUPLICATE_LIST_ELEMENT=      'Duplicates';
static const DUPLICATE_ELEMENT=           'Duplicate';
...
class CustomizationHandler {
    ...
    *                         not created
    */
   protected int openModFile(bool create = true)
   {
      xmlHandle := 0;
      if (!file_exists(m_modFile) && create) {
         ...
         if (xmlHandle < 0) {
            ...
            // make a copy of the bad file and delete the original so we can put something good in its place
            copy_file(m_modFile, m_modFile'.bad');
            delete_file(m_modFile);

            // try again, this will make a new file
    ...
    *                         and was not created
    */
   protected int getModCategory(int xmlHandle, bool create = true)
   {
    ...
    *                            was not created
    */
   protected int getModItem(int xmlHandle, int categoryNode, _str name, bool create = true)
   {
    ...
    */
   protected bool compareIdentifiers(UserControl &origItem, UserControl &curItem)
   {
    ...
    */
   protected int determineItemPosition(_str prevItem, _str nextItem, typeless (&list)[], bool strict)
   {
      newPos := NOT_FOUND;
      findAfter := -1;
      listLength := list._length();
    ...
    */
   protected int determinePositionFromSurroundingPositions(int beforeIndex, int afterIndex, int listLength, bool strict)
   {
      newPos := NOT_FOUND;
    ...
    *                            next one
    * 
    * @return                    identifier of next item
    */
    ...
    *                            previous one
    * 
    * @return                    identifier of previous item
    */
    ...
    */
   protected bool removeMods(int xmlHandle, int categoryNode, _str name)
   {
      ...
      if (importHandle > 0) {
         ...
         if (origHandle > 0) {
            ...
            if (origCategoryNode > 0 && importCategoryNode > 0) {
               ...
               _xmlcfg_save(origHandle, -1, VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE);
            }
    ...
    */
   protected bool compareSeparatorLists(Separator (&origSeps)[], Separator (&curSeps)[])
   {
   ...
   /**
    * Prints a list of controls, used for debugging purposes.
    *
    * @param list
    * @param msg
    */
   protected void debugPrintControlList(UserControl (&list)[], _str msg = 'PRINTING CONTROL LIST')
   {
      say('***'msg'***');
      for (i := 0; i < list._length(); i++) {
         say('   'i' - 'list[i].getIdentifier());
      }
      say('***************************');
   }
}
