...
class MenuCustomizationHandler : CustomizationHandler {
   ...
   { 
      m_modFile = _ConfigPath() :+ VSCFGFILE_USER_MENUS;
      m_elementName = 'Menu';
    ...
    *                            false otherwise
    */
   public bool saveMenuChanges(_str menuName, int origMenuIndex, int curMenuIndex)
   {
      ...
      do {
         if (menuItem.p_object == OI_MENU_ITEM && menuItem.p_caption == '-') {
            ...
            if (menuItem.p_object == OI_MENU) {
               ...
               buildMenu(getItemKey(caption, menuName), menuItem, menu, seps);
            } else {
   ...
   private void writeMenuMods(_str menuName, MenuModification (&mods)[], Separator (&seps)[], 
                              MenuModification (&dupes)[], bool new = false)
   {
      // open up our file
   ...
   private void getSubMenuPositionInfo(_str parentMenuName, int menuIndex, _str (&parentIndexTable):[], STRARRAY (&subMenus):[])
   {
      ...
      do {
         ...
         parse menuItem.p_caption with auto caption \t .;
         key := getItemKey(caption, parentMenuName);
    ...
    */
   private bool findMenuItem(_str caption, _str parentIndexTable:[], STRARRAY subMenus:[], typeless &parentIndex, int &position, int findAfter = -1)
   {
      position = NOT_FOUND;
    ...
    */
   private bool removeItemFromMenu(MenuModification mod, _str (&parentIndexTable):[], STRARRAY (&subMenus):[])
   {
    ...
    */
   private bool insertMenuItem(MenuModification mod, _str (&parentIndexTable):[], STRARRAY (&subMenus):[], 
                                  bool strictPositioning = false, bool allowDupes = false)
   {
    ...
    *                               otherwise
    */
   private bool insertItemIntoMenu(MenuModification mod, int parentIndex, int position, 
                                      ...
                                      bool allowDupes = false)
   {
      ...
      if (mod.getSubMenu()) flags = MF_SUBMENU;
		...
		key := getItemKey(mod.getCaption(), mod.getParentCaption());
      if (!allowDupes) {
         len := subMenus:[parentIndex]._length();
         if (((position < len)  && (subMenus:[parentIndex][position] == key)) ||
             ((position > 0) && (subMenus:[parentIndex][position - 1] == key)) ||
             ((position + 1 < len) && (subMenus:[parentIndex][position + 1] == key))) {
