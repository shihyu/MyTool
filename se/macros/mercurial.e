...
static const HG_CHILD_DIR_NAME=      '.hg';
static const HG_CYGWIN_PREFIX=       '/cygdrive/';
...
static bool gHgCancel=false;
// flag indicating whether or not push and pull should be interactive
bool def_hg_pushpull_interactive = true;
// flag indicating whether or not all output should be written to the output window
bool def_hg_output_all = true;

static int max_debug_shell_output = 25;

static const PULL_REQUIRES_MERGE=  2;
static const PULL_NO_CHANGES=      3;
static const COMMIT_MERGE_PENDING= 4;
 ...
 */
_str _HgGetExePath() {
   return _GetCachedExePath(def_hg_exe_path,_hg_cached_exe_path,("hg":+EXTENSION_EXE));
}

#if 0 //2:00pm 4/15/2019
/**
 ...
 */
_command int hg_push(_str repositoryName='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   ...
   def_vc_system = "Mercurial";
   svc_push_to_repository();
   def_vc_system = orig_def_vc;
   return 0;
#if 0 //11:19am 1/27/2015
 ...
 * @return int - 0 for success
 */
_command int hg_pull(_str repository="") name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   ...
   orig_def_vc := def_vc_system;
   def_vc_system = "Mercurial";
   svc_pull_from_repository();
   def_vc_system = orig_def_vc;
   return 0;
#if 0 //11:21am 1/27/2015
...
_command int hg_clone(_str repository="") name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
_command int hg_set_repository() name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
static int _HgUpdate(_str path="")
{
   // first, find out if this file is addable in hg and get the repository root for this file
   repositoryRoot := "";
   isHgFile := IsHgPath(path, repositoryRoot);
   if (isHgFile == false) {
 ...
 * @return int - 0 for success
 */
_command int hg_diff_with_tip(_str cmdline='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
                             ...
                             _str TagName='',_str lang='')
{
   ...
   _DiffModal(ro_opt' -r2 -bi2 -nomapping -file1title "':+filenamenq:+' (Version 'local_version' - Local' modstr')" -file2title "'filenamenq' (Version 'remote_version' - Remote)" '_maybe_quote_filename(filename)' 'temp_view_id.p_buf_id,"hg");
...
{
   caption := "";
   status := _HgShell("cat ":+checkout_options' '_maybe_quote_filename(filename),"","q",auto output=null,true,auto outputTempWID=0);
   if ( status && !quiet ) {
...
_command int hg_history_diff_predecessor() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   ...
   predecessor_index := _TreeGetNextSiblingIndex(version_index);
...
static int _HgDiffTwoVersions(_str filename, _str version1, _str version2)
{
   ...
   repositoryRoot := "";
   isHgFile := IsHgPath(filename, repositoryRoot);
   if (isHgFile == false) {
   ...
   _str cmd = 'cat -r 'version1' -o '_maybe_quote_filename(version1ContentsFilename)' '_maybe_quote_filename(remote_filename);
   ...
   cmd = 'cat -r 'version2' -o '_maybe_quote_filename(version2ContentsFilename)' '_maybe_quote_filename(remote_filename);
   ...
   status = _DiffModal('-r1 -r2 -nomapping -file1title "Version ('version1')" -file2title "Version ('version2')" '_maybe_quote_filename(version1ContentsFilename)' '_maybe_quote_filename(version2ContentsFilename), 'Mercurial');
 ...
 * @return int - 0 for success
 */
_command int hg_commit(typeless filename='', _str comment=NULL_COMMENT) name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   ...
   orig_def_vc := def_vc_system;
   def_vc_system = "Mercurial";
   if ( comment==NULL_COMMENT ) comment=null;
...
_command int hg_update(typeless filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
static bool isRepositoryRootDir(_str path,_str &repositoryRoot)
{
   _str local_path;
   parse ctllocal_path_label.p_caption with 'Local Path: ','i' local_path;

   if ( _file_eq(path,local_path) ) {
      return true;
   }
...
int _hg_update_history_button()
{
   ...
   return( hg_history(filename,true) );
}
...
//   parse ctllocal_path_label.p_caption with 'Local Path: ','i' auto local_path;
   ...
   if ( comment_is_filename ) {
      comment_info_str="-l ":+_maybe_quote_filename(comment);
   }else{
...
static bool hgCommandLineTooLongError(_str cmd, _str cap)
{
   threshold := 0;
   if (_isMac()) {
      threshold = 0x18000;
 ...
 * @return int - 0 for success
 */
_command int hg_add(_str filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
static int _HgAdd(_str filelist[])
{
   ...
   // run the add command
   cmd :=  'add -Ilistfile:'fileListFilename;
   status = _HgShell(cmd, "", 'Q', auto output);
   popd();
   if (status) {
 ...
 * @return int - 0 for success
 */
_command int hg_remove(_str filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
static int _HgRemove(_str filelist[])
{
   ...
   repositoryRoot := "";
   isHgFile := IsHgPath(filelist[0], repositoryRoot);
   if (isHgFile == false) {
   ...
   for ( i=0;i<len;++i ) {
      ...
      cmd:+=_maybe_quote_filename(filelist[i])' ';
   }
 ...
 * @return int - 0 for success
 */
_command int hg_revert(_str filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
static int _HgRevert(_str filelist[])
{
   ...
   // warn the user about what they're about to do
   msg := "Are you sure you want to revert ";
   if (filelist._length() == 1) {
      msg :+= "'"filelist[0]"'?";
   } else {
      msg :+= 'these 'filelist._length()' files?';
...
_command int hg_history(_str filename='', bool quiet=false, _str version=null) name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
...
{
   // hide the tree
   ctltree1.p_visible=ctl_close.p_visible=false;
   ...
   ctl_close.p_y=ctltree1.p_y_extent+(xbuffer*2);
   ...
   // now show the tree
   ctltree1.p_visible=ctl_close.p_visible=true;
}
#endif
 ...
 * @return int - 0 for success
 */
_command int hg_update_directory,hg_gui_mfupdate(_str path='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return VSRC_FEATURE_REQUIRES_PRO_EDITION;
   ...
   repositoryRoot := "";
   isHgPath := IsHgPath(path, repositoryRoot);
   if (isHgPath == false) {
...
static void HgSetupTree(_str rootPath, HG_STATUS_INFO fileList[])
{
   ...
   for ( i=0;i<fileList._length();++i ) {
      curFilename := stranslate(fileList[i].local_filename, FILESEP, '/');
      int parent_bitmap_index=_pic_fldopen;
      _str end_char=_last_char(curFilename);
      typeless isdir=isdirectory(curFilename);

      curPath := rootPath:+_file_path(curFilename);
      index1 := _HgTreeGetPathIndex(newindex, curPath, rootPath, PathIndexes1, _pic_fldopen, _pic_cvs_fld_m);
...
static int _HgTreeGetPathIndex(int rootIndex,_str Path,_str BasePath,int (&PathTable):[], int ExistFolderIndex=_pic_fldopen, int NoExistFolderIndex=_pic_cvs_fld_m, _str OurFilesep=FILESEP, int state=1)
{
   ...
   for (;;) {
      ...
      if (_file_eq(Path:+OurFilesep,BasePath) || _file_eq(tPath,Path)) 
         break;
      if (isunc_root(Path)) 
         break;
      Path=tPath;
   ...
   for (i=0;i<PathsToAdd._length();++i) {
      int bmindex;
      fullPath :=  BasePath :+ PathsToAdd[i];
 ...
 */
static bool IsHgPath(_str path, _str& hgRootPath)
{
   hgRootPath = '';
   ...
   isRootFolder := (isdirectory(path) != 0);
   HgPath := "";
   HgPathExists := false;
 ...
 */
_str _HgGetUrlForLocalRepository(_str repositoryRoot, bool quiet = false)
{
   repositoryUrl := "";
...
static _str _ConvertCygwinPrefix(_str url) {
   _str retVal = url;
   prefixMatch := substr(url, 1, length(HG_CYGWIN_PREFIX));
...
static int _HgGetLogInfoForFile(_str filename, HG_LOG_INFO &logInfo)
{
   ...
   cmd :=  "log --style=xml "_maybe_quote_filename(filename);
   _str output[];
   logOutputTempWID := -1;
 ...
 */
static int _HgShell(_str command,_str path,_str shell_options,_str (&output)[], bool suppressOutput=false,int &outputTempWID=0,
                    bool debug=false)
{
   pid := 0;
   status := 0;
   ...
   commandWithoutOutput := _maybe_quote_filename(_HgGetExePath())' 'command;
   _str commandWithOutput = commandWithoutOutput' > '_maybe_quote_filename(outputFilename)' 2>&1'; 
...
static void _HgWriteOutputFile(_str output[], _str &outputFilename)
{
   ...
   // save the buffer
   tempWid._save_file(_maybe_quote_filename(outputFilename));
   // clean up after ourselves
   _delete_temp_view(tempWid);
   p_window_id = origWid; 
}
 ...
 */
static void SetHgCancel(bool newval)
{
   gHgCancel=newval;
}
...
static int _HgGetFileStatus(_str filename, HG_FILE_STATE &fileStatus)
{
   ...
   remote_filename := substr(filename, repositoryRoot._length() + 1);
   // translate and \ chars into /
   remote_filename = stranslate(remote_filename, '/', '\');

   // run the status command
   cmd :=  'status '_maybe_quote_filename(remote_filename);
   _str output[];
...
static HG_FILE_STATE _HgGetStatusFlagsFromLine(_str line,_str &filename="")
{
...
static int _HgGetVerboseFileInfo(_str repositoryRoot, HG_STATUS_INFO (&fileList)[])
{
   ...
   // run the status command
   cmd :=  'status '_maybe_quote_filename(repositoryRoot);
   _str output[];
   int status=_HgShell(cmd, repositoryRoot, 'Q', output);
   if (status) {
      return status;
...
static bool HgParseFileStatus(_str line, HG_FILE_STATUS &fileStatus)
{
   actionFound := false;
   temp := "";
   // does it contain an indicator that this line is a new file being added?
   tokenPos := pos('new file:', line, 1, 'I');
   if (tokenPos != 0) {
...
int _hg_history_view_button()
{
   ...
   _str output[];
   cmd :=  'show 'version':'_maybe_quote_filename(remote_filename);
...
void ctlok.on_create(_str repository="",_str clonePath="")
{
   ...
   rightAlign := p_active_form.p_width-ctlrepository.p_prev.p_x;
   sizeBrowseButtonToTextBox(ctlrepository.p_window_id, ctlbrowsedir1.p_window_id, 0, rightAlign);
   sizeBrowseButtonToTextBox(ctlclonePath.p_window_id,  ctlbrowsedir2.p_window_id, 0, rightAlign);
}
