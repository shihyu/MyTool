...
_command for_all(_str commandArg='', _str shell_wait='', _str echo_command='', typeless command_view_id='') name_info(','VSARG2_REQUIRES_FILEMAN_MODE|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   if (p_line==1 && echo_command) {
      ...
      _shell_wid.p_user=_enable_non_modal_forms(false,_shell_wid);
   }
   up();
   status := 0;
   select_cmd := "";
   ...
   if ( shell_wait ) {
      ...
      if (_iswindow_valid(_shell_wid)) {
         ...
         _set_focus();
         _enable_non_modal_forms(true,_shell_wid,_shell_wid.p_user);
...
{
   return(_maybe_quote_filename(substr(line,DIR_FILE_COL)));
}
#else
/**
...
{
  //return(_maybe_quote_filename(substr(line,DIR_FILE_COL)));
  filename := substr(line,DIR_FILE_COL);
  path := "";
