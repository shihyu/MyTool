...
static int display_node(int id, int node, int parentnode)
{
   ...
   case VSXMLCFG_NODE_ATTRIBUTE:
        ...
        nodename :+= "="_xmlcfg_get_value(id, node);
        _TreeAddItem(parentnode, nodename, TREE_ADD_AS_CHILD, _pic_xml_attr, _pic_xml_attr);
        return parentnode;

   case VSXMLCFG_NODE_PCDATA:
   case VSXMLCFG_NODE_ELEMENT_START:
   case VSXMLCFG_NODE_ELEMENT_START_END:
         nodeindx = _TreeAddItem(parentnode,_xmlcfg_get_name(id, node),TREE_ADD_AS_CHILD, _pic_xml_tag, _pic_xml_tag);
         return nodeindx;
   }
   return parentnode;
}
