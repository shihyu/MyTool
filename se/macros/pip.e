...
static const PIPDB_FILE=                  'pipDB.dbs';
static const PIP_LOG=                     'pip';

static const PIP_SEND_PERIOD=             7;
static const PIP_TRIAL_SEND_PERIOD=       1;
static const PIP_CLEAR_PERIOD=            4;
static const PIP_TIME_INTERVAL=           DT_DAY;

static const PIP_URL=                     'http://productimprovement.slickedit.com';
static const PIP_FAQS_URL=                'http://www.slickedit.com/index.php?option=com_content&view=article&id=254';

static const PIP_SEND_TIMEOUT=            30000;
static const PIP_SEND_TIMER_FACTOR=       10;
static const PIP_NO_SEND_RESULT_YET=      'Send not complete.';

static const PIP_SCHEMA_VERSION=          "1.5";
...
void _pip_startup()
{
   ...
   if (def_pip_on) {
      ...
      _pip_maybe_send(AlwaysUpdate:true);
   }
}
...
{
   oldFile := _ConfigPath() :+ 'pipDB' :+ TAG_FILE_EXT;
 ...
 */
bool _pip_db_needs_upgrade()
{
   // in 15.0.1, we added file extension logging
   // in 19.0.0, we changed the date format
   if (_version_compare(pip_db_version, '19.0.0') < 0) {
      return true;
   }
 ...
 */
static void _pip_maybe_reset_date(bool force = false)
{
 ...
 */
void _pip_maybe_send(bool AlwaysUpdate=false)
{
   // we want to make sure that the editor is really idle - sometimes we skip 
   // this step (on startup, for instance)
   if (!AlwaysUpdate && _idle_time_elapsed() < def_pip_send_delay) return;
...
static void _pip_send()
{
   ...
   status := _xmlcfg_save(xmlHandle, -1, VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE | VSXMLCFG_SAVE_PCDATA_INLINE);
   if (status) {
...
static void _pip_start_result_timer()
{
   ...
   // now start it
   pip_result_timer = _set_timer(PIP_SEND_TIMEOUT intdiv PIP_SEND_TIMER_FACTOR, _pip_check_result);
}
 ...
 *                         are finishing up
 */
static void _pip_alert(bool starting)
{
   if (starting) {
...
void pip_log_regex_search(_str options)
{
   ...
   if (pos('R', options)) {
      ...
      pip_regex_search_flags |= VSSEARCHFLAG_VIMRE;
   } else if (pos('&', options)) {
...
static void writeRegexUsage(int xmlHandle, int parentNode)
{
   ...
   typeNode = _xmlcfg_add(xmlHandle, regexNode, 'Vim', VSXMLCFG_NODE_ELEMENT_START_END, VSXMLCFG_ADD_AS_CHILD);
   _xmlcfg_add(xmlHandle, typeNode, (int)((pip_regex_search_flags & VSSEARCHFLAG_VIMRE) != 0), VSXMLCFG_NODE_PCDATA, VSXMLCFG_ADD_AS_CHILD);
...
static void writeNotificationSettings(int xmlHandle, int settingsNode)
{
   ...
   writeAlertGroupInfo(xmlHandle, notificationsNode, ALERT_GRP_WARNING_ALERTS);
 ...
 */
static void writeAlertGroupInfo(int xmlHandle, int notificationsNode, _str groupId)
{
   // get our alert info
...
static void writeTagFileInfo(int xmlHandle, int tagFilesNode, _str file)
{
   ...
   if (status >= 0) {
      ...
      tag_close_db(absolute(file), true);
   }
}
 ...
 *                               it is a tool window
 */
static void writeToolbarInfo(int xmlHandle, int toolbarsNode, _TOOLBAR &tb, bool buttonBar)
{
   tbNode := 0;
   ...
   if (!buttonBar) {
      autohide := tw_is_auto_form(tb.FormName);
...
void _cb_participate.on_create()
{
   ...
   if (isEclipsePlugin()) {
      ctl_slickedit_pic.p_picture = _find_or_add_picture("vse_eclipse.ico@64");
   } else if (_isStandardEdition()) {
      ctl_slickedit_pic.p_picture = _find_or_add_picture("vse_standard.ico@64");
   } else if (_isCommunityEdition()) {
      ctl_slickedit_pic.p_picture = _find_or_add_picture("vse_community.ico@64");
   } else {
      ctl_slickedit_pic.p_picture = _find_or_add_picture("vse_profile.ico@64");
   }
}
