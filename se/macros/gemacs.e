 ...
 * 
 * @param key     key to check (gotten with get_event or pgetkey()
 * @param index   nametable index of command bound to this key returned 
 */
...
/**
 * Deletes all spaces (if any), and puts one in their place.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void just_one_space() name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL|VSARG2_CMDLINE)
...
/**
 * Like the EMACS delete_space command, except the column isn't preserved, 
 * and it deletes all of the spaces, not just from the cursor to the next 
 * non-blank char.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void gnu_delete_space() name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
//A-\
{
   init_command_op();
   typeless p;
...
/**
 * Marks from the current position to the beginning of the buffer (char)
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Marks from the current position to the end of the buffer (char)
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Character selects the entire buffer, and leaves you at the top of it.
 *
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
 ...
 * @categories Emacs_Commands
 */
_command void gnu_kill_rectangle() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
//C-x r k
{
   if (!select_active()) {
 ...
 * @categories Emacs_Commands
 */
_command void gnu_yank_rectangle() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
//C-x r y
{
      paste("gr");
}
...
/**
 * Performs a modal tab on selected text.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void gnu_indent_selection() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
//C-x r o
{
   if (!select_active()) {
      message('There must be an active selection');
...
/**
 * Indents the current pos to the gnu_comment_column column, or one past the
 * last position on the current line. 
 *  
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void c_indent_for_comment() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
//A-;  (c-mode)
{
   end_line();
...
/**
 * Sets the comment column (used in c_indent_for_comment) to the current column.
 * 
 * @param col 
 *  
 * @note This can't be bound to "C-x ;" in c-mode due to keytable problems.
 *  
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Moves the close brace to the end of the statement, and replaces it with the cursor
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void move_past_close_and_reindent() name_info(','VSARG2_MULTI_CURSOR|VSARG2_EDITORCTL)
//A-' (c-mode)
{
   //find('{','+NIXCS-');
...
/**
 * Counts the number of lines and chars in a selection
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/** 
 * @return 
 * Returns the OCT, DEC, HEX and seek positions of the current text 
 * as a stick message
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
...
//C-x =
{
   ...
   ch   := get_text(1);
   _dec := _UTF8Asc(ch);
   _hex := "0x":+_dec2hex(_dec,16);
   _oct := "0":+_dec2hex(_dec,8);
   ...
   restore_pos(p);
   msg := nls('char: %s (%s, %s, %s) point= %s of %s (%s%) column %s',
...
/**
 * Executes the last command that was run from the command line.
...
/**
 * Gnu emacs emulation command to jump to a tag.
 * 
 * @param proc_name     name of symbol to locate
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command gnu_goto_tag(_str proc_name="") name_info(TAG_ARG','VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
//C-x .
{
...
int _OnUpdate_gnu_goto_tag(CMDUI &cmdui,int target_wid,_str command)
{
   return _OnUpdate_push_tag(cmdui, target_wid, "find-tag");
}

/**
 * does a find-tag if user has def_one_file set, it will
 * tile all windows horizontally,
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command find_other_tag() name_info(TAG_ARG','VSARG2_NCW|VSARG2_ICON|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_EDITION)
//C-x 4 .
{
   if (!_haveContextTagging()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Context Tagging"VSREGISTEREDTM);
   ...
   if (_mdi.p_child.p_window_id==orig_wid) {
      ...
      if (_mdi.p_child.p_window_state=='M') {
         ...
         edit(' +w +b '_maybe_quote_filename(buf_name));
      }
      center_line();
      next_window();
      center_line();
...
int _OnUpdate_find_other_tag(CMDUI &cmdui,int target_wid,_str command)
{
   return _OnUpdate_push_tag(cmdui, target_wid, "find-tag");
}

/**
 * Creates a new buffer in the "other" window.
...
//C-x 4 b
{
   message_string := "Switch to buffer in other window: (default to buffer-list): ";
   ...
   if (_no_child_windows()) {
      ...
      status=edit(' +b '_maybe_quote_filename(buf_name));
      return(status);
   }
   ...
   if (strip(buf_name)=='') {
      ...
      if (old_wid==new_wid) {
         ...
         if (old_wid==new_wid) {
            ...
            edit(' +b '_maybe_quote_filename(old_buf_name));
            hsplit_window();
            edit(' +b '_maybe_quote_filename(new_buf_name));
         }
         return(status);
      }
      //_next_window();
      ...
      status=edit('+b '_maybe_quote_filename(absolute(buf_name)));
      if (status==FILE_NOT_FOUND_RC) {
         //The buffer wasn't found.  So I'll have to open a new one.
         clear_message();
         edit(' +t '_maybe_quote_filename(absolute(buf_name)));
      }
      return(status);
   }
}
...
/**
 * Creates a fileman-dir in the "other" window if "One file per window" is on. 
 * Otherwise, it just does a "dir".
 * 
 * @param directory     directory name to list
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Does a "dir" on the current directory.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * same as edit, but if def_one_file is set, it will tile the windows.
 * 
 * @param filename     same as edit
 *  
...
//C-x 4 r
{
   typeless status=0;
   if (filename:=='') {
      message_string := "Edit read only (Other Window): ";
 ...
 * @note This works only if there is a "next window".
...
/**
 * Goes to the bottom of the buffer in the next_window
 * @note This works only if there is a "next window"
 * @return 0 if successful, otherwise 1. 
 *  
 * @appliesTo  Edit_Window, Editor_Control
...
/**
 * This centers the current line (or selection) within the margins.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void center_paragraph(_str alt_margins="") name_info(','VSARG2_MULTI_CURSOR|VSARG2_EDITORCTL|VSARG2_MARK)
//A-S-s
{
   if (!select_active()) {
      ...
      begin_select();
      while (p_line<=line) {
         center_within_margins(alt_margins);
         down();
      }
      restore_pos(p);
   }
...
static int shell_it(_str cmd)
{
   ...
   if (status:!='') {
      edit('+b '_maybe_quote_filename(temp_out));
      p_modify=false;
      quit();
   }
   if ( temp_out=='' ) {
   ...
   if ( file_match(temp_out,1)!=temp_out ) {
      ...
      status=edit('-w '_maybe_quote_filename(temp_out));
      if (status) {
...
/**
 * Centers the current line to the current window.
 * If argument is a number, it will put the cursor on that relative line.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Selects (character) from the current point to the next word.
...
/**
 * Prompts user to delete to certain character.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Puts a space and an open paren into the file
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void space_paren() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_EDITORCTL)
//A-(  (c-mode)
{
   if (get_text(1):==' ') {
      keyin('()');
 ...
 * @categories Emacs_Commands
 */
_command int select_next_word() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_EDITORCTL|VSARG2_MARK)
//A-@
{
   typeless status=0;
   if (select_active() && upcase(_select_type()):=='CHAR' && pos('C',def_select_style)) {
      ...
      if (select_active()) {
         ...
         _deselect();
         search_string := '([~'p_word_chars']|^)['p_word_chars']';
...
/**
 * Cycles through the clipboard to paste into the current file
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void enum_paste() name_info(','VSARG2_MULTI_CURSOR|VSARG2_NCW|VSARG2_ICON|VSARG2_READ_ONLY|VSARG2_MARK)
//A-y
{
   ...
   if (_cbpaste > _Nofnulls) {     // wrap around
      _cbpaste=1;
   }
...
/**
 * Shells a command, and outputs stdout to a buffer.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command comint_run()
{
   ...
   typeless status=0;
   index := find_index(line,COMMAND_TYPE);
   if (index) {
...
/**
 * This shells a command on the selected text.  If CHAR
 * or BLOCK are the current selection, it changes them to LINE. 
 *  
 * @note Very much like the {@link vi_filter command}
 *  
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
...
/**
 * Re-indent selected text.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void gnu_c_rendent_selection() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
//C-A-q  (c-mode)
{
   typeless p;
   _save_pos2(p);
...
/**
 * Rigidly re-indent selected text.
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command void gnu_c_rendent_rigidly() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_AB_SELECTION)
{
   typeless p;
   _save_pos2(p);
   end_select(); 
...
/**
 * Indents from the cursor but not if prev line
 * has no indentation.  Used as the indent-line-function
 * 
 * @appliesTo  Edit_Window, Editor_Control
 * @categories Emacs_Commands
 */
_command int gnu_html_tab() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
//Tab  (html)
{
...
//l (fileman-mode)
{
   ...
   if (p_LangId=='fileman') {
      ...
      execute('quit');
      if (word1=='list') {
         execute('list '_maybe_quote_filename(rest));
      } else {
         execute('dir '_maybe_quote_filename(rest));
      }
      if (!_no_child_windows()) {
...
//! (fileman mode)
{
   ...
   command := strip(prompt('','Enter command to shell on file 'filename));
   for (;;) {
      if (command=='') {
