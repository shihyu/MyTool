...
static const CUR_SET_INDEX_INDEX=   0;
static const PAST_SET_INDEX_INDEX=  1;
static const LAST_FILE_INDEX_INDEX= 2;
static const MODIFIED_FILES_INDEX=  3;
static const MISC_TABLE_INDEX=      4;
...
static void LoadCSTreeCallback(int xml_handle,int xml_index,int tree_index)
{
   ...
   for ( i=0;i<len;++i ) {
      ...
      jlen := xml_comment_line_indexes._length();
      int j;
      for ( j=0;j<jlen;++j ) {
...
int _OnUpdate_cvs_add_to_current_commit_set(CMDUI &cmdui,int target_wid,_str command)
{
   return _OnUpdate_commit_set_add(cmdui, target_wid, command);
}
 ...
 */
_command int commit_set_add,cvs_add_to_current_commit_set(_str filename='') name_info(FILE_ARG'*,'VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
   ...
   if ( ncw && (filename=='' || beginsWith(filename,'.process')) ) {
   ...
   status=_xmlcfg_save(xml_handle,-1,VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE);
 ...
 */
_command void commit_sets,cvs_commit_sets() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveVersionControl()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Version control");
      return;
...
int _OnUpdate_current_sets(CMDUI &cmdui,int target_wid,_str command)
{
   return _OnUpdate_commit_set_add(cmdui, target_wid, command);
}
int _OnUpdate_cvs_commit_sets(CMDUI &cmdui,int target_wid,_str command)
{
   return _OnUpdate_commit_set_add(cmdui, target_wid, command);
}
...
{
   cindex := _TreeGetFirstChildIndex(parent_index);
   for ( ;cindex>0; ) {
      cur_name := _TreeGetCaption(cindex);
      int state,bm1,bm2;
...
{
   cap := stranslate(ctltree1._TreeGetCaption(tree_parent_index),'',' ');
...
/**
 * @return Returns true if there is no current (default) commit set in the tree
 */
static bool NoCurrentCommitSetInTree()
{
   index := _TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   found_bold := false;
   for (;;) {
...
void ctledit.lbutton_up()
{
   ...
   changed_commit_set := EditCommitSet(commit_set_name);
 ...
 */
static bool EditCommitSet(_str commit_set_name)
{
   CVS_COMMIT_SET cur_commit_set=GetDialogInfoHash(commit_set_name);
   _param1=null;
   changed := false;
...
int ctlreview.lbutton_up()
{
   ...
   index := _SVCGetProcIndex('commit_set_review_button');
   if ( index>0 ) {
...
static const COMMITSET_GENERAL_FAILURE= 1;
static const COMMITSET_EDIT_FAILURE=    2;
static const COMMITSET_NO_FILES_MODIFIED= 3;

int ctlcommit.lbutton_up()
{
   int status=COMMAND_NOT_FOUND_RC;
   index := _SVCGetProcIndex('commit_set_commit_button');
   if ( index>0 ) {
      status=call_index(index);
   }
   return(status);
 ...
 * 
 * @return bool 'false' if <B>newName</B> is unique
 */
static bool nameIsUnique(_str newName,int treeWID)
{
                                ...
                                int commit_set_tree_index)
{
   ...
   old_caption := _TreeGetCaption(commit_set_tree_index);
   int state,bm1,bm2;
...
static int CommitCommitSet(CVS_COMMIT_SET &cur_commit_set)
{
   ...
   index := _SVCGetProcIndex('maybe_warn_multifile_comments');
   if ( index>0 ) {
   ...
   if ( file_list._length() ) {
      ...
      list := "";
      len := file_list._length();
      for ( i=0;i<len;++i ) {
         list :+= ' '_maybe_quote_filename(file_list[i]);
      }
 ...
 */
static bool RemoveFilenameFromArray(_str filename,_str (&file_array)[])
{
   int len=file_array._length(),i;
   for ( i=0;i<len;++i ) {
      if ( _file_eq(filename,file_array[i]) ) {
 ...
 */
static void InsertArrayIntoCurView(_str (&line_array)[],bool clear_first=false)
{
   if ( clear_first ) {
      delete_all();
   }
...
static int SVNCommitSetFailsChecks(_str CommitSetName,SVN_STATUS_INFO (&svn_file_info)[],CVS_COMMIT_SET &cur_commit_set)
{
   ...
   for ( i=0;i<len;++i ) {
      ...
      bool conflict_marker;
      if ( _last_char(pcur->local_filename)!=FILESEP ) {
 ...
 */
static int FileHasConflictMarkers(_str filename,bool &conflict_marker)
{
...
void _cvs_current_commit_set_form.on_resize()
{
   ...
   ctlcomment_all.p_x=ctltree1.p_x_extent+xbuffer;
   ...
   ctltree1.p_y=first_label_wid.p_y_extent+ybuffer;
   ...
   comment_file_label_wid.p_y=ctlcomment_all.p_y_extent+ybuffer;

   ctlcomment_file.p_y=comment_file_label_wid.p_y_extent+ybuffer;
   ctltag.p_y=ctlcomment_file.p_y_extent+ybuffer;
...
void ctlok.on_create(CVS_COMMIT_SET &info)
{
   ...
   for ( i=0;i<len;++i ) {
      ...
      cur_filename=_maybe_quote_filename(cur_filename);
   ...
   ctlcomment_all.p_SoftWrap=true;
   ctlcomment_all.p_SoftWrapOnWord=true;
   ctlcomment_file.p_SoftWrap=true;
   ctlcomment_file.p_SoftWrapOnWord=true;
   p_window_id=wid;
}
...
static int GetPathsToAdd(_str filelist,_str (&dirlist_to_add)[])
{
   for ( ;; ) {
      ...
      bool path_is_root;
      if (_isUnix()) {
         path_is_root=_file_eq(path,FILESEP);
      } else {
...
void ctldiff.lbutton_up()
{
   ...
   filename := ctltree1._TreeGetCaption(index);
 ...
 * @return bool
 */
bool _CVSTagCheckFails(_str taglist,int text_box_wid=0)
{
...
void ctlok.lbutton_up()
{
   ...
   index := _TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   for ( ;index>-1; ) {
      cur_filename := _TreeGetCaption(index);
                                  ...
                                  typeless *pData=null)
{
   ...
   for ( i=0;i<len;++i ) {
      if ( lastdir=='' ) {
         ...
         list=_maybe_quote_filename(_SVCRelative(filelist[i],lastdir));
      } else {
         _str curdir=_file_path(filelist[i]);
         cur := _maybe_quote_filename(_SVCRelative(filelist[i],lastdir));
         curlen := length(cur);
         if ( length(list)+curlen>=MAX_COMMAND_LINE_LENGTH ||
              !_file_eq(lastdir,substr( curdir,1,length(lastdir))) ) {
            --i;
            module_name := "";
                             ...
                             _str CommitSetName,_str default_filename)
{
   ...
   for ( i=0;i<len;++i ) {
      ...
      newindex := -1;
      set_default := 0;
      if ( default_filename!='' && set_default>-1 && _file_eq(default_filename,files[i] ) ) {
         set_default=1;
      }
      int new_index;
      if ( index!=null ) {
...
void ctldiff.lbutton_up()
{
   ...
   svc_diff_with_tip(_maybe_quote_filename(filename));
...
{
   int status=COMMAND_NOT_FOUND_RC;
   index := _SVCGetProcIndex('commit_set_update_button');
   if ( index>0 ) {
      status=call_index(index);
   }
   return(status);
...
int _cvs_commit_set_update_button()
{
   ...
   output_file := "";
   status := 0;
   if ( button_caption==SVC_UPDATE_CAPTION_UPDATE ) {
      status=_SVCUpdate(temp,output_file);
   } else if ( button_caption==SVC_UPDATE_CAPTION_ADD ) {
      status=_SVCAdd(temp,output_file);
   }
