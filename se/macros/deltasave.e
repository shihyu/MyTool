...
_deltasave_versions_dialog.on_resize()
{
   ...
   int button_buffer_x=ctldiff.p_x-(ctlclose.p_x_extent);
   ...
   int button_y=ctltree1.p_y_extent+tree_buffer_y;
...
static const VERSIONS_DIALOG_INFO_INDEXES= 0;
...
int ctldiff.lbutton_up()
{
   ...
   diff('-modal -r2 -viewid2 -file2title "'filename' (Version 'curVersion')" '_maybe_quote_filename(filename)' 'curVersionViewId);
...
//
//   if ( !status ) {
//      status=_menu_set_state(output_handle,item_pos,MF_ENABLED,'P','&Backup history for 'p_buf_name'...',"deltasave-list-versions "_maybe_quote_filename(p_buf_name),'deltasavecreate');
//   }
