 ...
 */
_command void cics_enter() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
...
bool _cics_supports_syntax_indent(bool return_true_if_uses_syntax_indent_property=true) {
   return true;
}
/**
 * Handler for SPACE key in CICS mode
 */
_command void cics_space() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL|VSARG2_LASTKEY)
{
 ...
 */
int _cics_MaybeBuildTagFile(int &tfindex, bool withRefs=false, bool useThread=false, bool forceRebuild=false)
{
   return ext_MaybeBuildTagFile(tfindex,'cics','cics',"CICS Libraries", "", false, withRefs, useThread, forceRebuild);
}

// search for CICS command matches
static int cics_is_function_name(_str name, typeless tag_files, bool exact_match)
{
   i := 0;
...
{
   cics_offset := cics_find_command_start();
 ...
 * @since 11.0
 */
int _cics_get_expression_info(bool PossibleOperator, VS_TAG_IDEXP_INFO &idexp_info,
                              VS_TAG_RETURN_TYPE (&visited):[]=null, int depth=0)
{
   ...
   start_seekpos := cics_find_command_start();
...
bool _cics_autocomplete_after_replace(AUTO_COMPLETE_INFO &word,
                            ...
                            VS_TAG_RETURN_TYPE &prefix_rt=null)
{
   ...
   // argument to parameter, list data members in current context
      ...
      context_id := tag_get_current_context(auto cur_tag_name,auto cur_tag_flags,
                                            ...
                                            visited, depth+1);
      tag_files = tag_find_context_tags_filenamea('cob', context_flags);
      //SETagFilterFlags filter_flags=SE_TAG_FILTER_ANY_DATA;
...
{
   if (_chdebug) {
      isay(depth, "_cics_fcthelp_get_start");
   }
   errorArgs._makeempty();
   flags=0;
   ch := "";
...
//   typeless otherinfo;
   for (;;--stack_top) {
      ...
      status=_cics_get_expression_info(true,idexp_info,visited,depth+1);
      ...
      if (!status) {
         ...
         match_class := "";
         match_tag := "";
         int  match_flags = SE_TAG_FILTER_PROTOTYPE;
         ...
         tag_list_context_globals(0,0,idexp_info.prefixexp,false,tag_files,
                                  SE_TAG_FILTER_PROTOTYPE,SE_TAG_CONTEXT_ONLY_FUNCS,
                                  num_matches,def_tag_max_function_help_protos,
                                  false,false,visited,depth+1);
         ...
         for (i=0; i<num_unique; i++) {
            ...
            taginfo := tag_compose_tag_browse_info(cm);
            match_list[match_list._length()] = cm.member_name "\t" cm.arguments "\t" cm.return_type "\t" cm.file_name "\t" cm.line_no "\t" taginfo;
         }
         ...
         if (match_list._length()>0) {
            ...
            for (i=0; i<match_list._length(); i++) {
               ...
               parse match_list[i] with auto match_tag_name "\t" signature "\t" return_type "\t" auto match_file_name "\t" auto match_line_no "\t" auto match_taginfo;
               parse match_tag_name with . '<' auto tag_trailer;
               ...
               base_length := length(match_tag_name:+tag_trailer);
               ...
               while (argument != '') {
                  ...
                  parse argument with auto arg_name '(' .;
 ...
 */
static void cics_fix_tagname(_str &tag_name, _str &arguments, bool remove_brackets=false)
{
   ...
   word_chars := _clex_identifier_chars();
   p := pos('['word_chars']#[ \t]*$',before_lt,1,'r');
   if (p > 1) {
      first_word := "";
      last_word := strip(substr(before_lt,p));
 ...
 */
int cics_proc_search(_str &proc_name, int find_first)
{
   proc_name="";
