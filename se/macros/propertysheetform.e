...
static const BOOLEAN_PROPERTY_USES_SWITCH= true;

static const PROPERTY_SHEET=           'propertySheet';
static const PROPERTY_DEPENDENCIES=    'propertyDependencies';
static const HELP_HANDLE=              'helpHandle';

// default space between border of form and controls
const DEFAULT_DIALOG_BORDER= 120;

static const PS_PROMPT=       OP_QUICK_START + 1;

static const NEUTRAL_PROPERTY_VALUE=      'Languages Differ';

 // size of the modified property sheet column (column 0)
static const MODIFIED_OPTION_COLUMN_SIZE=    600;

double optColPerc = 0.55;           // percentage of remainding width that goes into option column (column 1)
bool resizing = false;           // whether we are currently resizing the property sheet
 ...
 */
int showPropertySheetOptions(_str xmlInfoOrPSName, bool setValues)
{
...
bool _property_sheet_form_is_modified()
{
   PropertySheet * ps = _property_sheet_form_get_property_sheet_data();
...
bool _property_sheet_form_apply(int &changeEventsTriggered, _str (&appliedProperties)[])
{
   PropertySheet * ps = _property_sheet_form_get_property_sheet_data();
   psName := getPropertySheetName();
   applySuccess := true;
   ...
   for (i := 0; i < ps -> getNumItems(); i++) {
      ...
      if (p != null && isPropertyModified(*p, i)) {
         ...
         if (propApplySuccess) {
            ...
            // Can't just translate all *. This deletes * from the entire caption!
   ...
   for (i = 0; i < ps -> getNumItems(); i++) {
      ...
      if (p != null) {
         ...
         if (p -> getPropertyType() == COLOR_PROPERTY) {
            ...
            if (color != null) {
               _ctl_property_sheet._TreeSetColor(treeIndex, 2, 0, _hex2dec(color), F_INHERIT_FG_COLOR);
            }
         }
      }
   }
...
void _property_sheet_form_reload_conditional_selects()
{
   PropertySheet * ps = _property_sheet_form_get_property_sheet_data();

   // go through the properties and find the Selects
   for (i := 0; i < ps -> getNumItems(); i++) {
      ...
      if (p != null) {
         ...
         // is this a select?
         // does it have choices that are only available sometimes?
         if (p -> getPropertyType() == SELECT_PROPERTY && ((Select)*p).hasConditionalChoices()) {
            _str choices[];
            ((Select)*p).getChoices(choices);
            _ctl_property_sheet._TreeSetComboDataNodeCol(i + 1, 2, choices);
         }
      }
   }
}
...
/**
 * Figure out a color for the Property Group rows that doesn't
 * clash too terribly with the existing color scheme.
 *
 * @return int
 */
int getPropertyGroupBackground()
{
   // get the base color of the first row
   typeless bg;
   _ctl_property_sheet._TreeGetColor(1, 1, auto fg, bg, auto ff);

   // get the hex value so we can split it up into r, g, b
   ...
   // turn it to decimal to manipulate it
   rDec := _hex2dec('0x'r);
   gDec := _hex2dec('0x'g);
   bDec := _hex2dec('0x'b);

   // if this is a light color, make it darker
   if (rDec + gDec + bDec < (175 * 3)) {
      rDec += 60;
      gDec += 60;
      bDec += 60;
   } else {
   ...
   // now put it all together and turn it into a decimal value
   bg = '0x'(r :+ g :+ b);
   bg = _hex2dec(bg);
   ...
   for (i := 0; i < psPtr -> getNumItems(); i++) {
      ...
      if (type == TMT_GROUP) {
         ...
         if (pg != null) {
            ...
            // if we haven't retrieved the property group color for this sheet, get it now
            if (pgColor == 0) pgColor = getPropertyGroupBackground();

            // change the background color to make it stand out
            _ctl_property_sheet._TreeSetRowColor(rowID, 0, pgColor, F_INHERIT_FG_COLOR);
...
void loadPropertySheetItems(PropertySheetItem (&ps)[], bool enabled = true)
{
   _ctl_property_sheet._TreeBeginUpdate(TREE_ROOT_INDEX);
   flags :=  enabled ? 0 : TREENODE_DISABLED;

   // now let's get the data in there!
   for (i := 0; i < ps._length(); i++) {
      caption := ''\tps[i].Caption\tps[i].Value;
      _ctl_property_sheet._TreeAddItem(TREE_ROOT_INDEX, caption, TREE_ADD_AS_CHILD, 0, 0, TREE_NODE_LEAF, flags, 0);
   }
 ...
 */
bool addPropertyToSheet(Property &p, int index, _str (&values):[], bool useSwitch = BOOLEAN_PROPERTY_USES_SWITCH)
{
   ...
   case COLOR_PROPERTY:
      _ctl_property_sheet._TreeSetColor(rowID, 2, 0, _hex2dec(p.getActualValue()), F_INHERIT_FG_COLOR);
 ...
 *                         otherwise
 */
static bool isPropertyModified(Property &p, int propIndex)
{
   if (p.getLanguage() == ALL_LANGUAGES_ID) {
      psName := getPropertySheetName();
...
static bool setLanguagePropertyValue(Property &p, _str psName, int propIndex, bool mustSet = true)
{
   didSet := false;
...
static _str getPropertyValueForAllLanguages(_str psName, _str propName, SettingInfo &info, _str exclusions = '')
{
   ...
   if (!_haveBuild()) exclusions :+= ',process,';
 ...
 *                         ONE language
 */
static bool setPropertyValueForAllLanguages(SettingInfo &info, _str value, _str exclusions = '')
{
   SettingInfo infoCopy = info;
   success := false;

   infoCopy.Language = ALL_LANGUAGES_ID;
   PropertyGetterSetter.setSetting(infoCopy, value);
   if (!_haveBuild()) exclusions :+= ',process,';
 ...
 */
static bool isPropertySheetEmbeddedInOptions()
{
...
void _property_sheet_form.on_create(int purpose = OP_CONFIG) 
{
   ...
   case OP_IMPORT:
      ...
      protect = _GetDialogInfoHt(se.options.OptionsTree.PROTECT, form);
      if (protect) {
   ...
   _ctl_property_sheet.p_LevelIndent = 0;
...
void _property_sheet_form.on_resize()
{
   ...
   if (isPropertySheetEmbeddedInOptions()) {
      ...
      heightDiff := height - (_ctl_ok.p_y_extent);
      if (heightDiff) {
...
void size_property_sheet_columns()
{
   ...
   if (_ctl_property_sheet._TreeGetNumColButtons() == 3) {
      ...
      // the other two should be 55/45 of the remaining space
...
static void changePropertyValue(int treeIndex, _str caption = '')
{
   ...
   if ((allow_blank_value || value != '' || caption == Path.DEFAULT_PATH) && value != curValue) {
      p -> updateValue(value);
      maybe_fire_change_callback(p);
...
static void changePropertyInTree(PropertySheet * ps, int psIndex)
{
   ...
   _ctl_property_sheet._TreeSetCaption(treeIndex, newCaption);
   if (p -> getPropertyType() == COLOR_PROPERTY) {
      _ctl_property_sheet._TreeSetColor(treeIndex, 2, 0, _hex2dec(p -> getActualValue()), F_INHERIT_FG_COLOR);
...
static void protectProperty(int treeIndex, _str protectColor, int protectIcon)
{
   ...
   _ctl_property_sheet._TreeSetColor(treeIndex, 1, 0, _hex2dec(protectColor), F_INHERIT_FG_COLOR);
   _ctl_property_sheet._TreeSetColor(treeIndex, 2, 0, _hex2dec(protectColor), F_INHERIT_FG_COLOR);
}
   ...
   case CHANGE_EDIT_CLOSE:
      if (index > 0) {
         ...
         if (purpose == OP_CONFIG || purpose == OP_QUICK_START || purpose == PS_PROMPT) {
            ...
            se.options.OptionsCheckboxTree * cbTree =_GetDialogInfoHt(sc.controls.CheckboxTree.CHECKBOXTREE, p_window_id, true);
            if ( cbTree != null ) {
...
static bool getColor(Property * p, _str &value)
{
   curValue := p -> getActualValue();
   color := _hex2dec(curValue);
   typeless new_color = show("-modal _color_picker_form", color);
   if (new_color == '' || new_color == color) {
      value = '';
   } else {
      value = "0x":+_dec2hex(new_color);
   }

   return (value != '');
}

static bool getPath(Property * p, _str &value)
{
   Path pathP = (Path)*p;
   if (pathP.allowMultiplePaths()) {
      value = show('-modal _edit_paths_form', pathP.getCaption(), pathP.getActualValue(), pathP.getDelimiter(), (pathP.getPathType() == Path.DIRECTORYPATH));
   } else {
 ...
 */
static bool isPropertyEnabled(int index)
{
