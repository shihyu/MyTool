...
*/
int _BufEdit(_str filename,_str load_options="",bool IgnoreNotFound=true,_str load_options2="",bool quiet=false)
{
   orig_view_id := 0;
   ...
   bool buffer_already_exists=(lowcase(load_options)!='+d' &&
...
static int _ini_set_value2(_str section_name,_str field_name,_str new_value,_str case_option)
{
   ...
   _ini_set_value3(field_name,new_value,case_option);
   status=_save_config_file();
   //messageNwait('h2 status='status' 'p_buf_name);
   return(status);
}

static int _ini_set_value3(_str field_name,_str new_value,_str case_option)
{
   _end_line();
   save_search(auto a,auto b,auto c,auto d);
...
static int _ini_find_item(_str section_name,_str item_name)
{
   // If section_name or item_name not found, arg(5) is returned
   // actual value in file not changed.
   if (_ini_find_section(section_name)) {
      return(STRING_NOT_FOUND_RC);  // String not found in section
   }
   _end_line();
   save_search(auto a,auto b,auto c,auto d);
   int status=search('^'_escape_re_chars(item_name)'$', 'rih@');
   restore_search(a,b,c,d);
   if (status) {
      return(STRING_NOT_FOUND_RC);
   }
...
int _ini_delete_item(_str filename,_str section_name,_str item_name)
{
   ini_view_id := 0;
   view_id := 0;
   int status=_open_temp_view(filename,ini_view_id,view_id);
   ...
   status=_ini_find_item(section_name,item_name);
   if (!status) {
      _delete_line();
      status=_save_config_file();
   }
...
/*
   ...
   section header line [...]
*/
void _ini_delete_section3(bool DeleteHeader)
{
   typeless status=0;
   typeless mark_id=_alloc_selection();
   if (DeleteHeader) {
...
static int _ini_get_value2(_str section_name, _str  field_name, _str &result,
                           _str DefaultValue='', bool DoFindSection=true,
...
int _ini_find_section(_str section_name)
{
   ...
   /* Searches for [section_name] at the beginning of a line in the current buffer */
   ...
   save_search(auto a,auto b,auto c,auto d);
   status := search(search_string,'rih@');
   restore_search(a,b,c,d);
   return(status);
}
...
bool _isfile_loaded(_str filename)
{
...
bool _ini_is_valid(_str filename)
{
   int orig_view_id;
   int temp_view_id;
...
/**
 * Retrieves the same value for each section and stores in a hashtable.
 *
 * @param filename      ini file
 * @param item          item name to find for each section
 * @param svt           table of section name to item value
 *
 * @return              0 if success, error code otherwise
 */
int _ini_get_section_value_table(_str filename, _str field, _str (&svt):[])
{
   ...
   while (!status) {
      ...
      if (name != '') {
         ...
         // now get the value
         _ini_get_value2(name, field, value, '', false);
         if (value != '') {
            svt:[name] = value;
         }
      }
...
/**
 * Sets the same field for each specified section.
 *
 * @param filename      ini file
 * @param field         field to set
 * @param svt           table of section names to values
 *
 * @return int
 */
int _ini_set_section_value_table(_str filename, _str field, _str (&svt):[])
{
   if (!svt._length()) return 0;
   ...
   // go through our list
   _str section, value;
   foreach (section => value in svt) {

      // see if we even have this section - if not, create it!
      status = _ini_find_section(section);
      if (!status) {
         status = _ini_set_value3(field, value, null);
      }
   }
