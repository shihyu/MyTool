...
static const PERL5DB_VERSION= "0.40";
...
static _str getPerl5dbDir()
{
   ...
   perl5db_dir :=  tools_dir:+FILESEP:+'perl5db-'PERL5DB_VERSION;
...
static void setProjectOptionsForConfig(int projectHandle, _str config, PerlOptions& perl_opts, DBGpOptions& perl5db_opts)
{
   //
   // Execute
   //
   ...
   int target_node = _ProjectGet_TargetNode(projectHandle,"Execute",config);
   cmdline := '"%(SLICKEDIT_PERL_EXE)"';
   if( perl_opts.interpreterArgs != "" ) {
      cmdline :+= ' 'perl_opts.interpreterArgs;
   }
   cmdline :+= ' %(SLICKEDIT_PERL_EXECUTE_ARGS)';
   if( perl_opts.defaultFile != "" ) {
      cmdline :+= " "_maybe_quote_filename(perl_opts.defaultFile)" %~other";
   } else {
      cmdline :+= ' "%f" %~other';
   }
   ...
   // Remote Mappings
   DBGpRemoteFileMapping remote_file_map;
...
static void setProjectOptions(int projectHandle, _str (&configList)[], PerlOptions (&perl_opts_list):[], DBGpOptions (&perl5db_opts_list):[])
{
   foreach( auto config in configList ) {
                            ...
                            bool quiet=false, _str& error_hint=null)
{
   ...
   isPerlProject := ( strieq("perl",_ProjectGet_Type(projectHandle,config)) );
   ...
   perlDir := _strip_filename(perlExePath,'N');
 ...
 * @return 0 on success, <0 on error.
 */
_command int set_perl_environment() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
                                 ...
                                 _str debugWorkingDir="")
{
   ...
   do {
      ...
      // DBGp features
      debugger_args :+= ' 'se.debug.dbgp.dbgp_make_debugger_args(perl5db_opts.dbgp_features);
   
      // Attempt to start debug session
      status = debug_begin("perl5db","","",debugArguments,def_debug_timeout,null,debugger_args,debugWorkingDir);
 ...
 */
_command int perl_debug(_str cmdArgs = '') name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
   ...
   if( _project_DebugCallbackName != "perl5db" ) {
      // What are we doing here?
      msg := "Project does not support perl5db. Cannot debug.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return INVALID_ARGUMENT_RC;
   }
   ...
   if( perl5db_opts.serverHost == "" || perl5db_opts.serverPort == "" ) {
      msg := "Invalid perl5db parameters. Cannot debug.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      return INVALID_ARGUMENT_RC;
   }
   ...
   do {
      ...
      already_listening := perl5db_is_listening(perl5db_opts.serverHost,perl5db_opts.serverPort);
      if( !already_listening || !perl5db_is_pending(perl5db_opts.serverHost,perl5db_opts.serverPort) ) {
         ...
         if( perl5db_dir == "" ) {
            msg := "Could not find perl5db directory. Cannot start debugger.";
            _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
         ...
         perl5db_cmdline :=  '-d -I'_maybe_quote_filename(perl5db_dir);
         //say('perl_debug: perl5db_cmdline='perl5db_cmdline);

         // SLICKEDIT_PERL_EXECUTE_ARGS
         old_args := get_env("SLICKEDIT_PERL_EXECUTE_ARGS");
         if( old_args == "" && rc == STRING_NOT_FOUND_RC ) {
            old_args = null;
         }
         set_env("SLICKEDIT_PERL_EXECUTE_ARGS",perl5db_cmdline);
         // PERLDB_OPTS
         //set("PERLDB_OPTS=RemotePort="host':'port' LogFile=C:\temp\perllog.log');
...
static const PERLOPTS_FORM_MINWIDTH=  8175;
static const PERLOPTS_FORM_MINHEIGHT= 7950;
...
static void getFormOptions(PerlOptions& perl_opts, DBGpOptions& perl5db_opts)
{
   //
   // Run tab
   //
   ...
   while( index >= 0 ) {
      ...
      i := perl5db_opts.remoteFileMap._length();
...
static bool validateFormOptions(PerlOptions& perl_opts, DBGpOptions& perl5db_opts)
{
   ...
   if( perl5db_opts.serverHost == "" ) {
      msg := "Invalid/missing perl5db host";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   ...
   if( perl5db_opts.serverPort == "0" ) {
      ...
      msg := "Invalid/missing perl5db port";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      ctl_options_tab.sstActivateTabByCaption("Debug");
...
static void setFormOptionsFromConfig(_str config,
                                     ...
                                     PerlOptions& default_perl_opts,
                                     DBGpOptions (&all_perl5db_opts):[],
                                     DBGpOptions& default_perl5db_opts)
{
   ...
   int i, n=perl5db_opts.remoteFileMap._length();
   for( i=0; i < n; ++i ) {
      mapline :=  perl5db_opts.remoteFileMap[i].remoteRoot"\t"perl5db_opts.remoteFileMap[i].localRoot;
...
static void savePerl5dbOptionsForConfig(_str config, DBGpOptions& perl5db_opts, DBGpOptions (&all_perl5db_opts):[])
{
   // Gather up all affected config names
   _str configIndices[];
   if( config == PROJ_ALL_CONFIGS ) {
...
static bool changeCurrentConfig(_str config)
{
   ...
   do {
      ...
      DBGpOptions (*pAllPerl5dbOpts):[] = _GetDialogInfoHtPtr("allPerl5dbOpts");
...
{
   wildcards := "Perl Files (*.pl;*.pm;*.perl;*.plx)";
   format_list := "";
...
_command void ctlinsertPerlInterpreterArg(_str text="", _str delim='%') name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if( substr(text,1,length('-I')) == '-I' ) {
      ...
      ctlinsert('-I'_maybe_quote_filename(result)' ',delim);
      return;
   }
   // Nothing special
...
void ctl_browse_perl_exe.lbutton_up()
{
   ...
   init_dir := "";
   init_filename := ctl_perl_exe_path.p_text;
   if( init_filename == "" ) {
                     ...
                     _str projectFilename=_project_name, bool isProjectTemplate=false)
{
   ...
   DBGpOptions allPerl5dbOpts:[] = null;
...
void _perl_options_form.on_load()
{
   ...
   if( setting_environment ) {
      perlExePath := ctl_perl_exe_path.p_text;
      msg := "";
      if( perlExePath != "" ) {
         ...
         msg :+= "Please set the Perl interpreter on the Options dialog that follows (\"Build\", \"Perl Options\").";
      }
...
{
   // Debug session frame
   //ctl_debug_session_frame.p_x_extent = childW - 180;
...
_command void perloptions(_str options="") name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveBuild()) {
   ...
   typeless result = ctlbutton_wid.call_event('_perl_options_form 'options,configName,ctlbutton_wid,LBUTTON_UP,'w');
