 ...
 */
static bool _ctItemListAddTemplate(_str filename)
{
   if( _ctIsTemplateFile(filename) ) {
      ...
      if( status!=0 ) {
         msg :=  "Error adding template \"":+filename:+"\". ":+get_message(status);
 ...
 */
bool _ctItemListInOnChange(int onoff=-1)
{
...
void _ctItemListInit(_str path)
{
   ...
   result := file_match(_maybe_quote_filename(path:+"*":+CT_EXT)' +T -X -D +S -P -V',1);
   while( result!="" ) {
      filename := absolute(result,path);
      if( _ctIsTemplateFile(filename) ) {
         _ctItemListAddTemplate(filename);
      }
      result = file_match(_maybe_quote_filename(path:+"*":+CT_EXT)' +T -X -D +S -P -V',0);
   }
 ...
 */
bool _ctItemListGetCurrentItem(_str& templateFilename=null, _str& itemName=null)
{
   index := _TreeCurIndex();
   if( index<=0 ) {
      return false;
 ...
 * @return true on success.
 */
bool _ctItemListSetCurrentItem(_str itemName)
{
