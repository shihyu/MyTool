...
static bool _ftpUseShortFilenames() {
   return (machine()=="WINDOWS" && _win32s()==1);
}

static const FTPBM_FTPCDUP= "_f_folder_up.svg";
static const FTPBM_FTPFILE= "_f_doc.svg";
static const FTPBM_FTPFOLD= "_f_folder.svg";
static const FTPBM_FTPLFOL= "_f_folder_link.svg";
static const FTPBM_FTPLFIL= "_f_doc_link.svg";
static const FTPBM_FTPFILD= "_f_doc_disabled.svg";
static const FTPBM_FTPFOD=  "_f_folder_disabled.svg";
static const FTPBM_FTPNFIL= "_f_doc_disabled.svg";
static const FTPBM_FTPNFOL= "_f_folder_disabled.svg";
static const FTPBM_FTPDFIL= "_f_doc_disabled.svg";
static const FTPBM_FTPDFOL= "_f_folder_disabled.svg";
static const FTPBM_FTPRFIL= "_f_doc_disabled.svg";
static const FTPBM_FTPRFOL= "_f_folder_disabled.svg";
...
_command void ftp_default_options() name_info(','VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveFTP()) {
 ...
 */
void _ftpSetBinary(bool binary)
{
   if( binary ) {
...
static bool gIniInitDone;
// Dictionary of profiles. Each profile has an array of CWDs
static _str _ftpCwdHist:[][];
static _str _ftpLastProfile;

definit()
{
   gIniInitDone=false;
   _ftpCwdHist._makeempty();
   _ftpLastProfile='';
...
/**
 * Fetch flags for tree item at <code>index</code>. Active
 * window must be a <code>_tree_view</code>.
 *
 * @param index 
 * @param _info  (optional) If set, use instead of fetching with
 *               <code>_TreeGetUserInfo()</code>
 *
 * @return One or more of FTPFILETYPE_*.
 */
int _ftpTreeItemUserInfo2TypeFlags(int index, typeless _info=null)
{
   res := 0;
   _str info = _info != null ? _info : _TreeGetUserInfo(index);
   // info := sort-col-info"\t"name-col-info"\t"size-col-info"\t"modified-col-info"\t"attribs-col-info
   name_col_info := "";
   hint := "";
   parse info with ."\t"name_col_info"\t".;
   // name-col-info := hint'-'filename
   parse name_col_info with hint'-'.;
   hint = lowcase(hint);
   if ( pos('d', hint) || pos('z', hint) ) {
      res |= FTPFILETYPE_DIR;
   }
   if ( pos('l', hint) || pos('x', hint) ) {
      res |= FTPFILETYPE_LINK;
   }
   if ( pos('t', hint) || pos('w', hint) ) {
      res |= FTPFILETYPE_FAKED;
   }
   return res;
}

/**
 * Return info suitable for use with 
 * <code>_TreeSetUserInfo()</code> to facilitate sorting 
 * filenames by <code>type</code>, and ascending/descending 
 * order via <code>sort_flags</code>, using 
 * <code>_TreeSortUserInfo()</code>. 
 * 
 * @param type  One or more of FTPFILETYPE_*
 * @param sort_flags  0, or TREE_BUTTON_SORT, or 
 *                    TREE_BUTTON_SORT_DESCENDING.
 * 
 * @return User info.
 */
_str _ftpTypeFlags2SortHint(int type, int sort_flags)
{
   bool descending = testFlag(sort_flags, TREE_BUTTON_SORT_DESCENDING);
   info := "";
 ...
 * <p>
 *
 * Active window must be a <code>_tree_view</code>.
 *
 * Set userinfo for all tree items to facilitate sorting by 
 * Name, Size, Modified, or Attributes, in ascending/descending 
 * order via the current sort column. Current sort flags are
 * used (so make sure you set them BEFORE calling this
 * function).
 */
void _ftpTreeSetAllUserInfo()
{
   ...
   for (;;) {
      ...
      if ( filename == '..' ) {
         // Special case '..' parent directory item
         // We ALWAYS want it at top regardless of which column is being sorted
         _str info = descending ? '~' : '!';
         _TreeSetUserInfo(idx, info);
      } else {
         _str info = _TreeGetUserInfo(idx);
         int type = _ftpTreeItemUserInfo2TypeFlags(idx, info);
         _str hint = _ftpTypeFlags2SortHint(type, name_sort_flags);
         // info := sort-col-info"\t"name-col-info"\t"size-col-info"\t"modified-col-info"\t"attribs-col-info
         _str sort_col_info, name_col_info, size_col_info, modified_col_info, attribs_col_info;
         parse info with sort_col_info"\t"name_col_info"\t"size_col_info"\t"modified_col_info"\t"attribs_col_info;
         // name-col-info := hint'-'name
         _str name;
         parse name_col_info with .'-'name;
         // New name-col-info with updated sort hint
         name_col_info = hint'-'name;
         info = name_col_info"\t"size_col_info"\t"modified_col_info"\t"attribs_col_info;
         // We sort by a specific column by making sure its info is prepended to tree item's user-info
         if ( sort_by_name ) {
            info = name_col_info"\t"info;
         } else if ( sort_by_size ) {
            info = size_col_info"\t"info;
         } else if ( sort_by_modified ) {
            info = modified_col_info"\t"info;
         } else if ( sort_by_attribs ) {
            info = attribs_col_info"\t"info;
         }
         //say('_ftpTreeSetAllUserInfo : info='info);
         _TreeSetUserInfo(idx, info);
      }
...
/**
 * Fetch sort flags for tree column <code>col</code>. Active 
 * window must be a <code>_tree_view</code>. 
 * 
 * @param col 
 * 
 * @return 0, TREE_BUTTON_SORT, or TREE_BUTTON_SORT_DESCENDING. 
 */
int _ftpTreeSortFlags(int col)
{
   if ( col < 0 ) {
      return 0;
   }
   typeless res = _GetDialogInfoHt('SortFlags-'col);
   if ( res == null || !isinteger(res) || res < 0 ) {
      res = 0;
   }
   return (int)res;
}
/**
 * Set sort <code>flags</code> for tree column <code>col</code>.
 * Active window must be a <code>_tree_view</code>. 
 * 
 * @param col 
 * @param flags  0, or TREE_BUTTON_SORT, or 
 *               TREE_BUTTON_SORT_DESCENDING.
 */
void _ftpSetTreeSortFlags(int col, int flags)
{
   if ( col < 0 ) {
      return;
   }
   _SetDialogInfoHt('SortFlags-'col, flags);
}

/**
 * Add <code>file</code> item to remote tree control as child of
 * TREE_ROOT_INDEX.
 *
 * <p>
 *
 * <code>name_sort_flags</code> are used to set sorting hints 
 * for the Name column (in order to keep directories on top 
 * regardless of sort order) and can be 0 or one of: 
 * <ul>
 * <li>TREE_BUTTON_SORT</li> 
 * <li>TREE_BUTTON_SORT_DESCENDING</li>. 
 * </ul>
 *
 * <p>
 *
 * <code>size_sort_flags</code> are used to set sorting hints
 * for the Size column and can be 0 or one of: 
 * <ul>
 * <li>TREE_BUTTON_SORT</li> 
 * <li>TREE_BUTTON_SORT_DESCENDING</li>. 
 * </ul>
 ...
 * <p>
 *
 * <code>modified_sort_flags</code> are used to set sorting
 * hints for the Modified column and can be 0 or one of:
 * <ul>
 * <li>TREE_BUTTON_SORT</li> 
 * <li>TREE_BUTTON_SORT_DESCENDING</li>. 
 * </ul>
 ...
 * <p>
 *
 * <code>attribs_sort_flags</code> are used to set sorting hints
 * for the Attributes column and can be 0 or one of:
 * <ul>
 * <li>TREE_BUTTON_SORT</li> 
 * <li>TREE_BUTTON_SORT_DESCENDING</li>. 
 * </ul>
 * 
 * @param file 
 * @param name_sort_flags 
 * @param size_sort_flags
 * @param modified_sort_flags
 * @param attribs_sort_flags
 * 
 * @return Tree control index of added item.
 */
int _ftpTreeAddItem(FtpFile& file, int name_sort_flags, int size_sort_flags, int modified_sort_flags, int attribs_sort_flags)
{
   bool sort_by_name = 0 != (name_sort_flags & (TREE_BUTTON_SORT | TREE_BUTTON_SORT_DESCENDING));
   bool sort_by_size = 0 != (size_sort_flags & (TREE_BUTTON_SORT | TREE_BUTTON_SORT_DESCENDING));
   bool sort_by_modified = 0 != (modified_sort_flags & (TREE_BUTTON_SORT | TREE_BUTTON_SORT_DESCENDING));
   ...
   _str filename = file.filename;
   int size = file.size;
   modified :=  file.year'-'file.month'-'file.day;
   if ( file.time != '' ) {
      // Dec-31-2000 + pad-2-spaces = 13 chars
      modified = _pad(modified, 13, ' ', 'R') :+ file.time;
   }
   _str attribs = file.attribs;
...
void _ftpTodoGetList()
{
   ...
   int info;
   for( ff:=1;;ff=0 ) {
      index := tree_wid._TreeGetNextSelectedIndex(ff,info);
      if( index<0 ) {
         break;
      }
...
bool isuncdirectory(_str path)
{
   ...
   if (file_match('+d ':+_maybe_quote_filename(path:+ALLFILES_RE),1)=='') {
      return(false);
   }
...
void _ftpLog(FtpConnProfile *fcp_p,_str buf)
{
   ...
   if( _ftpdebug&FTPDEBUG_SAVE_LOG ) {
      ...
      typeless status=save_file(_maybe_quote_filename(log_filename),'+o');
   }
   p_window_id=orig_view_id;

   // Make sure we are always at the bottom of the log window on the toolbar
   _control _ctl_ftp_sstab;
   int ftpclient_wid = _find_formobj("_tbFTPClient_form","N");
   if( ftpclient_wid > 0 ) {
 ...
 *   SOCK_TIMED_OUT_RC
 */
int _ftpCommand(FtpConnProfile *fcp_p,bool quiet,typeless cmd,...)
{
...
int _ftpType(FtpConnProfile* fcp_p, FtpXferType override_xfer_type=FTPXFER_DEFAULT)
{
...
_str _ftpLocalAbsolute(FtpConnProfile *fcp_p,_str local_path)
{
   ...
   abs_path := absolute(_maybe_quote_filename(local_path),localcwd);
...
bool _ftpIsDir(FtpConnProfile *fcp_p,_str remote_path)
{
   isdir := false;
   path := "";
   filename := "";
   ...
   case FTPSYST_VMS_MULTINET:
      ...
      break;
   case FTPSYST_VOS:
      if( _last_char(remote_path)=='>' ) isdir=true;
      break;
   case FTPSYST_VM:
   case FTPSYST_VMESA:
...
bool _ftpFileEq(FtpConnProfile *fcp_p,_str file1,_str file2)
{
...
bool _RemoteFilespecMatches(FtpConnProfile *fcp_p,_str filespec,_str filename,_str fcase="")
{
   if( fcase=="" ) {
 ...
 */
bool _ftpExists(FtpConnProfile *fcp_p,_str name,int &flags)
{
   FtpFile file;
...
bool _ftpIsConnectionAlive(FtpConnProfile *fcp_p)
{
   bool alive;
...
static bool _in_MaybeUpdate=false;
...
void _MaybeUpdateFTPClient(_str profile)
{
   ...
   if( _in_MaybeUpdate || gin_restore ) {
      return;
   }
   ...
   clientformWid=_find_formobj("_tbFTPClient_form","N");
   ...
   if( profile=="" || clientprofileWid.p_text=="" || clientprofileWid.p_text==profile ) {
      ...
      int openformWid=_find_formobj("_tbFTPOpen_form","N");
      if( openformWid ) {
...
void _MaybeUpdateFTPTab(_str profile)
{
   ...
   // the same connection as profile name passed in.
   openformWid=_find_formobj("_tbFTPOpen_form","N");
   ...
   if( profile=="" || openprofileWid.p_text=="" || openprofileWid.p_text==profile ) {
      ...
      #if 1
      // Now synch up the progress labels between the 2 toolbars
      int clientformWid=_find_formobj("_tbFTPClient_form","N");
      if( clientformWid ) {
         openlabel1Wid := openformWid._find_control("_ctl_operation");
         openlabel2Wid := openformWid._find_control("_ctl_nofbytes");
         clientlabel1Wid := clientformWid._find_control("_ctl_progress_label1");
...
_str ftpfile_match(_str name,int find_first)
{
   ...
   int formWid=_find_formobj("_ftpManualDownload_form","N");
   if( !formWid ) return("");
   ...
   for( ;_fm_idx<_fm_files._length(); ) {
      ...
      if( _RemoteFilespecMatches(fcp_p,filespec,filename) ) {
         _last_char=substr(filename,length(filename),1);
         switch( _last_char ) {
         case FTPSYST_VOS:
            isdir= (_last_char=='>');
            break;
...
_str ftplocalfile_match(_str name,int find_first,_str doDirMatch="")
{
   ...
   dir_match := (doDirMatch!="");

   fcp_p=null;
   int formWid=_find_formobj("_ftpManualDownload_form","N");
   if( !formWid ) {
      // Maybe it is _ftpChangeDir_form
      formWid=_find_formobj("_ftpChangeDir_form","N");
   }
   if( !formWid ) return("");
   // What you return depends on which toolbar you are in
   _str form_name=formWid.p_parent.p_active_form.p_name;
   ...
   if( name:!="" ) {
      ...
      if( !_file_eq(orig_name,substr(name,1,length(orig_name))) ) {
...
void _ctl_ok.on_create(_str src_filename="", _str dest_filename="",
                       FtpXferType xferType=FTPXFER_DEFAULT, _str caption="",
                       FtpConnProfile *fcp_p = null)
{
   if( caption!="" ) {
      p_active_form.p_caption=caption;
...
void _ctl_ok.lbutton_up()
{
   src_filename := _ctl_src_fname.p_text;
   if( src_filename=="" ) {
      p_window_id=_ctl_src_fname;
      _set_focus();
      msg := "You must specify a source filename";
      ftpDisplayInfo(msg);
      return;
   }
   dest_filename :=  ctl_dst_panel.p_visible ? _ctl_dest_fname.p_text : '';
   ...
   os400_lfs := (_ctl_os400_lfs.p_visible && _ctl_os400_lfs.p_value != 0 );
...
void _ctl_ok.on_create(_str remote_filename='', 
                       FtpXferType xferType=FTPXFER_DEFAULT, 
                       _str caption='',
                       FtpConnProfile* fcp_p=null,
                       _str edit_options='')
{
...
void _ctl_ok.lbutton_up()
{
   remote_filename := _ctl_fname.p_text;
   if( remote_filename=="" ) {
      p_window_id=_ctl_fname;
      _set_focus();
      msg := "You must specify a new filename";
      ftpDisplayInfo(msg);
      return;
   }
...
_command void ftpCloseAllConnections() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   _ftpCloseAllConnections();
}

static const FTPTEMP_DIR= 'ftp';
static const FTPTEMP_FILE_PREFIX= "vsftp";
...
_str _ftpMktemp()
{
   ...
   _maybe_strip_filesep(path);   // Strip trailing FILESEP
   if( file_match('+d +x -p ':+_maybe_quote_filename(path),1)=="" ) {
   ...
   path :+= FILESEP;   // Tack the trailing FILESEP back on
 ...
 */
bool _ftpIsTempFilename(_str name)
{
   temp_path := _ftpTempPath();
   _maybe_append_filesep(temp_path);

   name_path := _file_case(_strip_filename(name,'N'));
...
static _str FTPXLAT_FILENAME_CHARSET() {
   return  (_isUnix()?"\t":"[]:\\/<>|=+;,\t");
}
/**
...
_str _ftpRemoteToLocalPath(FtpConnProfile *fcp_p,_str remote_path)
{
   ...
   mvs_quoted := (fcp_p->system==FTPSYST_MVS && substr(remote_path,1,1)=="'" && _last_char(remote_path)=="'");
   if( mvs_quoted ) {
      remote_path=strip(remote_path,'B',"'");
   }
   switch( fcp_p->system ) {
      ...
      if( _ftpFileEq(fcp_p,substr(path_only,1,length(remoteroot)),remoteroot) ) {
         ...
         case FTPSYST_VMS_MULTINET:
            ...
            while( temp!="" ) {
               ...
               append_path :+= name:+FILESEP;
            }
            parse filename with filename ';' ver;

            // 10/26/2017 - rb
            // VMS system can have extensionless filename where the last char is '.'
            // Strip it if this is the case.
            _maybe_strip(filename, '.');

            append_path :+= _MaybeLongTo8Dot3(filename);
         ...
         if( !isdrive(temp_path) && file_match('+d +x -p ':+_maybe_quote_filename(temp_path),1)=="" ) {
   ...
   case FTPSYST_VMS:
   case FTPSYST_VMS_MULTINET:
      _maybe_strip(local_filename, '.');
      break;
   }

   // This tells us if the filename part was shortened to 8.3
   filename_was_reduced := (!isdir && local_filename!=remote_filename);
   ...
   _maybe_strip_filesep(local_path);
   if( file_match('+d +x -p ':+_maybe_quote_filename(local_path),1)=="" ) {
      // The path is not created yet, so create it
   ...
   if( local_filename!="" ) {
      ...
      if( filename_was_reduced ) {
         ...
         found=file_match("-p "_maybe_quote_filename(filename),1);
         for( i=0;i<10;++i ) {
            if( found=="" ) break;
            ...
            if( ext!="" ) filename :+= ".":+ext;
            found=file_match("-p "_maybe_quote_filename(filename),0);
         }
         if( found!="" ) {
            // Try again
            for( i=10;i<100;++i ) {
 ...
 * Expects the name of the file, including the full path, to work.
 */
bool _ftpIsLocalFilename(_str name)
{
   temp_path := _ftpTempPath();
   _maybe_append_filesep(temp_path);
...
bool ftpIsFTPDocname(_str name)
{
...
int _exit_ftp()
{
   ...
   temp_path := _ftpTempPath();
   _maybe_append_filesep(temp_path);
   name := file_match("-d ":+_maybe_quote_filename(temp_path:+FTPTEMP_FILE_PREFIX:+ALLFILES_RE),1);
   while( name!="" ) {
      if( !pos(" ":+_file_case(name):+" ",excluded,1) ) {
         ...
         if( status ) {
            ...
            if( errors!="" ) errors="\n":+errors;
            errors :+= name:+" - ":+get_message(status);
         }
      }
      name=file_match("-d ":+_maybe_quote_filename(temp_path:+FTPTEMP_FILE_PREFIX:+ALLFILES_RE),0);
   }
   if( errors!="" ) {
...
_command int ResetSockets() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   // Exit sockets layer
...
int _ftpSynchronousDisconnect(FtpConnProfile *fcp_p,bool quiet=false)
{
   bool progress_already_visible;
   formWid := 0;
   orig_view_id := 0;
   bool success;
   ...
   if( !quiet ) {
      ...
      formWid=_find_formobj("_ftpProgress_form",'N');
...
int _ftpSynchronousConnect(FtpConnProfile *fcp_p,bool &need_connect,bool quiet=false)
{
   int formWid;
   bool success;
   int orig_view_id;
...
void _init_menu_ftp(int menu_handle, int no_child_windows)
{
   if (_jaws_mode()) {
      ...
      int wid=_find_formobj('_tbFTPOpen_form._ctl_binary','N');
      if (!wid) {
         wid=_find_formobj('_tbFTPClient_form._ctl_binary','N');
      }
      if (wid) {
...
_command int ftpUpload() name_info(','VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveFTP()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION_1ARG, "FTP");
...
static _str _ftpAssembleUploadPath(FtpConnProfile *fcp_p,_str remote_path,bool quiet)
{
   path := "";
   filename := "";
   file := "";
   ...
   case FTPSYST_MVS:
      if( substr(path,1,1)=='/' ) {
         ...
         if( mvs_pds ) {
            ...
            if( path!="" ) {
               remote_path :+= path;
               _maybe_append(remote_path, '.');
            }
            remote_path :+= _ftpUploadCase(fcp_p,filename):+"'";
         }
      }
      break;
   case FTPSYST_VM:
   ...
   case FTPSYST_OS400:
      if( substr(path,1,1)=='/' ) {
         ...
         parse filename with file '.' member;
         if( member!="" ) {
            remote_path :+= file:+'.':+_ftpUploadCase(fcp_p,member);
         } else {
            remote_path :+= _ftpUploadCase(fcp_p,file);
         }
      }
      break;
   default:
...
static int _ftpSave(_str local_path,_str remote_address, 
                    bool force_prompt=false, bool quiet=false,
                    FtpXferType xfer_type=FTPXFER_DEFAULT)
{
   FtpConnProfile fcp;
   FtpQEvent event;
   ...
   line := file_match("-p +v "_maybe_quote_filename(local_path),1);
   if( line=="" ) {
      if( !quiet ) {
   ...
   if( fcp.serverType==FTPSERVERTYPE_SFTP ) {
      ...
      scmd.src=local_path;
      scmd.pasv=false;   // Ignored by SFTP
      scmd.size=size;
 ...
 * @return 0 on success.
 */
_command int ftpCopy(_str line="") name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   FtpConnProfile* fcp_p;

   if( line == "" ) {
      msg := "Not enough arguments. Usage: ftpCopy [+-A|+-B] [+-PROMPT] <source> <dest>";
      message(msg);
      return 1;
   }
...
/**
 * Determine if FTP profile exists
 */
bool _ftpIsConnProfile(_str ProfileName)
{
   return _plugin_has_profile(VSCFGPACKAGE_FTP_PROFILES,ProfileName);
}
...
int _ftpOpenConnProfile(_str ProfileName,FtpConnProfile *fcp_p)
{
   ...
   handle:=_plugin_get_profile(VSCFGPACKAGE_FTP_PROFILES,ProfileName);
   if( handle<0 ) {
      ftpDisplayError('Error opening profile "':+ProfileName:+'".  ':+get_message(handle));
      return(handle);
   }
   fcp_p->profileName=ProfileName;
   profile_node:=_xmlcfg_get_document_element(handle);
   node:=_xmlcfg_get_first_child_element(handle,profile_node);
   while( node>=0) {
      varble := "";
      val := "";
      varble=_xmlcfg_get_attribute(handle,node,VSXMLCFG_PROPERTY_NAME);
      val=_xmlcfg_get_attribute(handle,node,VSXMLCFG_PROPERTY_VALUE);
      varble=strip(varble);
      val=strip(val);
      switch( varble ) {
      case 'host':
...
{
   handle:=_xmlcfg_create_profile(auto profile_node,VSCFGPACKAGE_FTP_PROFILES,fcp_p->profileName,VSCFGPROFILE_FTP_VERSION);

   _xmlcfg_add_property(handle,profile_node,"host",fcp_p->host);
   _xmlcfg_add_property(handle,profile_node,"servertype",fcp_p->serverType);
   _xmlcfg_add_property(handle,profile_node,"sshauthtype",fcp_p->sshAuthType);
   _xmlcfg_add_property(handle,profile_node,"hosttype",fcp_p->system);
   _xmlcfg_add_property(handle,profile_node,"userid",fcp_p->userId);
   _xmlcfg_add_property(handle,profile_node,"password",fcp_p->password);   // Password is already encrypted
   _xmlcfg_add_property(handle,profile_node,"anonymous",fcp_p->anonymous);
   _xmlcfg_add_property(handle,profile_node,"savepassword",fcp_p->savePassword);
   _xmlcfg_add_property(handle,profile_node,"defremotehostdir",fcp_p->defRemoteDir);
   _xmlcfg_add_property(handle,profile_node,"deflocaldir",fcp_p->defLocalDir);
   _xmlcfg_add_property(handle,profile_node,"remotefilter",fcp_p->remoteFileFilter);
   _xmlcfg_add_property(handle,profile_node,"localfilter",fcp_p->localFileFilter);
   _xmlcfg_add_property(handle,profile_node,"autorefresh",fcp_p->autoRefresh);
   _xmlcfg_add_property(handle,profile_node,"remoteroot",fcp_p->remoteRoot);
   _xmlcfg_add_property(handle,profile_node,"localroot",fcp_p->localRoot);
   _xmlcfg_add_property(handle,profile_node,"xfertype",(int)fcp_p->xferType);
   _xmlcfg_add_property(handle,profile_node,"port",fcp_p->port);
   _xmlcfg_add_property(handle,profile_node,"timeout",fcp_p->timeout);
   _xmlcfg_add_property(handle,profile_node,"usefw",fcp_p->useFirewall);
   _xmlcfg_add_property(handle,profile_node,"keepalive",fcp_p->keepAlive);
   _xmlcfg_add_property(handle,profile_node,"uploadcase",fcp_p->uploadCase);
   _xmlcfg_add_property(handle,profile_node,"resolvelinks",fcp_p->resolveLinks);
...
{
   _plugin_delete_profile(VSCFGPACKAGE_FTP_PROFILES,ProfileName);

   return(0);
}
static void _reverse_array(_str (&templist)[],_str (&CwdHist)[]) {
   templist._makeempty();
   i := 0;
   for( i=CwdHist._length()-1;i>=0;--i ) {
      templist[templist._length()]=CwdHist[i];
   }
}
/**
...
int _ftpSaveCwdHist(_str ProfileName,_str CwdHist[])
{
   ...
   _reverse_array(templist,CwdHist);
   if (templist._length()>def_maxcombohist) {
      templist._deleteel(templist._length(),templist._length()-def_maxcombohist);
      
   }
   _ftpCwdHist:[lowcase(ProfileName)]=templist;
   return(0);
}
...
{
   _str (*parray)[];
   parray=_ftpCwdHist._indexin(lowcase(ProfileName));
   if (!parray) {
      CwdHist._makeempty();
      return 0;
   }
   _reverse_array(CwdHist,*parray);
   return(0);
}
 ...
 * @return 0 if match(es) are found. Otherwise non-zero is returned.
 */
int _ftpHostNameToProfileName(_str Host,_str (&matchingProfileNames)[]) {
   _str profileNames[];
   _plugin_list_profiles(VSCFGPACKAGE_FTP_PROFILES,profileNames);

   matchingProfileNames._makeempty();
   _str temparr[];
   temparr._makeempty();
   for (i:=0;i<profileNames._length();++i) {
      value:=_plugin_get_property(VSCFGPACKAGE_FTP_PROFILES,profileNames[i],'host');
      if( value==Host ) {
         matchingProfileNames[matchingProfileNames._length()]=profileNames[i];
      }
   }

   if( matchingProfileNames._length() ) {
      return(0);
   }
   // No match for Host
   return(1);
...
{
   handle:=_plugin_get_profile(VSCFGPACKAGE_FTP,VSCFGPROFILE_OPTIONS);
   if( handle<0 ) {
   ...
   while( node>=0) {
      ...
      case 'deflocaldir':
         ...
         break;
      case 'put':
         if( !isinteger(val) || val<=0 || val>=3) {
            val=FTPOPT_PROMPTED_PUT;
         }
         fo.put= (int)val;
         break;
      case 'resolvelinks':
...
{
   handle:=_xmlcfg_create_profile(auto profile_node,VSCFGPACKAGE_FTP,VSCFGPROFILE_FTP_OPTIONS,VSCFGPROFILE_FTP_VERSION);

   _xmlcfg_add_property(handle,profile_node,"email",fo_p->email);
   _xmlcfg_add_property(handle,profile_node,"deflocaldir",fo_p->deflocaldir);
   _xmlcfg_add_property(handle,profile_node,"put",fo_p->put);
   int val= (int)fo_p->resolvelinks;
   _xmlcfg_add_property(handle,profile_node,"resolvelinks",val);
   _xmlcfg_add_property(handle,profile_node,"timeout",fo_p->timeout);
   _xmlcfg_add_property(handle,profile_node,"port",fo_p->port);
   val= (int)fo_p->keepalive;
   _xmlcfg_add_property(handle,profile_node,"keepalive",val);
   _xmlcfg_add_property(handle,profile_node,"uploadcase",fo_p->uploadcase);
   _xmlcfg_add_property(handle,profile_node,"fwhost",fo_p->fwhost);
   _xmlcfg_add_property(handle,profile_node,"fwport",fo_p->fwport);
   _xmlcfg_add_property(handle,profile_node,"fwuserid",fo_p->fwuserid);
   _xmlcfg_add_property(handle,profile_node,"fwpassword",fo_p->fwpassword);   // Already encrypted
   _xmlcfg_add_property(handle,profile_node,"fwtype",fo_p->fwtype);
   val= (int)fo_p->fwpasv;
   _xmlcfg_add_property(handle,profile_node,"fwpasv",val);
   val= (int)fo_p->fwenable;
   _xmlcfg_add_property(handle,profile_node,"fwenable",val);
   _xmlcfg_add_property(handle,profile_node,"sshexe",fo_p->sshexe);
   _xmlcfg_add_property(handle,profile_node,"sshsubsystem",fo_p->sshsubsystem);
...
static const FCPTAB_GENERAL=  (0);
static const FCPTAB_ADVANCED= (1);

static bool gchange_server_type=true;
static bool gchange_auth_type=true;
static bool gchange_server_port=true;
static int gchange_old_server_type= -1;

defeventtab _ftpCreateProfile_form;

static FtpConnProfile *FCPDATAP(...) {
   if (arg()) _ctl_profile_sstab.p_user=arg(1);
   return  _ctl_profile_sstab.p_user;
}
static int FCPFLAGS(...) {
   if (arg()) _ctl_ok.p_user=arg(1);
   return _ctl_ok.p_user;
}
static _str FCPUSERID(...) {
   if (arg()) _ctl_user.p_user=arg(1);
   return _ctl_user.p_user;
}
static _str FCPPASS(...) {
   if (arg()) _ctl_pass.p_user=arg(1);
   return _ctl_pass.p_user;
}
static _str FCPORIG_PROFILENAME(...) {
   if (arg()) _ctl_profile.p_user=arg(1);
   return _ctl_profile.p_user;
}
...
{
   type_name := _ctl_server_type.p_text;
...
static int GetGeneralOptions(FtpConnProfile *fcp_p)
{
   ...
   server_type := strip(_ctl_server_type.p_text);
   if( server_type=="" ) {
      _ctl_profile_sstab.p_ActiveTab=FCPTAB_GENERAL;
   ...
   auth_type := strip(ctl_auth_type.p_text);
   if( auth_type=="" ) {
      _ctl_profile_sstab.p_ActiveTab=FCPTAB_GENERAL;
   ...
   host_type := strip(_ctl_host_type.p_text);
   if( host_type=="" ) {
      _ctl_profile_sstab.p_ActiveTab=FCPTAB_GENERAL;
...
void _ctl_server_type.on_change(int reason)
{
   ...
   type_name := _ctl_server_type.p_text;
   int type = _ftpservertype_list:[type_name];
   enabled := ( type==FTPSERVERTYPE_FTP );
...
static const FTPOPTIONSTAB_GENERAL=  (0);
static const FTPOPTIONSTAB_ADVANCED= (1);
static const FTPOPTIONSTAB_FIREWALL= (2);
static const FTPOPTIONSTAB_SSH=      (3);
static const FTPOPTIONSTAB_DEBUG=    (4);
...
{
   bool require_host,require_port,require_user,require_pass;
...
{
   status := 0;
   sshexe := _ctl_ssh_exe.p_text;
   if( sshexe!="" && !file_exists(sshexe) ) {
   ...
   sshexe_nopath := _strip_filename(sshexe,'PE');
   ...
   sshsubsystem := _ctl_ssh_subsystem.p_text;
   if( sshsubsystem=="" ) {
...
static FtpConnProfile* FPMDATAP(...) {
   if (arg()) _ctl_connect.p_user=arg(1);
   return  _ctl_connect.p_user;
}
...
static int _fpmFillProfileList()
{
   ...
   _ctl_list._lbclear();
   for (i:=0;i<profileNames._length();++i) {
      _ctl_list._lbadd_item(profileNames[i]);
   }
...
{
   _ctl_options.p_x = (ctlHelpLabelFilter.p_x_extent) - _ctl_options.p_width;
}
...
void _ctl_connect.lbutton_up()
{
   ...
   if( !status ) {
      _ftpLastProfile=strip(_ctl_list._lbget_seltext());
   }
...
void _ctl_list.rbutton_up()
{
   ...
   menu_name := "_ftpProfileManager_menu";
...
_command void ftpProfileManager() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   if (!_haveFTP()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_OR_STANDARD_EDITION_1ARG, "FTP");
      return;
...
_command void ftpProfileManagerAdd() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   FtpConnProfile fcp;

   int formWid=_find_formobj("_ftpProfileManager_form","N");
   if( !formWid ) return;
   listWid := formWid._find_control("_ctl_list");
   if( !listWid ) return;
   fcp._makeempty();
...
_command void ftpProfileManagerDelete() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   FtpConnProfile fcp;
...
_command void ftpProfileManagerEdit() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   FtpConnProfile fcp;
...
_command void ftpProfileManagerRename() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   typeless status=0;
   int formWid=_find_formobj("_ftpProfileManager_form","N");
   if( !formWid ) return;
   ...
   if( old_profile!="" ) {
      ...
      if( new_profile=="" || strieq(old_profile,new_profile)) return;
      ...
      _plugin_copy_profile(VSCFGPACKAGE_FTP_PROFILES,old_profile,new_profile);
      _plugin_delete_profile(VSCFGPACKAGE_FTP_PROFILES,old_profile);
...
_command void ftpProfileManagerCopy() name_info(','VSARG2_NCW|VSARG2_READ_ONLY|VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_REQUIRES_PRO_OR_STANDARD_EDITION)
{
   FtpConnProfile fcp;
   ...
   if( from_profile!="" ) {
      ...
      _plugin_copy_profile(VSCFGPACKAGE_FTP_PROFILES,from_profile,to_profile);
...
_ftpLog_form.on_resize()
{
   ...
   _ctl_ok.p_x= (int)(formW-_ctl_ok.p_width) intdiv 2;
   _ctl_ok.p_y=_ctl_log.p_y_extent+_dy2ly(SM_TWIP,4)-1;
}
...
// Store remove cwd history and last profile
int _srg_ftp_cwdhist(_str option='',_str info='')
{
   if ( option=='R' || option=='N' ) {
      parse info with auto Noflines .;
      down();
      get_line(_ftpLastProfile);--Noflines;
      _ftpCwdHist._makeempty();
      _str (*parray)[]=null;
      while (Noflines--) {
          if (down()) break;
          ...
          if (substr(line,1,1):==' ') {
             if (parray!=null) {
                (*parray)[parray->_length()]=substr(line,2);
             }
          } else {
             if (_plugin_has_profile(VSCFGPACKAGE_FTP_PROFILES,line)) {
                parray=&_ftpCwdHist:[lowcase(line)];
             } else {
                parray=null;
             }
          }
      ...
      insert_line('FTP_CWDHIST: 1');
      header_line := p_line;
      insert_line(_ftpLastProfile);
      NofLines := 1;
      _str profileName;
      for (profileName._makeempty();;) {
          _str (*parray)[]=&_ftpCwdHist._nextel(profileName);
          if (profileName._isempty()) break;
          if (profileName!='') {
             insert_line(strip(profileName));++NofLines;
             for (i:=0;i<parray->_length();++i) {
                insert_line(' '(*parray)[i]);++NofLines;
             }
          }
      }
      save_line := p_line;
      p_line = header_line;
      replace_line('FTP_CWDHIST: 'NofLines);
      p_line = save_line;
   }
   return(0);
}
