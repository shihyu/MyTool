 ...
 * Specify 0 for no bitmap.  The following are names of bitmaps used for debugging:
 * <dl>
 ...
 * <dt>_ed_watchpoint.svg</dt><dd>Indicates a line with an enabled watchpoint.</dd>
 * </dl>
 *
 ...
 *       0,4, find_index("_ed_breakpointt.svg", PICTURE_TYPE),
 *       type,"");
 *
 * // Add a line marker which draws a box
 * int type=_MarkerTypeAlloc();
 * int LineMarkerIndex=_LineMarkerAdd(p_window_id,p_line,
 *       0,4,find_index('_ed_plus.svg',PICTURE_TYPE),
 ...
 *
 */
extern int _LineMarkerAdd(int wid,int RealLineNum,bool isRealLineNum,int NofLines,int BMIndex,int type,_str Message);
 ...
 * // break point.
 * int type=_MarkerTypeAlloc();
 * int LineMarkerIndex=_LineMarkerAddB(p_buf_name,p_line,
 *       0,4, find_index("_breakpoint.ico", PICTURE_TYPE),
 *       type,"");
 *
 * // Add a line marker which draws a box
 * int type=_MarkerTypeAlloc();
 ...
 * current object.
 *
 * @param StartOffset    Bytes offset or real byte offset (not
 *                       counting no save lines).
 * 
 * @param Length         Number of bytes included starting from
 *                       StartOffset.
 *
 * @param isRealOffset 	Indicates whether <i>StartOffset</i> is
 * a real offset or not.
 *
 * @param BMIndex	Bitmap to display at the line specified.
 * Specify 0 for no bitmap.  The following are
 * names of bitmaps used for debugging:
 ...
 * @example
 * <pre>
 * // Color 4 bytes of text starting from the cursor location.
 * int type=_MarkerTypeAlloc();
 * int StreamMarkerIndex=_StreamMarkerAdd(p_window_id,_nrseek(),
 *       4, 0, 0,
 *       type,"");
 * int ColorIndex=_AllocColor(0xffffff,0xff);
 * _StreamMarkerSetTextColor(StreamMarkerIndex,ColorIndex);
 *
 *  // Draw red squiggly under 4 bytes of text starting from the cursor location.
 *  int type=_MarkerTypeAlloc();
 *  _MarkerTypeSetFlags(type,VSMARKERTYPEFLAG_DRAW_SQUIGGLY);
 *  int StreamMarkerIndex=_StreamMarkerAdd(p_window_id,_nrseek(),
 *        4, 0, 0,
 *        type,"");
 *  _StreamMarkerSetStyleColor(StreamMarkerIndex,0xff);
 *
 * </pre>
 *
 * @appliesTo Edit_Window, Editor_Control
 ...
 *
 */
extern int _StreamMarkerAdd(int wid, long StartOffset, long Length, bool isRealOffset, int BMIndex,int type,_str pszMessage);
 ...
 *
 */
extern int _StreamMarkerAddB(_str BufName, long StartOffset, long Length, bool isRealOffset, int BMIndex,int type,_str pszMessage);
 ...
 *                   which use MarkIds.
 * @param CheckNofLines
 *                   When on, only line markers that contain the
 *                   current line are returned.
 *
 */
extern void _LineMarkerFindList(int (&list)[],int wid,int LineNum,long LineOffset,bool CheckNofLines);
...
extern void _StreamMarkerSetUserDataInt64(int StreamMarkerIndex, long UserData);
extern long  _StreamMarkerGetUserDataInt64(int StreamMarkerIndex);
...
const RELOC_MARKER_WINDOW_SIZE= 5;
...
/**
 * All marker information to save/restore. 
 *  
 * @see _SaveMarkersInFile 
 * @see _RestoreMarkersInFile
 */
struct MarkerSaveInfo {
   BookmarkSaveInfo     bookmarks[];
   BreakpointSaveInfo   breakpoints[];
   AnnotationSaveInfo   annotations[];
   ReferenceSaveInfo    references[];
   MessageSaveInfo      messages[];
};
