 ...
 */
_command int get(_str src_filename='', _str obsolete='', _str insert_style='', int start_line=0, int end_line=0, _str load_options='') name_info(FILE_ARG','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   buf_id := p_buf_id;
   typeless junk;
   filename := absolute(strip_options(src_filename,junk));
   load_files('+bi 'buf_id);
   temp_view_id := 0;
   orig_view_id := 0;
   status := _open_temp_view(filename,
   ...
   if (!_on_line0()) {
      ...
      if (!p_buf_width && p_newline=="\r\n" && get_text()==_chr(26)) {
         ...
         LastLineHasNLChars=_line_length()!=_line_length(true);
      }
   }
   mark_view_id := 0;
...
static _str is_printer_device(_str dev_name)
{
   if (_isWindows()) {
      return (
        _file_eq(dev_name,'prn') ||
        /* Check for LPT follow by a digit. */
         (_file_eq(substr(dev_name,1,3),'lpt') && isinteger(substr(dev_name,4))) ||
         _file_eq(dev_name,def_tprint_device)
      );
   }
    return (
      _file_eq(dev_name,'/dev/lp') || _file_eq(dev_name,'/dev/lp1') ||
      _file_eq(dev_name,'/dev/lp2') || _file_eq(dev_name,'/dev/lp3') ||
      _file_eq(dev_name,'/dev/lp4') || _file_eq(dev_name,def_tprint_device)
    );
 ...
 */
static int put2(_str dest_filename,_str save_flags,_str msg,bool doAppend)
{
   ...
   p_AllowSave=true;
   if (isEclipsePlugin()) {
      status= save_file(_maybe_quote_filename(p_buf_name),build_save_options(p_buf_name) " "save_options);
   } else {
