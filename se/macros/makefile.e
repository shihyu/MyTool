...
{
   if (!_ProjectFileExists(ProjectFilename)) return(0);
...
void _projecttb_AddFile(int tree_wid,int index,_str filename,int &NofInserted,int (&StatusList)[],bool iUsingScc)
{
                                   ...
                                   )
{
   ...
   for (;;) {
      ...
      line=strip(line);
      stripped=0;
      if (_last_char(line)==ContinuationChar) {
         line=substr(line,1,length(line)-1);
         stripped=1;
      }
      if (strip(line)!='') {
         filename := strip(line,'B','"');
...
static int GetFileListFromVisualStudioFile(_str MakefileName,int FileListViewId,_str (&FileArray)[]=null,
                                           ...
                                           int handle=-1,
                                           bool ConvertToAbsolute=true,
                                           bool ResolveLinks=false,
                                           bool listboxFormat=false)
{
   MakefilePath := _strip_filename(MakefileName,'N');
   opened_file := false;
   if (handle<0) {
      opened_file=true;
      int status;
      if (_file_eq(_get_extension(MakefileName,true),VISUAL_STUDIO_VCPP_PROJECT_EXT)) {
         handle=_xmlcfg_open(MakefileName,status);
      } else {
         handle=_xmlcfg_open(MakefileName,status,VSXMLCFG_OPEN_ADD_NONWHITESPACE_PCDATA);
      }
      if (handle<0) {
         return(status);
      }
   ...
   // Visual C++ (vcxproj), Visual Studio 2010 and up
   else if(_file_eq(_get_extension(MakefileName,true),VISUAL_STUDIO_VCX_PROJECT_EXT)) {
      ...
      if (AppName:!='') {
         ...
         path := '/VisualStudioProject/'AppName'/Files/Include/File';
      ...
      for (curItemGroup=0;curItemGroup<itemGroupIndexes._length();++curItemGroup) {
         ...
         for (curCompile=0;curCompile<compileIndexes._length();++curCompile) {
            ...
            if (val!='') {
               val=translate(val,FILESEP,FILESEP2);
               if (ConvertToAbsolute) {
                  val=absolute(val,MakefilePath);
               } else {
                  val=ConvertFromVCPPRelFilename(val,MakefilePath);
         ...
         typeless referenceIndexes[]=null;
         _xmlcfg_find_simple_array(handle,'Reference/HintPath',referenceIndexes,itemGroupIndexes[curItemGroup]);
         //say('hit count='referenceIndexes._length());

         int curHint;
         for (curHint=0;curHint<referenceIndexes._length();++curHint) {
            child_node:=_xmlcfg_get_first_child(handle,referenceIndexes[curHint],VSXMLCFG_NODE_PCDATA);
            if (child_node<0) {
               //say('no child_node');
               continue;
            }
            _str val=_xmlcfg_get_value(handle,child_node);
            val=translate(val,FILESEP,FILESEP2);
            //say('val='val);

            if (file_eq(get_extension(val),'dll')) {
               //say('***************************');
               if (ConvertToAbsolute) {
                  val=absolute(val,MakefilePath);
               } else {
               ...
               if (!IndexTable._indexin(_file_case(val))) {
                  AddFileToView(FileListViewId,val,'',listboxFormat);
                  FileArray[FileArray._length()]=val;
                  IndexTable:[_file_case(val)]=referenceIndexes[curHint];
               }
            }
         }
      }
...
void GetFileListFromVSTemplateFile(_str filename, int FileListViewId, bool ConvertToAbsolute)
{
...
static void GetFileListFromVSDatabaseFile(_str filename, int FileListViewId, bool ConvertToAbsolute)
{
   _str file_array[];
   _str line;
                                             ...
                                             int index)
{
   ...
   if (iCurrentVCSIsScc && _haveVersionControl() && _SccGetCurProjectInfo(1)!="") {
...
static int GetFileListFromJBuilderFile(_str jbFilename, int FileListViewId, _str (&FileArray)[]=null,
                                       ...
                                       int handle=-1, bool ConvertToAbsolute=true,
                                       bool listboxFormat=false)
{
   status := 0;

   // get the path to the project for use in absolute()
   jbFilePath := _strip_filename(jbFilename, "N");
...
bool _ExcludeMatches(_str wildcard,_str filename)
{
   if (_last_char(wildcard)==FILESEP) {
      DirNamePart := substr(wildcard,1,length(wildcard)-1);
...
bool _FilespecMatches(_str filespec,_str filename)
{
   if( filespec=='*' ||  filespec==ALLFILES_RE) return(true);
   filespec=strip(filespec);
   _str filespec_re=_Wildcard2RE(filespec);
   typeless p=pos(filespec_re,filename,1,'r'_fpos_case);
   return( p!=0 );
}
int _GetEclipsePathList(_str MakefilePath,_str (&pathList)[],bool getDeps=false)
{
   status := 0;
   filename := MakefilePath:+'.classpath';
   int handle=_xmlcfg_open(filename,status);
   if (status) return(status);
   int index=_xmlcfg_get_first_child(handle,TREE_ROOT_INDEX);
   if (index>-1) {
      cap := _xmlcfg_get_name(handle,index);
      ...
      if (index>-1) {
         for (;;) {
            ...
            if (getDeps) {
               ...
               if (kind=='src') {
                  path := absolute(_xmlcfg_get_attribute(handle,index,"path",''),MakefilePath);
                  if (path!='') {
...
void _ExpandFileView2(int handle,int FileNode,_str RelFileName,_str (&pathList)[],
                      bool BuildTree=false, bool doAbsolute=true,bool listboxFormat=false,
                      int (&folderNodeHash):[]=null)
{
   ...
   for (i=0;i<pathList._length();++i) {
      // Use insert_file_list so get ant-like file matching
      int orig_wid=_create_temp_view(auto list_wid);
      insert_file_list('-v +p 'treeOption' '_maybe_quote_filename(absolute(RelFileName,pathList[i])));
      top();up();
      outerloop:
      while (!down()) {
      //for (ff=1;;ff=0) {
         //_str cur=file_match(_maybe_quote_filename(absolute(RelFileName,pathList[i])):+treeOption,ff);
         //if (cur=='') break;
         get_line(auto cur);cur=substr(cur,2);
         justname := _GetLastDirName(cur);
         if (justname=='.' || justname=='..') {
            continue;
         }

         // ignore directories if not building tree
         isFolder := (_strip_filename(cur,"P") == "");
         ...
         if (_FileRegexMatchExcludePath(excludeWildcards,cur)) {
            continue outerloop;
         }
         ...
         if (BuildTree) {
            ...
            if(!folderNodeHash._indexin(path)) {
               ...
               partialPath := relative(path, absolute(pathList[i]));
               rebuiltPath := substr(path, 1, length(path) - length(partialPath));
               ...
               for(;;) {
                  ...
                  nextFolder := "";
                  filesepPos := pos(FILESEP, partialPath);
                  if(filesepPos > 0) {
                  ...
                  rebuiltPath :+= nextFolder :+ FILESEP;
   ...
   //_showxml(handle, TREE_ROOT_INDEX, VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE);
}

bool _ExpandFileView(int handle,int FileListViewId,_str (&pathList)[],int append_after_ln=0,
                     bool listboxFormat=false)
{
   hasWildCards := false;
   orig_view_id := p_window_id;
...
void _ExpandXMLFilesNode(int handle,int FilesNode)
{
   ...
   for(i = 0; i < nodeList._length(); i++) {
      ...
      path := _strip_filename(name, "N");
      wildcard := _strip_filename(name, "P");
      _str pathList[] = null;
...
static const MSVC_HEADER_STRING= '# Microsoft Developer Studio';
static const MAKEFILE_INI_FILE= 'makefile.ini';

static const BEGIN_GROUP_PREFIX=      '# Begin Group "';
static const END_GROUP_PREFIX=        '# End Group';
static const EXT_LIST_PREFIX=         '# PROP Default_Filter "';
static const BEGIN_FILE_PREFIX=       '# Begin Source File';
static const END_FILE_PREFIX=         '# End Source File';
static const END_TARGET_PREFIX=       '# End Target';
static const SOURCE_FILE_PREFIX=      'SOURCE=';
static const EXCLUDE_FROM_BUILD_LINE= '# PROP Exclude_From_Build 1';
static const NAME_PREFIX=             '# Name';
...
{
   NewPath := _strip_filename(Path,'N');
...
static _str GetWin32FileCase(_str filename)
{
   ...
   newfilename := "";
   filename_is_path := _last_char(filename)==FILESEP;
   if (filename_is_path) {
   ...
   for (;;) {
      ...
      curfilename := file_match(_maybe_quote_filename(filename)' -p +d',1);
      ...
      if (newfilename=='') {
         ...
         //Strip off the path
         nextpiece := _strip_filename(curfilename,'P');
         ...
         //then strip the path
         nextpiece := _strip_filename(substr(curfilename,1,length(curfilename)-1),'P');
...
static const VERSION_MARKER_PREFIX= '# Microsoft Developer Studio Generated Build File, Format Version';
 ...
 */
static bool IsValidVisualStudioProjectFile(bool &isWhidbey_csharp=null)
{
   dummy := false;
 ...
 * @return True if we recognize it
 */
static bool IsValidDSPVersion(bool displayError=true)
{
   top();
 ...
 */
bool _CanWriteFileSection(_str AnyProjectFilename=_project_name,bool displayError=true)
{
   ...
   if(_file_eq( _get_extension(AnyProjectFilename,true), JBUILDER_PROJECT_EXT)) {
      return true;
   }
   ...
   if ( !_file_eq( ext,VCPP_PROJECT_FILE_EXT) &&
        !_file_eq( ext,VISUAL_STUDIO_CSHARP_PROJECT_EXT) &&
   ...
   bool isWhidbey_csharp;
   writeable := IsValidVisualStudioProjectFile(isWhidbey_csharp);
   if (!writeable) {
      writeable=IsValidDSPVersion(displayError);
   }
...
int SaveAssociatedProjectFiles(int _srcfile_list_view_id,_str ProjectFilename=_project_name)
{
   ...
   _str ext=_get_extension(makefilename,true);
   if ( _file_eq(ext, VCPP_PROJECT_FILE_EXT) ) {
      ...
      ProjectPath := _strip_filename(makefilename,'N');
      ...
      while (!down()) {
         ...
         get_line(line);
         temp := absolute(line,MakefilePath);
         if (_file_eq(_get_extension(temp),'rc')) {
            have_rc_file=true;
         }
      ...
      for (i=0;i<FilesInMakefile._length();++i) {
         ...
         if (_haveContextTagging()) {
            ...
            abs_filename := absolute(FilesInMakefile[i],_strip_filename(ProjectFilename,'N'));
      ...
      if (_haveContextTagging()) {
         ...
         len := AbsNewFiles._length();
         for (i=0;i<len;++i) {
            RetagFile(AbsNewFiles[i], useThread, 0, null, tag_filename);
   ...
   }else if(_file_eq(_get_extension(makefilename,true), PRJ_FILE_EXT)) {
...
static int AddFileToVisualStudioVCPPProject(_str FileList,_str VisualStudioProjectName,bool FileExistsOnDisk=true)
{
   int temp_view_id;
...
static int AddFileToVisualStudioVCXProject(_str FileList,_str VisualStudioProjectName,bool FileExistsOnDisk=true)
{
   int status;
   ...
   status=WriteVisualStudioVCXProjectFile2(VisualStudioProjectName,temp_view_id,0,handle,filter,FileExistsOnDisk,true);
   _delete_temp_view(temp_view_id);
   return(status);
}
                                            ...
                                            bool doTagging)
{
   ...
   int status;
   _str TagFilename=workspace_tags_filename_only();
   int FileToNode:[];
   ...
   if (RemoveViewId) {
      open_status := 0;
      if (_haveContextTagging() && doTagging) {
      ...
      for (;;) {
         ...
         if (open_status >= 0 && doTagging && _haveContextTagging()) {
   ...
   if (doTagging) {
      ...
      vse_project_name := _strip_filename(VisualStudioProjectFilename,'E'):+PRJ_FILE_EXT;
      tag_add_viewlist(TagFilename,temp_view_id,vse_project_name,false,FileExistsOnDisk,useThread);
   }
...
static int WriteVisualStudioVCPPProjectFile2(_str VisualStudioProjectName,
                                             ...
                                             bool doTagging=false,
                                             bool &removed_a_file=false)
{
   vsVCPPIndent := -1;  // use tabs
   ...
   if (doTagging) {
      useThread := _is_background_tagging_enabled(AUTOTAG_LANGUAGE_NO_THREADS);
      vse_project_name := _strip_filename(VisualStudioProjectName,'E'):+PRJ_FILE_EXT;
      tag_add_viewlist(TagFilename,temp_view_id,vse_project_name,false,FileExistsOnDisk,useThread);
   }
...
static int AddFileToVisualStudioStandardProject(_str FileList,_str VisualStudioProjectName,
                                                _str ext,bool FileExistsOnDisk=true)
{
   ...
   handle := _xmlcfg_open(VisualStudioProjectName, auto xcstatus, VSXMLCFG_OPEN_REFCOUNT|VSXMLCFG_OPEN_ADD_PCDATA);
   if (handle > 0) {
                                                ...
                                                _str ext)
{
   ...
   _str CompileExtensionList=GetVSStandardExt(ext);
   int status;
   int handle=_xmlcfg_open(VisualStudioProjectFilename,status,VSXMLCFG_OPEN_ADD_NONWHITESPACE_PCDATA);
   if (handle<0) {
      return(status);
   }
   FileListViewId := -1;
                                                 ...
                                                 bool &removed_a_file=false)
{
   int status;
   if (handle<0) {
      handle=_xmlcfg_open(VisualStudioProjectFilename,status,VSXMLCFG_OPEN_ADD_NONWHITESPACE_PCDATA);
      if (handle<0) {
         return(status);
      }
   }
   ...
   if(firstChild >= 0) {
      rootName := _xmlcfg_get_name(handle, firstChild);
      if(rootName == 'VisualStudioProject') {
         // This is a VS2002/2003 project file
         return WriteVisualStudioStandardProjectFile2003(VisualStudioProjectFilename,AppName,CompileExtensionList,RemoveViewId,AddViewId,handle,FileExistsOnDisk,doTagging,removed_a_file);
         ...
         return WriteVisualStudioStandardProjectFile2005(VisualStudioProjectFilename,AppName,CompileExtensionList,RemoveViewId,AddViewId,handle,FileExistsOnDisk,doTagging,removed_a_file);
      }
   }
   return -1;
}
                                                    ...
                                                    bool &removed_a_file=false)
{
      ...
      for (;;) {
         ...
         if (pnode) {
            ...
            if(GetVS2005DependentNodes(handle, nodeToDelete, RelFilename, dependentNodes, dependentNodeFileNames)) {
               ...
               numDeps := dependentNodes._length();
               depIdx := 0;
               typeless oldPos;
               save_pos(oldPos);
            ...
            removed_a_file=true;
            node_to_delete:=*pnode;
            if (_xmlcfg_get_name(handle,*pnode)=='HintPath') {
               parent_node:=_xmlcfg_get_parent(handle,*pnode);
               // Delete parent Reference node.
               node_to_delete=parent_node;
            }
            prev_node:=_xmlcfg_get_prev_sibling(handle,node_to_delete,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END|VSXMLCFG_NODE_PCDATA|VSXMLCFG_NODE_CDATA|VSXMLCFG_NODE_COMMENT|VSXMLCFG_NODE_PROCESSING_INSTRUCTION);
            if (prev_node>=0) {
               if (_xmlcfg_get_type(handle,prev_node)==VSXMLCFG_NODE_PCDATA) {
                  _xmlcfg_delete(handle,prev_node);
               }
            }
            _xmlcfg_delete(handle,node_to_delete);
         }
      }
      p_window_id=orig_view_id;
   }
   ...
   bool AutoAdd=_csproj2005Get_AutoAddWildcards(handle);
   ...
   while (!down()) {
      ...
      // Don't double-add files
      if (!FileToNode._indexin(_file_case(RelFilename))) {
         if (AutoAdd && !pos('..',RelFilename)) {
            // This file is beneath this project. Don't add an XML element.
            continue;
         }
         ...
         fileTaskName := "None";
         _str fileExt = _get_extension(RelFilename);
         if (file_eq(fileExt,'dll')) {
            int ref_node =_xmlcfg_add(handle,itemGroupNode,'Reference',VSXMLCFG_NODE_ELEMENT_START_END,flags);
            int prev_node=_xmlcfg_get_prev_sibling(handle,ref_node,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END|VSXMLCFG_NODE_PCDATA);
            if (prev_node>=0) {
               if (_xmlcfg_get_type(handle,prev_node)==VSXMLCFG_NODE_PCDATA) {
                  text:=_xmlcfg_get_value(handle,prev_node);
                  text=strip(text,'T'):+substr('',vs2005Indent*2);
                  _xmlcfg_set_value(handle,prev_node,text);
               }
            }
            _xmlcfg_add_attribute(handle,ref_node,"Include",_strip_filename(RelFilename,'e'),VSXMLCFG_ADD_ATTR_AT_END);
            // Add the Include attribute to the item that we just added
            int hint_node =_xmlcfg_add(handle,ref_node,'HintPath',VSXMLCFG_NODE_ELEMENT_START_END,VSXMLCFG_ADD_AS_CHILD);
            _xmlcfg_add(handle,hint_node,RelFilename,VSXMLCFG_NODE_PCDATA,VSXMLCFG_ADD_AS_CHILD);
         } else {
            ...
            int fileTaskNode =_xmlcfg_add(handle,parentGroupNode,fileTaskName,VSXMLCFG_NODE_ELEMENT_START_END,flags);
            int prev_node=_xmlcfg_get_prev_sibling(handle,fileTaskNode,VSXMLCFG_NODE_ELEMENT_START|VSXMLCFG_NODE_ELEMENT_START_END|VSXMLCFG_NODE_PCDATA);
            if (prev_node>=0) {
               if (_xmlcfg_get_type(handle,prev_node)==VSXMLCFG_NODE_PCDATA) {
                  text:=_xmlcfg_get_value(handle,prev_node);
                  text=strip(text,'T'):+substr('',vs2005Indent*2);
   ...
   // Can't sort this because this xml file was opened with the VSXMLCFG_OPEN_ADD_PCDATA flag.
...
{
   ExtToTask._makeempty();
   if (_file_eq(projectExt,VISUAL_STUDIO_CSHARP_PROJECT_EXT) ||
       _file_eq(projectExt,VISUAL_STUDIO_CSHARP_DEVICE_PROJECT_EXT)) {
      ExtToTask:['cs'] = 'Compile';
   }
   if (_file_eq(projectExt,VISUAL_STUDIO_VB_PROJECT_EXT) ||
       _file_eq(projectExt,VISUAL_STUDIO_VB_DEVICE_PROJECT_EXT)) {
      ExtToTask:['vb'] = 'Compile';
   }
   if (_file_eq(projectExt,VISUAL_STUDIO_FSHARP_PROJECT_EXT)) {
      ExtToTask:['fs'] = 'Compile';
   }
   if (_file_eq(projectExt,VISUAL_STUDIO_JSHARP_PROJECT_EXT)) {
...
static bool GetVS2005DependentNodes(int xmlHandle, int parentNode, _str parentNodeFilename, int (&depNodes)[], _str (&depNodeFiles)[])
{
   depNodes._makeempty();
   ...
   if(rootSearch > 0)
   {
      ...
      if(_xmlcfg_find_simple_array(xmlHandle, '//DependentUpon', dependencyNodes, rootSearch) == 0)
      {
         ...
         for(; dIdx < dMax; ++dIdx)
         {
            ...
            if(_file_eq(dependsUpon, parentNodeFilename))
            {
...
static int AddFileToVCPP6ProjectFile(_str GroupNames[],_str filename,_str MakefilePath)
{
   ...
   for (;;) {
      ...
      absfilename_path := absolute(_file_case(filename),MakefilePath);
      absfilename_path=_file_case(_strip_filename(absfilename_path,'N'));

      just_last_filename := _strip_filename(LastFilename,'P');
      just_cur_filename := _strip_filename(CurFilename,'P');

      if (just_filename >= just_last_filename &&
          just_filename <= just_cur_filename) {
         if (!had_filename) {
            had_filename=_file_eq(just_filename,just_last_filename)!=0;
         }
         ...
         if (
             ...
             (had_filename && !_file_eq(just_filename,just_cur_filename))
             ) {
                           ...
                           bool Quiet=false)
{
   if (VCPPProjectName=='') {
      ...
      if (status) return(status);
      if (_file_eq(_get_extension(VCPPProjectName,true),VISUAL_STUDIO_INTEL_CPP_PROJECT_EXT)) {
         VCPPProjectName = absolute(getICProjAssociatedProjectFile(VCPPProjectName),_strip_filename(_project_name,'N'));
      }

   } else if (_file_eq(_get_extension(VCPPProjectName,true),PRJ_FILE_EXT)) {
      //Find the associated makefile(.dsp file).
      //int status=_ini_get_value(_project_name,"ASSOCIATION","makefile",VCPPProjectName,'');
   ...
   for (;;) {
      ...
      if (open_status >= 0 && _haveContextTagging()) {
         RetagFile(_AbsoluteToProject(cur),useThread,0,null,tag_filename);
      }
   }
...
int _RemoveFileFromVCPPMakefile(_str filelist,_str VCPPProjectName='',
                                bool Quiet=false,bool &removed_a_file=false)
{
   if (VCPPProjectName=='') {
      //Find the associated makefile(.dsp file).
      //int status=_ini_get_value(_project_name,"ASSOCIATION","makefile",VCPPProjectName,'');
   ...
   vse_project_name := _strip_filename(VCPPProjectName,'E'):+PRJ_FILE_EXT;
   for (;;) {
      ...
      status=RemoveFileFromVCPP6ProjectFile(filename,vse_project_name,removed_a_file);
      if (status && !Quiet) {
...
int _RemoveFileFromVisualStudioProject(_str FileList,_str ProjectFilename,bool &removed_a_file=false)
{
   _str ext=_get_extension(ProjectFilename,true);
   if (_file_eq(ext,VISUAL_STUDIO_INTEL_CPP_PROJECT_EXT)) {
      ...
      ext=_get_extension(ProjectFilename,true);
   }
   if (_file_eq(ext,VISUAL_STUDIO_VCPP_PROJECT_EXT)) {
      return(RemoveFileFromVisualStudioVCPPProject(FileList,ProjectFilename,removed_a_file));
   }

   return(RemoveFileFromVisualStudioStandardProject(FileList,ProjectFilename,ext,removed_a_file));
}

int _AddFileToVisualStudioProject(_str FileList,_str ProjectFilename,bool FileExistsOnDisk=true)
{
   _str ext=_get_extension(ProjectFilename,true);
   if (_file_eq(ext,VISUAL_STUDIO_INTEL_CPP_PROJECT_EXT)) {
      ProjectFilename = absolute(getICProjAssociatedProjectFile(ProjectFilename),_strip_filename(ProjectFilename,'N'));
   ...
   if (_file_eq(ext,VISUAL_STUDIO_VCX_PROJECT_EXT)) {
      return(AddFileToVisualStudioVCXProject(FileList,ProjectFilename,FileExistsOnDisk));
   }
 ...
 * @return 0 if succesful
 */
static int RemoveFileFromVisualStudioVCPPProject(_str FileList,_str ProjectFilename,bool &removed_a_file=false)
{
   int temp_view_id;
   _str cur;
   int orig_view_id=_create_temp_view(temp_view_id);
   ...
   p_window_id=orig_view_id;
   int status=WriteVisualStudioVCPPProjectFile2(ProjectFilename,0,temp_view_id,-1,true,false,removed_a_file);
   _delete_temp_view(temp_view_id);
   return(status);
}
                                                     ...
                                                     bool &removed_a_file=false)
{
   ...
   int status=WriteVisualStudioStandardProjectFile2(ProjectFilename,
                                                ...
                                                CompileExtensionList,
                                                temp_view_id,
                                                0,-1,true,false,removed_a_file);
   _delete_temp_view(temp_view_id);
   _ProjectCache_Update(ProjectFilename);
   return(status);
}
 ...
 * @return 0 if successful
 */
static int RemoveFileFromVCPP6ProjectFile(_str RelativeFilename,_str project_name=_project_name,bool &removed_a_file=false)
{
...
static void GetVCPPGroups(int MakeFilenameViewId,_str (&Groups)[])
{
   ...
   for (;;) {
      ...
      cur_group := substr(line,length(EXT_LIST_PREFIX)+1);
 ...
 * @return 0 on success, <0 on failure
 */
int _RemoveFilesFromJBuilderProject(_str fileList, _str projectName,bool &removed_a_file=false)
{
   // create temp view and insert the file list one file per line
   int tempViewID;
   int origViewID = _create_temp_view(tempViewID);
   ...
   // update the jbuilder project
   int status=_WriteJBuilderProject(projectName, tempViewID, 0,-1,true,false,removed_a_file);
...
int _WriteJBuilderProject(_str projectName, int removeViewId, int addViewId, int handle = -1,
                          bool fileExistsOnDisk = true, bool doTagging = false,bool &removed_a_file=false)
{
   status := 0;
...
void _ConvertViewToAbsolute(int ViewId, _str WorkingDir, int iFirstLine=0,
                            bool listboxFormat=false,
                            bool parseProjectCommand=false,
                            bool convertToAbsolute=true)
{
   orig_view_id := p_window_id;
   p_window_id=ViewId;
...
bool _IsMSBuildProj(_str WorkspaceFilename, _str AssociatedFileType)
{
   if (_file_eq(_get_extension(WorkspaceFilename,true),VISUAL_STUDIO_SOLUTION_EXT)) {
      switch (AssociatedFileType) {
      case VISUAL_STUDIO_CSHARP_VENDOR_NAME:
      case VISUAL_STUDIO_VB_VENDOR_NAME:
      case VISUAL_STUDIO_CSHARP_DEVICE_VENDOR_NAME:
      case VISUAL_STUDIO_VB_DEVICE_VENDOR_NAME:
      case VISUAL_STUDIO_FSHARP_VENDOR_NAME:
      case VISUAL_STUDIO_JSHARP_VENDOR_NAME:
      case VISUAL_STUDIO_MSBUILD_VENDOR_NAME:
                                      ...
                                      int iProjectIndex)
{
   ...
   if (!strieq(AutoFolders,VPJ_AUTOFOLDERS_CUSTOMVIEW)) {
      ...
      _ProjectBuildTree(_workspace_filename,
                        ProjectName, 
                        temp_handle,
   ...
   if (_file_eq(ext,VISUAL_STUDIO_VCPP_PROJECT_EXT) ||
       _file_eq(ext,VISUAL_STUDIO_INTEL_CPP_PROJECT_EXT) ||
       _file_eq(ext, JBUILDER_PROJECT_EXT) ||
       _file_eq(ext, MACROMEDIA_FLASH_PROJECT_EXT)
       ) {
      int status;
      ...
      if (cachedHandle>=0) {
         ...
         if(_file_eq(_get_extension(AssociatedFile,true),JBUILDER_PROJECT_EXT)) {
      ...
      _InsertVCXProjectHierarchy(_workspace_filename, AssociatedFile, 0, p_window_id, iProjectIndex);

   } else if (_IsMSBuildProj(_workspace_filename, AssociatedFileType)) {
      path := _strip_filename(ProjectName,'N');
      temp_handle := _ProjectCreate(path);
      FilesNode := _ProjectGet_FilesNode(temp_handle, true);
      assoc_handle := _ProjectAssociatedHandle(AssociatedFile);
      _MSBuildGetCustomView(assoc_handle, temp_handle, FilesNode, path);
     ...
     _xmlcfg_close(temp_handle);
      ...
      _str vartab:[];
      ProjectDir := _strip_filename(AssociatedFile,'N');
      bool hashtab:[];
      iUsingScc := false;
      int StatusList[];
      ...
      pbxprojPath := absolute("project.pbxproj", AssociatedFile);
 ...
 */
void _convertJBuilderXML(int handle, bool toVSEFormat)
{
   ...
   //_showxml(handle, TREE_ROOT_INDEX, VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE, 3);
}
...
//Make file import section
static const MAXMAKEFILEINCLUDEDEPTH= 10;
...
static void MI_compressTrailingWS(_str &content) {
   if (content._length() && (_last_char(content) == ' ' || _last_char(content) == "\t")) {
...
static const RM_FAILURE= -1;
static const RM_NO_SUBSTITUTION= 0;
static const RM_SUBSTITUTION= 1;
...
int resolveMacros(MI_makefileMacro_t (&makefileMacros):[], _str& value, _str usedKeys:[], _str targetName = '', _str preReqNames = '') {
   ...
   posResult := macroName := macroExpansion := newValue := "";
   posStatus := macroPos(value, 1, macroName, matchPos, matchLen);
...
_str resolveSpecialMacros(_str value, _str targetName = '', _str preReqNames = '') {
   matchPos := matchLen := 0;
   matchPos0 := matchLen0 := 0;
   posResult := posResult0 := newValue := "";
   posStatus := pos(specialMacroRefRegex, value, 1, 'R');
   while (posStatus) {
      matchPos = pos('S');
      matchLen = pos('');
      matchPos0 = pos('S0');
...
int macroPos(_str value, int startPos, _str& matchStr, int &matchPos, int &matchLen) {
   status1 := pos('\${#0\$\@|[_:a\@\?\*\<\%\(\{]}', value, startPos, 'R');
   if (!status1) {
      return 0;
   }
   posLen := pos('0');
   ...
   if (posLen == 1) {
      ...
      firstChar := substr(value, matchPos + 1, 1);
...
static bool hasContinuation(_str s)
{
   c := _charAt(s, s._length());

   return c == "\\";
}
static _str removeContinuation(_str s)
{
   if (hasContinuation(s)) {
      return strip(substr(s, 1, s._length() - 1));
   } else {
      return s;
   }
}

// Assumes that the cursor is at the start of the RHS text, right after
// the = or :   Advances the cursor past the RHS, and returns it in a string.
static _str getRHSide()
{
   hitEnd := false;

   actCol := _text_colc(p_col, 'P'); 

   get_line(auto line);
   line = strip(line);
   line = substr(line, actCol);
   rv := '';

   for (;;) {
      rv :+= ' ' :+ removeContinuation(line);
      ln := p_line;
      p_line++;

      hitEnd = (ln == p_line);
      if (hitEnd || !hasContinuation(line)) break;
 ...
 * @return int
 */
int readMakefileMacros(_str makefileName, _str makefileDir, _str (&makefileFiles):[], MI_makefileMacro_t (&makefileMacros):[], bool checkIfMakefile, int includeDepth = 0)
{
 ...
 * @return int
 */
int readMakefileTargets(_str makefileName, _str startDirectory, _str (&makefileFiles):[], MI_makefileTarget_t (&makefileTargets):[], MI_makefileMacro_t (&makefileMacros):[], _str &firstTargetName, bool parseCommands = false, int includeDepth = 0)
{
   makefileName = absolute(makefileName);
   if (startDirectory :== '') {
...
void MI_parseRecursiveMakefileCalls(_str (&commands)[], _str startDirectory, bool debug = false) {
   ...
   for (i = 0; i < commands._length(); i++) {
      ...
      if (status) {
         if (substr(firstToken, pos('S'), pos('')) :== 'cd') {
            ...
            if (status) {
               newMakefile := substr(remainder' ', pos('S') + 2);
...
int MI_makefile_GetTargetList(_str filename, _str startDirectory, _str (&targetList)[], _str (&descriptionList)[], bool checkIfMakefile = true)
{
...
int MI_getFilesInMakefile(_str makefileName, _str makefileDir, 
                          ...
                          bool filterList = false, bool checkIfMakefile = true,
                          _str wildcards = '*', _str file_exclude = '',
                          bool checkRecursiveFiles = false,
                          bool isQTMakefile = false) {
   ...
   for (el._makeempty();;) {
       ...
       while (true) {
          ...
          if (file_exists(prereq) && !preReqFiles._indexin(prereq)) {
             ...
             prereqFilename := _strip_filename(prereq, 'P');
             prereqDirname := _strip_filename(prereq, 'N');
...
//int MI_temp_printCommands(_str makefilename, _str (&filesList)[], bool promptForFilters = false) {
//
//   if (!file_exists(makefilename)) return 1;
//   int status;
//   _str makefileFiles:[];
...
_command int checkQTMakefileDate() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   return 0;
}
_command int checkedQTMakefileDate() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   return 0;
}
...
static const MAKEFILECAPTION= "Open Makefile as Workspace";
static const QTMAKEFILECAPTION= "Open QT Makefile as Workspace";
...
{
   makefile := ctlListFile.p_text;
   makefileDirectory := get_env("HOME");
   ...
   ctlListFile.p_text = strip(makefile,'B','"');
}

static const OBJECTFILEEXCLUDES= "Object Files (*.o;*.so;*.a)";
static void _init_includeFileTypes(bool forceRefresh = false)
{
   if (forceRefresh) ctl_MI_includeFileTypes_combo.p_user = '';
   if (ctl_MI_includeFileTypes_combo.p_user == '') {
      ctl_MI_includeFileTypes_combo.add_filetypes_to_combo(def_file_types);
      ctl_MI_includeFileTypes_combo.p_user = 1; // Indicate that retrieve list has been done
   }
}

static void _init_excludeFileTypes(bool forceRefresh = false)
{
   if (forceRefresh) ctl_MI_excludeFileTypes_combo.p_user = '';
   if (ctl_MI_excludeFileTypes_combo.p_user == '') {
      ctl_MI_excludeFileTypes_combo.add_filetypes_to_combo(OBJECTFILEEXCLUDES);
      ctl_MI_excludeFileTypes_combo.p_user = 1; // Indicate that retrieve list has been done
   }
}

void ctl_MI_includeFileTypes_combo.on_create() {
   _init_includeFileTypes();
}

void ctl_MI_excludeFileTypes_combo.on_create() {
