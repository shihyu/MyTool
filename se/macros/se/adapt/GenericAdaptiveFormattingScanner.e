...
class GenericAdaptiveFormattingScanner : se.adapt.AdaptiveFormattingScannerBase {
   ...
   protected int m_indentMinCol=2;
...
#region Examine methods
   ...
   protected void examineIndentCaseFromSwitch(_str idChars,_str posOptions)
   {
      ...
      if (p_line > orig_line && p_col > beginCol) {
         // For this switch, the brace is in brace style 3, which means we
         // need to change the column we test against to determine if the case
         // is indented or not.
         beginCol = p_col;
      }
      right();
   ...
   protected int examineBeginEndStyle()
   {
      ...
      ret := 0;
      isBraceStyle1 := p_line==beginLine;
   ...
   private void examineIndent(_str &prevLine,int &prevPos,bool hitPrevChar) {
      ...
      */
      ...
      lineDiv4 := (length(expand_tabs(line)) % 4)==0;
   ...
   protected void scan(int orig_flags, int cap = MAX_SCAN)
   {
      ...
      doIndent := (flags & (AFF_INDENT_WITH_TABS|AFF_SYNTAX_INDENT|AFF_TABS))!=0;
      ...
      while (flags) {
         ...
         if ((flags & (AFF_BEGIN_END_STYLE)) && 
            ...
            if (be_count>=cap) {
               ...
               if (!doIndent && searchString=='') break;
               completeRE=makeCompleRE(doIndent,prevLine,searchString);
            }
         }
      ...
      doCasing := flags && (m_available & AFF_KEYWORD_CASING);
      if (doCasing) {
         top();
         findNextEmbeddedRegion(embedLex);
         kcount := 0;
