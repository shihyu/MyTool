...
class GenericAutoBracket : DefaultAutoBracket {
   ...
   protected bool genericCheckQuote(_str &key) {
      save_pos(auto p);
         ...
         if (_clex_find(0, 'g') == CFG_STRING) {
            ...
            default:
               if (!testStringOp(ch, 1)) {
                  /* 
                     Typing something like print(<type-quote-here>a shouldn't
                     be any different than print(<type-quote-here>abc. Just
                     setting status to 1 here makes these cases the same.
                  */ 
                  status=1;
                  /*right();
                  if (p_col <= _text_colc()) {
