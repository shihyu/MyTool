...
class EventUI {
   ...
   static void removeListener(IKeyEventCallback *ui, int bufID = -1)
   {
      if (bufID < 0) {
         bufID = p_buf_id;
      }
   ...
   static void beautifySave(IKeyEventCallback* (&cb)[], int (&markerIndex)[], long (&markers)[], long (&cursorMarkerIndices)[], long startOffset, long endOffset)
   {
      cb._makeempty();
      bufID := p_buf_id;
      if (!s_buffers._indexin(bufID) || s_buffers:[bufID]._isempty() ||
      ...
      while (id >= 0) {
         ...
         len := markers._length();
         if (ev->save(markers, cursorMarkerIndices, startOffset, endOffset)) {
   ...
   static bool enableKeyEvents(bool newValue)
   {
      oldValue := s_enabled;
      s_enabled = newValue;
      return oldValue;
   ...
   static bool hasCallbacks()
   {
      bufID := p_buf_id;
      if (!s_enabled ||
          !s_buffers._indexin(bufID) || s_buffers:[bufID]._isempty() ||
          s_buffers:[bufID].m_eventList._isempty() ||
          s_buffers:[bufID].m_eventList._length() == 0) {
         return false;
      }
   ...
   private static bool onKeyCallback(_str key)
   {
      if (!s_enabled) {
         return(false);
      }
