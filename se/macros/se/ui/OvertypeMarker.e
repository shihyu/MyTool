...
class OvertypeMarker {
   ...
   private static bool testKey(_str& key)
   {
      ch := get_text();
      if (key != ch) {
         return(false);
   ...
   private static void enableCallback(bool enable)
   {
      _str keyarray[];
...
bool OvertypeListenerKeyin(_str key)
{
