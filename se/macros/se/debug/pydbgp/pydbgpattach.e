...
const PYDBGP_ATTACH_NOTE= '' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'        :+
          'When debugging Python you will typically want to set up a Python ':+
          'project in order to take advantage of managed breakpoints, file ' :+
          'organization, and Context Tagging. '                              :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'            :+
          'This dialog allows you to initiate a Python debugging session '       :+
          'in the absence of an active Python project. This can be useful when:' :+
          '<ul>'                                                                 :+
          '<li>Your current project is not a Python project, and </li>'          :+
          ...
          '<li>You have inserted hard breaks into your Python code '             :+
          'which will break execution in the debugger.'                          :+
          '</ul>'                                                                :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'     :+
          'Otherwise you are better served by creating a Python project.' :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'      :+
          'The Python debugger uses the DBGp client script (pydbgp.py). '  :+
          'Set the local host and port to the host:port that pydbgp will ' :+
          'attempt to connect to when initiating a debug sesson. '         :+
          '<a href="slickc:help Running and Debugging Python">See the help for '  :+
          'more information about setting up debugging with pydbgp</a>.'   :+
          '</p>';

/**
 * Start actively listening for connection from pydbgp. The user
 * is prompted for host:port settings. 
 * 
 * @return 0 on success, <0 on error.
 */
_command int pydbgp_attach() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
   ...
   // Fire up the pydbgp Attach dialog which will prompt for host:port
   // parameters and accept a pydbgp connection.
   _str attach_info = show("-modal _debug_pydbgp_remote_form");
   if( attach_info == "" ) {
      // User cancelled
   ...
   int status = debug_remote("pydbgp",attach_info,session_name);
   if( status != 0 ) {
      // Clean up
...
//
// _debug_pydbgp_remote_form
//

defeventtab _debug_pydbgp_remote_form;

void _debug_pydbgp_remote_form.'!'(se.net.ServerConnection* server=null, se.net.SERVER_CONNECTION_STATUS status=SCS_NONE)
{
   if( !server ) {
...
static const PYDBGP_ATTACH_FORM_MINWIDTH= 8640;
static const PYDBGP_ATTACH_FORM_MINHEIGHT= 7665;

void _debug_pydbgp_remote_form.on_resize()
{
   // Enforce sanity on size
   // if the minimum width has not been set, it will return 0
   if (!_minimum_width()) {
      _set_minimum_size(PYDBGP_ATTACH_FORM_MINWIDTH, PYDBGP_ATTACH_FORM_MINHEIGHT);
   }
...
void ctl_listen.on_create(bool settings_only=false)
{
   ...
   session_name := "";
   max_width := ctl_session_combo.debug_load_session_names("pydbgp", session_name);
   ...
   ctl_note.p_text = PYDBGP_ATTACH_NOTE;
   // Dialog color
...
void ctl_listen.lbutton_up()
{
   ...
   int status_or_socket = pydbgp_wait_and_accept(host,port,timeout,&dlg,true);
   ...
   int handle = pydbgp_peek_packet(status_or_socket);
   if( handle < 0 ) {
      // Error
