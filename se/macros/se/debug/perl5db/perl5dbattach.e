...
/**
 * Start actively listening for connection from perl5db. The user
 * is prompted for host:port settings. 
 * 
 * @return 0 on success, <0 on error.
 */
_command int perl5db_attach()
{
   if( debug_active() ) {
   ...
   // Fire up the perl5db Attach dialog which will prompt for host:port
   // parameters and accept a perl5db connection.
   _str attach_info = show("-modal _debug_perl5db_remote_form");
   if( attach_info == "" ) {
      // User cancelled
   ...
   int status = debug_remote("perl5db",attach_info,session_name);
   if( status != 0 ) {
      // Clean up
...
//
// _debug_perl5db_remote_form
//

defeventtab _debug_perl5db_remote_form;

void _debug_perl5db_remote_form.'!'(se.net.ServerConnection* server=null, se.net.SERVER_CONNECTION_STATUS status=SCS_NONE)
{
   if( !server ) {
...
static const PERL5DB_ATTACH_FORM_MINWIDTH= 8640;
static const PERL5DB_ATTACH_FORM_MINHEIGHT= 7665;

void _debug_perl5db_remote_form.on_resize()
{
   // Enforce sanity on size
   // if the minimum width has not been set, it will return 0
   if (!_minimum_width()) {
      _set_minimum_size(PERL5DB_ATTACH_FORM_MINWIDTH, PERL5DB_ATTACH_FORM_MINHEIGHT);
   }
...
void ctl_listen.on_create(bool settings_only=false)
{
   ...
   session_name := "";
   max_width := ctl_session_combo.debug_load_session_names("perl5db", session_name);
   ...
   ctl_note.p_text = PERL5DB_ATTACH_NOTE;
   // Dialog color
...
void ctl_listen.lbutton_up()
{
   ...
   int status_or_socket = perl5db_wait_and_accept(host,port,timeout,&dlg,true);
   ...
   int handle = perl5db_peek_packet(status_or_socket);
   if( handle < 0 ) {
      // Error
   ...
   perl5db_make_default_options(perl5db_opts);
   debugger_args :+= ' 'se.debug.dbgp.dbgp_make_debugger_args(perl5db_opts.dbgp_features);

   attach_info :=  'file='fileuri',,path=,args='debugger_args",session="session_name;
