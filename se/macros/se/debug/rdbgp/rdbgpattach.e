...
const RDBGP_ATTACH_NOTE= '' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'        :+
          'When debugging Ruby you will typically want to set up a Ruby ':+
          'project in order to take advantage of managed breakpoints, file ' :+
          'organization, and Context Tagging. '                              :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'            :+
          'This dialog allows you to initiate a Ruby debugging session '       :+
          'in the absence of an active Ruby project. This can be useful when:' :+
          '<ul>'                                                                 :+
          '<li>Your current project is not a Ruby project, and </li>'          :+
          ...
          '<li>You have inserted hard breaks into your Ruby code '             :+
          'which will break execution in the debugger.'                          :+
          '</ul>'                                                                :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'     :+
          'Otherwise you are better served by creating a Ruby project.' :+
          '</p>' :+
          '<p style="font-family:'VSDEFAULT_DIALOG_FONT_NAME'; font-size:10pt">'      :+
          'The Ruby debugger uses the DBGp client script (rdbgp.rb). '  :+
          'Set the local host and port to the host:port that rdbgp will ' :+
          'attempt to connect to when initiating a debug sesson. '         :+
          '<a href="slickc:help Running and Debugging Ruby">See the help for '  :+
          'more information about setting up debugging with rdbgp</a>.'   :+
          '</p>';
 ...
 */
_command int rdbgp_attach() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
...
static const RDBGP_ATTACH_FORM_MINWIDTH= 8640;
static const RDBGP_ATTACH_FORM_MINHEIGHT= 7665;
