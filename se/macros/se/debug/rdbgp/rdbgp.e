...
DBGpOptions rdbgp_make_default_options(DBGpOptions& rdbgp_opts=null)
{
 ...
 */
void rdbgp_project_get_options_for_config(int projectHandle, _str config, DBGpOptions& rdbgp_opts, DBGpOptions& default_rdbgp_opts=null)
{
   ...
   listenInBackground := default_rdbgp_opts.listenInBackground;
   DBGpRemoteFileMapping remoteFileMap[] = default_rdbgp_opts.remoteFileMap;
 ...
 *                             options present for config.
 */
void rdbgp_project_get_options(int projectHandle, _str (&configList)[], DBGpOptions (&rdbgp_opts_list):[], DBGpOptions& default_rdbgp_opts=null)
{
 ...
 */
bool rdbgp_is_pending(_str host, _str port)
{
 ...
 */
int rdbgp_wait(_str host, _str port, int timeout, ServerConnectionObserver* observer, bool close_on_error)
{
 ...
 */
int rdbgp_wait_and_accept(_str host, _str port, int timeout, ServerConnectionObserver* observer, bool close_if_first)
{
 ...
 */
bool rdbgp_is_listening(_str host, _str port)
{
 ...
 */
bool rdbgp_get_server_address(_str& host, _str& port)
{
...
static int rdbgp_get_options(_str projectName, DBGpOptions& rdbgp_opts)
{
   if( projectName != "" ) {
 ...
 * @return 0 on success, <0 on error.
 */
_command int rdbgp_project_watch(_str projectName=_project_name, bool honorListenInBackground=false) name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
   ...
   if( status != 0 ) {
      if( status != COMMAND_CANCELLED_RC ) {
         msg := "Invalid rdbgp host/port. Cannot listen for connection.";
 ...
 *                     name.
 */
_command void rdbgp_project_shutdown(_str projectName=_project_name) name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
      return;
   }
...
_command void rdbgp_project_watch_toggle(_str projectName=_project_name) name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Debugging");
      return;
