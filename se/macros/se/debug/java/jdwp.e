 ...
 * @return int 
 */
_command int debug_jdwp_start() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
...
int _OnUpdate_debug_jdwp_start(CMDUI &cmdui,int target_wid,_str command)
{
   if (!_haveDebugging()) {
...
///////////////////////////////////////////////////////////////////////////
// Callbacks for JDWP debugger integration
//
bool _jdwp_ToolbarSupported(_str FormName)
{
   switch (FormName) {
...
static bool is_gradle_dbg_cmd(_str debug_cmd)
{
   return (pos('gradle', debug_cmd, 1, 'i') != 0) && (pos('--debug-jvm', debug_cmd, 1, 'i') != 0);
}
...
bool _jdwp_ConfigNeedsDebugMenu(_str debug_command)
{
   return(is_gradle_dbg_cmd(debug_command) || pos('-Xrunjdwp:',debug_command,1,'i')!=0);
}
bool _jdwp_DebugCommandCaptureOutputRequiresConcurrentProcess(_str debug_command)
{
...
///////////////////////////////////////////////////////////////////////////
// Callbacks for JDWP debug other executable attach form
//
defeventtab _debug_jdwp_executable_form;
static void debug_gui_executable_ok(_str command)
{
   ...
   // get the class path, pass it by uncensored
   class_path := ctl_classpath.p_text;
   ...
   // that's all folks
   _save_form_response();
   p_active_form._delete_window("command="command",app="program_name",dir="dir_name",args="program_args",session="session_name",cp="class_path);
}
 ...
 */
static void _debug_jdwp_executable_form_initial_alignment()
{
...
void dbg_jdwp_list_exceptions(_str (&exception_list)[])
{
   ...
   for (i=0; i<n; ++i) {
      if (debug_class_derives_from(i,"java/lang/Exception",class_list,parent_list,result_list,tag_files,visited,1) ||
          debug_class_derives_from(i,"java.lang/Exception",class_list,parent_list,result_list,tag_files,visited,1)) {
