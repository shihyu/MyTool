 ...
 */
bool proto_is_pending(_str proto, _str host, _str port)
{
 ...
 */
bool proto_is_listening(_str proto, _str host, _str port)
{
 ...
 */
bool proto_get_server_address(_str proto, _str& host, _str& port)
{
   ServerConnection* server = ServerConnectionPool.findByAddr(host,port);
   if( !server ) {
      return false;
...
// DEBUG
_command void dbgp_list_all() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
...
// DEBUG
_command void dbgp_release_all() name_info(','VSARG2_REQUIRES_PRO_EDITION)
{
   if (!_haveDebugging()) {
      popup_nls_message(VSRC_FEATURE_REQUIRES_PRO_EDITION_1ARG, "Debugging");
      return;
