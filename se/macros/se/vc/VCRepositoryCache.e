...
static _str VCCACHEUPDATER_EXE() {
   return  ("vccacheupdtr":+EXTENSION_EXE);
}
...
class VCRepositoryCache {
   const SVN_CACHE_SUCCESS= 0;
   const SVN_INVALID_ARGS= -1;
   const SVN_DATABASE_ERROR= -2;
   const SVN_SHELL_ERROR= -3;
   const SVN_INCAPABLE_ERROR= -4;
   const SVN_LOG_PARSE_ERROR= -5;
   const SVN_TOO_LONG_ERROR= -6;
   ...
   */
    ...
    */
   public bool init(_str rootFolder, _str repositoryUrl, bool isReRun=false) 
   {
    ...
    * 
    * @return bool : true if async update is required, false if not
    */
   public bool requiresAsyncUpdate()
   {
      retVal := false;
      lastUpdateTimestamp := "";
      ...
      if ( lastUpdateTimestamp!="" ) {
         ...
         DateTime timeStamp = DateTime.fromTimeF(lastUpdateTimestamp);
    ...
    * @return int 
    */
   public int updateVersionCache(bool asyncUpdate)
   {
      if ( m_processID!=-1 ) return PROCESS_ALREADY_RUNNING_RC;

      // shell the vccacheupdtr executable
      exePath := get_env("VSLICKBIN1"):+VCCACHEUPDATER_EXE();
      if (file_exists(exePath) == false) {
         _message_box(nls("Unable to find %s.",VCCACHEUPDATER_EXE()));
         return -1;
      }
      // build the command
      command :=  exePath:+' ':+_SVNGetExeAndOptions()' ':+_maybe_quote_filename(m_repositoryUrl):+' ':+_maybe_quote_filename(m_cacheFileName):+' ':+_maybe_quote_filename(m_tempLogFileName):+' ':+CACHE_DB_VERSION;
      shellOptions := "PQ";
...
//      command :+= ' 1';
    ...
    */
   public int getRevisions(_str fileName, VCBranch& rootBranch, VCLabel (&labels)[], bool getAllBranches=true, bool getLabels=true)
   {
      i := j := k := 0;
      branchID := 0;
      VCBranch branch;
      ...
      if (lowcase(substr(url, 1, m_repositoryUrl._length())) != lowcase(m_repositoryUrl)) {
         ...
         trimmedUrl := substr(url, m_repositoryUrl._length() + 1);
   ...
   private void maybeLogDebugMessage(_str msg, bool includeTimestamp=false)
   {
      if (def_vccache_debug) {
   ...
   public bool isSvnCapable()
   {
      _str shellCmd = _SVNGetExeAndOptions()' log --xml -v -rHEAD ':+_maybe_quote_filename(m_repositoryUrl):+' > ':+_maybe_quote_filename(m_tempLogFileName);
