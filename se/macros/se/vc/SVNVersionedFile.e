...
class SVNVersionedFile: IVersionedFile {
   ...
   SVNVersionedFile(_str localFilename="",_str branchName="") {
   ...
   private int getHistoryInformation(_str localFilename) {
      status := 0;
      if ( m_historyInfo==null ) {
         mou_hour_glass(true);
         IVersionControl *pInterface = svcGetInterface("subversion");
         SVCHistoryInfo historyInfo[];
         status = pInterface->getHistoryInformation(m_localFilename,historyInfo);
         if ( status ) {
            return status;
         }
   ...
   int enumerateVersions(STRARRAY &versionList) {
      status := getHistoryInformation(m_localFilename);
      if ( status ) {
         return status;
      }
