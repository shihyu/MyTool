...
const OPTIONS_XML_LANGUAGE_ATTRIBUTE=          'Language';
const OPTIONS_XML_VC_PROVIDER_ATTRIBUTE=       'VCProvider';
...
class OptionsConfigurationXMLParser {
    ...
    */
   private bool doesXMLConfigFileExist()
   {
      return (m_xmlConfigHandle > 0);
   }
   ...
   public void close()
   {
      ...
      if (m_xmlConfigHandle > 0 && doesXMLConfigFileExist()) {
         _xmlcfg_save(m_xmlConfigHandle, -1, VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE);
   ...
   public void buildOptionsHistory(int changeFlags, _str (&history)[])
   {
      ...
      if (historyIndex > 0) {
         ...
         for (i = 0; i < history._length(); i++) {
            ...
            origPath := path;
            fillInLanguageOrVersionControlName(path, OPTIONS_XML_LANGUAGE_ATTRIBUTE);
            if (origPath == path) {
               fillInLanguageOrVersionControlName(path, OPTIONS_XML_VC_PROVIDER_ATTRIBUTE);
            }
   ...
   private void fillInLanguageOrVersionControlName(_str &path, _str searchString)
   {
      ...
      if (attrPos) {
         ...
         switch (searchString) {
         case OPTIONS_XML_LANGUAGE_ATTRIBUTE:
            if (id == ALL_LANGUAGES_ID) {
               id = 'All Languages';
            } else {
               id = _LangGetModeName(id);
            }
            if (id != '') section = id;
            break;
         case OPTIONS_XML_VC_PROVIDER_ATTRIBUTE:
   ...
   private _str convertTime(_str time)
   {
      ...
      hours := temps intdiv (60 * 60 * 1000);
      temps -= (hours * 60 * 60 * 1000);
      minutes := temps intdiv (60 * 1000);
      temps -= (minutes * 60 * 1000);
      seconds := temps intdiv 1000;
