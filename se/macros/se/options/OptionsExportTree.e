...
static const OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE=      "Export Options";
static const OPTIONS_EXPORT_ERROR_MESSAGE_TYPE=        "Export Options Error Message";
...
class OptionsExportTree : OptionsTree {
    ...
    *                      otherwise
    */
   public bool doesGroupHaveProtections()
   {
   ...
   public void export(_str package, _str protectionCode = "", _str groupName = "")
   {
      ...
      se.messages.MessageCollection* mCollection = setupErrorLog(OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE, OPTIONS_EXPORT_ERROR_MESSAGE_TYPE);
      
      // prepare the status dialog...
      progressWid := show_cancel_form("Exporting options", null, true, true);
      progressWid.refresh();
      len := nodes._length();
            ...
            for (j := 0; j < ps.getNumItems(); j++) {
               ...
               if (type == TMT_PROPERTY) {
                  ...
                  if (groupName == "" || properties:[pIndex] == EIIS_TRUE) {
                     if (p -> getDisplayValue() == PropertyGetterSetter.RETRIEVE_ERROR) {
                        // log this as an error
                        errorCaption := m_parser.getFrameCaption(itemIndex) " > "p -> getCaption();
                        logErrors(mCollection, 
                                  errorCaption, 
                                  PropertyGetterSetter.RETRIEVE_ERROR,
                                  OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE, 
                                  OPTIONS_EXPORT_ERROR_MESSAGE_TYPE);
                        error = true;
                     } else {
                        // now save it!
      ...
      if (!exportItems._length()) {
          ...
          if (status) {
             logErrors(mCollection, 
                       OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE,
                       "Error creating export package.  Error code "status,
                       OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE, 
                       OPTIONS_EXPORT_ERROR_MESSAGE_TYPE);
             error =  true;
          }
      ...
      endErrorLog(error, OPTIONS_EXPORT_ERROR_MESSAGE_SOURCE);
   }
   ...
   private int buildExportPackage(_str zipFile, _str (&files)[])
   {
      // create the zip file
      int zipStatus[];
      status := _ZipCreate(zipFile, files, zipStatus);
