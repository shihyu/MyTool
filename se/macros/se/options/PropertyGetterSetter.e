...
class PropertyGetterSetter {
   const RETRIEVE_ERROR= 'Error retrieving value';
    ...
    */
   private static bool displayToInt(_str value, int &newValue, ResolutionInfo resInfo) {
      if (isinteger(value)) {
         if (resInfo != null) {
    ...
    * @return bool      success of conversion
    */
   private static bool displayToLong(_str value, long &newValue, ResolutionInfo resInfo)
   {
      if (isnumber(value)) {
         if (resInfo != null) {
            // resInfo.Divide specifies how you get from value to
    ...
    * @return bool      success of conversion
    */
   private static bool displayToFloat(_str value, double &newValue, ResolutionInfo resInfo)
   {
      if (isnumber(value)) {
         if (resInfo != null) {
            // resInfo.Divide specifies how you get from value to 
    ...
    * @return bool      success of conversion   
    */
   private static bool displayToFlag(_str value, int &newValue, int flag, bool backwards)
   {
      trueFalse := strieq(value, 'true');
   ...
   private static bool translateFlagValue(SettingInfo &info, int &value)
   {
      curSet := getSetting(info, false);
      if (!isinteger(curSet)) {
         return false;
      }
      curValue := (int)curSet;
    ...
    * @return bool      success of conversion
    */
   private static bool displayToBool(_str value, typeless &newValue, BooleanInfo &binfo)
   {
    ...
    * @return bool      success of conversion
    */
   private static bool translateDisplayToValue(SettingInfo &info, typeless &value)
   {
      typeless newValue = 0;
      status := false;
    ...
    * @return _str      boolean string
    */
   private static _str flagToDisplay(int value, int flag, bool backwards)
   {
      trueFalse := ((value & flag) == flag);
   ...
   private static _str intToDisplay(int value, ResolutionInfo &resInfo)
   {
      if (resInfo != null && isinteger(value)) {
         ...
         if (resInfo.Divide) {
            remainder:=(int)value%resInfo.Value;
            value = value / resInfo.Value;
            if (remainder>=(resInfo.Value intdiv 2)) {
               ++value;
            }
         } else{
            value = value * resInfo.Value;
   ...
   private static _str longToDisplay(long value, ResolutionInfo &resInfo)
   {
      if (resInfo != null && isnumber(value)) {
         ...
         // the value to the display value
         if (resInfo.Divide) {
            remainder:=(long)value%resInfo.Value;
            value = value / resInfo.Value;
            if (remainder>=(resInfo.Value intdiv 2)) {
               ++value;
            }
   ...
   private static _str floatToDisplay(double value, ResolutionInfo &resInfo)
   {
      ...
      if (resInfo != null && isnumber(value)) {
         if (resInfo.Divide) {
            remainder:=(double)value%resInfo.Value;
    ...
    * @return bool      success of conversion
    */
   private static bool translateValueToDisplay(SettingInfo &info, typeless &value)
   {
      bool status;
   ...
   public static void getCheckedSetting(SettingInfo &info, _str &value, int &checkState, bool translateToDisplay = true)
   {
    ...
    *                string.
    */
   public static _str getSetting(SettingInfo &info, bool translateToDisplay = true)
   {
      PropertySetting ps;
    ...
    * 
    * @return bool      success of calling _spell_option - does 
    *                   not verify that value was changed.
    */
   private static bool setSpellOption(SetOptionInfo &soi, _str value)
   {
    ...
    * 
    * @return bool      success of calling _default_option - does 
    *                   not verify that value was changed.
    */
   private static bool setDefaultOption(SetOptionInfo soi, _str value)
   {
    ...
    *  
    * NOTE: This function does not support setting colors with an 
    * embedded value. If there is an embedded value, it is 
    * preserved.
    * 
    ...
    * 
    * @return bool      success of calling _default_color - does 
    *                   not verify that color was set correctly.
    */
   private static bool setDefaultColor(SetDefaultColorInfo &scoi, _str value)
   {
      typeless fg, bg, ff;
      ...
      if (scoi.Value) {
         ...
         _macro_call("_default_color("scoi.Name", ":+
                     ...
                     bg:+", ":+
                     "0x":+_dec2hex(ff):+");");
         property_name := lowcase(substr(scoi.Name,5));
         int handle=_plugin_get_property_xml(VSCFGPACKAGE_COLOR_PROFILES,def_color_scheme,property_name);
         if (handle>=0) {
            int attrs_node=_xmlcfg_get_document_element(handle);
            _xmlcfg_set_attribute(handle,attrs_node,"fg","0x":+_dec2hex(0x0000000FFFFFFFF & fg /*c.getForegroundColor(&this)*/));
            _xmlcfg_set_attribute(handle,attrs_node,"bg","0x":+_dec2hex(0x0000000FFFFFFFF & bg /*c.getBackgroundColor(&this)*/));
            _xmlcfg_set_attribute(handle,attrs_node,"flags","0x":+_dec2hex(ff /*c.getFontFlags(&this)*/));
            _plugin_set_property_xml(VSCFGPACKAGE_COLOR_PROFILES,def_color_scheme,VSCFGPROFILE_COLOR_VERSION,property_name,handle);
    ...
    * 
    * @return bool      whether set was successful - does not 
    *                   verify the new value of variable
    */
   private static bool setVariableProperty(SettingInfo &info, _str value)
   {
      SetVariableInfo svi = info.SettingTypeInfo;
      if (svi.Index > 0) {
         _set_var(svi.Index, value);
   
         if (info.ValueType == INT) _macro_append(svi.Name"=0x"_dec2hex((long)value)";");
    ...
    * 
    * @return bool      success of function call - does not verify 
    *                   that value was actually set
    *         
    * See {@link _getFunctionProperty}.
    */
   private static bool setFunctionProperty(SettingInfo &info, _str value, int checkState = null)
   {
      SetFunctionInfo sfi = info.SettingTypeInfo;
      if (sfi.Index > 0) {
         typeless extraArg = '';
    ...
    */
   private static bool setLanguageSetting(SettingInfo &info, _str value, int checkState = null)
   {
      SetLanguageSettingInfo slsi = info.SettingTypeInfo;
      if (index_callable(slsi.SetIndex)) {
         if (slsi.CreateNewLang && !_LangIsDefined(info.Language)) {
            if (_GetDefaultLanguageOptions(info.Language, auto options)) {
               _LangInitOptions(options,true, info.Language);
               _SetDefaultLanguageOptions(info.Language, options);
            }
         }
         if (info.GetCheckState) {
    ...
    */
   private static bool setVersionControlSetting(SettingInfo &info, _str value, int checkState = null)
   {
      SetVersionControlSettingInfo svcsi = info.SettingTypeInfo;
   ...
   public static bool setCheckedSetting(SettingInfo &info, _str value, int checkState)
   {
    ...
    * @param value      value to be set
    * 
    * @return bool      success of set operation - does not verify 
    *                   that value was actually changed.
    */
   public static bool setSetting(SettingInfo &info, _str value)
   {
      if (translateDisplayToValue(info, value)) {
         if (info.SettingType == VARIABLE) {
