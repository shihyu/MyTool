...
class FileWatcherManager : Timer {
   ...
   FileWatcherManager() {
      if ( !isinteger(def_svn_update_interval) ) {
   ...
   void addWatchedPath(_str path,WatchedPathTypes type,bool recursive,typeless *pfnCallback) {
      ...
      if ( temp.changeType == WATCHEDPATH_GIT ) {
         casedPath =  path;
      }
