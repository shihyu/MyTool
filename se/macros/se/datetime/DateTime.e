...
class DateTime :
   ...
   sc.lang.IIndexable
{
   static public bool validDate (int yyyy=0, int mm=0, int dd=0);
   static public bool validTime (int hh=0, int mm=0, int ss=0, double fs=0);
   static public void utcOffset (int& hh, int& mm);  
   static public bool isLeapYear (int iyear);
   public long toTimeB ();
   public long toTimeF ();
   ...
   static public DateTime fromUnixTime(int t);
   public bool equalsByInterval (sc.lang.IEquals& rhs,
    ...
    * 
    * @return (bool) True is if <i>iyear</i> is a leap year.
    */
   static public bool isLeapYear (int iyear) {
    ...
    *  
    * @return Unix time value corresponding to this date/time value.
    */
   public int toUnixTime()
   {
      seconds := 0;
      tmpDateTime := this;
      while (tmpDateTime.year(DT_UTCTIME) > 1970) {
         seconds += 60*60*24*365;
         tmpDateTime = tmpDateTime.add(-365,DT_DAY);
      }
      seconds += 60*60*24*(tmpDateTime.dayOfYear(DT_UTCTIME) - 1);
      seconds += 60*60*tmpDateTime.hour(DT_UTCTIME);
      seconds += 60*tmpDateTime.minute(DT_UTCTIME);
      seconds += tmpDateTime.second(DT_UTCTIME);
      return seconds;
   }
   ...
   static public DateTime fromTimeB (_str strTB='')
   {
      ...
      thours := temps intdiv (60 * 60 * 1000);
      temps -= (thours * 60 * 60 * 1000);
      tminutes := temps intdiv (60 * 1000);
      temps -= (tminutes * 60 * 1000);
      tseconds := temps intdiv 1000;
   ...
   {
      // use now for default parameter
      if (strTF == "" || strTF=="0") strTF = _time('F');
    ...
    * 
    * @return DateTime object corresponding to the given Unix time value.
    */
   static public DateTime fromUnixTime(int t)
   {
      DateTime alpha(1970, 1, 1, 0, 0, 0, 1, DT_UTCTIME);
      return alpha.add(t,DT_SECOND);
   }
   ...
   public bool equals (sc.lang.IEquals& rhs)
   {
      ...
      if (this.m_ISO8601_DateTime == null) return true;
   ...
   public DateTime add(double value=0, DTSpan interval=DT_FRACTIONALSECOND)
   {
      ...
      if (interval == DT_FRACTIONALSECOND) {
         tmpFSecond += (value - iValue);
         tmpSecond  += iValue;
         if (tmpFSecond < 0.0) {
            --tmpSecond;
      ...
      if (interval == DT_SECOND) tmpSecond += iValue;
      if (interval == DT_MINUTE) tmpMinute += iValue;
      if (interval == DT_HOUR)   tmpHour   += iValue;
      if (interval == DT_DAY)    tmpDay    += iValue;
      ...
      if (interval == DT_YEAR) {
         ...
         // speed up large second, minute, and hour based calculations.
         tmpMinute += (tmpSecond intdiv 60);
         tmpSecond  = (tmpSecond % 60);
         if (tmpSecond < 0) {
            --tmpMinute;
            tmpSecond = tmpSecond + 60;
         }
         tmpHour  += (tmpMinute intdiv 60);
         tmpMinute = (tmpMinute % 60);
         if (tmpMinute < 0) {
            --tmpHour;
            tmpMinute = tmpMinute + 60;
         }
         tmpDay += (tmpHour intdiv 24);
         tmpHour = (tmpHour % 24);
         if (tmpHour < 0) {
            --tmpDay;
            tmpHour = tmpHour + 24;
         ...
         // days and months are more difficult
         if (tmpDay <= 0) {
            do {
               if (tmpMonth > 1) {
                  --tmpMonth;
         ...
         // finally, adjust months and years
         while (tmpMonth > 12) {
            ++tmpYear;
            tmpMonth = tmpMonth - 12;
         }
   ...
   public int dayOfWeek (_str& dOW=null, DTType dateType=DT_LOCALTIME)
   {
      ...
      int y = shortYear + shortYear intdiv 4;
      int century = (year-shortYear) intdiv 100;
      int c = (3 - (century%4))*2;
