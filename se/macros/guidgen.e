 ...
 * @see copy_guid
 */
_command void insert_guid(_str outputFormat = 'B') name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @see guid_create_string
 */
static bool guid_parse(_str guidString, GUID& rguidDestination)
{
 ...
 * @see guid_init
 */
static bool guid_is_empty(GUID& rGuid)
{
...
static _str _guid_format(GUID& rGuid, _str formatString='B')
{
   ...
   opt := substr(upcase(formatString), 1, 1, 'B');
   retVal := "";
...
static bool _guid_parse(_str guidString, GUID& rGuid)
{
   // Regular expression to support parsing GUID formats B, P, D, and N
   formatParseRE := '^[\{\(]{0,1}([0-9a-fA-F]{8,8})[\-]{0,1}([0-9a-fA-F]{4,4})[\-]{0,1}([0-9a-fA-F]{4,4})[\-]{0,1}([0-9a-fA-F]{4,4})[\-]{0,1}([0-9a-fA-F]{12,12})[\}\)]{0,1}$';
   // We are looking to parse out 5 groups of hex digits
   // 8 - 4 - 4 - 4 - 12
   inputGuid := strip(guidString);
   if(pos(formatParseRE, inputGuid, 1, 'UI'))
   {
         ...
         length(part5) == 12)
      {
         ...
         // into 4 integers
         val1Hi := _hex2dec('0x'substr(part1, 1, 4));
         val1Lo := _hex2dec('0x'substr(part1, 5, 4));

         val2Hi := _hex2dec('0x'part2);
         val2Lo := _hex2dec('0x'part3);

         val3Hi := _hex2dec('0x'part4);
         val3Lo := _hex2dec('0x'substr(part5, 1, 4));

         val4Hi := _hex2dec('0x'substr(part5, 5, 4));
         val4Lo := _hex2dec('0x'substr(part5, 9, 4));
      
         rGuid.Data1 = _makeLongI(val1Lo, val1Hi);
         rGuid.Data23 = _makeLongI(val2Lo, val2Hi);
         rGuid.Data4A = _makeLongI(val3Lo, val3Hi);
         rGuid.Data4B = _makeLongI(val4Lo, val4Hi);
         return true;
      }
   }
   guid_init(rGuid);
...
{
   hexString := _dec2hex(0x00000000FFFFFFFFL & data);
   n := length(hexString);
   if (n < 8) {
...
void ctlbtn_insert.lbutton_up()
{
   ...
   if (_mdi.p_child > 0)
   {
      if (_mdi.p_child._isEditorCtl())
      {
         ...
         formatSpec := ctlcombo_formats.p_text;
         formatChar := "";
...
void ctlcombo_formats.on_create()
{
   ...
   if (_no_child_windows()) {
      ctlbtn_insert.p_enabled = false;
   }
}

void ctlcombo_formats.on_change(int reason)
{
   if(reason == CHANGE_CLINE || CHANGE_CLINE_NOTVIS) {
      formatSpec := ctlcombo_formats.p_text;
      formatChar := "";
      parse formatSpec with .'('formatChar')';
      ctllabel_current_guid.p_caption = guid_to_string(_currentGuid, formatChar);
...
_command guid_unittest_duplicates(int numGuids = 1000) name_info(','VSARG2_MACRO|VSARG2_MARK|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   _str tmB = _time('B');
   lastSeven := substr(tmB, length(tmB) - 6, 7);
