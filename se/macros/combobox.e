...
void _ul2_combobx.\27-\255()
{
   ...
   case_sense := p_cb.p_case_sensitive?'e':'i';
   //case_sense='i';
...
void _ul2_combobx.on_change2(int reason,_str value="")
{
   if ( reason==CHANGE_DELKEY_2  
        && p_style==PSCBO_EDIT
        && p_AllowDeleteHistory ) {
      status := _ComboBoxDeleteHistory(value);
   }
...
int _cbi_search(_str case_sense='', _str search_string='')
{
   ...
   if (p_style==PSCBO_NOEDIT) text :+= '$';
   options := '@r'case_sense;
   top();
   status := search(text:+search_string,options);
