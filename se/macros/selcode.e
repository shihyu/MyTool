...
static const CS_BODYONLY=   0;         // 1 = select code body only and
                                       //     exclude header
                                       // 0 = select everything
...
bool def_selcode_cursor_end=true;
 ...
 */ 
_command int select_code_block()  name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   // if this language does not have callbacks for doing this the hard way
   // then do it the easy way
   if ( !_FindLanguageCallbackIndex( 'cs_%s_nextBlockToken')           &&
        !_FindLanguageCallbackIndex( 'cs_%s_blockBoundary')            &&
        !_FindLanguageCallbackIndex( 'cs_%s_skipOverBlockStartToken')  &&
        !_FindLanguageCallbackIndex( 'cs_%s_skipOverBlockEndToken')    &&
        !_FindLanguageCallbackIndex( 'cs_%s_findProcBlock')            &&
        !_FindLanguageCallbackIndex( 'cs_%s_procBlockBoundary') ) {
      // if we have statement tagging, then fire the big guns
      if (_are_statements_supported()) {
         status := cs_selectCodeBlock_statements();
         return(status);
      }
      // list-tags style context tagging, still better than nothing
      if (_istagging_supported() && _FindLanguageCallbackIndex("vs%s_list_tags")) {
         status := cs_selectCodeBlock_context();
         return(status);
      }
   }
   ...
   if ( name == 'select-code-block' || name == 'select-toggle' ) same_command = true;
   //say( 'name='name' same_command='same_command );

   // Remember the position:
   save_pos( cs_ori_position );

   int old_def_pmatch_max_diff=def_pmatch_max_diff_ksize;
   def_pmatch_max_diff_ksize=10000;
   ...
   if ( cs_selection_exists() && same_command ) {
      status = cs_expand_codeblock( CS_BODYONLY!=0, def_initial_scope );
   } else {
      status = cs_select_newcodeblock( CS_BODYONLY!=0, def_initial_scope, true );
   }
   if ( status ) restore_pos( cs_ori_position );

   // Restore old paren matching limits:
   def_pmatch_max_diff_ksize=old_def_pmatch_max_diff;
...
int _OnUpdate_select_code_block(CMDUI &cmdui,int target_wid,_str command)
{
   ...
   real_extension := "";
   lang := target_wid.cs_getExtension(real_extension);
   ...
   // check for more modern langauge-specific support callback
   if ( _FindLanguageCallbackIndex('selectCodeBlock_%s', lang)) return MF_ENABLED;

   // check for statement tagging, if we have it then we always have select-code-block
   if (target_wid._are_statements_supported(lang)) return MF_ENABLED;
...
/**
 * Global variable to keep track of the last context ID used for select code block.
 */
static int _selectcb_last_context_id = 0;

/**
 * Implement select-code-block for any language that has statement tagging implemented. 
 * 
 * @return int 0 on success, and 1 on failure.
 */
static int cs_selectCodeBlock_statements()
{
   save_pos(auto p);

   if (!select_active()) {
      // No previous selection.
      _selectcb_last_context_id = 0;
   }

   // If current selection not in same file, deselect it:
   start_sel_col := end_sel_col := buf_id := -1;
   if (_get_selinfo(start_sel_col, end_sel_col, buf_id) == TEXT_NOT_SELECTED_RC || buf_id != p_buf_id) {
      _selectcb_last_context_id = 0;
      _deselect();
   }
   ...
   left();
   cxtid := 0;
   if (_selectcb_last_context_id) {
      tag_get_detail2(VS_TAGDETAIL_context_outer, _selectcb_last_context_id, cxtid);
      if (cxtid == 0) {
         // The last context is the outer-most context.  Simply reuse it.
         cxtid = _selectcb_last_context_id;
      }
   } else {
   ...
   tag_get_statement_browse_info(cxtid, auto cm);
   if (cm.seekpos < 0) {
   ...
   if (cm.end_seekpos < cm.seekpos) {
      cm.end_seekpos = cm.seekpos;
   }
   ...
   _selectcb_last_context_id = cxtid;
   restore_pos(p);
   return 0;
}
/**
 * Implement select-proc for any language that has context tagging implemented. 
 * 
 * @return int 0 on success, and 1 on failure.
 */
static int cs_selectCodeBlock_context()
{
   save_pos(auto p);
   ...
   tag_get_context_browse_info(cxtid, auto cm);
   if (cm.seekpos < 0) {
      // A valid seek position is never negative, however, in Python
      // sometimes a seek position of -1 is used for the package context,
      // as an ugly hack to work around the package context not including
   ...
   if ( cs_selection_exists2() && expandBlock ) {
      ...
      if (_LanguageInheritsFrom("plsql",lang)) {
         ...
         */
         ...
         status = 0;
         while (!found && !status) {
            status = cs_expand_codeblock(true, "b");
         ...
         status = cs_select_newcodeblock( true, 'b', false );
...
int cs_hide_code_block()
{
   ...
   old_def_pmatch_max_diff=def_pmatch_max_diff_ksize;
   def_pmatch_max_diff_ksize='10000';
...
// Retn:  0 for OK, 1 for error.
static int cs_select_newcodeblock( bool bodyonly, _str mode, bool always_select )
{
   // Re-init stuff:
   level := 0;
   found := false;
...
// Retn:  0 for OK, 1 for error.
static int cs_expand_codeblock( bool bodyonly, _str mode )
{
   ...
   if ( cs_selected_level == 0 ) {
      bottom();
      cs_select_codetext( false, '-', 1, 1, p_line, p_col, 0, 0, 0,
...
// Retn:  0 for OK, 1 for error.
static int cs_list_all_codeblocks( int level, _str mode, bool bodyonly, 
                                   int bbsl, int bbsc, int bbel, int bbec,
                                   int target_line, int target_col, bool &found )
{
...
static typeless cs_awk_locateStartEnd( int &sbstart_line, int &sbstart_col )
{
   ...
   while ( !found ) {
      if ( ch == '(' ) {
         if ( _find_matching_paren(def_pmatch_max_diff_ksize) ) return( '' );
         seek_pos = _nrseek() + 1;
      ...
      if ( type == CFG_KEYWORD ) {
         ...
         if ( ch == 'if' || ch == 'fo' || ch == 'wh' ) {
            ...
            if ( cs_awk_next_openParen( true ) ) return( '' );
            if ( _find_matching_paren(def_pmatch_max_diff_ksize) ) return( '' );
            seek_pos = _nrseek() + 1;
            if ( _nrseek( seek_pos ) == '' ) return( '' );
            if ( cs_skipSpaces() == -1 ) return( '' );
                        ...
                        int &end_line,   int &end_col )
{
   ...
   sentry.lockContext(false);
   _UpdateContext(true,false);
   start_ctxId := tag_current_context();

   // Find the starting and ending code block symbol:
   //    C         -- { and }
   start_line = p_line; start_col = p_col;
   block_start_line := block_end_line := 0;
...
static void cs_select_codetext( bool bodyonly, _str mode, 
                                int sl, int sc, int el, int ec, 
...
static int cs_pas_next_proc( bool no_skip, typeless &type )
{
   temp_l := p_line; 
   temp_c := p_col;
   for(;;) {
...
// Retn:  0 for OK, 1 for not found.
static int cs_plsql_next_proc( bool no_skip, typeless &type, int includeIfBegin )
{
   int temp_l, temp_c;
...
// Retn:  0 for OK, 1 for not found.
static int cs_pas_next_begin( bool no_skip )
{
   temp_l := p_line; 
   temp_c := p_col;
   for(;;) {
                          ...
                          int &end_line, int &end_col )
{
   ...
   _nrseek( seek_pos );
   typeless type=0;
   typeless status = cs_pas_next_proc( true, type );
   if ( status ) {
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         start_line = p_line; start_col = p_col;
         //messageNwait( 'h1' );
         status = cs_pas_next_begin( false );
         //messageNwait( 'h2' );
         if ( _find_matching_paren(def_pmatch_max_diff_ksize) ) return( 1 );
         ...
         // Find next proc:
         status = cs_pas_next_proc( false, type );
      }
   ...
   top();
   status = cs_pas_next_proc( true, type );
                            ...
                            int &end_line, int &end_col )
{
   ...
   */
   ...
   typeless type=0;
   status = cs_plsql_next_proc( true, type, 0 );
   //messageNwait("h2 status="status);
   if ( status ) {
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         ...
         if (p_line < start_line) {
            ...
            cs_nextSemicolon();
            status = cs_plsql_next_proc( false, type, 0 );
            continue;
         }
         ...
         status = cs_plsql_next_proc( false, type, 1 );
                                 ...
                                 int &sbstart_line, int &sbstart_col )
{
   ...
   // Search for the next '{':
   if ( cs_c_next_openCurly( true ) ) return( '' );
   sbstart_line = p_line; sbstart_col = p_col;
                                   ...
                                   int &sbstart_line, int &sbstart_col )
{
   ...
   if ( ch == 'prog' ) {
      ...
      typeless type=0;
      if ( cs_pas_next_proc( false, type ) ) return( '' );
      end_line = p_line - 1; end_col = p_col;
      return( 'prog' );
   }

   // Search for the next 'begin':
   if ( cs_pas_next_begin( true ) ) return( '' );
   sbstart_line = p_line; sbstart_col = p_col;
                                   ...
                                   int &sbstart_line, int &sbstart_col )
{
   ...
   if ( ch == 'modu' ) {
      sbstart_line = start_line + 1; sbstart_col = 1;
      typeless type=0;
      if ( cs_mod_next_proc( false, type ) ) return( '' );
      end_line = p_line - 1; end_col = p_col;
      return( 'modu' );
   }
...
{
   // Search for the next '{':
   if ( cs_c_next_openCurly( true ) ) return( 1 );
   start_line = p_line;
...
// Retn:  0 for OK, 1 for not found.
static int cs_c_next_openCurly( bool no_skip )
{
   temp_l := p_line; 
   temp_c := p_col;
                           ...
                           int &end_line,   int &end_col )
{
   ...
   _nrseek( seek_pos );
   typeless type=0;
   typeless status = cs_java_next_proc( true, type );
   if ( status ) return( 1 );
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         start_line = p_line; start_col = p_col;
         status = cs_c_next_openCurly( false );
         if ( _find_matching_paren(def_pmatch_max_diff_ksize) ) return( 1 );
         ...
         // Find next proc:
         status = cs_java_next_proc( false, type );
      }
      return( 1 );
   }
   ...
   // Go back all the way to the beginning of the file:
   top();
   status = cs_java_next_proc( true, type );
...
// Retn:  0 for OK, 1 for not found.
static int cs_java_next_proc( bool no_skip, typeless &type )
{
   temp_l := p_line; 
   temp_c := p_col;
   for(;;) {
                          ...
                          int &end_line,   int &end_col )
{
   ...
   _nrseek( seek_pos );
   typeless type=0;
   typeless status = cs_mod_next_proc( true, type );
   if ( status ) {
      // Maybe inside 'main' code block:
      p_line = ori_line; p_col = ori_col;
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         ...
         // Find next proc:
         status = cs_mod_next_proc( false, type );
      }
   ...
   // Go back all the way to the beginning of the file:
   top();
   status = cs_mod_next_proc( true, type );
...
// Retn:  0 for OK, 1 for not found.
static int cs_mod_next_proc( bool no_skip, typeless &type )
{
   temp_l := p_line; 
   temp_c := p_col;
   for(;;) {
...
// Retn:  0 for OK, 1 for not found.
static int cs_ada_next_proc( bool no_skip, typeless &type )
{
   temp_l := p_line; 
   temp_c := p_col;
   temp_line := 0;
                          ...
                          int &end_line,   int &end_col )
{
   ...
   _nrseek( seek_pos );
   typeless type = 0;
   typeless status = cs_ada_next_proc( true, type );
   if ( status ) return( 1 );
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         ...
         // Find next proc:
         status = cs_ada_next_proc( false, type );
      }
   ...
   // Go back all the way to the beginning of the file:
   top();
   status = cs_ada_next_proc( true, type );
                          ...
                          int &end_line,   int &end_col )
{
   ...
   last_col := p_col;
   typeless type = 0;
   typeless status = cs_awk_next_proc( true, type );
   if ( status ) return( 1 );
   ...
   if ( p_line <= ori_line ) {
      while ( !status ) {
         ...
         // Find next proc:
         status = cs_awk_next_proc( false, type );
   ...
   status = cs_awk_next_proc( true, type );
...
//        type               Returned type: 'proc', 'func', 'task', 'pack'
// Retn:  0 for OK, 1 for not found.
static int cs_awk_next_proc( bool no_skip, typeless &type )
{
   temp_l := p_line; 
   temp_c := p_col;
   if ( cs_skipSpaces() == -1 ) return( 1 );
...
static typeless cs_awk_nextEOLN( bool no_skip )
{
   temp_l := p_line; 
   temp_c := p_col;
   last_line := p_line; 
...
// Para:  no_skip            Flag: 1=if already on char, do nothing (don't skip)
// Retn:  0 for OK, 1 for not found.
static int cs_awk_next_openParen( bool no_skip )
{
   temp_l := p_line; 
   temp_c := p_col;
   if ( cs_skipSpaces() == -1 ) return( 1 );
                           ...
                           int &end_line,   int &end_col )
{
   ...
   if ( seek_pos < 1 ) seek_pos = 0;
   _nrseek( seek_pos );
   typeless status = cs_c_next_openCurly( true );
                          ...
                          int &end_line,   int &end_col )
{
   ...
   if ( seek_pos < 1 ) seek_pos = 0;
   _nrseek( seek_pos );
   typeless status = cs_for_next_proc( true );
   typeless type = 0;
   stopLabel := "";
   if ( status ) return( 1 );
   typeless sbstart_line=0, sbstart_col=0;
...
// Para:  no_skip            Flag: 1=if already on proc, do nothing (don't skip)
// Retn:  0 for OK, 1 for error.
static int cs_for_next_proc( bool no_skip )
{
...
static int cs_buildCallIndexProcList()
{
   ...
   index := find_index("def_cs_stepback_size_"lang,VAR_TYPE);
   if (index) {
...
//                             cob, ada, for, pas, awk,
//                             mod, html, plsql, php
bool _isSelectCodeBlock_supported()
{
   if (cs_buildCallIndexProcList()) {
      return(false);
   }
