...
static const ISPF_FILL_CHAR= ' ';
static const ISPF_TAB_POSN_CHAR= '*';
static const ISPF_LBND_POSN_CHAR= '<';
static const ISPF_RBND_POSN_CHAR= '>';
static const ISPF_LABEL_PREFIX=   '.';
static const ISPF_RULER_LINE=   "----|----1----|----2----|----3----|----4----|----5----|----6----|----7----|----8----|----9----|----0----|----1----|----2----|----3----";
static const ISPF_MAX_LINE=      132;
static const ISPF_EXCLUDE_SINGLE= "---------- %s LINE NOT DISPLAYED ----------";
static const ISPF_EXCLUDE_PLURAL= "---------- %s LINES NOT DISPLAYED ----------";
static const ISPF_EXCLUDE_RE=     "^---------- {[0-9]+} LINE[A-Z ]* ----------";
...
struct ISPF_LC_INFO {
   ...
   bool mc_is_copy;         // true if above is a CC, false for MM
   bool mc_was_used;        // true after found A, B, or O command
   bool z_selection;        // do we already have a 'z' selection?
...
static const ISPF_LC_COLOR_PASS=     1;    // parse and color prefix area
static const ISPF_LC_EDIT_PASS=      2;    // Perform processing commands
static const ISPF_LC_UPDATE_PASS=    3;    // Update informational comamnds, dg BNDS
 ...
 */
int ispf_find_move_copy(int &startLine, int &endLine, bool &isCopy)
{
   ...
   for (i=0; i<_LCQNofLineCommands(); ++i) {
      ...
      line_number := _LCQLineNumberAtIndex(i);
 ...
 */
void ispf_insert_exclude(int n, bool replace=false)
{
...
static _str tab_settings(_str tab_line)
{
   ...
   start_pos := pos(ISPF_TAB_POSN_CHAR,tab_line,1);
...
bool g_continuous_insert = false;
/**
...
int ispf_process_lc(int pass_number, struct ISPF_LC_INFO &lcinfo)
{
   ...
   for (i=0; i<_LCQNofLineCommands(); ++i) {
      ...
      lc_end_line := 0;
      lc_do_block := false;
      lc_remove_command := true;
      lc_remove_start := false;
      switch (lc_str) {
      // commands operating on the next 'n' lines
      case 'C':     // copy
      case 'M':     // move
         ...
         if (pass_number==ISPF_LC_EDIT_PASS) {
            ...
            if (_lineflags() & NOSAVE_LF) {
               if (!lc_bounds_modified) {
                  lt_pos := pos(ISPF_LBND_POSN_CHAR,cur_line);
                  gt_pos := pos(ISPF_RBND_POSN_CHAR,cur_line);
                  if (gt_pos>0 && !lt_pos) {
                     lt_pos=gt_pos;
                  }
      ...
      case 'TF':
         if (pass_number==ISPF_LC_EDIT_PASS) {
            ...
            if (lc_val <= 0) lc_val=_TruncateLengthC();
...
static int ispf_delete_lines(int start_line, int end_line)
{
   ...
   p_line=start_line;
   real_line := p_RLine;
   is_no_save := (_lineflags() & NOSAVE_LF)? true:false;
   _select_line(mark_id,'C');
   p_line=end_line;
 ...
 * @return 0 on success, <0 on error.
 */
static int ispf_case_lines(int start_line, int end_line, bool upcase_option)
{
...
static int ispf_shift_lines(int start_line, int end_line, int num_cols,
                            bool left_shift, bool smart_shift)
{
   // allocate a selection
   int orig_mark_id=_duplicate_selection('');
...
static _str bounds_positions(int start_col, int end_col)
{
   ...
   result := substr('',1,start_col-1):+ISPF_LBND_POSN_CHAR;
 ...
 */
int ispf_parse_label(_str word, bool displayError=true)
{
   // is it a label
...
_command void ispf_flip(_str cmdline="") name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   for (i=endLine; i>=startLine; --i) {
      ...
      if (_lineflags() & NOSAVE_LF) {
         is_excl := (ispf_is_excluded_line() > 0);
         if (numHidden > 0) {
 ...
 * @categories ISPF_Primary_Commands
 */
_command void ispf_nonumber() name_info(','VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ispf_number("OFF");
}
/**
 ...
 */
static int parse_number_options(_str cmdline, int renumber_flags, bool off_cmds=true)
{
   // parse through the arguments
   saw_std_or_cobol := false;
   show_usage := (cmdline=='');
   while (cmdline!='') {
      word := "";
      parse strip(cmdline) with word cmdline;
 ...
 * @categories ISPF_Primary_Commands
 */
_command void ispf_number,ispf_num(_str arglist="") name_info(','VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   // parse through the arguments
   ...
   if (renumber_flags & VSRENUMBER_STD) {
      renumber_lines(73,80,'0',false,false,true);
   }
 ...
 * @categories ISPF_Primary_Commands
 */
_command void ispf_renumber,ispf_renum(_str arglist="") name_info(','VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   if (arglist=='') {
      renumber();
      return;
 ...
 * @categories ISPF_Primary_Commands
 */
_command void ispf_unnum,ispf_unnumber(_str arglist="") name_info(','VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   int renumber_flags = parse_number_options(arglist,0,false);
   if (renumber_flags < 0) {
   ...
   if (renumber_flags & VSRENUMBER_STD) {
      renumber_lines(73,80,'0',true,false,true);
   }
   if (renumber_flags & VSRENUMBER_COBOL) {
...
_command void ispf_cut(_str arglist="") name_info(','VSARG2_READ_ONLY|VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   typeless p;
   _save_pos2(p);
   p_line=startLine; _LCSetData('');
   _select_line(mark_id,'C');
   p_line=endLine; _LCSetData('');
   _show_selection(mark_id);
 ...
 * @categories ISPF_Primary_Commands
 */
_command void ispf_paste(_str arglist="") name_info(','VSARG2_ICON|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   // parse through the arguments
   pasteLine := -1;
   insert_style := 'A';
   ...
   cmdline := strip(arglist);
   show_usage := false;
   line_command := strip(_LCQData());
   if (line_command != '') {
      switch (upcase(line_command)) {
      case 'A':
         insert_style='A';
         break;
      case 'B':
   ...
   if (clipboard_name=='') {
      if (_Nofnulls < _Nofclipboards) {
         clipboard_name = '1'; // first unnamed
      }
   }
...
_command void ispf_bounds,ispf_bnds(_str cmdline='') name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   if (cmdline=='') {
      ...
      if (isEclipsePlugin()) {
         _str bnd;
         if (p_BoundsStart > 0) {
            bnd = (p_BoundsStart' 'p_BoundsEnd);
         } else {
            bnd = '';
         }
         status := textBoxDialog("Set Bounds", 0, 0, "", "", "", 
                                 "Bounds:":+bnd);
         if (status < 0) {
            return;
         }
         cmdline = strip(_param1);
         if (cmdline :== '') {
            return;
         }
...
_command void ispf_reset,ispf_res(_str cmdline="") name_info(','VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   ...
   if (reset_seldisp && (p_Nofhidden || p_NofSelDispBitmaps)) {
      ...
      up();
      for (;;) {
         num_excluded := ispf_is_excluded_line();
         if (num_excluded>0) {
            down(num_excluded);
         } else {
   ...
   for (i=0; i<_LCQNofLineCommands(); ++i) {
      _str lc_str, lc_arg; int lc_val;
      line_command := strip(_LCQDataAtIndex(i));
      line_number := _LCQLineNumberAtIndex(i);
      line_flags := _LCQFlagsAtIndex(i);
      p_line=line_number;
      ...
      if (line_number>=startLine && line_number<=endLine) {
         ...
         // blow away labels
         if (substr(line_command,1,1)==ISPF_LABEL_PREFIX) {
            if (reset_label) {
               _LCSetDataAtIndex(i,'');
               --i;
            }
