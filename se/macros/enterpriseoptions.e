 ...
 */
_metadata enum_flags EnterpriseAdminOptions {
...
static void init_cached_settings() {
   def_eao_disabled_flags = 0;
   def_hotfix_auto_prompt=true;
...
static void _eao_read_config()
{
   ...
   file := _getSlickEditInstallPath():+'Admin.xml';

   // if file does not exist, then we can't do anything
   if (!file_exists(file)) {
      // if the date was already blank, then there is nothing really to do here
      if (def_eao_file_date2 != ''  && def_eao_file_date2 != 0) {
         // blank out the date
         def_eao_file_date2 = '';
         _config_modify_flags(CFGMODIFY_DEFVAR);
         // reset cached settings
         init_cached_settings();

         // reset all the options
         // 2018-07-30 - rb
         // Do NOT reset all options to default. The original idea was probably
         // to handle the case where 'Admin.xml' gets deleted (by an administrator?) 
         // and we reset all options back to their default, but that is not smart 
         // enough. If the user set an option (e.g. PIP) to Off via the Options
         // dialog, then this code would turn it back On (not what the user wants).
      ...
      if (def_eao_file_date2 != '' && def_eao_file_date2 != 0 && def_eao_file_date2 == fileDate) return;

      // otherwise we save this date
      def_eao_file_date2 = fileDate;
...
static void _eao_read_config_version1501(int xmlHandle)
{
   ...
   // 2018-07-30 - rb
   // Do NOT reset unhandled options to default. The original idea was probably
   // to handle the case where an option gets removed (by an administrator?) 
   // and we reset that option back to its default, but that is not smart 
   // enough. If the user set an option (e.g. PIP) to Off via the Options
   // dialog, then this code would turn it back On (not what the user wants).
