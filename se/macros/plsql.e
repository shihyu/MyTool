...
static const PLSQL_LANGUAGE_ID=  'plsql';
...
_command void plsql_enter() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_ICON|VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   generic_enter_handler(_plsql_expand_enter);
}
bool _plsql_supports_syntax_indent(bool return_true_if_uses_syntax_indent_property=true) {
   return true;
}
_command void plsql_semi() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_REQUIRES_EDITORCTL)
{
   if ( command_state() ) {
      call_root_key(';');
      return;
...
_command plsql_space() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
...
_str _plsql_keyword_case(_str s, bool confirm=true, _str sample="")
{
...
/*
    Returns true if nothing is done.
*/
static bool plsql_expand_space()
{
   updateAdaptiveFormattingSettings(AFF_SYNTAX_INDENT);
   syntax_indent := p_SyntaxIndent;
   ...
   aliasfilename := "";
   _str word=min_abbrev2(orig_word,plsql_space_words,'',aliasfilename);
   ...
   if ( word=='if' ) {
      replace_line(_word_case(line:+'  then',false,sample));
      insert_line(indent_string(width)_word_case('end if;',false,sample));
      set_surround_mode_end_line();
      up();_end_line();p_col-=5;
   } else if (word=='while') {
      replace_line(_word_case(line:+'  loop',false,sample));
      insert_line(indent_string(width)_word_case('end loop;',false,sample));
      set_surround_mode_end_line();
      up();_end_line();p_col-=5;
   } else if (word=='for') {
      replace_line(_word_case(line:+'  in  loop',false,sample));
      insert_line(indent_string(width)_word_case('end loop;',false,sample));
      set_surround_mode_end_line();
      ...
      if (col<=0) {
         ...
         restore_pos(p2);
         replace_line(indent_string(col-1)_word_case('begin',false,sample));
         insert_line(indent_string(col-1)_word_case('end;',false,sample));
         set_surround_mode_end_line();
         up();_end_line();++p_col;
      }
      ...
      if (col<0) {
         ...
         restore_pos(p2);
         replace_line(indent_string(col-1)_word_case('declare',false,sample));
         insert_line(indent_string(col-1)_word_case('begin',false,sample));
         insert_line(indent_string(col-1)_word_case('end;',false,sample));
         set_surround_mode_end_line();
         up(2);_end_line();++p_col;
      }
      ...
      col:=_plsql_find_block_col(block_info,true,true);
      if (col) {
         replace_line(indent_string(col-1+syntax_indent)_word_case(orig_word'  then',false,sample));
         _end_line();p_col-=5;
      } else {
...
int plsql_proc_search(_str &proc_name,int find_first)
{
   typeless status=0;
   if ( find_first ) {
      word_chars := _clex_identifier_chars();
      variable_re := '(['word_chars'.]#)';
      re := '{#1(procedure|function|trigger|cursor)}[ \t]+{#0'variable_re'}';
 ...
 */
int _plsql_MaybeBuildTagFile(int &tfindex, bool withRefs=false, bool useThread=false, bool forceRebuild=false)
{
   return ext_MaybeBuildTagFile(tfindex, "plsql", "plsql",
                                "Oracle SQL Builtins",
...
int _proc_search(var proc_name,find_first)
{
   if ( find_first ) {
...
/*
   ...
   RETURN
      ...
      -1   Indent to previous statement.
*/
int _plsql_find_begin_block_col(bool skipFirstBegin)
{
   typeless orig_pos;
   save_pos(orig_pos);
...
*/
int _plsql_find_block_col(_str &block_info/* currently just block word */,bool restoreCursor,bool returnFirstNonBlank)
{
   typeless orig_p2=0;
   typeless orig_pos;
   save_pos(orig_pos);
...
*/
static int plsql_begin_stat_col(bool RestorePos,bool SkipFirstHit,bool ReturnFirstNonBlank,
...
*/
int plsql_indent_col(int non_blank_col, bool pasting_open_block = false)
{
   orig_col := p_col;
   orig_linenum := p_line;
   save_pos(auto p);
...
int plsql_smartpaste(bool char_cbtype,int first_col,int Noflines,bool allow_col_1=false)
{
...
static bool plsql_enter_col2(int &enter_col)
{
...
_command void plsql_maybe_case_word() name_info(','VSARG2_MULTI_CURSOR|VSARG2_CMDLINE|VSARG2_LASTKEY|VSARG2_REQUIRES_EDITORCTL)
{
   _maybe_case_word(LanguageSettings.getAutoCaseKeywords(PLSQL_LANGUAGE_ID),gWord,gWordEndOffset);
}

_command void plsql_maybe_case_backspace() name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
...
*/
int _plsql_find_matching_word(bool quiet,int pmatch_max_diff_ksize=MAXINT,int pmatch_max_level=MAXINT)
{
