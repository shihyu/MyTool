...
{
   return ( _getSysconfigPath():+CT_ROOT_DIR );
}
...
{
   int status = _xmlcfg_save(handle,-1,VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE|VSXMLCFG_SAVE_PCDATA_INLINE,filename);
   return status;
}

static const CTTAG_ITEM_ROOT= "SETemplate";

static const CTTAG_TEMPLATEDETAILS= "TemplateDetails";
static const CTTAG_TEMPLATECONTENT= "TemplateContent";
static const CTTAG_NAME= "Name";
static const CTTAG_DESCRIPTION= "Description";
static const CTTAG_ICON= "Icon";
static const CTTAG_CATEGORY= "Category";
static const CTTAG_SORTORDER= "SortOrder";
static const CTTAG_DEFAULTNAME= "DefaultName";
static const CTTAG_PARAMETERS= "Parameters";
static const CTTAG_PARAMETER= "Parameter";
static const CTTAG_FILES= "Files";
static const CTTAG_FILE= "File";

static const CTXPATH_ITEM_ROOT= "/"CTTAG_ITEM_ROOT;
static const CTXPATH_ITEM_TEMPLATEDETAILS= CTXPATH_ITEM_ROOT"/"CTTAG_TEMPLATEDETAILS;
static const CTXPATH_ITEM_NAME= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_NAME;
static const CTXPATH_ITEM_DESCRIPTION= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_DESCRIPTION;
static const CTXPATH_ITEM_ICON= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_ICON;
static const CTXPATH_ITEM_CATEGORY= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_CATEGORY;
static const CTXPATH_ITEM_SORTORDER= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_SORTORDER;
static const CTXPATH_ITEM_DEFAULTNAME= CTXPATH_ITEM_TEMPLATEDETAILS"/"CTTAG_DEFAULTNAME;
static const CTXPATH_ITEM_TEMPLATECONTENT= CTXPATH_ITEM_ROOT"/"CTTAG_TEMPLATECONTENT;
static const CTXPATH_ITEM_FILES= CTXPATH_ITEM_TEMPLATECONTENT"/"CTTAG_FILES;
static const CTXPATH_ITEM_FILE= CTXPATH_ITEM_FILES"/"CTTAG_FILE;
static const CTXPATH_ITEM_PARAMETERS= CTXPATH_ITEM_TEMPLATECONTENT"/"CTTAG_PARAMETERS;
static const CTXPATH_ITEM_PARAMETER= CTXPATH_ITEM_PARAMETERS"/"CTTAG_PARAMETER;

static const CTTAG_OPTIONS_ROOT= "Options";

static const CTXPATH_OPTIONS_ROOT= "/"CTTAG_OPTIONS_ROOT;
static const CTXPATH_OPTIONS_PARAMETERS= CTXPATH_OPTIONS_ROOT"/"CTTAG_PARAMETERS;
static const CTXPATH_OPTIONS_PARAMETER=  CTXPATH_OPTIONS_PARAMETERS"/"CTTAG_PARAMETER;

static const CTOPTIONS_DTD_PATH= "http://www.slickedit.com/dtd/vse/setemplate/1.0/options.dtd";
...
int _ctTemplate_SetTemplateContent(int handle, ctTemplateContent_t& templateContent)
{
   ...
   if( 0==status ) {
      ...
      if( 0==status ) {
         ...
         templateDir := absolute(_xmlcfg_get_filename(handle));
 ...
 */
bool _ctIsTemplateFile(_str filename)
{
 ...
 */
bool _ctIsOptionsFile(_str filename)
{
   filename=strip(filename,'B','"');
   filenamenopath := _strip_filename(filename,'p');
   return ( _file_eq(filenamenopath,CTOPTIONS_FILENAME) && file_exists(filename) );
}
 ...
 */
bool _ctIsSysItemTemplatePath(_str path)
{
   _str root = _ctGetSysItemTemplatesDir();
   if( _file_eq(root,substr(path,1,length(root))) ) {
      return true;
   }
 ...
 */
bool _ctIsUserItemTemplatePath(_str path)
{
 ...
 */
bool _ctIsTemplateDirectory(_str dir)
{
   ...
   int status = insert_file_list("+hsa ":+_maybe_quote_filename(dir:+'*':+CT_EXT));
   if (!status && !p_Noflines) {
      status=1;
   }
...
bool _ctTemplateIsValidVersion(_str Version)
{
...
bool _ctTemplateIsValidType(_str Type)
{
...
int _ctOptionsMaybeCreate(_str filename)
{
   ...
   if( recreate ) {
      status=_xmlcfg_save(handle,-1,VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE|VSXMLCFG_SAVE_PCDATA_INLINE,filename);
      if( status!=0 ) {
         _xmlcfg_close(handle);
         return status;
      }
...
bool _ctOptionsIsValidVersion(_str Version)
{
   _str testMajor, testMinor, testRev, testBuild;
   normalizeVSEVersion(Version,testMajor,testMinor,testRev,testBuild);
   _str major, minor, rev, build;
...
static const CODEHELP_DEFAULT_WORD_CHARS= "A-Za-z0-9_";
...
static bool isJavaProjectType(_str projectType)
{
   if( projectType=="" ) {
      return false;
   }
...
static bool isCSharpProjectType(_str projectType)
{
   if( projectType=="" ) {
      return false;
   }
...
static bool isCppProjectType(_str projectType)
{
   if( projectType=="" ) {
      return false;
   }
...
static bool isJavaBuildCmdline(_str cmdline)
{
   if( cmdline=="" ) {
      return false;
   }
...
static bool isCSharpBuildCmdline(_str cmdline)
{
   if( cmdline=="" ) {
      return false;
   }
...
static bool isCppBuildCmdline(_str cmdline)
{
   if( cmdline=="" ) {
      return false;
   }
...
static _str findMainNamespace()
{
   ...
   if (_haveContextTagging()) {
      ...
      int status = tag_read_db(workspace_tag_file);
      if( status>=0 ) {
         ...
         while( status==0 ) {
            ...
            if( _FileExistsInCurrentProject(filename) ) {
               ...
               if( _LanguageInheritsFrom("java",lang) &&
                  ...
                  if( 
                      ( 0!=(tag_flags & SE_TAG_FLAG_STATIC) && tag_tree_type_is_func(type_name) ) ||
                  ...
                  if( 0!=(tag_flags & SE_TAG_FLAG_STATIC) && tag_tree_type_is_func(type_name) ) {
                     sig_match=true;
                  }
               ...
               if( sig_match ) {
                  ...
                  if (_LanguageInheritsFrom("c",lang)) {
                     parse tag_class with package VS_TAGSEPARATOR_package .;
                     //package=translate(package,'::',VS_TAGSEPARATOR_package);
                  } else {
                     parse tag_class with package VS_TAGSEPARATOR_class .;
                     /* For C# and Java, should 
                        assume last / is for class
                     */
                     i:=lastpos(VS_TAGSEPARATOR_package,package);
                     if (i) {
                        package=substr(package,1,i-1);
                     }
                     package=translate(package,'.',VS_TAGSEPARATOR_package);
                  }
...
bool _ctIsPredefinedParameter(_str Name)
{
   switch( Name ) {
   case "itemname":
   case "inputfilename":
                             ...
                             _str delim='$', _str wordChars=CODEHELP_DEFAULT_WORD_CHARS, _str lang="")
{
   ...
   _str result = s;
   i := pos("{"re"}",result,1,'er');
   while( i>0 ) {
      _str rep = null;
      parm := substr(result,pos('S0'),pos('0'));
      _str orig_parm = parm;
      before := substr(result,1,pos('S0')-1);
      after := substr(result,pos('S0')+pos('0'));
      ...
      if( parm=="" ) {
         ...
         case "fileinputname":
            {
               ...
               itemNameName := _strip_filename(itemName,'pe');
               rep=itemNameName;
            }
            break;
         case "lowcaseitemname":
         ...
         case "rootpackage":
            ...
            // Would be better provide a list the user can choose from.
            if (rep!='') {
...
int _ctPostProcessContent_format(int wid, _str itemName, _str itemLocation)
{
    if (_haveBeautifiers()) {
       ...
       if( _file_eq('.'lang,CT_EXT) || _LanguageInheritsFrom("xml",lang) ) {
          // Special case:
                                     ...
                                     _str& statusMessage=null)
{
   ...
   DefaultNameName := _strip_filename(DefaultName,'pe');
   ...
   for( i=0; i<content.Files._length(); ++i ) {
      ReplaceParameters := content.Files[i].ReplaceParameters;
      Filename := absolute(content.Files[i].Filename,srcDir);
      FilenameName := _strip_filename(Filename,'pe');
      ...
      TargetPath := _strip_filename(TargetFilename,'n');
      TargetName := _strip_filename(TargetFilename,'pe');
                                    ...
                                    _str& statusMessage=null)
{
   ...
   for( i=0; i<content.Files._length(); ++i ) {
      ...
      TargetDir := _strip_filename(TargetFilename,'n');
