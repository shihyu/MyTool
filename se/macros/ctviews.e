 ...
 *               Defaults to true.
 */
void _ctDetailsViewEnable(bool enable=true)
{
 ...
 *                 Defaults to true.
 */
void _ctDetailsViewReadOnly(bool readOnly=true)
{
   _control ctl_name, ctl_description, ctl_defaultname, ctl_sortorder;
...
static bool verifyName(_str Name)
{
   // Anything goes!
   return true;
}

static bool verifyDescription(_str Description)
{
   // Anything goes!
   return true;
}

static bool verifyDefaultName(_str DefaultName)
{
...
static bool verifySortOrder(_str SortOrder)
{
 ...
 */
bool _ctDetailsViewVerifyInput(bool quiet=false, bool selectError=true, _str& statusMessage=null)
{
   _control ctl_name, ctl_description, ctl_defaultname, ctl_sortorder;
   // Name
   ...
   // Description
   Description := strip(ctl_description.p_text);
   ...
   // DefaultName
   DefaultName := strip(ctl_defaultname.p_text);
   ...
   // SortOrder
   SortOrderStr := strip(ctl_sortorder.p_text);
...
static const CT_SORTORDER_INCREMENT= 10;

void ctl_sortorder_spin.on_spin_down()
{
   _control ctl_sortorder;
   SortOrder := ctl_sortorder.p_text;
...
void ctl_files.on_change(int reason, int nodeIndex=0)
{
   ...
   old_inOnChange := p_parent._ctFilesViewInOnChange(1);
   switch( reason ) {
...
static int findFilesTreeOrDie(int wid=0)
{
   ...
   if( wid==0 ) {
      msg := "Unable to locate Files tree.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   }
   return wid;
}
...
static bool _ctViewAddEnabled(int treeWid)
{
...
static bool _ctViewEditEnabled(int treeWid)
{
   // Note:
...
static bool _ctViewMoveEnabled(int treeWid)
{
   if( _GetDialogInfoHt("readOnly",treeWid.p_parent) ) {
      // View is read-only
      return false;
...
_command void ctFilesViewAddFile(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   ...
   do {
      ...
      while( index>0 ) {
         line := treeWid._TreeGetCaption(index);
         _str Filename, TargetFilename, ReplaceParameters;
         parse line with Filename"\t"TargetFilename"\t"ReplaceParameters"\t".;
         if( _file_eq(target,TargetFilename) ) {
            msg := "Duplicate target found. Cannot add file.";
            _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   ...
   if( copy ) {
      ...
      name := _strip_filename(src,'p');
      new_src :=  templateDir:+name;
      if( !_file_eq(src,new_src) ) {
...
_command void ctFilesViewEditFile(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   treeWid=findFilesTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return;
   ...
   caption := "Edit File";
   readOnly := _GetDialogInfoHt("readOnly",treeWid.p_parent);
   if( readOnly ) {
      caption="View File";
   }
...
_command void ctFilesViewOpenFile(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   treeWid=findFilesTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return;
   ...
   _mdi.p_child.edit(_maybe_quote_filename(Filename));
}

static bool moveFileUpDown(int treeWid=0, _str direction='')
{
   treeWid=findFilesTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return false;
...
_command void ctFilesViewMoveFileUp(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
...
_command void ctFilesViewMoveFileDown(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   moveFileUpDown(treeWid,'-');
}
...
_command void ctFilesViewRemoveFile(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   treeWid=findFilesTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return;
 ...
 */
bool _ctFilesViewInOnChange(int onoff=-1)
{
   old_onoff := _GetDialogInfoHt("inOnChange",p_window_id);
   if( onoff > -1 ) {
...
{
   old_inOnChange := _ctFilesViewInOnChange(1);
   ...
   if( content.Files!=null ) {
      ...
      for( i=0; i<content.Files._length(); ++i ) {
         ...
         line :=  File.Filename"\t"File.TargetFilename"\t"File.ReplaceParameters;
 ...
 *               Defaults to true.
 */
void _ctFilesViewEnable(bool enable=true)
{
 ...
 *                 Defaults to true.
 */
void _ctFilesViewReadOnly(bool readOnly=true)
{
   _control ctl_files, ctl_file_add, ctl_file_edit, ctl_file_remove;
 ...
 * @return true on success (no errors in input).
 */
bool _ctFilesViewVerifyInput(bool quiet=false, bool selectError=true, _str& statusMessage=null)
{
   ctTemplateContent_t templateContent = _ctViewContent();
   templateContent.Files._makeempty();
   _control ctl_files;
   index := ctl_files._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   while( index>0 ) {
      line := ctl_files._TreeGetCaption(index);
      _str Filename, TargetFilename, ReplaceParameters;
      parse line with Filename"\t"TargetFilename"\t"ReplaceParameters"\t".;
      i := templateContent.Files._length();
...
void _ctParametersView_etab.on_resize()
{
   ...
   int after_tree_x = ctl_param_add.p_x - (ctl_params.p_x_extent);
   after_tree_y := 0;
   if( ctl_delimiter.p_visible ) {
      // Force the tree to take up entire height of frame if not showing the delimiter
      after_tree_y = ctl_delimiter.p_y - (ctl_params.p_y_extent);
   }
   ...
   ctl_param_add.p_x=ctl_params.p_x_extent + after_tree_x;
   ctl_param_edit.p_x=ctl_param_add.p_x;
   ctl_param_remove.p_x=ctl_param_add.p_x;
   // Delimiter label, text box, note
   ctl_delimiter.p_y=ctl_params.p_y_extent + after_tree_y;
...
void ctl_params.on_change(int reason, int nodeIndex)
{
   ...
   old_inOnChange := p_parent._ctParametersViewInOnChange(1);
   switch( reason ) {
   case CHANGE_SELECTED:
      {
...
{
   menu_name := "_ctParametersView_menu";
   int index = find_index(menu_name,oi2type(OI_MENU));
   if( index==0 ) {
      return;
   }
...
static int findParametersTreeOrDie(int wid=0)
{
   ...
   if( wid==0 ) {
      msg := "Unable to locate Parameters tree.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   }
   return wid;
}
...
_command void ctParametersViewAddParameter(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   ...
   do {
      ...
      while( index>0 ) {
         ...
         if( lowcase(n)==lowcase(Name) ) {
            msg := "Duplicate parameter found. Cannot add parameter.";
            _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
            treeWid._TreeSetCurIndex(index);
            continue prompt;
         }
...
_command void ctParametersViewEditParameter(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   treeWid=findParametersTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return;
...
_command void ctParametersViewRemoveParameter(int treeWid=0) name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
   treeWid=findParametersTreeOrDie(treeWid);
   if( treeWid==0 ) {
      return;
 ...
 * @return true on success (no errors in input).
 */
bool _ctParametersViewVerifyInput(bool quiet=false, bool selectError=true, _str& statusMessage=null)
{
   ctTemplateContent_t templateContent = _ctViewContent();
   templateContent.Parameters._makeempty();
   _control ctl_params;
   index := ctl_params._TreeGetFirstChildIndex(TREE_ROOT_INDEX);
   while( index>0 ) {
      line := ctl_params._TreeGetCaption(index);
      _str Name, Value, Prompt, PromptString;
      parse line with Name"\t"Value"\t"Prompt"\t"PromptString"\t".;
 ...
 * @return Previous value. Current value if -1 specified.
 */
bool _ctParametersViewInOnChange(int onoff=-1)
{
   old_onoff := _GetDialogInfoHt("inOnChange",p_window_id);
   if( onoff > -1 ) {
      _SetDialogInfoHt("inOnChange",onoff!=0,p_window_id);
...
void _ctParametersViewInit(_str templateFilename,
                           ctTemplateDetails_t& details,
                           ctTemplateContent_t& content,
                           bool hideDelimiter=false)
{
   old_inOnChange := _ctParametersViewInOnChange(1);
   ...
   if( content.Parameters!=null ) {
      ...
      for( Name._makeempty();; ) {
         ...
         line :=  Name"\t"Param.Value"\t"Param.Prompt"\t"Param.PromptString;
   ...
   if( hideDelimiter ) {
      ...
      ctl_params.p_height=ctl_delimiter.p_y_extent;
   ...
   index := ctl_params._TreeCurIndex();
 ...
 *               Defaults to true.
 */
void _ctParametersViewEnable(bool enable=true)
{
 ...
 *                 Defaults to true.
 */
void _ctParametersViewReadOnly(bool readOnly=true)
{
   _control ctl_params, ctl_param_add, ctl_param_edit, ctl_param_remove, ctl_delimiter;
...
{
   rightAlign := ctl_preview_frame.p_x_extent;
...
static bool verifyFileFormInput(_str& src, bool& copySrcToTemplateDir, _str& target, bool& replaceParams)
{
   ...
   if( src=="" || !file_exists(src) ) {
      msg := "Missing or bad source file name. Please provide a valid source file name.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
   ...
   target=strip(target);
   if( target=="" ) {
      msg := "Missing or bad target file name. Please provide a valid target file name.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
...
static const TARGET_EXAMPLE_FORMATSPEC= "Example: $fileinputname$.%s";
static const PREVIEW_FORMATSPEC= "%s\n\nwould be copied to:\n\n%s";
...
static void updatePreview()
{
   ...
   if( target=="" ) {
      ...
      if( !_file_eq(target,absolute(target)) &&
          !isdrive(target) &&
          !( _isWindows() && substr(target,1,2)=='\\' && length(target)==2 ) ) {
...
void ctl_source.on_change()
{
   ...
   if( templateDir!="" ) {
      _maybe_append_filesep(templateDir);
      if( _file_eq(templateDir,substr(src,1,length(templateDir))) ) {
         copy=1;
         enabled=false;
      }
   } else {
...
_command void ctInsertSubstitution(_str text="") name_info(','VSARG2_EXECUTE_FROM_MENU_ONLY|VSARG2_READ_ONLY)
{
...
void ctl_ok.on_create(_str Name="", _str Value="", bool Prompt=false, _str PromptString="",
                      _str caption="", bool readOnly=false)
{
   if( caption!="" ) {
      p_active_form.p_caption=caption;
   }
...
static bool verifyParameterFormInput(_str& Name, _str& Value, bool& Prompt, _str& PromptString)
{
   _control ctl_name, ctl_value, ctl_prompt, ctl_promptstring;

   Name=strip(ctl_name.p_text);
   if( Name=="" ) {
      msg := "Missing parameter name. Please provide a valid parameter name.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      p_window_id=ctl_name;
      _set_sel(1,length(p_text)+1);_set_focus();
      return false;
   ...
   if( 1 != pos('{#0'CTPARAMETER_NAME_RE'}',Name,1,'er') || pos('0') < length(Name) ) {
      msg := "Invalid parameter name. Please provide a valid parameter name.";
      _message_box(msg,"",MB_OK|MB_ICONEXCLAMATION);
      p_window_id=ctl_name;
      _set_sel(1,length(p_text)+1);_set_focus();
      return false;
