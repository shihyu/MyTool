...
static const VAREDITSV_SAFE_EXIT= 0x1;

int def_sv_flags=0;

static const VAREDITSV_ELEMENT= 1;
static const VAREDITSV_ARRAY= 2;
static const VAREDITSV_HASHTAB= 3;
static const VAREDITSV_NOTINIT= 4;
...
void _ctlsv_value.on_change()
{
   ...
   wid := p_window_id;p_window_id=_ctlsv_list;
...
static void svinit_list_font()
{
   ...
   parse _getSelectionListFont() with font_name',' font_size','font_flags . ;
...
static void collapse_var()
{
   typeless p;
   _ctlsv_list.save_pos(p);
   oldwid := p_window_id;p_window_id=_ctlsv_list;
   subscriptlist := "";
   indent := pic := 0;
...
/* Fills in the value textbox, and sets its enabled property properly.  Also
   makes not initialized message visible if necessary.*/
void _ctlsv_list.on_change(int reason)
{
   ...
   case CHANGE_CLINE:
      ...
      if (pic!='') {
         ...
         _ctlsv_notinit_message.p_visible=false;
      ...
      case VF_EMPTY:
         _ctlsv_notinit_message.p_visible=true;
         _ctlsv_value.p_enabled=_ctlsv_value_label.p_enabled=false;
         _ctlsv_ptr_deref_message.p_visible=false;
         break;
      default:
         if (ptr->_varformat()==VF_ARRAY||ptr->_varformat()==VF_HASHTAB||ptr->_varformat()==VF_OBJECT) {
            _ctlsv_value.p_enabled=_ctlsv_value_label.p_enabled=false;
         }else{
            _ctlsv_value.p_enabled=_ctlsv_value_label.p_enabled=true;
         }
         _ctlsv_notinit_message.p_visible=false;
         _ctlsv_ptr_deref_message.p_visible=isptr;
         break;
...
void _ctlsv_delete.lbutton_up()
{
   ...
   shift := !_lbdelete_item();//Delete the line that we're on
   if (shift) _lbup();
   ...
   isarray := (ptr->_varformat()==VF_ARRAY);
   if (isarray&&shift) {
...
void _var_editor_form.on_resize()
{
   ...
   if (p_active_form.p_width<_ctlsv_format.p_x_extent+border_width+_ctlsv_ok.p_x) {
      //p_active_form.p_width=_ctlsv_format.p_x_extent+border_width+_ctlsv_ok.p_x;
   }

   _ctlsv_ok.p_visible=_ctlsv_expand_all.p_visible=_ctlsv_collapse_all.p_visible=_ctlsv_update.p_visible=_ctlsv_refresh.p_visible=_ctlsv_compact.p_visible=false;

   _ctlsv_list.p_width=p_active_form.p_width-(_ctlsv_list.p_x+(border_width*2));
   int button_buffer=_ctlsv_insert.p_y-(_ctlsv_ok.p_y_extent);
   ...
   _ctlsv_list.p_y_extent = _ctlsv_value.p_y-100;

   _ctlsv_ok.p_visible=_ctlsv_expand_all.p_visible=_ctlsv_collapse_all.p_visible=_ctlsv_update.p_visible=_ctlsv_refresh.p_visible=_ctlsv_compact.p_visible=true;
#if 1
...
_ctlsvi_ok.on_create()
{
   ...
   if (no_notinit) {
      ...
      _ctl_notinit.p_visible=false;
   }
   p_active_form.p_height=_ctlsvi_ok.p_y_extent+200+p_active_form._top_height()+p_active_form._bottom_height();
   if (arg(4)!='') {
      switch (arg(4)) {
      case VF_EMPTY:
