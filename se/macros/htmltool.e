...
static const MAX_HLINE_THICKNESS= 10;
static const MAX_BORDER_THICKNESS= 15;
static const IMG_FILE_EXT= "*.bmp;*.jpg;*.jpeg;*.gif;*.svg";
static const GIF_FILE_EXT= "*.gif";
static const HTML_FILE_EXT="*.htm;*.html;*.shtml;*.asp;*.jsp;*.php3;*.php;*.rhtml";
static const APPLET_FILE_EXT="*.class";
static const JAVASCRIPT_FILE_EXT="*.js";
static const PHP_FILE_EXT="*.php3;*.php";
static const STYLE_FILE_EXT="*.css;*.htm;*.html";
static const VBSCRIPT_FILE_EXT="*.bas;*.vbs";
static const RGB_DEFAULT= 0x000000;
static const MAX_HEX_LENGTH= 6;
static const MAX_HTML_SIZE= 7;
static const DEFAULT_HTML_FONT= 3;
 ...
 */
static _str convert_rgb_to_html_color(_str val, bool use_name=false)
{
   // the value is already a color name
   if (!isinteger(val) || _first_char(val) == '#') {
      return val;
   }

   // Check to see if the word name is to be used
   if (LanguageSettings.getUseColorNames(html_langId()) || use_name) {
      for (x := 0; x < ColorEntryList._length(); x++) {
         if (ColorEntryList[x].rgb_val == (int)val) {
   ...
   html_val := _dec2hex((int)val,16,6);
...
/**
 * Convert an HTML color name to an RGB value.
 */
static int convert_html_color_to_rgb(_str val)
{
   if (isinteger(val)) {
      return (int)val;
   }
   if (_first_char(val) == '#') {
      return _hex2dec(substr(val,2));
   }
   for (x := 0; x < ColorEntryList._length(); x++) {
      if (ColorEntryList[x].color_name == val) {
         return (ColorEntryList[x].rgb_val);
      }
   }
   // should not get here
   return 0;
...
{
   if (LanguageSettings.getQuoteNumberValues(html_langId())) {  //Check to see if they want quotes around numbers
      return('"':+value:+'"');
   }
   return(value);
}
...
_str case_html_string( _str tag, int type = 0)
{
   ...
   if (!type) {  //If it is not an attrib, use casing for tags
      ...
      if (LanguageSettings.getQuoteWordValues(html_langId())) {
         tag = '"'tag'"';
      }
   }
   ...
   if (scase < 0) {
      // Default to lower on error or "PRESERVE", since PRESERVE
      // has no meaning when we're creating new tags.
      scase = WORDCASE_LOWER;
   }
 ...
 * @return cased tag
 */
_str case_html_tag( _str tag, bool attrib = false, _str sample="")
{
   ...
   if ( scase < 0) {
      if (sample != "") {
         if (upcase(sample)==sample) {
            /* Upper case language key words. */
            return(upcase(tag));
         } else if (upcase(_first_char(sample)) == _first_char(sample)) {
            camelCasePrefix := "Access Content OnCue OnDbl OnDouble OnSingle OnScroll OnKey OnLoaded OnLoad OnTime OnRate OnMouse OnEvent OnCanPlay OnVolume OnDuration On";
            foreach (auto camelCaseWord in camelCasePrefix) {
               if (lowcase(substr(tag,1,length(camelCaseWord)))==lowcase(camelCaseWord)) {
                  return(camelCaseWord:+upcase(substr(tag,length(camelCaseWord)+1,1)):+lowcase(substr(tag,length(camelCaseWord)+2)));
               }
            }
...
_str _html_keyword_case(_str s, bool confirm=true, _str sample="")
{
   return case_html_tag(s, false);
}
...
static int getAttributeCasing()
{
   ...
   setting := get_format_setting(VSCFGP_BEAUTIFIER_WC_ATTR_NAME);
   if (setting != '') {
      return (int)setting;
   }
 ...
 */
bool checkHTMLContext(bool quiet=false)
{
 ...
 * @return
 */
_command insert_html_bold() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
...
static void insert_single_line_tags(_str start_tag, _str end_tag)
{
   ...
   int standalone_val = markup_standalone(tagname);
   int begin_val = markup_num_lines_before(tagname);
   int after_val = markup_num_lines_after(tagname);
   indent_col := p_col;  //Establish the current indent level
   ...
   if (select_active()) {
      ...
      if (!begin_val && !after_val) {
         _insert_text(start_tag);
         save_pos(auto pp);
         _insert_text(end_tag);
         restore_pos(pp);
         return;
      }
   }
...
static void insert_block_tags(_str begin_tag,_str end_tag)
{
   ...
   if (tagname!= '') {
      indent = (int)markup_indent_content(tagname);
   }
   indent_col := p_col;  //Establish indent level
   if (select_active()) {   //Check to see if we are working with selected text
      ...
      if (_select_type() == 'CHAR') {
         ...
         _str textBeforeCursor=_expand_tabsc(1,p_col-1,'S');
         if (textBeforeCursor!="") {
            textAfterCursor := strip(_expand_tabsc(p_col,-1,'S'),'L');
            _delete_text(-1);
            insert_line(indent_string(indent_col-1):+textAfterCursor);
            sline = p_line;
 ...
 * @return
 */
_command insert_html_italic() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('I');
}
 ...
 * @return
 */
_command insert_html_em() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('Em');
}
 ...
 * @return
 */
_command insert_html_code() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_uline() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('U');
}
 ...
 * @return
 */
_command insert_html_table_caption() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_table_row() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('Tr');
}
 ...
 * @return
 */
_command insert_html_table_col() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('Td');
}
 ...
 * @return
 */
_command insert_html_table_header() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   insert_html_tag('Th');
}
 ...
 * @return
 */
_command insert_html_paragraph() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_center() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_right() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_left() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
...
void insert_html40_attributes(_str &line)
{
   ...
   if (_param8!='') {
      line :+= case_html_string(' Style="',1):+_param8:+'"';
   }
   if (_param9!='') {
      line :+= case_html_string(' Class="',1):+_param9:+'"';
   }
   if (_param10!='') {
      line :+= case_html_string(' Id="',1):+_param10:+'"';
   }
}
 ...
 * @return
 */
_command insert_html_table() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   begin_tag :=  table_line:+_param1:+'>';
   if (!markup_endtag('table')) {
      /*     indent_col=p_col;
 ...
 * @return
 */
_command insert_rgb_value() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');

   //Bring up the _html_insert_rgb dialog
   result := show('-modal _html_insert_rgb_color_form');
   if (result=='') {
      return(COMMAND_CANCELLED_RC);
   }
   ...
   if (_LanguageInheritsFrom('bbc')) {
      insert_html_tag('[color='convert_rgb_to_html_color(result)']','[/color]',true,false);
      return 0;
   }

   _insert_text(html_use_quote(convert_rgb_to_html_color(result)));
}
 ...
 */
void insert_html_tag( _str tagopen = '', _str tagclose = '', bool preset_tags = false, bool caps = true)
{
   start_tag := "";      //Opening tag
   end_tag := "";      //Ending tag
   if (caps) {
      tagopen = case_html_tag(tagopen);
      tagclose = case_html_tag(tagclose);
   }
 ...
 * @return
 */
_command insert_html_script()name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   script_line :+= case_html_string(' Language="',1):+_param1:+'"';
   //<SCRIPT LANGUAGE="" SRC="">
   begin_tag := "";
   if (_param3) {
      ...
      insert_html_tag(begin_tag,end_tag,true,false);
      int value = markup_standalone('script');
      ...
      if (_param2 != '') {
         ...
         get(_maybe_quote_filename(_param2));
      }
   }
}
 ...
 * @return
 */
_command insert_html_heading() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
 ...
 * @return
 */
_command insert_html_applet()name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   if (_param1 != '') {
      applet_line :+= case_html_string(' Codebase="',1):+_param1:+'"';

   }

   applet_line :+= case_html_string(' Code="',1):+_param2:+'"';

   if (_param3 !='') {
      if (_param4 !='') {
         applet_line :+= case_html_string(' Width=',1):+html_use_quote(_param3);
         applet_line :+= case_html_string(' Height=',1):+html_use_quote(_param4);
      }
   }
   if (_param5 !='') {
      applet_line :+= case_html_string(' Alt="',1):+_param5:+'"';
   }
   if (_param6 !='' && _param6 != 0) {
      applet_line :+= case_html_string(' Border=',1):+html_use_quote(_param6);
   }

   begin_tag :=  applet_line:+'>';
 ...
 * @return
 */
_command insert_html_anchor() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   begin_tag :+= case_html_string(' Name="',1):+_param1:+'">';
 ...
 * @return
 */
_command insert_html_hline() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   if (_param1 !='') {
      hline_line :+= case_html_string(' Width=',1):+html_use_quote(_param1);
   }
   if (_param2 != '') {
      hline_line :+= case_html_string(' Size=',1):+html_use_quote(_param2);
   }
   if (_param3 != '') {
      hline_line :+= case_html_string(' Align=',1):+case_html_string(_param3,2);
   }
   if (_param4) {
      hline_line :+= case_html_string(' Noshade',1);
   }
   hline_line :+= '>';
 ...
 * @return
 */
_command insert_html_link() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   ...
   _str begin_tag = case_html_string('<A');
   insert_html40_attributes(begin_tag);
   begin_tag :+= case_html_string(' Href="',1):+_param1:+'">';
   _str end_tag = case_html_string('</A>');
   //<A HREF=""></A>
   insert_html_tag(begin_tag,end_tag,true,false);
   _insert_text(_param2);   //Insert text for the link
 ...
 * @return
 */
_command insert_html_image() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if (!checkHTMLContext()) return('');
   image_line := "";
   ...
   image_line :+= case_html_string(' Src="',1):+_param1:+'"';
   if (_param2 != '') {
      image_line :+= case_html_string(' Height=',1):+html_use_quote(_param2);
   }
   if (_param3 != '') {
      image_line :+= case_html_string(' Width=',1):+html_use_quote(_param3);
   }
   ...
   if (_param4 != '(Default)') {
      ...
      image_line :+= case_html_string(' Align=',1):+case_html_string(_param4,2);
   }
   if (_param6 != '' && _param6 != 0) {
      image_line :+= case_html_string(' Border=',1):+html_use_quote(_param6);

   }
   if (_param5 != '') {
      //Check for Alternate text.  If there is any, insert it
      image_line :+= case_html_string(' Alt="',1):+_param5:+'"';
   }
   image_line :+= '>';
 ...
 * @return
 */
_command insert_html_font() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
...
_command insert_html_list()name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL)
{
   if (!checkHTMLContext()) return('');
   ...
   if (_param3 != '') {
      start_tag :+= case_html_string(' Start=',1):+html_use_quote(_param3);
   }
   if (_param2 != '') {
      start_tag :+= case_html_string(' Type="',1):+_param2:+'"';
   }
   start_tag :+= '>';
   col := p_col;
...
_command insert_html_body()name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_REQUIRES_EDITORCTL){
...
void extract_body_attributes(struct tag_attribute (&attribute_list)[])
{
   ...
   for (x = 0; x < attribute_names._length() ;x++) {
      begin_select();
      status := search(attribute_names[x]:+'=','@+hIWXC');
      if (!status) {
         if (attribute_names[x] == "BACKGROUND" ) {
            ...
            if (loc) { //We have a hex val
               ...
               red := substr(attrib_val,1,2);
               green := substr(attrib_val,3,2);
               blue := substr(attrib_val,5,2);
               attribute_list[y].attribute_color= _hex2dec('0x':+blue:+green:+red);
...
void _styles_link.lbutton_up(){
   if (_styles_link.p_value) {
      _styles_loc.p_enabled = true;
      ctl_stylesbrowse.p_enabled = true;
      _styles_url.p_enabled=true;
      _styles_file.p_enabled=true;
   } else {
      _styles_loc.p_enabled = false;
      ctl_stylesbrowse.p_enabled = false;
      _styles_url.p_enabled=false;
      _styles_file.p_enabled=false;
   }
}
void ctl_stylesbrowse.lbutton_up()
{
   format_list := 'Style sheets('STYLE_FILE_EXT')';
   _str default_exts= STYLE_FILE_EXT;
   format_list :+= ', All Files('ALLFILES_RE')';
...
ctlok.on_create()
{
   ...
   _bg_color_sample.p_enabled = false;
   _bg_border_sample.p_enabled = false;
   _bg_color_sample.p_user = null;
   _bg_border_sample.p_user = null;
}
void ctlbgcolors.lbutton_up(){

   result := show('-modal _html_insert_rgb_color_form');
   if (result != '') {
      _bg_color_sample.p_enabled = true;
      _bg_color_sample.p_backcolor = convert_html_color_to_rgb(result);
      _bg_color_sample.p_user = result;
...
void ctlbgborder.lbutton_up(){
   ...
   result := show('-modal _html_insert_rgb_color_form');
   if (result != '') {
      _bg_border_sample.p_enabled = true;
      _bg_border_sample.p_backcolor = convert_html_color_to_rgb(result);
      _bg_border_sample.p_user = result;

   }
}

static _str get_html_color_attribute_value(_str text=null)
{
   if (text == null) text = p_user;
   if (text != null && !isinteger(text) && convert_html_color_to_rgb(text) == p_backcolor) {
      return text;
   }
   color := _form_parent().convert_rgb_to_html_color(p_backcolor);
   return color;
}
...
void ctlok.lbutton_up(){
   ...
   if (!_table_none.p_value) {
      _param1 :+= _form_parent().case_html_string(' Align=',1);
      if (_table_left.p_value) {
         _param1 :+= _form_parent().case_html_string('Left',2);
      } else if (_table_right.p_value) {
         _param1 :+= _form_parent().case_html_string('Right',2);
      } else {
         _param1 :+= _form_parent().case_html_string('Center',2);
      }
   }
   if (_bg_color_sample.p_enabled) {
      color := _bg_color_sample.get_html_color_attribute_value();
      _param1 :+= _form_parent().case_html_string(' Bgcolor="',1):+color:+'"';
   }
   if (_table_border.p_text !='') {
      ...
      _param1 :+= _form_parent().case_html_string(' Border=',1):+_form_parent().html_use_quote(_table_border.p_text);

   }
   if (_bg_border_sample.p_enabled) {
      color := _bg_border_sample.get_html_color_attribute_value();
      _param1 :+= _form_parent().case_html_string(' Bordercolor="',1):+color:+'"';
   }
   if (_table_padding.p_text !='') {
      ...
      _param1 :+= _form_parent().case_html_string(' Cellpadding=',1):+_form_parent().html_use_quote(_table_padding.p_text);
   ...
   if (_table_spacing.p_text !='') {
      ...
      _param1 :+= _form_parent().case_html_string(' Cellspacing=',1):+_form_parent().html_use_quote(_table_spacing.p_text);
   ...
   if (_table_width.p_text !='') {    //Checks for the table Width
      ...
      _param1 :+= _form_parent().case_html_string(' Width=',1):+_form_parent().html_use_quote(_table_width.p_text:+percent);
   ...
   if (_table_height.p_text != '') {   //Checks for the table Height
      ...
      _param1 :+= _form_parent().case_html_string(' Height=',1):+_form_parent().html_use_quote(percent:+_table_height.p_text);
   }
   if (_table_hspace.p_text !='') {    //Checks for the table HSpace
      ...
      _param1 :+= _form_parent().case_html_string(' Hspace=',1):+_form_parent().html_use_quote(_table_hspace.p_text);
   ...
   if (_table_vspace.p_text !='') {    //Checks for the table VSpace
      ...
      _param1 :+= _form_parent().case_html_string(' Vspace=',1):+_form_parent().html_use_quote(_table_vspace.p_text);
...
static _str HTMLTOOL_CHANGING_NAME_LIST(...) {
   if (arg()) _font_name_list.p_user=arg(1);
   return _font_name_list.p_user;
}

ctlok.on_create(bool isgeneric = false){
   initialize_style_vals();
   FillInFontNameList(isgeneric);
}
ctlok.lbutton_up() {
   p_active_form._delete_window(_font_name_list.get_font_name_from_list());
}
 ...
 */
static _str generic_font_types[] = {"Monospace","Fantasy","Serif","Sans Serif","Cursive"};
static void FillInFontNameList(bool isgeneric = false)
{
   ...
   for (;;) {
      if (down()) break;
      name := get_font_name_from_list(true);
      switch (name) {
...
{
   if (HTMLTOOL_CHANGING_NAME_LIST) {
      _str font_name=get_font_name_from_list();
      _str lbtext=get_font_name_from_list(true);
...
static void _html_insert_font_form_initial_alignment()
{
   ...
   sizeBrowseButtonToTextBox(_fontnameg.p_window_id, ctlfont_browseg.p_window_id, 0, rightAlign);
}
...
void ctlcolors.lbutton_up(){
   ...
   result := show('-modal _html_insert_rgb_color_form');
   if (result != '') {
      _color_sample.p_enabled = true;
      _color_sample.p_backcolor = convert_html_color_to_rgb(result);
      _color_sample.p_user = result;
   }
}

void ctlok.lbutton_up()
{
   alignval :=  _param1 = _param2 = '';

   if (_fontname1.p_text !='' || _fontname2.p_text !='' || _fontname3.p_text != ''
       || _fontnameg.p_text != '' || _color_sample.p_enabled || _font_size.p_text !=''|| _param1 == '') {
      htmlfonts := "";
      ...
      _param1 :+= _form_parent().case_html_string('<Font');
      if (_color_sample.p_enabled) {
         color := _color_sample.get_html_color_attribute_value();
         _param1 :+= _form_parent().case_html_string(' Color="',1):+color:+'"';
      }

      if (htmlfonts != '') {
         _param1 :+= _form_parent().case_html_string(' Face="',1):+htmlfonts:+'"';
      }

      if (_font_size.p_text !='') {
         _param1 :+= _form_parent().case_html_string(' Size=',1):+_form_parent().html_use_quote(_font_size.p_text);
      }
      _param1 :+= '>';
...
void ctlok.on_create(){
   ...
   _retrieve_prev_form();
   _color_sample.p_enabled = false;
   _color_sample.p_user = null;
...
void ctlok.lbutton_up()
{
   ...
   if (_script_file.p_value && _script_filename.p_text != '') {
      ...
      if ((file_match(_maybe_quote_filename(_param2)'  -p',1))== '') {                                         //If there is no such file...
...
void _script_file.on_got_focus(){
   ctl_scriptbrowse.p_enabled = true;
}
void _script_url.on_got_focus(){
   ctl_scriptbrowse.p_enabled = false;
}
...
void _anchor_selected.lbutton_up()
{
   ...
   anchor_list._makeempty();
   filename := _link_name.p_text;
   path := _strip_filename(_form_parent().p_buf_name,'N');
...
void ctl_linkbrowse.on_create(bool supportBBC=false)
{
   initialize_style_vals();
...
void ctlok.lbutton_up()
{
   ...
   if (_param1 == '') {
      if (_anchor_current.p_value && !_anchor_list._lbfind_selected(true)) {
...
void ctl_imagebrowse.on_create(bool supportBBC=false)
{
...
void ctl_imagebrowse.lbutton_up()
{
   ...
   if (pos(ext,IMG_FILE_EXT,1,'I')) {
      ...
      } else if (_file_eq(ext,'jpg') || _file_eq(ext,'jpeg')) {
...
static const UNASSIGNED_COLOR_TEXT = '(Unassigned)';
...
ctlok.lbutton_up()
{
   ...
   if ((_body_image.p_text!='')) {
      _param1 :+= _form_parent().case_html_string(' Background="',1):+_body_image.p_text:+'"';
   }
   if ((_bg_color.p_text!='') && (_bg_color.p_text!=UNASSIGNED_COLOR_TEXT)) {
      color := _bg_sample.get_html_color_attribute_value(_bg_color.p_text);
      _param1 :+= _form_parent().case_html_string(' Bgcolor=',1):+_form_parent().html_use_quote(color);
   }
   if ((_text_color.p_text!='') && (_text_color.p_text!=UNASSIGNED_COLOR_TEXT)) {
      color := _text_sample.get_html_color_attribute_value(_text_color.p_text);
      _param1 :+= _form_parent().case_html_string(' Text=',1):+_form_parent().html_use_quote(color);
   }
   if ((_link_color.p_text!='') && (_link_color.p_text!=UNASSIGNED_COLOR_TEXT)) {
      color := _link_sample.get_html_color_attribute_value(_link_color.p_text);
      _param1 :+= _form_parent().case_html_string(' Link=',1):+_form_parent().html_use_quote(color);
   }
   if ((_vlink_color.p_text!='') && (_vlink_color.p_text!=UNASSIGNED_COLOR_TEXT)) {
      color := _vlink_sample.get_html_color_attribute_value(_vlink_color.p_text);
      _param1 :+= _form_parent().case_html_string(' Vlink=',1):+_form_parent().html_use_quote(color);
   }
   if ((_alink_color.p_text!='') && (_alink_color.p_text!=UNASSIGNED_COLOR_TEXT)) {
      color := _alink_sample.get_html_color_attribute_value(_alink_color.p_text);
      _param1 :+= _form_parent().case_html_string(' Alink=',1):+_form_parent().html_use_quote(color);
   }
   p_active_form._delete_window(1);
}
...
{
   sizeBrowseButtonToTextBox(_body_image.p_window_id, ctl_image_browse.p_window_id, 0, _bg_sample.p_x_extent);
...
void ctl_bg_color.lbutton_up()
{
   ...
   if (result != '') {
      ...
      _link_sample.p_backcolor=
      _text_sample.p_backcolor=
      _bg_sample.p_backcolor = convert_html_color_to_rgb(result);

      _bg_color.p_ReadOnly=false;
      _bg_color.p_text=_form_parent().convert_rgb_to_html_color(result);
      _bg_color.p_ReadOnly=true;
...
void ctl_text_color.lbutton_up()
{
   ...
   result := show(' -modal _html_insert_rgb_color_form');
   if (result != '') {
      _text_sample.p_forecolor = convert_html_color_to_rgb(result);
      _text_color.p_ReadOnly=false;
      _text_color.p_text=_form_parent().convert_rgb_to_html_color(result);
      _text_color.p_ReadOnly=true;
...
void ctl_link_color.lbutton_up()
{
   ...
   result := show(' -modal _html_insert_rgb_color_form');
   if (result != '') {
      _link_sample.p_forecolor = convert_html_color_to_rgb(result);
      _link_color.p_ReadOnly=false;
      _link_color.p_text=_form_parent().convert_rgb_to_html_color(result);
      _link_color.p_ReadOnly=true;
...
void ctl_alink_color.lbutton_up()
{
   ...
   result := show(' -modal _html_insert_rgb_color_form');
   if (result != '') {
      _alink_sample.p_forecolor = convert_html_color_to_rgb(result);
      _alink_color.p_ReadOnly=false;
      _alink_color.p_text=_form_parent().convert_rgb_to_html_color(result);
      _alink_color.p_ReadOnly=true;
...
void ctl_vlink_color.lbutton_up()
{
   ...
   result := show(' -modal _html_insert_rgb_color_form');
   if (result != '') {
      _vlink_sample.p_forecolor = convert_html_color_to_rgb(result);
      _vlink_color.p_ReadOnly=false;
      _vlink_color.p_text=_form_parent().convert_rgb_to_html_color(result);
      _vlink_color.p_ReadOnly=true;
...
{
   this_color := sample.p_backcolor;
   red   := (this_color & 0x0000FF);
   green := (this_color & 0x00FF00) >> 8;
   blue  := (this_color & 0xFF0000) >> 16;
   if (!_rgb_dec.p_value) {
      _ignore_change=1;
      _red.p_text   = _dec2hex(red,16,2);
      _green.p_text = _dec2hex(green,16,2);
      _blue.p_text  = _dec2hex(blue,16,2);
      _ignore_change=0;
   } else {
      _ignore_change=1;
      _red.p_text   = red;
...
void ctlok.lbutton_up(){
   ...
   color := sample.p_backcolor;

   if (_colorlb._lbisline_selected()) {
      colorName := _colorlb._lbget_seltext();
      if (colorName != "") {
         for (x := 0; x < ColorEntryList._length(); x++ ) {
            if (ColorEntryList[x].rgb_val == color && ColorEntryList[x].color_name == colorName) {
               p_active_form._delete_window(colorName);
               return;
            }
         }
      }
...
void _rgb_spin_box.on_change()
{
   ...
   if (_rgb_hex.p_value && (_hex2dec('0x'p_text) < 0 || _hex2dec('0x'p_text) > 255) && _hex2dec('0x'p_text)!='') {
      _message_box('RGB values must be between 0 and 255!');
      //look up default value here maybe
      return;
   }
...
void get_rgb_colors(typeless &red = 0, typeless &green = 0, typeless &blue = 0)
{
   ...
   if (_rgb_hex.p_value) {
      red   = _hex2dec('0x'red);
      green = _hex2dec('0x'green);
      blue  = _hex2dec('0x'blue);
   }
