 ...
 * 
 */
_command indent_previous() name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
...
static long findMatchingWordBoundary(_str direction, long origOffset, long matchOffset)
{
   ...
   loop {
      ...
      // landed on an identifier character, jump to the beginning of word
      if ( isalnum(get_text()) ) {
         if ( direction=='-' ) {
            _begin_identifier();
         } else {
 ...
 */
static bool isMatchingWord(_str direction, _str &matchWord,
                              ...
                              long *otherWordOffset=null, long *otherEndOffset=null)
{
   ...
   for ( count := 0; count < def_pmatch_max_diff_ksize*10; ++count ) {
      // find another match
      status = _find_matching_paren(def_pmatch_max_diff_ksize,true);
      if ( status ) break;
   ...
   if ( isMatchWord && otherWordOffset != null && otherEndOffset != null ) {
      ...
      _find_matching_paren(def_pmatch_max_diff_ksize,true);
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int next_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int prev_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   return(do_next_prev_sexp('-'));
}
/**
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int forward_down_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int backward_down_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   switch ( get_text() ) {
   case ')':
   case ']':
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int backward_up_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   return do_up_sexp('-');
}
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int forward_up_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   return do_up_sexp('');
}
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int transpose_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int select_next_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int select_prev_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   if ( _select_type() != 'CHAR' ) {
      select_char();
   }
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int cut_next_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
   save_last_index := last_index('','C');
   doAppend := ( name_name(prev_index('','C'))=='cut-next-sexp' );
 ...
 * @categories CursorMovement_Functions, Edit_Window_Methods, Editor_Control_Methods
 */
_command int cut_prev_sexp() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
   save_last_index := last_index('','C');
   bool doAppend = ( name_name(prev_index('','C'))=='cut-prev-sexp' ||
 ...
 * 
 */
_command next_level() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   return(next_level2(1));
}
static _str next_level2(_str direction,_str option='',bool push=false)
{
   /* Search for '(','{' or '[' */
 ...
 * 
 */ 
_command prev_level() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   return(next_level2(''));
}
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command cut_prev_level() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command cut_level() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 */
_command void emacs_select_block() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods, Selection_Functions
 */
_command emacs_select_char(...) name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   _cua_select=0;
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command emacs_paste() name_info(','VSARG2_MULTI_CURSOR|VSARG2_MARK|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL|VSARG2_REQUIRES_CLIPBOARD)
{
   _str old_mark=_duplicate_selection('');
   _free_selection(emacs_mark);
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command cut_region() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   set_region();
   cut();
}
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command copy_region() name_info(','VSARG2_MULTI_CURSOR|VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   set_region();
   copy_to_clipboard();
   clear_message();
 ...
 * 
 */
_command indent_region() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   set_region();
   indent_selection();
 ...
 * 
 */
_command indent_rigidly() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   if ( _argument=='' ) {
 ...
 * 
 */ 
_command tabify_region() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
 ...
 * 
 */ 
_command untabify_region() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command delete_space() name_info(','VSARG2_MULTI_CURSOR|VSARG2_TEXT_BOX|VSARG2_REQUIRES_EDITORCTL)
{
   init_command_op();
   col := p_col;
   search('[ \t]#|?|$|^','@rh-');
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command void center_within_margins(_str alt_margins="") name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
   typeless lm,rm,i;
   if (length(alt_margins) > 0) {
      parse alt_margins with lm rm .;
   } else {
   ...
   line = strip(line);
   wm := (rm-lm+1);
   cline := center(line,wm);
   i = verify(cline,' ');
   if ( ! i ) {
      i=1;
   }
   if (length(line) > wm) {
      lm -= ((length(line) - wm) intdiv 2);
   }
   replace_line(indent_string(lm+i-1):+strip(line));
}
 ...
 * 
 */ 
_command reflow_region(_str alt_margins="") name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL|VSARG2_MARK)
{
   typeless lm, rm;
   if (length(alt_margins) > 0) {
      parse alt_margins with lm rm .;
...
/**
 * Used in EMACS emulation.  Loads a file from disk and closes 
 * the current buffer with it. Prompts you for the file to load 
 * on the command line.
 * 
 * @see edit
 * @see gui_visit_file
...
_command visit_file(_str line="") name_info(FILE_ARG','VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   if ( ! status ) {
      ...
      status=edit('-w '_maybe_quote_filename(strip(line)));
      if (!status ){
         int buf_id=p_buf_id;
...
_command show_point() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_EDITORCTL)
{
   ...
   sticky_message(nls('seek=%s buffer size=%s char is decimal=%s hex=0x%s',_QROffset(),p_RBufSize,chr_num,_dec2hex(chr_num)));
}
/**
...
{
   return(replace_string2(nls('Query replace:')' ',GO_SEARCH|def_re_search_flags));
...
static _str replace_string2(_str prompt,int flags)
{
   ...
   old_search_flags = (old_search_flags &~(INCREMENTAL_SEARCH|RE_SEARCH|VSSEARCHFLAG_PERLRE|VSSEARCHFLAG_VIMRE/*|UNIXRE_SEARCH|BRIEFRE_SEARCH*/))|
 ...
 * @categories Edit_Window_Methods, Editor_Control_Methods
 */
_command delete_blank_lines() name_info(','VSARG2_MULTI_CURSOR|VSARG2_REQUIRES_EDITORCTL)
{
...
_command emacs_quit() name_info(','VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   bool killing_current=_file_eq(doc_name,absolute(kill_buf_name)) ||
      _file_eq(strip(doc_name),kill_buf_name);
   wid := p_window_id;
   int buf_id=p_buf_id;
   if ( ! killing_current ) {
      attempt := absolute(kill_buf_name);
      if ( attempt=='' ) {
         return '';
      }
      status=edit('+b '_maybe_quote_filename(attempt));
      if ( status ) {
        clear_message();
        status=edit('+b '_maybe_quote_filename(kill_buf_name));
      }
      if ( status ) {
   ...
   kill_wid := p_window_id;
   int kill_buf_id=p_buf_id;
   modify := p_modify;
   nls_chars := yes := no := line := "";
   p_window_id=wid;p_buf_id=buf_id;
   ...
   if ( killing_current ) {
      ...
      clear_message();
      if (  _file_eq(buf_name,absolute(kill_buf_name)) ||
           _file_eq(strip(buf_name),kill_buf_name) ) {
         return(1);
      }
      p_modify=false;
      quit();
...
_command select_buffer(_str buffer_name="") name_info(EMACS_BUFFER_ARG','VSARG2_READ_ONLY|VSARG2_REQUIRES_MDI_EDITORCTL)
{
   ...
   if ( status ) {
      /* Create a new buffer */
      clear_message();
      status=edit('+t '_maybe_quote_filename(buffer_name));
   }
...
static void get_default_sb(var filename)
{
   ...
   if ( _file_eq(p_buf_name,absolute(filename)) || filename=='' ||
 ...
 * 
 */
_command gui_find_backward_regex() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   if ( def_re_search_flags&VSSEARCHFLAG_PERLRE ) {
      return( gui_find('-l') );
   } else if ( def_re_search_flags&VSSEARCHFLAG_VIMRE ) {
 ...
 * 
 */
_command gui_find_regex() name_info(','VSARG2_REQUIRES_EDITORCTL|VSARG2_READ_ONLY)
{
   if (def_re_search_flags & VSSEARCHFLAG_PERLRE) {
      return( gui_find('l') );
   } else if (def_re_search_flags & VSSEARCHFLAG_VIMRE) {
