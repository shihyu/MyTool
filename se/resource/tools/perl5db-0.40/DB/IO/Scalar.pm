...
sub TIEHANDLE {
    ((defined($_[1]) && UNIVERSAL::isa($_[1], "DB::IO::Scalar"))
     ? $_[1]
     : shift->new(@_));
}
...
Attempting to use these functions with an DB::IO::Scalar will not work
prior to 5.005_57. DB::IO::Scalar will not have the relevant methods
...
on an DB::IO::Scalar with an old Perl.  The remedy is to simply
...
in L<DB::IO::Stringy/"CHANGE LOG">.  But just the same, special
...
L<DB::IO::String>, which is quite similar but which was designed
