 ...
 *  
 */
struct VSDLLEXPORT str_match_t {
   int m_match_length;
   int m_match_start;   
   //unsigned int m_group_hit_flags;
   int m_strmatch_flags;
   //re_group_t m_group[MAX_RE_GROUPS];

   //slickedit::SEArray<cmRegexEngine::cmRegexTaggedExpression> *m_group_array;
   void *m_pgroup_array;
   ...
   * 
   * <p>
   * For SlickEdit regular expressions the first tagged expression
   * is 0.  For Perl, the first tagged expression is 1.
   *  
