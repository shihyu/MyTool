...
#include "tags/SEIDExpressionInfo.h"
...
#define VS_TAG_MAX_SIGNATURE  4096
#define VS_TAG_MAX_COMMENTS   8192
...
#define VS_TAG_MAX_OCCURRENCES_NAME_LIST  8192
#define VS_TAG_MAX_OCCURRENCES_NAME_PREFIX 2
...
int VSAPI vsTagFindExtCallback(const char *callback_name, 
                               const char *language_id = nullptr);
 ...
 */
EXTERN_C
int VSAPI vsTagGetDate(const char *pszFilename, char *pszDate = nullptr,const char *pszIncludedBy = nullptr);
 ...
 * @param pszIncludedBy    File path of source file which includes pszFilename 
 * @param pszLangId        Language ID for the file being modified 
 *
 * @return 0 on success, &lt;0 on error, BT_RECORD_NOT_FOUND_RC if the file is not found.
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI vsTagSetDate(const char *pszFilename, 
                       const char *pszDate = nullptr,
                       int file_type = 0, 
                       const char *pszIncludedBy = nullptr,
                       const char *pszLangId = nullptr);
 ...
 * @param langId           (optional) language ID for the file 
 *
 * @return 0 on success, &lt;0 on error, BT_RECORD_NOT_FOUND_RC if the file is not found.
 *
 * @categories Tagging_Functions
...
int VSAPI SETagSetFileDate(int dbHandle,
                           ...
                           int fileType = 0, 
                           const slickedit::SEString &includedBy = (const char *)nullptr,
                           const slickedit::SEString &langId = (const char *)nullptr);
...
int VSAPI vsTagGetLanguage(const char *pszFilename, 
                           char *pszLangId = nullptr,
                           int   maxLangId = VS_TAG_MAX_FILEEXT);
 ...
 *
 * @param dbHandle        database handle 
 * @param fileName        File path of source file to set tagging date for.
...
/**
 * Add an additional language ID to the tag database.  This is primarily used 
 * to make sure that embedded code segments are represented in the tag file's 
 * language ID table. 
 * 
 * @param dbHandle        database handle 
 * @param langId          p_LangId property
 * @param allowCreate     allow the language ID to be added to the tag database? 
 * 
 * @return 0 on success, &lt;0 on error, BT_RECORD_NOT_FOUND_RC if the file is not found.
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI SETagGetLanguageId(int dbHandle, const slickedit::SEString &langId, bool allowCreate=true);

/**
 * Retrieve the database file ID for the given file.
 * 
 * @param dbHandle        database handle 
 * @param fileName        File path of source file to look for.
 *
 * @return ID &gt;0 on success, &lt;0 on error, 
 *         BT_RECORD_NOT_FOUND_RC if the file is not found.
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI SETagGetFileID(int dbHandle, const slickedit::SEString& fileName);

/**
 * Remove the given file from the given tag database session.
 *
 * @param dbHandle        database handle 
 * @param fileName        File path of source file to remove.
 * @param removeFile      non-zero to remove file entirely, otherwise, 
 *                        only the symbols and references are removed.
 *
 * @return 0 on success, &lt;0 on error, BT_RECORD_NOT_FOUND_RC if the file is not found.
 *
 * @categories Tagging_Functions
 */
EXTERN_C int VSAPI SETagRemoveFromFile(int dbHandle, 
                                       const slickedit::SEString &fileName,
                                       int RemoveFile/*=0*/);
...
int VSAPI vsTagFindExtension(char *pszLangId, 
                             const char *pszSearchFor = nullptr, 
                             int find_first = 1);
 ...
 */
EXTERN_C
int VSAPI vsTagFindFile(char *pszFilename,const char *pszPrefix = nullptr, int find_first = 1);
 ...
 */
EXTERN_C
int VSAPI vsTagOpenDB(const char *pszFilename, int read_only = 1);
...
int VSAPI vsTagCreateDB(const char *pszFilename, 
                        int dbFlags = 0,
                        const char *dbDescription = nullptr);
 ...
 */
EXTERN_C
int VSAPI vsTagCloseDB(const char *pszFilename = nullptr, 
                       int leave_open = 0, 
                       int dbHandle = -1,
                       bool releaseExtraWriteLock = false);
 ...
 */
EXTERN_C
int VSAPI vsTagFlushDB(int dbHandle = -1);

/** 
 * Check if it is recommended to flush the given database session to disk 
 * at this point.  This is to prevent the database from accumulating too many 
 * unwritten blocks.
 *  
 * @param dbHandle    Explicit database handle to close. 
 *
 * @return Returns 1 if flush is recommended, 0 if not, &lt;0 on error.
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI vsTagNeedToFlushDB(int dbHandle = -1);
 ...
 */
EXTERN_C
int VSAPI vsTagRetagFile(const char *pszFilename,const char *pszExt = nullptr,int reserved = 0);
...
int VSAPI SETagInsertFileStart(int dbHandle, 
                               ...
                               const slickedit::SEStringSet *pExcludeNestedFiles = nullptr);
 ...
 * @param tag_type            type of tag, (see SE_TAG_TYPE_*)
...
int VSAPI vsTagInsertTag(const char *tag_name, const char *tag_type,
                         ...
                         const char *class_parents = nullptr,
                         const char *template_signature = nullptr);

/**
 * API function for inserting a tag entry with supporting info
 *
 * @param tag_name            name of entry
 * @param type_id             type of tag, (see SE_TAG_TYPE_*)
 * @param file_name           path to file that is located in
 * @param line_no             line number that tag is positioned on
 * @param class_name          name of class that tag belongs to
 * @param tag_flags           tag attributes (see SE_TAG_FLAG_*)
 ...
 */
EXTERN_C
int VSAPI vsTagInsertSimpleTag(const char *tag_name, 
                               const SETagType type_id,
                               const char *file_name, int line_no,
                               const char *class_name,
                               const SETagFlags tag_flags,
                               const char *signature = nullptr,
 ...
 * @param tagExceptions  (optional) function exceptions clause
 * @param doctype        (optional) documentation comment type (null, javadoc, etc) 
 * @param docComments    (optional) documentation comment text
 *
 * @return 0 on success, &lt;0 on error.
 *
 * @categories Tagging_Functions
...
int VSAPI SETagInsertInDatabase(int dbHandle,
                                slickedit::SEHashSet<slickedit::SETagInformation> *oldTags,
                                slickedit::SEHashSet<slickedit::SETagInformation> *relatedTags,
                                const slickedit::SEString &tag_name,   
                                const SETagType tag_type,
                                const slickedit::SEString &file_name,  
                                const unsigned int line_no,
                                const slickedit::SEString &class_name, 
                                const SETagFlags tag_flags,
                                const slickedit::SEString &signature     = (const char *)nullptr,
                                const slickedit::SEString &class_parents = (const char *)nullptr,
                                const slickedit::SEString &template_sig  = (const char *)nullptr,
                                const slickedit::SEString &tagExceptions = (const char *)nullptr,
                                const SETagDocCommentType docType = SE_TAG_DOCUMENTATION_NULL, 
                                const slickedit::SEString &docComments   = (const char *)nullptr );
 ...
 * @param relatedTags         tag information of symbols used to propagate tag flags 
 ...
 * @categories Tagging_Functions 
 *  
 * @deprecated Use SETagInsertContext() instead 
 */
EXTERN_C VSDEPRECATED
int VSAPI vsTagInsertContext(int outer_context,
                             ...
                             const SETagFlags tag_flags = SE_TAG_FLAG_NULL,
                             const char *signature = nullptr,
                             const char *class_parents = nullptr,
                             const char *template_sig = nullptr,
                             int name_line_no = 0, 
                             int name_seekpos = 0
                             );
 ...
 *
 * @param outer_context    context ID for the outer context (eg. class/struct)
 * @param tagInfo          tag information to insert into current context
 *
 * @return sequence number (context_id) of tag context on success, or &lt;0 on error.
 * @categories Tagging_Functions
 */
EXTERN_C int VSAPI
SETagInsertInfoInContext(int outer_context, const slickedit::SETagInformation &tagInfo);

/**
 * Transfer a pointer to a token list to the current context.
 * 
 * @param pTokenList   Pointer to token list.  This pointer must be a pointer 
 *                     acquired from {@link SEListTagsTarget}. 
 */
EXTERN_C int VSAPI
SETagSetContextTokenList(const slickedit::SETokenList *pTokenList);

EXTERN_C const slickedit::SETokenList * VSAPI SETagGetContextTokenList();

/**
 * Copy in the given set of tags, potentially with accompanying token list, 
 * to the current context. 
 * 
 * @param pContext     Asynchronous context tagging results to copy in
 */
EXTERN_C int VSAPI
SETagSetListTagsContext(const slickedit::SEListTagsTarget &context);

EXTERN_C int VSAPI
vsTagCurrentContext(int bufferId, const char *fileName, int lineNum, int seekPos);

EXTERN_C int VSAPI
vsTagCurrentStatement(int bufferId, const char *fileName, int lineNum, int seekPos);

EXTERN_C int VSAPI
vsTagCurrentLocal(int bufferId, const char *fileName, int lineNum, int seekPos);

EXTERN_C int VSAPI
vsTagGetCurrentContextID(const bool allowOutlineOnly=false);

EXTERN_C int VSAPI
vsTagGetCurrentStatementID(const bool allowOutlineOnly=false);

EXTERN_C int VSAPI
vsTagGetCurrentLocalID(const bool allowOutlineOnly=false);
 ...
 * @categories Tagging_Functions 
 *  
 * @deprecated use SETagInsertLocal() instead 
 */
EXTERN_C VSDEPRECATED
 ...
 *                         use concatenation (as defined by language rules)
 *                         to specify names of inner classes.
 * @param tag_flags        (optional) see SE_TAG_FLAG_* above.
 * @param signature        (optional) tag signature (return type, arguments, etc)
 * @param class_parents    (optional) classes that this class inherits from
 * @param template_sig     (optional) template signature
 * @param tagExceptions    (optional) function exceptions clause
 ...
 * prior to invoking this function.
 *
 * @param tagInfo             tag information to insert into locals
 *
 * @return sequence number (local_id) of local variable on success, or &lt;0 on error.
 *
 * @categories Tagging_Functions
 */
EXTERN_C int VSAPI
SETagInsertInfoInLocals(const slickedit::SETagInformation &tagInfo);

/**
 * Copy in the given set of tags to the current set of local variables.
 * 
 * @param pLocals     Asynchronous local variable tagging results to copy in
 */
EXTERN_C int VSAPI
SETagSetListTagsLocals(const slickedit::SEListTagsTarget &locals);
...
/**
 * Set the class inheritance for the given match tag.
 * <p> 
 * For synchronization, threads should perform a tag_lock_matches(true) 
 * prior to invoking this function.
 *
 * @param match_id         id for the match to modify
 * @param parents          parents of the match item
 *
 * @return 0 on success, &lt;0 on error.
 */
EXTERN_C
int VSAPI SETagSetMatchParents(int match_id, const slickedit::SEString &parents);

/**
 * Set the template signature for the given match tag.
 * <p> 
 * For synchronization, threads should perform a tag_lock_matches(true) 
 * prior to invoking this function.
 *
 * @param match_id         id for the match to modify
 * @param template_sig     template signature of the match item
 *
 * @return 0 on success, &lt;0 on error.
 */
EXTERN_C
int VSAPI SETagSetMatchTemplateSignature(int match_id, const slickedit::SEString &template_sig);
 ...
 */
EXTERN_C
int VSAPI vsTagUpdateContext(int ltf_flags = 0);
 ...
 */
EXTERN_C
int VSAPI vsTagUpdateLocals(int list_all = 1);
 ...
 * mode if necessary.
 *
 * @param orig_values   VSHVAR used to save the original mode name
 ...
 *
 * @param orig_values   VSHVAR used to restore the original mode.
 *
 ...
 */
EXTERN_C
int VSAPI vsTagRemoveFromFile(const char *pszFilename,int RemoveFile = 0);
 ...
 */
EXTERN_C
int VSAPI vsTagFindInFile(const char *pszFilename, int find_first = 1);
 ...
 */
EXTERN_C
int VSAPI vsTagFindClosest(const char *pszTagName, const char *pszFilename, int line_no, int case_sensitive = 0);
 ...
 * @param tag_name_len          length of tag name pattern
 * @param symbol_name           symbol name to check (must be non-null)
 * @param symbol_len            length of symbol name
 ...
 * @param pattern_char_bitset64 case-sensitive bitset of which ascii chars are used in 'tag_name_pattern'
 * @param pattern_char_bitset32 case-insensitive bitset of which ascii chars are used in 'tag_name_pattern'
 * 
 * @return Returns 'true' if the symbol matches the given pattern.
 *
 * @categories Tagging_Functions 
 * @see vsTagMakeSymbolNamePatternCharBitset 
 */
EXTERN_C bool VSAPI
vsTagMatchesSymbolNamePattern(const char *tag_name_pattern, const int tag_name_len,
                              const char *symbol_name, const int symbol_len,
                              bool exact_match, bool case_sensitive,
                              const SETagContextFlags context_flags,
                              const VSUINT64 pattern_char_bitset64=0,
                              const unsigned pattern_char_bitset32=0);
 ...
 */
EXTERN_C bool VSAPI
vsTagMatchesPrefixWithCorrections(const slickedit::SEString &identifierPrefix,
                                  const slickedit::SEString &symbolName,
                                  const bool caseSensitive=true,
                                  const size_t start_col=0);
 ...
 * @param context_flags    (optional) optional flags for specifying symbol 
 *                         pattern matching strategy (SE_TAG_CONTEXT_MATCH_*) 
 *
 * @return 0 on success, &lt;0 on error, BT_RECORD_NOT_FOUND_RC if not found.
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI vsTagFindEqual(const char *tag_name, const char *class_name=nullptr,
                         bool exact_match=true, bool case_sensitive=true,
                         bool find_first=true, bool skip_duplicates=false,
                         VSHREFVAR visited=0, int depth=0,
                         SETagContextFlags context_flags=SE_TAG_CONTEXT_NULL);
 ...
 *
 * @param type_id          Type id (SE_TAG_TYPE_*)
 *                         If (type_id&lt;0), returns tags with any user defined tag types.
 * @param mask             Tag attribute flags set (SE_TAG_FLAG_*).
 ...
 * <PRE>
 *      vsTagFindGlobalOfType(SE_TAG_TYPE_FUNCTION, SE_TAG_FLAG_INLINE, 1);
 * </PRE>
 *
 * @categories Tagging_Functions
 */
EXTERN_C
int VSAPI vsTagFindGlobal(const SETagType type_id, 
                          const SETagFlags mask = SE_TAG_FLAG_NULL,
                          const bool nzero = false,
...
int VSAPI vsTagFindRegex(const char *pszTagRegex,
                         const char *pszSearchOptions = nullptr, 
                         int find_first = 1);
 ...
 * @param type_id          tag type to search for tag_name having (SE_TAG_TYPE_*)
 * @param pszClassName     class name to search for tag_name having
 * @param find_first       find first match, or next match
 * @param pszSignature     function signature, ignored if no match found
 * @param pszFileName      file to expect tag to be in
 *
 ...
 */
EXTERN_C
int VSAPI vsTagFindTag(const char *pszTagName, 
                       const SETagType type_id,
                       const char *pszClassName=nullptr,
                       const bool find_first=true, 
                       const char *pszSignature=nullptr,
 ...
 */
EXTERN_C
int VSAPI vsTagFindClass(char *pszClassName, const char *pszSearch = nullptr,
                         int normalize = 0, int case_sensitive = 1,
                         int find_first = 1, const char *cur_class_name = nullptr);
 ...
 */
EXTERN_C
int VSAPI vsTagFindMember(const char *pszClassName, 
                          const char *pszFilename = nullptr, 
                          int find_first = 1, 
 ...
 * @param pszTypeName      Tag type, corresponding to SE_TAG_TYPE_*
 ...
 * @param tag_flags        Set of tag attribute flags, SE_TAG_FLAG_*.
...
const char * VSAPI vsTagMakeCaption(const char *pszTagName, const char *pszTypeName,
                                    ...
                                    const SETagFlags tag_flags = SE_TAG_FLAG_NULL,
                                    const char *pszArguments = nullptr,
                                    const bool include_tab = false,
                                    const char *pszFileName = nullptr,
                                    const char *pszTemplateArgs = nullptr);
 ...
 * @param pszTypeName      type of tag, (see SE_TAG_TYPE_*)
 ...
 */
EXTERN_C
int VSAPI tag_tree_insert_tag_outline(int tree_wid, int tree_index,
                                      int include_tab, int tree_flags,
                                      const char *pszTagName,
                                      const char *pszTypeName,
                                      const char *pszFileName, int line_no,
                                      const char *pszClassName = nullptr,
                                      VSUINT64Param tag_flags = 0,
                                      const char *pszSignature = nullptr,
                                      const char *pszTemplateArgs = nullptr,
                                      const char *pszNodeText = nullptr,
                                      VSHVAR hvarUserInfo = 0);
...
vsTagGetBitmap(int filter_flags_1, int filter_flags_2,
               const char *type_name, const char *class_name, 
               const SETagFlags tag_flags,
 ...
 * @param tag_flags        (Output) Tag flags indicating symbol attributes, SE_TAG_FLAG_*
 ...
 */
EXTERN_C
int VSAPI vsTagGetTagInfo(char *pszTagName, 
                          char *pszTypeName, 
                          SETagFlags &tagFlags,
                          char *pszFilename, 
                          int &line_no,
                          char *pszClassName = nullptr,
                          char *pszReturnType = nullptr,
                          char *pszArguments = nullptr,
                          char *pszExceptions = nullptr,
                          char *pszClassParents = nullptr,
                          char *pszTmplSignature = nullptr);
 ...
 */
EXTERN_C
int VSAPI vsTagGetDetailI(int detail_id, int item_id, int &iValue);
 ...
 */
EXTERN_C
int VSAPI vsTagGetDetailZ(int detail_id, int item_id, char *pszValue, int max_len = VS_TAG_MAX_FILENAME);
...
SETagInsertOccurrence(int dbHandle,
                      ...
                      const slickedit::SEString &file_name=(const char *)nullptr,
                      slickedit::SEHashTable<int, slickedit::SEStringSet> *occurHashTable=nullptr);
 ...
 */
EXTERN_C int VSAPI
vsTagFindOccurrence(const char *tag_name, int exact_match = 0,
                    int case_sensitive = 0, int find_first = 1);
...
vsTagContextMatchOccurrence(VSHREFVAR errorArgs,
                            ...
                            int &num_matches, int max_matches = VS_TAG_MAX_MATCHES);
 ...
 * @param visited         (optional, reference) hash table of past tagging results 
 * @param depth           (optional) search depth, to cap recursion 
 * @param pIDExprInfo     (optional, output) set to ID expression info
 * @param pReturnTypeInfo (optional, output) set to return type of prefix expression
 * @param pTokenList      (optional) token list for current file
 *
 * @return 0 on success, &lt;0 on error.
 *
 * @categories Tagging_Functions
...
vsTagContextMatchTags(VSHREFVAR errorArgs,
                      ...
                      VSHREFVAR visited=0, int depth=0,
                      slickedit::SEIDExpressionInfo *pIDExprInfo=nullptr,
                      slickedit::SEReturnTypeInfo *pReturnTypeInfo=nullptr,
                      const slickedit::SETokenList *pTokenList=nullptr);

/**
 * Compute the identifier and prefix expression information for the symbol 
 * at the given seek position. 
 * 
 * @param seekPosition    seek position of symbol under cursor
 * @param possibleOperator was a member access operator just typed?
 * @param langId          current language mode
 * @param idexpInfo       (output) identifier / prefix expression information
 * @param pTokenList      (optional) token list object
 ...
 */
EXTERN_C int VSAPI
vsTagGetExpressionInfo(seSeekPos seekPosition,
                       bool possibleOperator,
                       const slickedit::SEString &langId,
                       slickedit::SEIDExpressionInfo &idexpInfo,
                       slickedit::SEArray<slickedit::SEString> &errorArgs,
                       const slickedit::SETokenList *pTokenList=nullptr,
                       int depth=0);
 ...
 * @param tag_name        Name of occurrence to match against
 * @param case_sensitive  Case sensitive tag search? 
 * @param for_callers_tree Search is for the callers tree, not references.
 * @param file_name       Path of file tag to search for is in
 * @param line_no         Line number tag is located on 
 * @param tagged_line_no  Line number tag is located on according to tag database 
 * @param filter_flags    SE_TAG_FILTER_*, tag filter flags
 * @param context_flags   SE_TAG_CONTEXT_*, tag context filter flags
 * @param src_file_name   name of source file to search
 * @param lang_id         language for tag search 
...
vsTagMatchOccurrencesInFile(VSHREFVAR errorArgs,
                            ...
                            const bool for_callers_tree,
                            const char *file_name, 
                            const int line_no,
                            const int tagged_line_no,
                            const char *lang_id,
...
vsTagMatchUsesInFile(VSHREFVAR errorArgs,
                     ...
                     const SETagFilterFlags filter_flags, 
                     int caller_id,
                     const char *source_file_name,
                     int start_seekpos/*=0*/, int stop_seekpos/*=0*/,
                     int &num_matches, int max_matches,
 ...
 * @param line_number     (optional) line number where reference to class name in 'file_name'
 *
 * @return 0 on success, &lt;0 on error.
 *
 * @categories Tagging_Functions
...
vsTagNormalizeClasses(const char *class_parents,
                      ...
                      char *normal_types = nullptr, int max_type_len = 0,
                      char *normal_files = nullptr, int max_file_len = 0,
                      VSHREFVAR template_args = 0,
                      VSHREFVAR visited = 0, int depth = 0,
                      int line_number = 1);
 ...
 */
EXTERN_C int VSAPI
vsTagCompareClasses(const char *c1, const char *c2, bool case_sensitive = true);
 ...
 * @param is_importing    (optional) Are we already process an import statement? 
 *
...
vsTagListSymbolsInClass(int treewid, int tree_index, VSHREFVAR tag_files,
                        ...
                        const SETagFilterFlags pushtag_flags,
                        const SETagContextFlags context_flags,
                        int &num_matches, int max_matches,
                        int exact_match, int case_sensitive,
                        int depth = 0, int find_all = 1,
                        VSHREFVAR template_args = 0,
                        const char *friend_list = nullptr,
                        VSHREFVAR visited = 0,
 ...
 * @param cur_flags       type (SE_TAG_TYPE_*) of the current tag
 * @param cur_type_name   type ID (SE_TAG_TYPE_*) of the current tag
 *                        (allocate VS_TAG_MAX_TYPENAME bytes)
 * @param cur_type_id     bitset of SE_TAG_FLAG_* for the current tag
 ...
 */
EXTERN_C int VSAPI
vsTagGetCurrentContext(char *cur_tag_name, int &cur_flags,
                       char *cur_type_name, int &cur_type_id,
...
vsTagMatchSymbolInContext(const char *prefix, const char *search_class,
                          ...
                          int &num_matches,int max_matches,
                          SETagFilterFlags pushtag_flags, 
                          SETagContextFlags context_flags,
                          int exact_match = 0, int case_sensitive = 1,
                          int strict = 1,
                          int find_parents = 0, int find_all = 0,
 ...
 * @param tag_flags       (optional) integer tag flags (see SE_TAG_FLAG_*)
 ...
 * @param pszTypeName     (reference) the tag type, (see SE_TAG_TYPE_*)
 ...
 * @categories Tagging_Functions
 */
EXTERN_C VSPSZ VSAPI vsTagGetType(const SETagType type_id);
...
vsTagExtInheritsFrom(const char *parent, const char *lang = nullptr);
...
int vsTagCheckForPackage(const char* symbol, VSHREFVAR tag_files,
                         int exact_match, int case_sensitive,
                         char *aliased_to = nullptr, 
                         int aliased_to_len = 0,
                         VSHREFVAR visited = 0, int depth = 0);
 ...
 */
EXTERN_C int VSAPI vsTagQueueAsyncTarget(const slickedit::SEListTagsTarget &context, bool maybeWaitForSpace=false);
...
/**
 * Tag database open options.
 */
enum SETagDatabaseOpenMode {
   /**
    * Open the tag database for write access.  Only one thread can be 
    * writing to a database at a time, and no readers are allowed when 
    * there is a writer.
    */
   SE_TAG_DATABASE_OPEN_READ_WRITE = false,
   /**
    * Open the tag database for read access.  Multiple threads can read the 
    * same database at the same time, as long as there are no writers. 
    */
   SE_TAG_DATABASE_OPEN_READ_ONLY = true,
};

/**
 * Tag database close options.
 */
enum SETagDatabaseCloseMode : bool {
   /**
    * Close the database instance completely.
    */
   SE_TAG_DATABASE_CLOSE_COMPLETELY,
   /**
    * Close the database for writing, but leave the tag database open for reading.
    */
   SE_TAG_DATABASE_REOPEN_READ_ONLY,
};

/**
 * Tag database close options.
 */
enum SETagDatabaseUnlockMode : bool {
   /**
    * Just close the database, no unlock action.  This is the default mode.
    */
   SE_TAG_DATABASE_CLOSE_ONLY,
   /**
    * If this database was opened using {@link SETagOpenWriterDB()}, and then 
    * locked for writing in advance using {@link SETagWriteLockDB()}, release
    * the extra write lock now.  This is done to prevent a race condition that
    * could lead to deadlock.
    */
   SE_TAG_DATABASE_UNLOCK_AND_CLOSE,
};

/**
 * Open an existing database for read-only or read-write access.
 * The database type (tags, references) is automatically detected.
 * BSC files can not be opened read-write.
 *
 * @param tagFilename      File path of database to open.
 * @param readMode         Option to open the tag database for read access or not.
 *
 * @return database handle >= 0 on success, < 0 on error.
 *
 * @categories Tagging_Functions
 */
extern
int VSDLLEXPORT SETagOpenDB(const slickedit::SEString &tagFilename, 
                            const SETagDatabaseOpenMode readMode);

/**
 * Close the current database.
 *  
 * @param dbHandle         Database handle >= 0 from {@link SETagOpenDB()}.
 * @param closeOption      Options for closing the tag database. 
 * @param unlockOption     Options for unlocking tag database before closing.
 *
 * @return database handle >= 0 on success, < 0 on error.
 *
 * @categories Tagging_Functions
 */
extern
int VSDLLEXPORT SETagCloseDB(const int dbHandle,
                             const SETagDatabaseCloseMode closeOption=SE_TAG_DATABASE_CLOSE_COMPLETELY,
                             const SETagDatabaseUnlockMode unlockOption=SE_TAG_DATABASE_CLOSE_ONLY);
