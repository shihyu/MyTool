...
public:
   ...
   /**
    * Debugging function, prints memory usage statistics to stdout.
    */
   ...
   /**
    * Set the number of alloators to use.
    * 
    * @param n   number of allocators (2 <= n <= 16)
    */
   static void setNumberOfAllocators(const size_t n);
   /** 
    * @return 
    * Return the number of alloators to use (2 <= n <= 16).
    */
   static const size_t getNumberOfAllocators();
...
protected:
   ...
   struct SegmentInfo {
      ...
      struct Header {
         ...
         unsigned int mAllocatorIndex:4;
         // Actual size is this value in bytes
         unsigned int mSegmentSize:26;
         union {
...
public:
   ...
   enum {
      ...
      // minimum number of allocators to have to reduce thread contention
      MIN_ALLOCATORS =  2,

      // maximum number of allocators to have to reduce thread contention
      MAX_ALLOCATORS = 16,

      // default number of allocators
      DEFAULT_NUM_ALLOCATORS = 8,

      // maximum segment size (+1) we can store in mSegmentSize (26 bits)
      MAXINT_SEGMENT_SIZE = 0x04000000
   };
...
private:
   ...
   cmAllocator( const cmAllocator& rhs );
   // hide assignment operator
   cmAllocator& operator=( const cmAllocator& rhs );
   ...
   // max number of allocators to use to reduce thread contention
   static unsigned char mMaxAllocators;
...
inline size_t cmAllocator::getNumberOfBytesPerBlock()
{
   return BLOCK_SIZE;
}
