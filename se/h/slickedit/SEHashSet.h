...
public:
    ...
    */
   SEHashSet<T> &operator = (SEHashSet<T>&& src);
   ...
   int add(T &&item,int dupStatus=0);
...
template <class T> inline
SEHashSet<T>::SEHashSet(SEHashSet<T> && src) :
   // initialize list of items
   _kvlist(src._kvlist),
   _NofItems(src._NofItems),
   _NofChained(src._NofChained),
...
template <class T> inline
SEHashSet<T>& SEHashSet<T>::operator=(SEHashSet<T> && src)
{
   if (this != &src) {
...
template <class T> inline
int SEHashSet<T>::addPrivate(KVpair *pkv, unsigned int h)
{
...
template <class T> inline
int SEHashSet<T>::add(T &&item,int dupStatus)
{
   // compute hash bucket for new item
   unsigned int h=hashKey(item);
   ...
   // allocate the key-value pair
   KVpair *pkv = new KVpair(std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
...
int SEHashSet<T>::addDuplicate(const T &item)
{
   ...
   // add new item to the chain
   return addPrivate(pkv, hashKey(item));
}
template <class T> inline
int SEHashSet<T>::addDuplicate(T &&item)
{
   // allocate the key-value pair
   KVpair *pkv = new KVpair(std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
