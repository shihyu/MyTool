 ...
 * @return hash value
 */
VSDLLEXPORT unsigned int SEHashString(const char * const &p);
VSDLLEXPORT unsigned int SEHashWideString(const wchar_t * const &p);
...
/**
 * Callback for comparing two "const char*" strings used as hash table keys. 
 *  
 * @param a    key 1 
 * @param b    key 2 
 *  
 * @return &lt;0 for less than, 0 for equality, &gt;0 for greater than 
 */
VSDLLEXPORT int SEStrCmp(const char * const &a, const char * const &b);
VSDLLEXPORT int SEStrICmp(const char * const &a, const char * const &b);
 ...
 * @return hash value
 */
VSDLLEXPORT unsigned int SEHashPLstr(const VSLSTR * const &plstr);
...
public:
   ...
   typedef unsigned int (*HashProc)(const K &item);
   typedef int (*KeyCompareProc)(const K &k1, const K &k2);
    ...
    */
   SEHashTable(const size_t initialCapacity=89);
   ~SEHashTable();
    ...
    * Move constructor
    */
   SEHashTable(SEHashTable<K,T> && src);
   /**
    * Assignment operator
    */
    ...
    */
   SEHashTable<K,T> &operator = (SEHashTable<K,T> && src);
   ...
   int add(const K &key, T &&item);
   int add(K &&key, const T &item);
   int add(K &&key, T &&item);
   ...
   int addDuplicate(const K &key, T &&item);
   int addDuplicate(K &&key, const T &item);
   int addDuplicate(K &&key, T &&item);
    ...
    */
   size_t length() const { return _NofItems; }
   int length32() const { return _NofItems; }
...
private:
   ...
   inline int isKeyEqual(const K &k1, const K &k2) const
   {
   ...
   T* nextItemInternal(SEIterator &iter, K *pKey=nullptr) const;
   T* prevItemInternal(SEIterator &iter, K *pKey=nullptr) const;
   T* currentItemInternal(const SEIterator &iter, K *pKey=nullptr) const;

   int addPrivate(KVpair *pkv, unsigned int h);
   KVpair *findPrivate(const K &key, unsigned int h);
};

//////////////////////////////////////////////////////////////////////////

template <class K, class T> inline
SEHashTable<K,T>::SEHashTable(const size_t initialCapacity) :
   ...
   _pfnHash(0)
{
   ...
   if (initialCapacity > 0) {
      _kvlist = (KVpair**) SEAllocate(sizeof(KVpair*)*initialCapacity);
      if (!_kvlist) {
         return;
      }
      memset(_kvlist,0,initialCapacity*sizeof(KVpair*));
      _NofAllocated=static_cast<int>(initialCapacity);
   }
}
template <class K, class T> inline
...
template <class K, class T> inline
SEHashTable<K,T>::SEHashTable(SEHashTable<K,T> && src) :
   // initialize list of items
   _kvlist(src._kvlist),
   _NofItems(src._NofItems),
   _NofChained(src._NofChained),
...
{
   // initial function pointers
   src._pfnKeyCompare=0;
   src._pfnValCompare=0;
   src._pfnHash=0;
   // initialize list of items
   src._kvlist=0;
   src._NofItems=0;
...
SEHashTable<K,T>& SEHashTable<K,T>::operator=(const SEHashTable<K,T>& src)
{
   if (this != &src) {
      ...
      int initialCapacity = src._NofAllocated;
      if (initialCapacity > 0) {
         _kvlist = (KVpair**) SEReallocate(_kvlist, sizeof(KVpair*)*initialCapacity);
         memset(_kvlist,0,initialCapacity*sizeof(KVpair*));
         _NofAllocated=initialCapacity;
      }
...
template <class K, class T> inline
SEHashTable<K,T>& SEHashTable<K,T>::operator=(SEHashTable<K,T> && src)
{
   if (this != &src) {
      // out with the old
      clear();
      // initialize list of items
      this->_kvlist = src._kvlist;
      this->_NofItems = src._NofItems;
      this->_NofChained = src._NofChained;
      this->_NofAllocated = src._NofAllocated;
      this->_pfnHash = src._pfnHash;
      this->_pfnKeyCompare = src._pfnKeyCompare;
      this->_pfnValCompare = src._pfnValCompare;
      // null out the source hash table
      src._pfnKeyCompare=0;
      src._pfnValCompare=0;
      src._pfnHash=0;
      src._kvlist=0;
...
T* SEHashTable<K,T>::nextItemInternal(SEIterator &iter, K *pKey) const
{
   ...
   if (!iter.mFindFirst && pkv != nullptr) {
      pkv = pkv->pnext;
      if (pkv != nullptr) {
         iter.mpNode = pkv;
         if (pKey != nullptr) {
...
{
   // first check the linked lists
   if (!iter.mFindFirst && iter.mpNode != nullptr) {
...
int SEHashTable<K,T>::removeItem(SEIterator &iter)
{
   ...
   if (!iter.mFindFirst && pkv != nullptr) {
      ...
      if (firstNode == pkv) {
         ...
         if (pkv->pnext != nullptr) --_NofChained;
         --_NofItems;
         delete pkv;
...
template <class K, class T> inline
typename SEHashTable<K,T>::KVpair *SEHashTable<K,T>::findPrivate(const K &key, unsigned int h)
{
   // if we have no items, no need to find the item
   if (_NofItems == 0) {
      return nullptr;
...
template <class K, class T> inline
int SEHashTable<K,T>::addPrivate(KVpair *pkv, unsigned int h)
{
   // the key-value pair object must not be null
   assert(pkv != nullptr);
...
int SEHashTable<K,T>::add(const K &key, const T &item)
{
   ...
   // allocate the key-value pair, and add new item to the chain
   pkv = new KVpair(key,item);
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,h);
}
template <class K, class T> inline
int SEHashTable<K,T>::add(K &&key, const T &item)
{
   // check if this key is already in the table
   unsigned int h=hashKey(key);
   KVpair* pkv = findPrivate(key,h);
   if (pkv != nullptr) {
      pkv->key = std::move(key);
      pkv->val = item;
      return 0;
   }

   // allocate the key-value pair, and add new item to the chain
   pkv = new KVpair(std::forward<K>(key),item);
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,h);
}
template <class K, class T> inline
int SEHashTable<K,T>::add(const K &key, T &&item)
{
   // check if this key is already in the table
   unsigned int h=hashKey(key);
   KVpair* pkv = findPrivate(key,h);
   if (pkv != nullptr) {
      pkv->key = key;
      pkv->val = std::move(item);
      return 0;
   }

   // allocate the key-value pair, and add new item to the chain
   pkv = new KVpair(key,std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,h);
}
template <class K, class T> inline
int SEHashTable<K,T>::add(K &&key, T &&item)
{
   // check if this key is already in the table
   unsigned int h=hashKey(key);
   KVpair* pkv = findPrivate(key,h);
   ...
   // allocate the key-value pair, and add new item to the chain
   pkv = new KVpair(std::forward<K>(key),std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,h);
}
...
int SEHashTable<K,T>::addDuplicate(const K &key, const T &item)
{
   ...
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,hashKey(key));
}
template <class K, class T> inline
int SEHashTable<K,T>::addDuplicate(K &&key, const T &item)
{
   // allocate the key-value pair, and add new item to the chain
   KVpair *pkv = new KVpair(std::forward<K>(key),item);
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,hashKey(key));
}
template <class K, class T> inline
int SEHashTable<K,T>::addDuplicate(const K &key, T &&item)
{
   // allocate the key-value pair, and add new item to the chain
   KVpair *pkv = new KVpair(key,std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,hashKey(key));
}
template <class K, class T> inline
int SEHashTable<K,T>::addDuplicate(K &&key, T &&item)
{
   // allocate the key-value pair, and add new item to the chain
   KVpair *pkv = new KVpair(std::forward<K>(key),std::forward<T>(item));
   if (!pkv) return INSUFFICIENT_MEMORY_RC;
   return addPrivate(pkv,hashKey(key));
}
...
template <class K, class T> inline
int SEHashTable<K,T>::rehash(size_t new_NofAllocated)
{
   ...
   KVpair **new_kvlist = (KVpair**) SEAllocate(sizeof(KVpair*)*new_NofAllocated);
   if (!new_kvlist) {
      return INSUFFICIENT_MEMORY_RC;
   }
   ...
   SEDeallocate(_kvlist);
   _kvlist=new_kvlist;
   _NofAllocated = static_cast<int>(new_NofAllocated);
   return (0);
}
