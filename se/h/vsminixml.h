...
#define VSXMLCFG_COPY_SRC_CHILDREN           0x200
#define VSXMLCFG_COPY_CHILDREN               (VSXMLCFG_COPY_SRC_CHILDREN|VSXMLCFG_COPY_AS_CHILD)
#define VSXMLCFG_COPY_BEFORE                 VSXMLCFG_ADD_BEFORE
#define VSXMLCFG_COPY_AS_CHILD               VSXMLCFG_ADD_AS_CHILD
#define VSXMLCFG_COPY_AS_FIRST_CHILD         VSXMLCFG_ADD_AS_FIRST_CHILD
 ...
 * @categories XMLCFG_Functions
 */
int VSAPI vsXMLCFGOpenFromBuffer(int wid,int &status,int flags VSDEFAULT(0),int StartRealSeekPos VSDEFAULT(0),int EndRealSeekPos VSDEFAULT(0x7fffffff),void *preserved VSDEFAULT(0));
/**
 * Opens and parses an XML file into a tree.  The XMLCFG function are intended to be used on valid XML files for reading and writing various configuration information.  The XMLCFG functions are not for validating or checking for well formedness.
 * 
 * @categories XMLCFG_Functions
 * @param pBuffer    Utf-8 string buffer
 ...
 * @return Returns XMLCFG tree handle if successful.  Otherwise a negative error code is returned.
 */
int VSAPI vsXMLCFGOpenFromString(const char *pBuffer,int BufLen,int &status,int OpenFlags=0);

//Use VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE or VSXMLCFG_SAVE_PRESERVE_PCDATA
...
#define VSXMLCFG_SAVE_SPACE_AFTER_LAST_ATTRIBUTE   (0x80 | VSXMLCFG_SAVE_ATTRS_ON_ONE_LINE)

#define VSXMLCFG_SAVE_PRESERVE_PCDATA             0x800
...
/* 
   ...
   <attrs a="a' b="b" c="c"/>
*/ 
#define VSXMLCFG_SAVE_SORT_ATTRIBUTES     0x10000
...
int VSAPI vsXMLCFGSave(int iHandle,int iIndentAmount,int iFlags,const char *pszAlternateFilename VSDEFAULT(0),
                       int encoding VSDEFAULT(-1),int NodeIndex VSDEFAULT(0));
/**
 ...
 * @categories XMLCFG_Functions
 */
int VSAPI vsXMLCFGSaveToBuffer(int wid,int iHandle,int iIndentAmount,int iFlags,int NodeIndex VSDEFAULT(0));
 ...
 * @categories XMLCFG_Functions
 */
VSDEPRECATED const char *VSAPI vsXMLCFGGetName(int iHandle,int NodeIndex,int *pNameLen VSDEFAULT(0));
/**
 ...
 * @categories XMLCFG_Functions
 */
VSDEPRECATED const char * VSAPI vsXMLCFGGetValue(int iHandle,int NodeIndex,int *pValueLen VSDEFAULT(0));
/**
...
#define   VSXMLCFG_ADD_AFTER           0
#define   VSXMLCFG_ADD_BEFORE          1
#define   VSXMLCFG_ADD_AS_CHILD        2
#define   VSXMLCFG_ADD_AS_FIRST_CHILD  3
...
/**
 * Adds either TEXT or CDATA child node to XMLCFG tree depending
 * on the contents of the pText. Use the vsXMLCFGGetText()
 * function to get the text contents of the node.
 *
 * @param iHandle  Handle to an XMLCFG tree returned by vsXMLCFGOpen(), vsXMLCFGCreate(), or vsXMLCFGOpenFromBuffer().
 * @param iIndex   Tree node index to add attribute to.
 * @param pText
 *                 Text to add as child
 ...
 * @categories XMLCFG_Functions
 */
int VSAPI vsXMLCFGAddChildText(int iHandle,int iIndex,const char *pText,int TextLen);
 ...
 * @categories XMLCFG_Functions
 */
int VSAPI vsXMLCFGSetAttribute(int iHandle,int NodeIndex,const char * pszAttrName,const char *pAttrValue,int AttrValueLen=-1,int iFlags=0);
/**
 ...
 *     <dl>
 *     <dt>VSXMLCFG_FIND_APPEND<dd>Append to array
 *     </dl>
 *
 ...
 * @categories XMLCFG_Functions
 */
void VSAPI vsXMLCFGFindSimpleArray(int iHandle,VSPSZ pszQueryStr,int NodeIndex,slickedit::SEArray<int> &nodeArray,int FindFlags VSDEFAULT(0),int maxAddCount= -1);
void VSAPI vsXMLCFGFindSimpleCMArray(int iHandle,VSPSZ pszQueryStr,int NodeIndex,cmArray<int> &nodeArray,int FindFlags VSDEFAULT(0),int maxAddCount=-1);
 ...
 *         vsXmlCfgClose to free the xml.
 */
int VSAPI vsPluginGetUserOptions(int optionLevel=0);
/**
 * 
 ...
 * profile. 
 *  
 * @param pszEscapedProfilePackage  The escaped profile package. 
 *                                  Specifies the package this
 *                                  profile (pszProfileName) is
 *                                  contained in.
 * @param pszProfileName Name of the profile to retrieve (for 
 *                       display unescaped)
 * @param optionLevel   optionLevel=0 specifies the user level 
 *                      settings. There may be project and
 *                      workspace levels in the future.
 * 
 * @return If successful, returns xml cfg handle to XML profile
 *         property settings. Otherwise, a negative error code
 *         is returned. Use vsXmlCfgClose to free the xml.
 */
int VSAPI vsPluginGetProfile(const char *pszEscapedProfilePackage, const char *pszProfileName,int optionLevel=0);
 ...
 *                      "options" element. Usually, this is what
 *                      you want when importing/exporting single
 *                      profiles in .cfg.xml files.
 * 
 * @return If successful, returns xml cfg handle to XML profile
 *         property settings. Otherwise, a negative error code
 *         is returned. Use vsXmlCfgClose to free the xml.
 *  
 * @see vsPluginGetProfile 
 */
int VSAPI vsPluginGetUserProfile(const char *pszEscapedProfilePackage, const char *pszProfileName,bool create_inside_options_node=true,int optionLevel=0);

/**
 * Retrieves property from a profile which has an XML value.
 *  
 * <p>Xml contains all (merged) settings for this 
 * profile. 
 *  
 ...
 *         is returned. Use vsXmlCfgClose to free the xml.
 */
int VSAPI vsPluginGetPropertyXml(VSPSZ pszEscapedProfilePackage, VSPSZ pszProfileName,VSPSZ pszName, bool *pApply=0,int optionLevel=0);

/**
 * Fetches a property from a profile from a .cfg.xml file.
 *  
 * @param handle
 * @param node
 * @param pszName
 ...
 * @param pszDefaultValue
 * 
 * @return Returns the value of the property. If the property 
 *         does not exists or the input parameters are invalid,
 *         pszDefaultValue is returned.
 */
VSPSZ VSAPI vsXMLCFGGetProperty(int handle,int node,VSPSZ pszName,VSPSZ pszDefaultValue="",bool *pApply=0);
/**
 * Sets property in a profile which has an XML value.
 *  
 * @param pszEscapedProfilePackage  The escaped profile package. 
 *                                  Specifies the package this
 *                                  profile (pszProfileName) is
 ...
 *                       display unescaped)
 * @param pszName
 * @param handle        Handle to Xml value.
 * @param pApply
 * @param optionLevel   optionLevel=0 specifies the user level 
 *                      settings. There may be project and
 *                      workspace levels in the future.
 ...
 *         is returned. Use vsXmlCfgClose to free the xml.
 */
void VSAPI vsPluginSetPropertyXml(VSPSZ pszEscapedProfilePackage, VSPSZ pszProfileName,VSPSZ profileVersion,VSPSZ pszName, int handle, bool *pApply=0,int optionLevel=0);
/**
 * Sets a property in a profile from a .cfg.xml file.
 * 
 * @param handle
 * @param node
 * @param pszName
 ...
 * @param pApply
 */
void VSAPI vsXMLCFGSetProperty(int handle,int node,VSPSZ pszName,VSPSZ pszValue,bool *pApply=0);


/**
 * List properties by a property name prefix match or a value 
 * match. 
 *  
 * @param array         (Output only) Set to nodes that match. 
 * @param handle        Handle to Xml value.
 * @param node          Profile node
 * @param pszMatchNamePrefix   property name prefix to match. 
 *                             Case sensitive match of property
 *                             name is based on profile node
 *                             specification.
 * @param pszMatchNameSearchOptions
 *               When !=null, this specifies vsSearch() style
 *               search options and a contains match (not prefix
 *               match) is performed. Typically
 *               used to match a regular expressions. (Ex.
 *               pszMatchNameSearchOptions="L" where
 *               pszMatchNamePrefix="^[^;]+;something$")
 * @param pszMatchValue       Optional value to match
 * @param pszMatchValueSearchOptions
 *               When !=null, this specifies vsSearch() style
 ...
 *               pszMatchNameSearchOptions="L" where
 *               pszMatchValue="^[^;]+;something$")
 */
void VSAPI vsXMLCFGListProperties(slickedit::SEArray<int> &array,int handle,int node,VSPSZ pszMatchNamePrefix="",VSPSZ pszMatchNameSearchOptions=0, VSPSZ pszMatchValue=0,VSPSZ pszMatchValueSearchOptions=0);

/**
 * Find a property in a profile from a .cfg.xml file.
 * 
 * @param handle
 * @param node
 * @param pszName
 * 
 * @return If successful, return node>=0. Otherwise, a negative 
 *         error code is returned.
 */
int VSAPI vsXMLCFGFindProperty(int handle,int node,VSPSZ pszName);
 ...
 *                 (vsPluginGetProfile, vsXmlCfgCreate, etc.)
 * @param optionLevel
 *                optionLevel=0 specifies the user level
 *                settings. There may be project and
 *                workspace levels in the future.
 */
void VSAPI vsPluginSetProfile(int iHandle,int optionLevel=0);
 ...
 *                workspace levels in the future.
 */
void VSAPI vsPluginSetUserProfile(int iHandle,int optionLevel=0);
 ...
 * @param NodeIndex index of element
 *
 * @return Returns the node text value of the node and all child
 *         nodes.
 *
 * @categories XMLCFG_Functions
 */
extern VSDLLEXPORT
slickedit::SEString vsXMLCFGGetText(int iHandle,int NodeIndex);
 ...
 */
extern VSDLLEXPORT
slickedit::SEString vsXMLCFGSaveToString(int iHandle,int iIndentAmount,int iFlags,int NodeIndex VSDEFAULT(0));
cmStringUtf8 vsXMLCFGSaveToCMString(int iHandle, int iIndentAmount, int iFlags, int NodeIndex=0);
slickedit::SEString vsPluginFindLongestProperty(const char *pszEscapedProfilePackage, const char *pszProfileName, const char *pszString, bool doPrefixMatch=true, slickedit::SEString *pValue = 0, bool *pApply = 0, int optionLevel = 0);
...
extern VSDLLEXPORT
int vsXMLCFGAddCMString(int iHandle, int iIndex,
                        const cmROStringUtf8 &NameOrValue,
                        int NodeType, int iFlags);
...
extern VSDLLEXPORT
cmStringUtf8 vsXMLCFGGetAttributeCMString(int iHandle,int NodeIndex,
                                          VSPSZ pszAttrName,
                                          const char *pszDefaultValue=0);
...
extern VSDLLEXPORT
int vsXMLCFGSetAttributeCMString(int iHandle, int NodeIndex,
                                 const char *pszAttrName,
                                 const cmROStringUtf8 &AttrValue,
                                 int iFlags=0 );
 ...
 */
extern VSDLLEXPORT
int vsXMLCFGAddAttributeCMString(int iHandle,int iIndex,
                                 const cmROStringUtf8 &attrName, 
                                 const cmROStringUtf8 &attrValue,
                                 int iFlags VSDEFAULT(0));
...
extern VSDLLEXPORT
cmStringUtf8 vsXMLCFGGetNameCMString(int iHandle,int NodeIndex);
...
extern VSDLLEXPORT
int VSAPI vsXMLCFGSetNameCMString(int iHandle, int NodeIndex,
                                  const cmROStringUtf8 &Name);
...
extern VSDLLEXPORT
cmStringUtf8 vsXMLCFGGetValueCMString(int iHandle,int NodeIndex);
